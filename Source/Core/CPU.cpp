...
static bool gCPUStopOnSimpleState{
    false}; // When stopping, try to stop in a 'simple' state (i.e. no RSP
            // running and not in a branch delay slot)
...
void CPU_AddEvent(s32 count, ECPUEventType event_type) {
  ...
  for (event_idx = 0; event_idx < gCPUState.NumEvents; ++event_idx) {
    ...
    if (count <= event.mCount) {
      //
      //	This event belongs before the subsequent one so insert a space
      //for it here and break out 	Don't forget to decrement the counter for the
      //subsequent event
      //
      event.mCount -= count;
      ...
      if (num_to_copy > 0) {
        memmove(&gCPUState.Events[event_idx + 1], &gCPUState.Events[event_idx],
                num_to_copy * sizeof(CPUEvent));
      }
      break;
    }
...
#endif
    //
    //	Remove any existing compare events. Need to adjust any subsequent
    //timer's count.
    //
    for (u32 i{}; i < gCPUState.NumEvents; ++i) {
      if (gCPUState.Events[i].mEventType == CPU_EVENT_COMPARE) {
        ...
        if (i + 1 < gCPUState.NumEvents) {
          ...
          memmove(&gCPUState.Events[i], &gCPUState.Events[i + 1],
                  num_to_copy * sizeof(CPUEvent));
        }
        gCPUState.NumEvents--;
        break;
...
static ECPUEventType CPU_PopEvent() {
  ...
  DAEDALUS_ASSERT(gCPUState.Events[0].mCount <= 0,
                  "Popping event when cycles remain");
  // DAEDALUS_ASSERT( gCPUState.Events[ 0 ].mCount == 0, "Popping event with a
  // bit of underflow" );
#endif
  ...
  if (num_to_copy > 0) {
    memmove(&gCPUState.Events[0], &gCPUState.Events[1],
            num_to_copy * sizeof(CPUEvent));
  }
  gCPUState.NumEvents--;
...
bool CPU_RomOpen() {
  ...
  // gCPUState.CPUControl[C0_SR]._u32   = 0x70400004;	//*SR_FR |*/ SR_ERL |
  // SR_CU2|SR_CU1|SR_CU0;
  R4300_SetSR(0x70400004);
  gCPUState.CPUControl[C0_PRID]._u32 =
      0x00000b10; // Was 0xb00 - test rom reports 0xb10!!
  ...
  ((u32 *)g_pMemoryBuffers[MEM_RI_REG])[3] =
      1; // RI_CONFIG_REG Skips most of init
...
bool CPU_Run() {
  ...
  while (true) {
    ...
    gCPUStopOnSimpleState = false;
#ifdef DAEDALUS_ENABLE_ASSERTS
    DAEDALUS_ASSERT(gSaveStateOperation == SSO_NONE,
                    "Shouldn't have a save state operation queued.");
#endif
    RESET_EVENT_QUEUE_LOCK();
...
void CPU_HANDLE_COUNT_INTERRUPT() {
#ifdef DAEDALUS_ENABLE_ASSERTS
  DAEDALUS_ASSERT(gCPUState.NumEvents > 0,
                  "Should always have at least one event queued up");
#endif
  switch (CPU_PopEvent()) {
  case CPU_EVENT_VBL: {
    ...
    if (vertical_sync_reg == 0) {
      ...
      VI_INTR_CYCLES =
          (vertical_sync_reg + 1) * (gVideoRateMatch ? gVISyncRate : 1500);
    }
    ...
    //   because it means we can remain responsive even if the game is not
    //   rendering frames (e.g. if it's slow starting up) Alternatively, we
    //   could add a special-purpose CPU even that triggers every N cycles, but
    //   that would have a small impact on framerate (it would interrupt the
    //   dynamo tracer for instance)
  ...
  case CPU_EVENT_AUDIO: {
    u32 status{Memory_SP_SetRegisterBits(
        SP_STATUS_REG, SP_STATUS_TASKDONE | SP_STATUS_YIELDED |
...
// DBGConsole_Msg(0, "COMPARE set to 0x%08x Count is 0x%08x.", value,
// gCPUState.CPUControl[C0_COUNT]._u32);
  ...
  if (value == gCPUState.CPUControl[C0_COMPARE]._u32) {
    ...
    if (value != 0) {
      // NB, value can be less than COUNT here, which indicates that the counter
      // is close to wrapping. Don't do anything special to handle this - just
      // treat delta as an unsigned value.
      u32 delta{value - gCPUState.CPUControl[C0_COUNT]._u32};

      // This fires a lot for Zelda OoT. It's benign.
      // If seems to keep setting a delta of 140624981 when the counter is close
      // to wrapping. if (value < gCPUState.CPUControl[C0_COUNT]._u32)
      // {
      // 	DBGConsole_Msg(0, "SetCompare wrapping: %d -> %d = %d",
      // gCPUState.CPUControl[C0_COUNT]._u32, value, delta);
      // }
...
u32 CPU_ProduceRegisterHash() {
  ...
  if (DAED_SYNC_MASK & DAED_SYNC_REG_CCR0) {
    hash = murmur2_hash((u8 *)&(gCPUState.CPUControl[0]),
                        sizeof(gCPUState.CPUControl), hash);
  }
  ...
  if (DAED_SYNC_MASK & DAED_SYNC_REG_CCR1) {
    hash = murmur2_hash((u8 *)&(gCPUState.FPUControl[0]),
                        sizeof(gCPUState.FPUControl), hash);
  }
...
void CPU_ExecuteOpRaw(u32 count, u32 address, OpCode op_code,
                      CPU_Instruction p_instruction, bool *p_branch_taken) {
...
#endif
  // SYNCH_POINT( DAED_SYNC_FRAGMENT_PC, ops_executed, "Number of executed ops
  // doesn't match" );
...
void CPU_UpdateCounterNoInterrupt(u32 ops_executed) {
  ...
  if (ops_executed > 0) {
    ...
    use(ready);
    DAEDALUS_ASSERT(
        !ready, "Ignoring Count interrupt"); // Just a test - remove eventually
