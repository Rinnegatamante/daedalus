...
//
//*****************************************************************************
enum EDelayType { NO_DELAY = 0, EXEC_DELAY, DO_DELAY };

#ifdef DAEDALUS_BREAKPOINTS_ENABLED
struct DBG_BreakPoint {
  OpCode mOriginalOp;
  bool mEnabled;
  bool mTemporaryDisable; // Set to true when BP is activated - this lets us
                          // type go immediately after a bp and execution will
                          // resume. otherwise it would keep activating the bp
...
#endif
{
  ...
  u32 Delay;                 // 0x288 ..			Delay state (NO_DELAY,
                             // EXEC_DELAY, DO_DELAY)
  ...
  CPUEvent Events[MAX_CPU_EVENTS]; // 0x2B0 //In practice there should only ever
                                   // be 2 CPU_EVENTS
  u32 NumEvents;
...
void CPU_EnableBreakPoint(
    u32 address,
    bool enable); // Enable/Disable the breakpoint as the specified address
#endif
bool CPU_IsRunning();
...
// For PSP, we just keep running forever. For other platforms we need to bail
// when the user quits.
#ifdef DAEDALUS_PSP
...
inline void CPU_TakeBranch(u32 new_pc) {
