...
static void CheatCodes_Apply(u32 index, u32 mode) {
  ...
  while (num--) {
    ...
    case 0xA0:
      ...
      // Set CHEAT_CODE_MAGIC_VALUE as well to make sure we can save the most
      // recent value later on
      if (!enable) {
        value = code->orig;
...
void CheatCodes_Activate(CHEAT_MODE mode) {
  for (u32 i{}; i < codegroupcount; i++) {
    ...
    if (codegrouplist[i].active) {
      ...
      // this flag will signal that we need to restore the hacked value to
      // normal
...
void CheatCodes_Delete(u32 index)
{
        if(index < codegroupcount - 1)
        {
                ...
                {
                        memcpy(&codegrouplist[j], &codegrouplist[j + 1],
sizeof(CODEGROUP));
                }
        }
        codegroupcount--;
...
bool CheatCodes_Read(const char *rom_name, const char *file, u8 countryID) {
  ...
  char line[2048]{},
      romname[256]{} /*, errormessage[400]*/; // changed line length to 2048
                                              // previous was 256
  bool bfound;
  u32 c1{}, c2{};
  FILE *stream{};

  // Add country ID to this ROM name, to avoid mixing cheat code of different
  // region in the same entry Only the country id is important (first char)
  sprintf(current_rom_name, "%s (%c)", rom_name,
          ROM_GetCountryNameFromID(countryID)[0]);
  ...
  if (bfound) {
    ...
    if (fgets(line, 256, stream)) {
      ...
      if (strncmp(line, "NumberOfGroups=", 15) == 0) {
        ...
        // If for some reason NumberOfGroups is incorrect or invalid, just set
        // the max of cheatcodes we allow
        numberofgroups = MAX_CHEATCODE_PER_LOAD;
      }
    } else {
    ...
    //
    //		printf("number of cheats loaded %d | %d kbs used of
    //memory\n",numberofgroups,(numberofgroups *sizeof(CODEGROUP))/ 1024);
    codegrouplist =
        (CODEGROUP *)malloc_volatile(numberofgroups * sizeof(CODEGROUP));
    if (codegrouplist == nullptr) {
    ...
    codegroupcount = 0;
    while (codegroupcount < numberofgroups && fgets(line, 32767, stream) &&
           strlen(line) > 8) // 32767 makes sure the entire line is read
    {
      // Codes for the group are in the string line[]
      for (c1 = 0; line[c1] != '=' && line[c1] != '\0'; c1++)
        codegrouplist[codegroupcount].name[c1] = line[c1];

      // FIXME: xocde reckons this is dodgy - if c1 is 0 (first char is '='),
      // this will index [-1]!
      ...
      if (line[c1 + 1] == '"') {
        ...
        for (c3 = 0; line[c3 + c1 + 2] != '"' && line[c3 + c1 + 2] != '\0';
             c3++) {
      ...
      u32 addr{}, value{};
      codegrouplist[codegroupcount].active =
          (line[c1 + 1] - '0') ? true : false;
      codegrouplist[codegroupcount].enable = false;
      codegrouplist[codegroupcount].codecount = 0;

      c1 += 2;

      for (c2 = 0; c2 < (strlen(line) - c1 - 1) / 14;
           c2++, codegrouplist[codegroupcount].codecount++) {
        if (c2 < MAX_CHEATCODE_PER_ENTRY) {
          sscanf(line + c1 + 1 + c2 * 14, "%08x-%04x", &addr, &value);
          if (c2 > 0 &&
              ((codegrouplist[codegroupcount].codelist[c2 - 1].addr >> 24) &
               0xFF) == 0x50) {
            // ToDO: Uncompress Serial Repeater cheat code; ex addr = (temp +
            // offset) ^ U8_TWIDDLE Then we can convert it to a normal cheat code
            // by just filling each slot with the repeater counter, but it would
            // alot of memory.. The way compressed cheat codes work is in pairs,
            // the serial repeater 0x50xxx and the actual code 0x80/81xxx; ex
            // 50001801-0001,80118444-0000 For now if previous cheatcode was
            // compressed, don't preswap
          } else {
            // Pre-swap address
            switch ((addr >> 24) & 0xFF) {
            case 0x80:
            ...
            case 0x50:
              break;
            default:
              // Unhandled cheat code? I think we handle most if not all cheat
              // code->types?
              break;
            }
          }

          codegrouplist[codegroupcount].codelist[c2].orig =
              CHEAT_CODE_MAGIC_VALUE;
          ...
          /*sprintf (errormessage,
                       "Too many codes for cheat: %s (Max = %d)! Cheat will be
          truncated and won't work!", codegrouplist[codegroupcount].name,
