...
void DMA_SP_CopyFromRDRAM() {
  ...
  u32 rdram_address{(rdram_address_reg & 0x00FFFFFF) &
                    ~7}; // Align to 8 byte boundary
  u32 spmem_address{(spmem_address_reg & 0x1FFF) &
                    ~7};                      // Align to 8 byte boundary
  ...
  for (u32 c{}; c < count; c++) {
    ...
    fast_memcpy_swizzle(&g_pu8SpMemBase[spmem_address],
                        &g_pu8RamBase[rdram_address], length);
...
void DMA_SP_CopyToRDRAM() {
  ...
  if ((spmem_address_reg & 0x1000) == 0) {
    ...
    fast_memcpy(&g_pu8RamBase[(rdram_address_reg & 0xFFFFFF)],
                &g_pu8SpMemBase[(spmem_address_reg & 0xFFF)],
                (wrlen_reg & 0xFFF) + 1);
  }
...
#endif
    fast_memcpy_swizzle(&g_pu8RamBase[rdram_address],
                        &g_pu8SpMemBase[spmem_address], length);
    rdram_address += length + skip;
    spmem_address += length;
  }
...
void DMA_SI_CopyToDRAM() {
  ...
  // Skipping this IRQ fixes allows Body Harvest, Nightmare Creatures and
  // Cruisn' USA to boot ToDo: Implement Delay SI, PJ64 uses that option to make
  // these games boot..
...
bool DMA_HandleTransfer(u8 *p_dst, u32 dst_offset, u32 dst_size,
                        ...
                        u32 length) {
  if ((s32(length) <= 0) || (src_offset + length) > src_size ||
...
static void OnCopiedRom() {
  if (!gDMAUsed) {
    ...
    // ToDO: try to reapply patches - certain roms load in more of the OS after
...
#endif
  // DAEDALUS_ASSERT(!IsDom1Addr1(cart_address), "The code below doesn't handle
  // dom1/addr1 correctly"); DAEDALUS_ASSERT(!IsDom1Addr3(cart_address), "The
  // code below doesn't handle dom1/addr3 correctly");
  ...
  if (cart_address < 0x10000000) {
    if (IsFlashDomAddr(cart_address)) {
      ...
      if (g_ROM.settings.SaveType != SAVE_TYPE_FLASH)
        DMA_HandleTransfer(g_pu8RamBase, mem_address, gRamSize, p_src,
                           cart_address, src_size, pi_length_reg);
      else
    ...
    if (cart_address < 0x1fc00000) {
      ...
      RomBuffer::CopyToRam(g_pu8RamBase, mem_address, gRamSize, cart_address,
                           pi_length_reg);
...
void DMA_PI_CopyFromRDRAM() {
  ...
  {
          DBGConsole_Msg(0, "PI Copy RDRAM to CART %db from %08X to %08X",
  pi_length_reg, cart_address|0xA0000000, mem_address); DBGConsole_Msg(0,
  "Warning, PI DMA, odd length");
  ...
  */
  ...
  if (IsFlashDomAddr(cart_address)) {
    ...
    if (g_ROM.settings.SaveType != SAVE_TYPE_FLASH)
      DMA_HandleTransfer(p_dst, cart_address, dst_size, g_pu8RamBase,
                         mem_address, gRamSize, pi_length_reg);
    else
