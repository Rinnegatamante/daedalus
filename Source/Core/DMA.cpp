...
void DMA_SP_CopyFromRDRAM()
{
	...
	auto rdram_address {(rdram_address_reg&0x00FFFFFF)	& ~7};	// Align to 8 byte boundary
	auto spmem_address {(spmem_address_reg&0x1FFF)		& ~7};	// Align to 8 byte boundary
	auto length {((rdlen_reg    &0x0FFF) | 7)+1};					// Round up to 8 bytes
	auto count  {((rdlen_reg>>12)&0x00FF)+1};
	auto skip   {((rdlen_reg>>20)&0x0FFF)};
...
{
	auto spmem_address_reg {Memory_SP_GetRegister(SP_MEM_ADDR_REG)};
	auto rdram_address_reg {Memory_SP_GetRegister(SP_DRAM_ADDR_REG)};
	auto wrlen_reg         {Memory_SP_GetRegister(SP_WR_LEN_REG)};
	...
	auto rdram_address {(rdram_address_reg&0x00FFFFFF)	& ~7};	// Align to 8 byte boundary
	auto spmem_address {(spmem_address_reg&0x1FFF)		& ~7};	// Align to 8 byte boundary
	auto length {((wrlen_reg    &0x0FFF) | 7)+1};				// Round up to 8 bytes
	auto count  {((wrlen_reg>>12)&0x00FF)+1};
	auto skip   {((wrlen_reg>>20)&0x0FFF)};
...
{
	auto mem {Memory_SI_GetRegister(SI_DRAM_ADDR_REG) & 0x1fffffff};
	auto *p_dst {(u32 *)g_pMemoryBuffers[MEM_PIF_RAM]};
	auto *p_src {(u32 *)(g_pu8RamBase + mem)};
...
void DMA_SI_CopyToDRAM( )
{
	...
	auto mem {Memory_SI_GetRegister(SI_DRAM_ADDR_REG) & 0x1fffffff};
	auto * p_src {(u32 *)g_pMemoryBuffers[MEM_PIF_RAM]};
	auto * p_dst {(u32 *)(g_pu8RamBase + mem)};
...
static void OnCopiedRom()
{
	if (!gDMAUsed)
	{
		...
		// Set RDRAM size
		auto addr {(g_ROM.cic_chip != CIC_6105) ? (u32)0x318 : (u32)0x3F0};
...
{
	auto mem_address  {Memory_PI_GetRegister(PI_DRAM_ADDR_REG) & 0x00FFFFFF};
	auto cart_address {Memory_PI_GetRegister(PI_CART_ADDR_REG)  & 0xFFFFFFFF};
	auto pi_length_reg {(Memory_PI_GetRegister(PI_WR_LEN_REG) & 0xFFFFFFFF) + 1};
	...
	if (cart_address < 0x10000000)
    {
		...
		{
           	const auto *p_src( (const u8 *)g_pMemoryBuffers[MEM_SAVE] );
			auto	src_size( ( MemoryRegionSizes[MEM_SAVE] ) );
...
{
	auto mem_address  {Memory_PI_GetRegister(PI_DRAM_ADDR_REG) & 0xFFFFFFFF};
	auto cart_address {Memory_PI_GetRegister(PI_CART_ADDR_REG)  & 0xFFFFFFFF};
	auto pi_length_reg {(Memory_PI_GetRegister(PI_RD_LEN_REG)  & 0xFFFFFFFF) + 1};
	...
	*/
	...
	if(IsFlashDomAddr(cart_address))
	{
		auto *p_dst {(u8 *)g_pMemoryBuffers[MEM_SAVE] };
		auto dst_size {MemoryRegionSizes[MEM_SAVE] };
		cart_address -= PI_DOM2_ADDR2;
