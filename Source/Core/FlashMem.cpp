...
#define SetFlashStatus(x)                                                      \
  FlashStatus[0] = (u32)((x) >> 32), FlashStatus[1] = (u32)(x)
...
bool DMA_FLASH_CopyToDRAM(u32 dest, u32 StartOffset, u32 len) {
  ...
  case FLASHRAM_MODE_READ: {
    ...
    DMA_HandleTransfer(g_pu8RamBase, dest, gRamSize,
                       (u8 *)g_pMemoryBuffers[MEM_SAVE], StartOffset,
                       MemoryRegionSizes[MEM_SAVE], len);
    return true;
  }
  case FLASHRAM_MODE_STATUS:
#ifdef DAEDALUS_ENABLE_ASSERTS
    DAEDALUS_ASSERT(len == sizeof(u32) * 2,
                    "Len is not correct when fetch status.");
#endif
...
void Flash_DoCommand(u32 FlashRAM_Command) {
  ...
  case 0xD2000000:
    switch (FlashFlag) {
      ...
      DBGConsole_Msg(0,
                     "Writing %X to flash ram command register\nFlashFlag: %d",
                     FlashRAM_Command, FlashFlag);
#endif
    ...
    case FLASHRAM_MODE_WRITE:
      memcpy((u8 *)g_pMemoryBuffers[MEM_SAVE] + FlashRAM_Offset, FlashBlock,
             128);
      Save_MarkSaveDirty();
      break;
#ifdef DAEDALUS_DEBUG_CONSOLE
