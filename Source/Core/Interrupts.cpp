...
static bool gExceptionWasDelay(
    false); // Was exception operation in a branch-delay slot?
...
//
//*****************************************************************************
void R4300_Exception_TLB(u32 virtual_address, u32 exception_code,
  ...
  gCPUState.CPUControl[C0_CONTEXT]._u32 &=
      0xFF800000; // Mask off bottom 23 bits
  gCPUState.CPUControl[C0_CONTEXT]._u32 |= ((virtual_address >> 13) << 4);

  gCPUState.CPUControl[C0_ENTRYHI]._u32 &=
      0x00001FFF; // Mask off the top bit 13-31
...
// These should be set before we end up here...
#ifdef DAEDALUS_ENABLE_ASSERTS
  DAEDALUS_ASSERT(gExceptionVector != u32(~0), "Exception vector not set: %08x",
                  gCPUState.GetStuffToDo());
  ...
  // Reset these, to ensure we set them correctly before next call to this
  // function
  gExceptionVector = ~0;
  gExceptionPC = ~0;
}
...
void R4300_Handle_Interrupt() {
  ...
  DAEDALUS_ASSERT(mi_interrupt_set == cause_int_3_set,
                  "CAUSE_IP3 inconsistant with MI_INTR_REG (%08x)",
                  Memory_MI_GetRegister(MI_INTR_MASK_REG) &
                      Memory_MI_GetRegister(MI_INTR_REG));
#endif

  if ((gCPUState.CPUControl[C0_SR]._u32 & (SR_EXL | SR_ERL | SR_IE)) ==
      SR_IE) // Ensure ERL/EXL are "0" and IE is "1"
  {
    if (gCPUState.CPUControl[C0_SR]._u32 & gCPUState.CPUControl[C0_CAUSE]._u32 &
        CAUSE_IPMASK) // Are interrupts pending/wanted?
    {
#ifdef DAEDALUS_PROFILE_EXECUTION
      gNumInterrupts++;
#endif
