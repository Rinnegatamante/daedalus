...
// static void EmitYUVTileLine_SwapY1Y2(const s16 *y, const s16 *u, u32
// address);
static void EmitRGBATileLine(const s16 *y, const s16 *u, u32 address);

/* macroblocks operations */
static void DecodeMacroblock1(s16 *macroblock, s32 *y_dc, s32 *u_dc, s32 *v_dc,
                              const s16 *qtable);
static void DecodeMacroblock2(s16 *macroblock, u32 subblock_count,
                              const s16 qtables[3][SUBBLOCK_SIZE]);
// static void DecodeMacroblock3(s16 *macroblock, u32 subblock_count, const s16
// qtables[3][SUBBLOCK_SIZE]);
static void EmitTilesMode0(const tile_line_emitter_t emit_line,
                           const s16 *macroblock, u32 address);
static void EmitTilesMode2(const tile_line_emitter_t emit_line,
                           const s16 *macroblock, u32 address);
...
static void MultSubBlocks(s16 *dst, const s16 *src1, const s16 *src2,
                          u32 shift);
...
const s16 DEFAULT_QTABLE[SUBBLOCK_SIZE] = {
    16, 12, 14, 14, 18,  24,  49,  72,  11, 12, 13, 17, 22,  35,  64,  92,
...
void jpeg_decode_PS(OSTask *task) {
  ...
  for (mb = 0; mb < macroblock_count; ++mb) {
    ...
    DecodeMacroblock2(macroblock, subblock_count,
                      (const s16(*)[SUBBLOCK_SIZE])qtables);
...
void jpeg_decode_OB(OSTask *task) {
  ...
  for (mb = 0; mb < macroblock_count; ++mb) {
    ...
    DecodeMacroblock1(macroblock, &y_dc, &u_dc, &v_dc,
                      (qscale != 0) ? qtable : nullptr);
...
static u8 clamp_u8(s16 x) { return (x & (0xff00)) ? ((-x) >> 15) & 0xff : x; }
...
static u32 GetUYVY(s16 y1, s16 y2, s16 u, s16 v) {
  return (u32)clamp_u8(u) << 24 | (u32)clamp_u8(y1) << 16 |
         (u32)clamp_u8(v) << 8 | (u32)clamp_u8(y2);
}
