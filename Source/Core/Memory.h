...
enum MEMBANKTYPE {
  MEM_UNUSED = 0, // Simplifies code so that we don't have to check for illegal
                  // memory accesses
  ...
  MEM_SAVE, // 0x20000 128KBytes EEPROM (512 bytes), 4X EEPROM (2Kbytes), SRAM
            // (32Kbytes), FlashRAM (128Kbytes)
...
//#define MEMORY_CHECK_ALIGN( address, align )	DAEDALUS_ASSERT( (address &
//~(align-1)) == 0, "Unaligned memory access" )
...
// inline void Write64Bits_NoSwizzle( u32 address, u64 data ){
// MEMORY_CHECK_ALIGN( address, 8 ); *(u64 *)WriteAddress( address ) = (data>>32)
// + (data<<32); }
inline void Write32Bits_NoSwizzle(u32 address, u32 data) {
  MEMORY_CHECK_ALIGN(address, 4);
  WriteAddress(address, data);
}
inline void Write16Bits_NoSwizzle(u32 address, u16 data) {
  MEMORY_CHECK_ALIGN(address, 2);
  *(u16 *)ReadAddress(address) = data;
}
inline void Write8Bits_NoSwizzle(u32 address, u8 data) {
  *(u8 *)ReadAddress(address) = data;
}
...
#define REGISTER_FUNCTIONS(set, base_reg, memory_buffer)                       \
  ...
  inline u32 Memory_##set##_GetRegister(u32 reg) {                             \
    return ((u32 *)g_pMemoryBuffers[memory_buffer])[(reg - (base_reg)) / 4];   \
                                                                               ...
                                                                               \
  inline u32 Memory_##set##_SetRegisterBits(u32 reg, u32 and_bits,             \
                                            u32 or_bits) {                     \
