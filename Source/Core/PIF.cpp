...
/*
In the exemplary embodiment, if processor 100 writes "0x00", "0xFD", "0xFE" or
"0xFF" as TxData Size, the data is not recognized as TxData size but has a
special function as indicated below. They become effective when processor 100
sets format bit (0x1FC007FC b0) by using Wr64B or Wr4B.

"0x00"=Channel Skip

If 0x00 is written as TxData Size, respective JoyChannel transaction is not
executed.

"0xFD"=Channel Reset

If 0xFD is written as TxData Size, PIF outputs reset signal to respective
JoyChannel.

"0xFE"=Format End

If 0xFE is written as TxData Size, TxData/RxData assignment is end at this
")xFE". In other words, the TxData Size or RxData Size after "0xFE" is ignored.

"0xFF"=Dummy Data

TxData Size's 0xFF is used as the dummy data for word aligning the data area.

Each Channel has four flags. Two of them have information from processor 100 to
JoyChannel and others from JoyChannel to processor 100.

Skip=Channel Skip

If processor 100 sets this flag to "1", respective JoyChannel transaction is not
executed. This flag becomes effective without formal flag.

Res=Channel Reset

If 64 bit CPU set this flag to "1", PIF outputs reset signal to respective
JoyChannel. This flag becomes effective without format flag.

NR=No Response to JoyChannel

When each JoyChannel's peripheral device does not respond, the respective NR bit
is set to "1". This is the way to detect the number of currently connected
peripheral devices.

Err=JoyChannel Error

When communication error has occurred between PIF and peripheral device, Err
flag is set to "1".

If the 64 bit CPU 100 wants to change JoyChannel's Tx/RxData assignment, a 32
bit format flag is used, where a certain bit(s) specify the desired format. For
example, when Wr64B or Wr4B is issued when this flag is "1", PIF executes each
JoyChannel's Tx/RxData assignment based on each channel's Tx/Rx Size. In other
words, unless this flag is set to "1" with Wr64B or Wr4B, Tx/RxData area
assignment does not change. After Tx/RxData assignment, this flag is reset to
"0" automatically.
*/
...
#define DPF_PIF(...)                                                           \
  {                                                                            \
    if (mDebugFile) {                                                          \
      fprintf(mDebugFile, __VA_ARGS__);                                        \
      fprintf(mDebugFile, "\n");                                               \
    }                                                                          \
  }
#else
...
IController::IController() : mpPifRam(nullptr), mpEepromData(nullptr) {
#ifdef DAEDALUS_DEBUG_PIF
  ...
  // Only one controller is enabled, this has to be revised once mltiplayer is
  // introduced
  mContPresent[0] = true;
}
...
bool IController::OnRomOpen() {
  ...
  if (save_type == SAVE_TYPE_EEP4K) {
    ...
    DBGConsole_Msg(0, "Initialising EEPROM to [M%d] bytes",
                   4096 / 8); // 4k bits
#endif
  ...
  for (u32 channel{}; channel < NUM_CONTROLLERS; channel++) {
    mMemPack[channel] =
        (u8 *)g_pMemoryBuffers[MEM_MEMPACK] + channel * 0x400 * 32;
  }
...
void IController::Process() {
  ...
  // Read controller data here (here gets called fewer times than
  // CONT_READ_CONTROLLER)
  ...
  while (count < 64) {
    ...
    // dummy data..
    if ((cmd[0] == CONT_TX_SIZE_DUMMYDATA) ||
        (cmd[0] == CONT_TX_SIZE_CHANRESET)) {
      count++;
      continue;
    }

#ifdef DAEDALUS_ENABLE_ASSERTS
    DAEDALUS_ASSERT((cmd[0] != 0xB4) || (cmd[0] != 0x56) || (cmd[0] != 0xB8),
                    "PIF : NOP command? %02x", cmd[0]);
#endif
    // next channel
    ...
    // 4 = eeprom channel
      ...
      DAEDALUS_ERROR("Trying to write from invalid controller channel! %d",
                     channel);
      break;
    }
#endif
  ...
  for (u32 x{}; x < 64; x += 8) {
    DPF_PIF("0x%02x%02x%02x%02x : 0x%02x%02x%02x%02x  |  0x%02x%02x%02x%02x : "
            "0x%02x%02x%02x%02x",
            mpInput[(x + 0)], mpInput[(x + 1)], mpInput[(x + 2)],
            mpInput[(x + 3)], mpInput[(x + 4)], mpInput[(x + 5)],
            mpInput[(x + 6)], mpInput[(x + 7)], mpPifRam[(x + 0)],
            mpPifRam[(x + 1)], mpPifRam[(x + 2)], mpPifRam[(x + 3)],
            mpPifRam[(x + 4)], mpPifRam[(x + 5)], mpPifRam[(x + 6)],
            mpPifRam[(x + 7)]);
  }
  DPF_PIF("");
  DPF_PIF("");
...
bool IController::ProcessController(u8 *cmd, u32 channel) {
  ...
  // From the patent, it says that if a controller is plugged in and the memory
  // card is removed, the CONT_CARD_PULL flag will be set. Cleared if CONT_RESET
  // or CONT_GET_STATUS is issued. Might need to set this if mContMemPackPresent
  // is false?
...
#ifdef DAEDALUS_ENABLE_ASSERTS
  DAEDALUS_ASSERT(IsEepromPresent(),
                  "ROM is accessing the EEPROM, but none is present");
#endif
  switch (cmd[2]) {
  case CONT_RESET:
  case CONT_GET_STATUS:
...
void IController::n64_cic_nus_6105() {
  ...
  char lut0[0x10] = {0x4, 0x7, 0xA, 0x7, 0xE, 0x5, 0xE, 0x1,
                     0xC, 0xF, 0x8, 0xF, 0x6, 0x3, 0x6, 0x9};
  char lut1[0x10] = {0x4, 0x1, 0xA, 0x7, 0xE, 0x5, 0xE, 0x1,
