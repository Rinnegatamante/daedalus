...
#else
#include <float.h>
#define DAEDALUS_128BIT_MULT // On PSP we only handle 64bit mults for
                             // performance
#endif

#ifdef DAEDALUS_POSIX
#include <fenv.h>
// Accurate cvt for W32/OSX, convert using the rounding mode specified in the
// Floating Control/Status register (FCSR)
...
#define R4300_CALL_MAKE_OP(var)                                                \
  OpCode var;                                                                  \
  (var)._u32 = op_code_bits
...
inline void SET_ROUND_MODE(ERoundingMode mode) {
  // This is very expensive on the PSP, so is disabled, also this is skipt in
  // the dynarec as well. Note: Do not enable this, since it causes conflicts in
  // the dynarec because we dont set rounding mode there. See Mario Party 3
  // TODO: Map 1:1 N64 and PSP round mode? perphaps write this in asm and just
  // fiddle with the PSP CTC1 opcode? pspFpuSetRoundmode( gNativeRoundingModes[
  // mode ] );
}
...
void SET_ROUND_MODE(ERoundingMode mode) {
  ...
  DAEDALUS_ERROR(
      "Floating point rounding modes not implemented on this platform");
#endif
}

#endif

// If the hardware doesn't support doubles in hardware - use 32 bits floats and
// accept the loss in precision
#ifdef SIM_DOUBLES
typedef f32 d64;
#else
typedef f64 d64;
...
DAEDALUS_FORCEINLINE void SpeedHack(u32 pc, u32 new_pc) {
  ...
  if (pc == new_pc) {
    ...
    if (next_op == 0) {
      // XXXX if we leave the counter at 1, then we always terminate traces with
      // a delay slot active. Need a more permenant fix to for this - i.e.
      // making tracing more robust.
      CPU_SkipToNextEvent();
    }
    ...
    /*else
    {
            ...
            if (!warned)
            {
                    DBGConsole_Msg(0, "Missed Speedhack 0x%08x",
    gCPUState.CurrentPC); warned = true;
            }
    }*/
  }
#endif
...
//		LWC1/LDC1/SWC1/SDC1		are unformatted, so no exceptions
//raised 		MTC1/MFC1/DMTC1/DMFC1	are unformatted, so no exceptions raised 		Word
//accessed must be 4 byte aligned, DWord accesses must be 8 byte aligned
//
...
// These ASM routines convert float to int and puts the value in CPU to sign
// extend, rather than FPU since the PSP doesn't have 64bit instructions //Corn
// These can be risky since the N64 is expecting float to int64 and thus float
// can be larger than int, this happens with trunc_w_s on the 4th level of DK64..
#ifdef DAEDALUS_ENABLE_ASSERTS
inline s32 cvt_w_s(f32 x) {
  DAEDALUS_ASSERT(x >= LONG_MIN && x <= LONG_MAX,
                  "Float too large, can't convert with 32bit PSP instruction");
  s32 r;
  asm volatile("cvt.w.s %1, %1\nmfc1 %0,%1\n" : "=r"(r) : "f"(x));
  return r;
}
inline s32 trunc_w_s(f32 x) {
  DAEDALUS_ASSERT(x >= LONG_MIN && x <= LONG_MAX,
                  "Float too large, can't convert with 32bit PSP instruction");
  s32 r;
  asm volatile("trunc.w.s %1, %1\nmfc1 %0,%1\n" : "=r"(r) : "f"(x));
  return r;
}
inline s32 round_w_s(f32 x) {
  DAEDALUS_ASSERT(x >= LONG_MIN && x <= LONG_MAX,
                  "Float too large, can't convert with 32bit PSP instruction");
  s32 r;
  asm volatile("round.w.s %1, %1\nmfc1 %0,%1\n" : "=r"(r) : "f"(x));
  return r;
}
inline s32 ceil_w_s(f32 x) {
  DAEDALUS_ASSERT(x >= LONG_MIN && x <= LONG_MAX,
                  "Float too large, can't convert with 32bit PSP instruction");
  s32 r;
  asm volatile("ceil.w.s  %1, %1\nmfc1 %0,%1\n" : "=r"(r) : "f"(x));
  return r;
}
inline s32 floor_w_s(f32 x) {
  DAEDALUS_ASSERT(x >= LONG_MIN && x <= LONG_MAX,
                  "Float too large, can't convert with 32bit PSP instruction");
  s32 r;
  asm volatile("floor.w.s %1, %1\nmfc1 %0,%1\n" : "=r"(r) : "f"(x));
  return r;
}
#else
inline s32 cvt_w_s(f32 x) {
...
// inline s64 f32_to_s64_trunc( f32 x )				{ return
// (s64)trunc_w_s( x ); }
...
DAEDALUS_FORCEINLINE s32 f32_to_s32_trunc(f32 x) {
  SET_ROUND_MODE(RM_TRUNC);
  return (s32)truncf(x);
}
DAEDALUS_FORCEINLINE s32 f32_to_s32_round(f32 x) {
  SET_ROUND_MODE(RM_ROUND);
  return (s32)roundf(x);
}
DAEDALUS_FORCEINLINE s32 f32_to_s32_ceil(f32 x) {
  SET_ROUND_MODE(RM_CEIL);
  return (s32)ceilf(x);
}
DAEDALUS_FORCEINLINE s32 f32_to_s32_floor(f32 x) {
  SET_ROUND_MODE(RM_FLOOR);
  return (s32)floorf(x);
}
DAEDALUS_FORCEINLINE s32 f32_to_s32(f32 x) {
  ...
  case FPCSR_RM_RN:
    return f32_to_s32_round(x);
  case FPCSR_RM_RZ:
    return f32_to_s32_trunc(x);
  case FPCSR_RM_RP:
    return f32_to_s32_ceil(x);
  case FPCSR_RM_RM:
    return f32_to_s32_floor(x);
  default:
    return (s32)x;
  }
#else
...
DAEDALUS_FORCEINLINE s64 f32_to_s64_trunc(f32 x) {
  SET_ROUND_MODE(RM_TRUNC);
  return (s64)truncf(x);
}
DAEDALUS_FORCEINLINE s64 f32_to_s64_round(f32 x) {
  SET_ROUND_MODE(RM_ROUND);
  return (s64)roundf(x);
}
DAEDALUS_FORCEINLINE s64 f32_to_s64_ceil(f32 x) {
  SET_ROUND_MODE(RM_CEIL);
  return (s64)ceilf(x);
}
DAEDALUS_FORCEINLINE s64 f32_to_s64_floor(f32 x) {
  SET_ROUND_MODE(RM_FLOOR);
  return (s64)floorf(x);
}
DAEDALUS_FORCEINLINE s64 f32_to_s64(f32 x) {
  ...
  switch (gCPUState.FPUControl[31]._u32 & FPCSR_RM_MASK) {
  case FPCSR_RM_RN:
    return f32_to_s64_round(x);
  case FPCSR_RM_RZ:
    return f32_to_s64_trunc(x);
  case FPCSR_RM_RP:
    return f32_to_s64_ceil(x);
  case FPCSR_RM_RM:
    return f32_to_s64_floor(x);
  default:
    return (s64)x;
  }
#else
...
DAEDALUS_FORCEINLINE s32 d64_to_s32_trunc(d64 x) {
  SET_ROUND_MODE(RM_TRUNC);
  return (s32)trunc(x);
}
DAEDALUS_FORCEINLINE s32 d64_to_s32_round(d64 x) {
  SET_ROUND_MODE(RM_ROUND);
  return (s32)round(x);
}
DAEDALUS_FORCEINLINE s32 d64_to_s32_ceil(d64 x) {
  SET_ROUND_MODE(RM_CEIL);
  return (s32)ceil(x);
}
DAEDALUS_FORCEINLINE s32 d64_to_s32_floor(d64 x) {
  SET_ROUND_MODE(RM_FLOOR);
  return (s32)floor(x);
}
DAEDALUS_FORCEINLINE s32 d64_to_s32(d64 x) {
  ...
  switch (gCPUState.FPUControl[31]._u32 & FPCSR_RM_MASK) {
  case FPCSR_RM_RN:
    return d64_to_s32_round(x);
  case FPCSR_RM_RZ:
    return d64_to_s32_trunc(x);
  case FPCSR_RM_RP:
    return d64_to_s32_ceil(x);
  case FPCSR_RM_RM:
    return d64_to_s32_floor(x);
  default:
    return (s32)x;
  }
#else
...
DAEDALUS_FORCEINLINE s64 d64_to_s64_trunc(d64 x) {
  SET_ROUND_MODE(RM_TRUNC);
  return (s64)trunc(x);
}
DAEDALUS_FORCEINLINE s64 d64_to_s64_round(d64 x) {
  SET_ROUND_MODE(RM_ROUND);
  return (s64)round(x);
}
DAEDALUS_FORCEINLINE s64 d64_to_s64_ceil(d64 x) {
  SET_ROUND_MODE(RM_CEIL);
  return (s64)ceil(x);
}
DAEDALUS_FORCEINLINE s64 d64_to_s64_floor(d64 x) {
  SET_ROUND_MODE(RM_FLOOR);
  return (s64)floor(x);
}
DAEDALUS_FORCEINLINE s64 d64_to_s64(d64 x) {
  ...
  switch (gCPUState.FPUControl[31]._u32 & FPCSR_RM_MASK) {
  case FPCSR_RM_RN:
    return d64_to_s64_round(x);
  case FPCSR_RM_RZ:
    return d64_to_s64_trunc(x);
  case FPCSR_RM_RP:
    return d64_to_s64_ceil(x);
  case FPCSR_RM_RM:
    return d64_to_s64_floor(x);
  default:
    return (s64)x;
  }
#else
...
bool R4300_InstructionHandlerNeedsPC(OpCode op_code) {
  switch (op_code.op) {
  // FIXME: These can potentially trow if memory is accessed through a function
  // call?! //Salvy
  case OP_LWL:
  case OP_SWL:
  case OP_LWR:
  ...
  case OP_SPECOP:
    ...
    case SpecOp_MULT:
    case SpecOp_MULTU:
    case SpecOp_DIV:  // Need to remove this if we can throw exception on divide
                      // by 0
...
void R4300_CALL_TYPE R4300_SetSR(u32 new_value) {
  ...
  if ((gCPUState.CPUControl[C0_SR]._u32 & SR_FR) != (new_value & SR_FR)) {
    DBGConsole_Msg(0, "[MChanging FP to %s, STATUS=%08X",
                   (new_value & SR_FR) ? "64bit" : "32bit",
                   (new_value & SR_FR));
  }
#endif

  bool interrupts_enabled_before =
      (gCPUState.CPUControl[C0_SR]._u32 & SR_IE) != 0;

  gCPUState.CPUControl[C0_SR]._u32 = new_value;

  bool interrupts_enabled_after =
      (gCPUState.CPUControl[C0_SR]._u32 & SR_IE) != 0;
  ...
  if (!interrupts_enabled_before && interrupts_enabled_after) {
    if (gCPUState.CPUControl[C0_SR]._u32 & gCPUState.CPUControl[C0_CAUSE]._u32 &
        CAUSE_IPMASK) {
...
#define WARN_NOEXIST(inf)                                                      \
  { DAEDALUS_ASSERT(false, "Instruction Unknown"); }
#define WARN_NOIMPL(op)                                                        \
  { DAEDALUS_ASSERT(false, "Instruction Not Implemented"); }
#else
#define WARN_NOEXIST(inf)                                                      \
  {}
#define WARN_NOIMPL(op)                                                        \
...
static void R4300_CALL_TYPE R4300_Unk(R4300_CALL_SIGNATURE) {
  WARN_NOEXIST("R4300_Unk");
}
static void R4300_CALL_TYPE R4300_CoPro1_Disabled(R4300_CALL_SIGNATURE)
...
// Cop1 Unusable
  ...
  DAEDALUS_ASSERT((gCPUState.CPUControl[C0_SR]._u32 & SR_CU1) == 0,
                  "COP1 usable flag in inconsistant state!");
#endif
  R4300_Exception_CopUnusuable();
}

// These are the only unimplemented R4300 instructions now: Will be implemented
// someday..
...
static void R4300_CALL_TYPE
R4300_BNE(R4300_CALL_SIGNATURE) // Branch on Not Equal
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE
R4300_BLEZ(R4300_CALL_SIGNATURE) // Branch on Less than of Equal to Zero
{
  R4300_CALL_MAKE_OP(op_code);
  // branch if rs <= 0
...
static void R4300_CALL_TYPE
R4300_BGTZ(R4300_CALL_SIGNATURE) // Branch on Greater than Zero
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE
R4300_DADDI(R4300_CALL_SIGNATURE) // Doubleword ADD Immediate
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE
R4300_DADDIU(R4300_CALL_SIGNATURE) // Doubleword ADD Immediate Unsigned
{
  R4300_CALL_MAKE_OP(op_code);
  gGPR[op_code.rt]._s64 = gGPR[op_code.rs]._s64 + (s32)(s16)op_code.immediate;
}
...
static void R4300_CALL_TYPE R4300_ADDI(R4300_CALL_SIGNATURE) {
  R4300_CALL_MAKE_OP(op_code);
  gGPR[op_code.rt]._s64 =
      (s64)(s32)(gGPR[op_code.rs]._s32_0 + (s32)(s16)op_code.immediate);
}

static void R4300_CALL_TYPE
R4300_ADDIU(R4300_CALL_SIGNATURE) // Add Immediate Unsigned
{
  R4300_CALL_MAKE_OP(op_code);
  gGPR[op_code.rt]._s64 =
      (s64)(s32)(gGPR[op_code.rs]._s32_0 + (s32)(s16)op_code.immediate);
...
static void R4300_CALL_TYPE
R4300_SLTI(R4300_CALL_SIGNATURE) // Set on Less Than Immediate
{
  R4300_CALL_MAKE_OP(op_code);
...
#if 1 // 1->default, 0->"hack" that speeds up SM64 when sound is off (just for
      // reference) //Salvy
...
static void R4300_CALL_TYPE
R4300_SLTIU(R4300_CALL_SIGNATURE) // Set on Less Than Immediate Unsigned
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE
R4300_LUI(R4300_CALL_SIGNATURE) // Load Upper Immediate
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE
R4300_BEQL(R4300_CALL_SIGNATURE) // Branch on Equal Likely
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE
R4300_BNEL(R4300_CALL_SIGNATURE) // Branch on Not Equal Likely
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE
R4300_BLEZL(R4300_CALL_SIGNATURE) // Branch on Less than or Equal to Zero Likely
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE
R4300_BGTZL(R4300_CALL_SIGNATURE) // Branch on Greater than Zero Likely
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE
R4300_LBU(R4300_CALL_SIGNATURE) // Load Byte Unsigned -- Zero extend byte...
{
  R4300_CALL_MAKE_OP(op_code);
#ifdef DAEDALUS_DEBUG_CONSOLE
...
static void R4300_CALL_TYPE
R4300_LHU(R4300_CALL_SIGNATURE) // Load Halfword Unsigned -- Zero extend word
{
  R4300_CALL_MAKE_OP(op_code);
#ifdef DAEDALUS_DEBUG_CONSOLE
...
static void R4300_CALL_TYPE R4300_LWL(R4300_CALL_SIGNATURE) // Load Word Left
{
  ...
  case 0:
    ...
    break;
  case 1:
    nReg = ((nReg & 0x000000FF) | (nMem << 8));
    break;
  case 2:
    nReg = ((nReg & 0x0000FFFF) | (nMem << 16));
    break;
  case 3:
    nReg = ((nReg & 0x00FFFFFF) | (nMem << 24));
    break;
  }
#endif
...
static void R4300_CALL_TYPE R4300_LDL(R4300_CALL_SIGNATURE) {
  ...
  case 0:
    ...
    break;
  case 1:
    nReg = ((nReg & 0x00000000000000FFLL) | (nMem << 8));
    break;
  case 2:
    nReg = ((nReg & 0x000000000000FFFFLL) | (nMem << 16));
    break;
  case 3:
    nReg = ((nReg & 0x0000000000FFFFFFLL) | (nMem << 24));
    break;
  case 4:
    nReg = ((nReg & 0x00000000FFFFFFFFLL) | (nMem << 32));
    break;
  case 5:
    nReg = ((nReg & 0x000000FFFFFFFFFFLL) | (nMem << 40));
    break;
  case 6:
    nReg = ((nReg & 0x0000FFFFFFFFFFFFLL) | (nMem << 48));
    break;
  case 7:
    nReg = ((nReg & 0x00FFFFFFFFFFFFFFLL) | (nMem << 56));
    break;
  }
#endif
...
#if 1 // 1-> tighter code, 0->old way //Corn
  nReg = (nReg & (~0 << (((address & 0x3) + 1) << 3))) |
         (nMem >> ((~address & 0x3) << 3));
#else
  switch (address % 4) {
  case 0:
    nReg = (nReg & 0xFFFFFF00) | (nMem >> 24);
    break;
  case 1:
    nReg = (nReg & 0xFFFF0000) | (nMem >> 16);
    break;
  case 2:
    nReg = (nReg & 0xFF000000) | (nMem >> 8);
    break;
  case 3:
    nReg = nMem;
    break;
...
#if 1 // 1-> tighter code, 0->old way //Corn
  nReg = (nReg & (~0LL << (((address & 0x7) + 1) << 3))) |
         (nMem >> ((~address & 0x7) << 3));
#else
  switch (address % 8) {
  case 0:
    nReg = (nReg & 0xFFFFFFFFFFFFFF00LL) | (nMem >> 56);
    break;
  case 1:
    nReg = (nReg & 0xFFFFFFFFFFFF0000LL) | (nMem >> 48);
    break;
  case 2:
    nReg = (nReg & 0xFFFFFFFFFF000000LL) | (nMem >> 40);
    break;
  case 3:
    nReg = (nReg & 0xFFFFFFFF00000000LL) | (nMem >> 32);
    break;
  case 4:
    nReg = (nReg & 0xFFFFFF0000000000LL) | (nMem >> 24);
    break;
  case 5:
    nReg = (nReg & 0xFFFF000000000000LL) | (nMem >> 16);
    break;
  case 6:
    nReg = (nReg & 0xFF00000000000000LL) | (nMem >> 8);
    break;
  case 7:
    nReg = nMem;
    break;
...
static void R4300_CALL_TYPE
R4300_LWU(R4300_CALL_SIGNATURE) // Load Word Unsigned
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE R4300_SWL(R4300_CALL_SIGNATURE) // Store Word Left
{
...
#if 1 // 1-> tighter code, 0->old way //Corn
  u32 dNew{(dMem & ~(((u32)~0 >> ((address & 0x3) << 3)))) |
           (dReg >> ((address & 0x3) << 3))};
#else
  u32 dNew{};
  switch (address % 4) {
  case 0:
    ...
    break; // Aligned
  case 1:
    dNew = (dMem & 0xFF000000) | (dReg >> 8);
    break;
  case 2:
    dNew = (dMem & 0xFFFF0000) | (dReg >> 16);
    break;
  default:
    dNew = (dMem & 0xFFFFFF00) | (dReg >> 24);
    break;
  }
#endif
...
#if 1 // 1-> tighter code, 0->old way //Corn
  u32 dNew{(dMem & ~(~0 << ((~address & 0x3) << 3))) |
           (dReg << ((~address & 0x3) << 3))};
#else
  u32 dNew{};
  switch (address % 4) {
  case 0:
    dNew = (dMem & 0x00FFFFFF) | (dReg << 24);
    break;
  case 1:
    dNew = (dMem & 0x0000FFFF) | (dReg << 16);
    break;
  case 2:
    dNew = (dMem & 0x000000FF) | (dReg << 8);
    break;
  default:
    dNew = dReg;
    break; // Aligned
...
static void R4300_CALL_TYPE R4300_SDL(R4300_CALL_SIGNATURE) // CYRUS64
{
  ...
  u64 nReg{gGPR[op_code.rt]._u64};
  u64 nNew = (nMem & ~(((u64)~0LL >> ((address & 0x7) << 3)))) |
             (nReg >> ((address & 0x7) << 3));
...
static void R4300_CALL_TYPE R4300_SDR(R4300_CALL_SIGNATURE) // CYRUS64
{
  ...
  u64 nMem{QuickRead64Bits(base, 0x0)};
  u64 nReg{gGPR[op_code.rt]._u64};
  u64 nNew = (nMem & ~(~0LL << ((~address & 0x7) << 3))) |
             (nReg << ((~address & 0x7) << 3));
  QuickWrite64Bits(base, 0x0, nNew);
}
...
static void R4300_CALL_TYPE R4300_CACHE(R4300_CALL_SIGNATURE) {
  ...
  // DBGConsole_Msg(0, "CACHE %s/%d, 0x%08x", gCacheNames[dwCache], dwAction,
  // address);
#endif
}

static void R4300_CALL_TYPE
R4300_LWC1(R4300_CALL_SIGNATURE) // Load Word to Copro 1 (FPU)
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE
R4300_LDC1(R4300_CALL_SIGNATURE) // Load Doubleword to Copro 1 (FPU)
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE
R4300_SWC1(R4300_CALL_SIGNATURE) // Store Word From Copro 1
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE
R4300_SDC1(R4300_CALL_SIGNATURE) // Store Doubleword From Copro 1
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE R4300_Special_Unk(R4300_CALL_SIGNATURE) {
  WARN_NOEXIST("R4300_Special_Unk");
}
static void R4300_CALL_TYPE
R4300_Special_SLL(R4300_CALL_SIGNATURE) // Shift word Left Logical
{
  R4300_CALL_MAKE_OP(op_code);
  ...
  gGPR[op_code.rd]._s64 =
      (s64)(s32)((gGPR[op_code.rt]._u32_0 << op_code.sa) & 0xFFFFFFFF);
}

static void R4300_CALL_TYPE
R4300_Special_SRL(R4300_CALL_SIGNATURE) // Shift word Right Logical
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE
R4300_Special_SRA(R4300_CALL_SIGNATURE) // Shift word Right Arithmetic
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE
R4300_Special_SLLV(R4300_CALL_SIGNATURE) // Shift word Left Logical Variable
{
  R4300_CALL_MAKE_OP(op_code);
  gGPR[op_code.rd]._s64 =
      (s64)(s32)((gGPR[op_code.rt]._u32_0 << (gGPR[op_code.rs]._u32_0 & 0x1F)) &
                 0xFFFFFFFF);
}

static void R4300_CALL_TYPE
R4300_Special_SRLV(R4300_CALL_SIGNATURE) // Shift word Right Logical Variable
{
  R4300_CALL_MAKE_OP(op_code);
  gGPR[op_code.rd]._s64 =
      (s64)(s32)(gGPR[op_code.rt]._u32_0 >> (gGPR[op_code.rs]._u32_0 & 0x1F));
...
static void R4300_CALL_TYPE
R4300_Special_SRAV(R4300_CALL_SIGNATURE) // Shift word Right Arithmetic Variable
{
  R4300_CALL_MAKE_OP(op_code);
  gGPR[op_code.rd]._s64 =
      (s64)(s32)(gGPR[op_code.rt]._s32_0 >> (gGPR[op_code.rs]._u32_0 & 0x1F));
}

static void R4300_CALL_TYPE
R4300_Special_JR(R4300_CALL_SIGNATURE) // Jump Register
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE
R4300_Special_JALR(R4300_CALL_SIGNATURE) // Jump and Link register
{
  R4300_CALL_MAKE_OP(op_code);
  ...
  u32 new_pc(gGPR[op_code.rs]._u32_0);
  gGPR[op_code.rd]._s64 =
      (s64)(s32)(gCPUState.CurrentPC + 8); // Store return address;
  CPU_TakeBranch(new_pc);
}
...
static void R4300_CALL_TYPE R4300_Special_BREAK(R4300_CALL_SIGNATURE) // BREAK
{
  ...
  DPF(DEBUG_INTR, "BREAK Called. PC: 0x%08x. COUNT: 0x%08x",
      gCPUState.CurrentPC, gCPUState.CPUControl[C0_COUNT]._u32);
#endif
  R4300_Exception_Break();
}

static void
    R4300_CALL_TYPE R4300_Special_SYNC(R4300_CALL_SIGNATURE) { /* Just ignore */
}

static void R4300_CALL_TYPE
R4300_Special_MFHI(R4300_CALL_SIGNATURE) // Move From MultHI
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE
R4300_Special_MTHI(R4300_CALL_SIGNATURE) // Move To MultHI
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE
R4300_Special_MFLO(R4300_CALL_SIGNATURE) // Move From MultLO
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE
R4300_Special_MTLO(R4300_CALL_SIGNATURE) // Move To MultLO
{
  R4300_CALL_MAKE_OP(op_code);
...
// BEGIN MODIFIED BY Lkb - 8/jun/2001 - changed 0x1f to 0x3f because the value
// to be shifted is 64-bit long
static void R4300_CALL_TYPE R4300_Special_DSLLV(R4300_CALL_SIGNATURE) {
  R4300_CALL_MAKE_OP(op_code);

  // Reserved Instruction exception
  gGPR[op_code.rd]._u64 = gGPR[op_code.rt]._u64
                          << (gGPR[op_code.rs]._u32_0 & 0x3F);
}
...
static void R4300_CALL_TYPE R4300_Special_DSRLV(R4300_CALL_SIGNATURE) {
  ...
  // Reserved Instruction exception
  gGPR[op_code.rd]._u64 =
      gGPR[op_code.rt]._u64 >> (gGPR[op_code.rs]._u32_0 & 0x3F);
}
...
static void R4300_CALL_TYPE R4300_Special_DSRAV(R4300_CALL_SIGNATURE) {
  ...
  // Reserved Instruction exception
  gGPR[op_code.rd]._u64 =
      gGPR[op_code.rt]._s64 >> (gGPR[op_code.rs]._u32_0 & 0x3F);
}

static void R4300_CALL_TYPE
R4300_Special_MULT(R4300_CALL_SIGNATURE) // MULTiply Signed
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE
R4300_Special_MULTU(R4300_CALL_SIGNATURE) // MULTiply Unsigned
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE
R4300_Special_DIVU(R4300_CALL_SIGNATURE) // DIVide Unsigned
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE
R4300_Special_DMULT(R4300_CALL_SIGNATURE) // Double Multiply
{
  R4300_CALL_MAKE_OP(op_code);
  // Reserved Instruction exception
...
static void R4300_CALL_TYPE
R4300_Special_DMULTU(R4300_CALL_SIGNATURE) // Double Multiply Unsigned
{
  R4300_CALL_MAKE_OP(op_code);
  // Reserved Instruction exception
...
static void R4300_CALL_TYPE
R4300_Special_DDIVU(R4300_CALL_SIGNATURE) // Double Divide Unsigned
{
  R4300_CALL_MAKE_OP(op_code);

  // Check if this operation can be done in 32bit rather than 64bit //Corn
  if ((gGPR[op_code.rs]._u32_1 | gGPR[op_code.rt]._u32_1) == 0) { // 32bit
...
static void R4300_CALL_TYPE
R4300_Special_ADD(R4300_CALL_SIGNATURE) // ADD signed - may throw exception
{
  R4300_CALL_MAKE_OP(op_code);

  // Can generate overflow exception
  gGPR[op_code.rd]._s64 =
      (s64)(s32)(gGPR[op_code.rs]._s32_0 + gGPR[op_code.rt]._s32_0);
}

static void R4300_CALL_TYPE R4300_Special_ADDU(
    R4300_CALL_SIGNATURE) // ADD Unsigned - doesn't throw exception
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE
R4300_Special_SUB(R4300_CALL_SIGNATURE) // SUB Signed - may throw exception
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE R4300_Special_SUBU(
    R4300_CALL_SIGNATURE) // SUB Unsigned - doesn't throw exception
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE
R4300_Special_AND(R4300_CALL_SIGNATURE) // logical AND
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE
R4300_Special_XOR(R4300_CALL_SIGNATURE) // logical XOR
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE
R4300_Special_NOR(R4300_CALL_SIGNATURE) // logical Not OR
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE
R4300_Special_SLT(R4300_CALL_SIGNATURE) // Set on Less Than
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE
R4300_Special_SLTU(R4300_CALL_SIGNATURE) // Set on Less Than Unsigned
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE R4300_Special_DADDU(R4300_CALL_SIGNATURE) // CYRUS64
{
  ...
  // I don't know why Excite bike only works if the operand is 32bit.. this for
  // sure is wrong as docs say. Also this causes Conker to fail to display a
  // cutscene in the final boss!
  // gGPR[ op_code.rd ]._s64 = (s64)( gGPR[ op_code.rt ]._s32_0 + gGPR[
  // op_code.rs ]._s32_0 );
}
...
static void R4300_CALL_TYPE R4300_Special_DSUBU(R4300_CALL_SIGNATURE) {
  ...
  // It caused several lock ups in games ex Animal Crossing, and Conker to crash
  // in last boss Also signed extended to be safe
  gGPR[op_code.rd]._u64 = gGPR[op_code.rs]._u64 - gGPR[op_code.rt]._u64;
}
...
static void R4300_CALL_TYPE
R4300_Special_DSLL32(R4300_CALL_SIGNATURE) // Double Shift Left Logical 32
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE
R4300_Special_DSRL32(R4300_CALL_SIGNATURE) // Double Shift Right Logical 32
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE
R4300_Special_DSRA32(R4300_CALL_SIGNATURE) // Double Shift Right Arithmetic 32
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE R4300_RegImm_Unk(R4300_CALL_SIGNATURE) {
  WARN_NOEXIST("R4300_RegImm_Unk");
}

static void R4300_CALL_TYPE
R4300_RegImm_BLTZ(R4300_CALL_SIGNATURE) // Branch on Less than Zero
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE
R4300_RegImm_BLTZL(R4300_CALL_SIGNATURE) // Branch on Less than Zero Likely
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE
R4300_RegImm_BLTZAL(R4300_CALL_SIGNATURE) // Branch on Less than Zero And Link
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE R4300_RegImm_BGEZ(
    R4300_CALL_SIGNATURE) // Branch on Greater than or Equal to Zero
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE R4300_RegImm_BGEZL(
    R4300_CALL_SIGNATURE) // Branch on Greater than or Equal to Zero Likely
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE R4300_RegImm_BGEZAL(
    R4300_CALL_SIGNATURE) // Branch on Greater than or Equal to Zero And Link
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE R4300_Cop0_Unk(R4300_CALL_SIGNATURE) {
  WARN_NOEXIST("R4300_Cop0_Unk");
}
static void R4300_CALL_TYPE R4300_TLB_Unk(R4300_CALL_SIGNATURE) {
  WARN_NOEXIST("R4300_TLB_Unk");
}
...
static void R4300_CALL_TYPE R4300_Cop0_MFC0(R4300_CALL_SIGNATURE) {
  ...
  if (op_code.fs == C0_CAUSE) {
    bool mi_interrupt_set{(Memory_MI_GetRegister(MI_INTR_MASK_REG) &
                           Memory_MI_GetRegister(MI_INTR_REG)) != 0};
    bool cause_int_3_set(
        {(gCPUState.CPUControl[C0_CAUSE]._u32 & CAUSE_IP3) != 0});

    DAEDALUS_ASSERT(mi_interrupt_set == cause_int_3_set,
                    "CAUSE_IP3 inconsistant with MI_INTR_REG");
...
// All these registers are read only - make sure that software doesn't write to
// them
#ifdef DAEDALUS_DEBUG_CONSOLE
    DBGConsole_Msg(
        0, "MTC0. Software attempted to write to read only reg %s: 0x%08x",
        Cop0RegNames[op_code.fs], new_value);
#endif
    break;
...
#endif
#ifdef DAEDALUS_DEBUG_CONSOLE
    if ((new_value & ~(CAUSE_SW1 | CAUSE_SW2)) !=
        (gCPUState.CPUControl[C0_CAUSE]._u32 & ~(CAUSE_SW1 | CAUSE_SW2))) {
      DBGConsole_Msg(0, "[MWas previously clobbering CAUSE REGISTER");
    }
#endif
#ifdef DAEDALUS_PROFILER
    DPF(DEBUG_REGS, "CAUSE set to 0x%08x (was: 0x%08x)", new_value,
        gGPR[op_code.rt]._u32_0);
#endif

    gCPUState.CPUControl[C0_CAUSE]._u32 &= ~(CAUSE_SW1 | CAUSE_SW2);
    gCPUState.CPUControl[C0_CAUSE]._u32 |=
        (new_value & (CAUSE_SW1 | CAUSE_SW2));
    break;
  case C0_SR:
    // Software can enable/disable interrupts here. We check if Interrupt Enable
    // is
    //  set, and if there are any pending interrupts. If there are, then we set
    //  the CHECK_POSTPONED_INTERRUPTS flag to make sure we check for interrupts
    //  that have occurred since we disabled interrupts
    R4300_SetSR(new_value);
    break;

  case C0_COUNT: {
    // See comments below for COMPARE.
    // When this register is set, we need to check whether the next timed
    // interrupt will
  ...
  case C0_COMPARE: {
    // When the value of COUNT equals the value of COMPARE, IP7 of the CAUSE
    // register is set (Timer interrupt). Writing to this register clears the
    ...
    // WatchHi/WatchLo are used to create a Watch Trap. This may not need
    // implementing, but we should Probably provide a warning on writes, just so
    // that we know
    // case C0_WATCHLO:
...
static void R4300_CALL_TYPE R4300_TLB_TLBR(R4300_CALL_SIGNATURE) // TLB Read
{
  ...
  gCPUState.CPUControl[C0_ENTRYHI]._u32 =
      g_TLBs[index].hi & (~g_TLBs[index].pagemask);
...
#ifdef DAEDALUS_PROFILER
  DPF(DEBUG_TLB, "TLBR: INDEX: 0x%04x. PAGEMASK: 0x%08x.", index,
      gCPUState.CPUControl[C0_PAGEMASK]._u32);
  DPF(DEBUG_TLB, "      ENTRYHI: 0x%08x. ENTRYLO1: 0x%08x. ENTRYLO0: 0x%08x",
      gCPUState.CPUControl[C0_ENTRYHI]._u32,
...
static void R4300_CALL_TYPE
R4300_TLB_TLBWI(R4300_CALL_SIGNATURE) // TLB Write Index
{
...
static void R4300_CALL_TYPE R4300_TLB_TLBP(R4300_CALL_SIGNATURE) // TLB Probe
{
  ...
  for (u32 i{}; i < 32; i++) {
    if (((g_TLBs[i].hi & TLBHI_VPN2MASK) == (entryH & TLBHI_VPN2MASK)) &&
        ((g_TLBs[i].g) ||
...
static void R4300_CALL_TYPE R4300_TLB_ERET(R4300_CALL_SIGNATURE) {
  ...
  // Point to previous instruction (as we increment the pointer immediately
  // afterwards
  DECREMENT_PC();
...
static void R4300_CALL_TYPE R4300_Cop1_Unk(R4300_CALL_SIGNATURE) {
  WARN_NOEXIST("R4300_Cop1_Unk");
}
...
static void R4300_CALL_TYPE
R4300_Cop1_CFC1(R4300_CALL_SIGNATURE) // move Control word From Copro 1
{
  R4300_CALL_MAKE_OP(op_code);
  ...
  // if( !((op_code.fs + 1) & 0x1E) )
  {
    ...
    // gGPR[ op_code.rt ]._s32_0 = gCPUState.FPUControl[ op_code.fs ]._s32;
    // //copy only low part
  }
}

static void R4300_CALL_TYPE
R4300_Cop1_CTC1(R4300_CALL_SIGNATURE) // move Control word To Copro 1
{
  R4300_CALL_MAKE_OP(op_code);
...
// Fixes collision issues in the final boss of DK64 and camera icon not
// rotating, fixes collision issues in Rayman, and JFG too
#ifdef DAEDALUS_PSP
...
static void R4300_CALL_TYPE
R4300_BC1_BC1F(R4300_CALL_SIGNATURE) // Branch on FPU False
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE
R4300_BC1_BC1T(R4300_CALL_SIGNATURE) // Branch on FPU True
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE
R4300_BC1_BC1FL(R4300_CALL_SIGNATURE) // Branch on FPU False Likely
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE
R4300_BC1_BC1TL(R4300_CALL_SIGNATURE) // Branch on FPU True Likely
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE R4300_Cop1_S_Unk(R4300_CALL_SIGNATURE) {
  WARN_NOEXIST("R4300_Cop1_S_Unk");
}
...
// Not sure if is worth to handle this, I have yet to see a game that fails due
// this..
#ifdef DAEDALUS_ENABLE_ASSERTS
  ...
  /*if ( fDivisor == 0 )
  {
          ...
          {
                  // Exception
                  DBGConsole_Msg( 0, "[MShould trigger FPU exception for /0
  here" );
          }
          else
          {
...
// Used by Mario Party Draft mini game, Earth Worm Jim, Tom and Jerry, Power
// Puff Girls' disable esimulate double hack Convert float to double...
#ifdef SIM_DOUBLES
...
static void R4300_CALL_TYPE
R4300_Cop1_S_EQ(R4300_CALL_SIGNATURE) // Compare for Equality
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE
R4300_Cop1_S_LT(R4300_CALL_SIGNATURE) // Compare for Equality
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE
R4300_Cop1_S_LE(R4300_CALL_SIGNATURE) // Compare for Equality
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE R4300_Cop1_D_Unk(R4300_CALL_SIGNATURE) {
  WARN_NOEXIST("R4300_Cop1_D_Unk");
}
...
static void R4300_CALL_TYPE
R4300_Cop1_D_EQ(R4300_CALL_SIGNATURE) // Compare for Equality
{
  R4300_CALL_MAKE_OP(op_code);
...
static void R4300_CALL_TYPE
R4300_Cop1_D_LE(R4300_CALL_SIGNATURE) // Compare for Less Than or Equal
{
  R4300_CALL_MAKE_OP(op_code);
...
// Used to swap functions(apply hacks) in interpreter mode (used for the PSP
// only)
void R4300_Init() {
#ifdef SIM_DOUBLES
  ...
  // Mario Party Draft mini game, Earth Worm Jim, Tom and Jerry, Power Puff
  // Girls
