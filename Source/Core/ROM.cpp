...
// The "Header" is actually something to do with the PI_DOM_*_OFS values...
  ...
  DBGConsole_Msg(0, "Header:          0x%02x%02x%02x%02x", header.x1, header.x2,
                 header.x3, header.x4);
  ...
  DBGConsole_Msg(0, "CountryID:       0x%02x - '%c'", header.CountryID,
                 (char)header.CountryID);
...
static void ROM_SimulatePIFBoot(ECicType cic_chip, u32 Country) {
  ...
  RomBuffer::GetRomBytesRaw(
      (u8 *)g_pMemoryBuffers[MEM_SP_MEM] + RAMROM_BOOTSTRAP_OFFSET,
      RAMROM_BOOTSTRAP_OFFSET, RAMROM_GAME_OFFSET - RAMROM_BOOTSTRAP_OFFSET);
  ...
  //
  // gCPUState.CPUControl[C0_SR]		= 0x34000000;	//*SR_FR |*/ SR_ERL |
...
bool ROM_ReBoot() {
  ...
  if (g_ROM.cic_chip == CIC_UNKNOWN) {
    ...
    /* goodn64 already tell us if the rom is good or bad
       ...
       || crc2 != RomBuffer::ReadValueRaw< u32 >( 0x14 ))
                            {
                                    DBGConsole_Msg(0, "[MWarning, CRC values
       don't match, fixing]");
                    ...
                    {
                            // Unable to checksum - just continue with what we
       have
                    }
    */
  }
  ...
  // This will always return false since we always simulate boot code instead of
  // loading from file
  //
...
// Most hacks are for the PSP, due the limitations of the hardware, and because
...
void SpecificGameHacks(const ROMHeader &id) {
  ...
  case 0x4154:
    ...
    break;
  case 0x4643:
    g_ROM.GameHacks = CLAY_FIGHTER_63;
    break;
  case 0x504A:
...
bool ROM_LoadFile() {
  ...
  if (ROM_GetRomDetailsByFilename(g_ROM.mFileName, &rom_id, &rom_size,
                                  &boot_type)) {
...
bool ROM_LoadFile(const RomID &rom_id, const RomSettings &settings,
  ...
  // But we do this when ROM is loaded too, to allow any forced enabled cheats
  // to work.
  if (gCheatsEnabled) {
    CheatCodes_Read(g_ROM.settings.GameName.c_str(), "Daedalus.cht",
                    g_ROM.rh.CountryID);
  }
  ...
  DBGConsole_Msg(0, "This game has been certified as [G%s] (%s)",
                 g_ROM.settings.Comment.c_str(), g_ROM.settings.Info.c_str());
  DBGConsole_Msg(0, "SaveType: [G%s]",
                 ROM_GetSaveTypeName(g_ROM.settings.SaveType));
  DBGConsole_Msg(0, "ApplyPatches: [G%s]", gOSHooksEnabled ? "on" : "off");
  DBGConsole_Msg(0, "Check Texture Hash Freq: [G%d]",
...
bool ROM_GetRomDetailsByFilename(const char *filename, RomID *id, u32 *rom_size,
                                 ECicType *boot_type) {
  return CRomDB::Get()->QueryByFilename(filename, id, rom_size, boot_type);
}

bool ROM_GetRomDetailsByID(const RomID &id, u32 *rom_size,
                           ECicType *boot_type) {
...
static const CountryIDInfo g_CountryCodeInfo[] = {
    {0, "0", OS_TV_NTSC},          {'7', "Beta", OS_TV_NTSC},
    {'A', "NTSC", OS_TV_NTSC},     {'D', "Germany", OS_TV_PAL},
    {'E', "USA", OS_TV_NTSC},      {'F', "France", OS_TV_PAL},
    {'I', "Italy", OS_TV_PAL},     {'J', "Japan", OS_TV_NTSC},
    {'P', "Europe", OS_TV_PAL},    {'S', "Spain", OS_TV_PAL},
    {'U', "Australia", OS_TV_PAL}, {'X', "PAL", OS_TV_PAL},
    {'Y', "PAL", OS_TV_PAL}};
