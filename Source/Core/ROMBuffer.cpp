...
#ifdef DAEDALUS_COMPRESSED_ROM_SUPPORT
ROMFile *DecompressRom(ROMFile *p_rom_file, const char *temp_filename,
                       COutputStream &messages) {
  ROMFile *p_new_file(nullptr);
  FILE *fh(fopen(temp_filename, "wb"));

  if (fh == nullptr) {
    messages << "Unable to create temporary rom '" << temp_filename
             << "' for decompression\n";
  } else {
    bool failed = false;
    ...
    while (length_remaining > 0) {
#ifdef DAEDALUS_DEBUG_CONSOLE
      if ((offset % 0x8000) == 0) {
        CDebugConsole::Get()->MsgOverwrite(0, "Converted [M%d / %d] KB",
      ...
      if (fwrite(p_temp_buffer, 1, length_to_process, fh) !=
          length_to_process) {
        failed = true;
        break;
      }
    ...
    if (failed) {
      messages << "Failed to decompress rom to '" << temp_filename
               << "' - out of disk space?\n";
    } else {
      //
      ...
      if (p_new_file == nullptr) {
        messages << "Failed to open temporary rom '" << temp_filename
                 << "' we just created\n";
      } else {
...
bool RomBuffer::Open() {
  ...
  if (ShouldLoadAsFixed(sRomSize)) {
    ...
    while (offset < sRomSize) {
      ...
      intraFontPrintf(ltn8, 480 / 2, (272 >> 1), "Buffering ROM %d%%...",
                      offset * 100 / sRomSize);
    ...
    if (DECOMPRESS_ROMS) {
      ...
      if (compressed) // || byteswapped)
      {
        ...
        DBGConsole_Msg(0,
                       "Decompressing rom to [C%s] (this may take some time)",
                       temp_filename);
#endif
        CNullOutputStream local_messages;

        ROMFile *p_new_file =
            DecompressRom(p_rom_file, temp_filename, local_messages);
#ifdef DAEDALUS_DEBUG_CONSOLE
        ...
        if (p_new_file != nullptr) {
#ifdef DAEDALUS_DEBUG_CONSOLE
          DBGConsole_Msg(
              0, "Decompression [gsuccessful]. Booting using decompressed rom");
#endif
          delete p_rom_file;
          ...
          DBGConsole_Msg(0,
                         "Decompression [rfailed]. Booting using original rom");
        }
#endif
      }
    }
...
namespace {
void CopyBytesRaw(ROMFileCache *p_cache, u8 *p_dst, u32 rom_offset,
                  u32 length) {
  ...
  while (length > 0) {
    ...
    if (!p_cache->GetChunk(src_offset, &p_chunk_base, &chunk_offset,
                           &chunk_size)) {
      // Out of range
      break;
    }
    ...
    DAEDALUS_ASSERT(s32(bytes_this_pass) > 0,
                    "How come we're trying to copy <= 0 bytes across?");
#endif
    // Copy this chunk across
    memcpy(p_dst + dst_offset, p_chunk_base + offset_into_chunk,
           bytes_this_pass);

    // Update the src/dst pointers and reduce length by the number of copied
    // bytes
...
#ifdef DAEDALUS_ENABLE_ASSERTS
    DAEDALUS_ASSERT(spRomFileCache != nullptr,
                    "How come we have no file cache?");
#endif
    CopyBytesRaw(spRomFileCache, reinterpret_cast<u8 *>(p_dst), rom_start,
                 length);
  }
}
...
#ifdef DAEDALUS_ENABLE_ASSERTS
  DAEDALUS_ASSERT(IsRomAddressFixed(),
                  "Cannot put rom bytes when the data isn't fixed");
#endif
...
// Read the cached bytes into our scratch buffer, and return that
      ...
      DAEDALUS_ASSERT(spRomFileCache != nullptr,
                      "How come we have no file cache?");
#endif
      CopyBytesRaw(spRomFileCache, sScratchBuffer, rom_start,
...
//
//*****************************************************************************
void RomBuffer::CopyToRam(u8 *p_dst, u32 dst_offset, u32 dst_size,
                          u32 src_offset, u32 length) {
  if (sRomFixed) {
    const u8 *p_src((const u8 *)spRomData);
    u32 src_size(sRomSize);

    DMA_HandleTransfer(p_dst, dst_offset, dst_size, p_src, src_offset, src_size,
                       length);
  } else {
    while (length > 0) {
      u8 *p_chunk_base{};
      ...
      if (!spRomFileCache->GetChunk(src_offset, &p_chunk_base, &chunk_offset,
                                    &chunk_size)) {
        // Out of range
        break;
      }
...
#endif
      // Copy this chunk across
      if (!DMA_HandleTransfer(p_dst, dst_offset, dst_size, p_chunk_base,
                              offset_into_chunk, chunk_size, bytes_this_pass)) {
        break;
      }
...
//
//*****************************************************************************
bool RomBuffer::IsRomAddressFixed() { return sRomFixed; }
...
const void *RomBuffer::GetFixedRomBaseAddress() {
  ...
  DAEDALUS_ASSERT(IsRomAddressFixed(),
                  "Trying to access the rom base address when it's not fixed");
#endif
  return spRomData;
}
