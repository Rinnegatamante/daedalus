...
void RSP_HLE_Finished(u32 setbits) {
  ...
  //
  // Set the SP flags appropriately. The RSP is not running anyway, no need to
  // stop it
  //
...
EProcessResult RSP_HLE_Jpeg(OSTask *task) {
  ...
  // most ucode_boot procedure copy 0xf80 bytes of ucode whatever the ucode_size
  // is. For practical purpose we use a ucode_size = min(0xf80,
  // task->ucode_size)
  u32 sum{sum_bytes(g_pu8RamBase + (u32)task->t.ucode,
                    Min<u32>(task->t.ucode_size, 0xf80) >> 1)};
...
void RSP_HLE_ProcessTask() {
  ...
  default:
    ...
    // This can be easily handled, need to find first a game that uses this
    // though
    ...
    //	RDP_DumpRSPCode("boot",    0xDEAFF00D, (u32*)(g_pu8RamBase +
    //(((u32)pTask->t.ucode_boot)&0x00FFFFFF)), 0x04001000,
    //pTask->t.ucode_boot_size); 	RDP_DumpRSPCode("unkcode", 0xDEAFF00D,
    //(u32*)(g_pu8RamBase + (((u32)pTask->t.ucode)&0x00FFFFFF)), 0x04001080,
    //0x1000 - 0x80);//pTask->t.ucode_size);
