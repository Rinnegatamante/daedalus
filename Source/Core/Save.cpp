...
bool Save_Reset() {
  ...
  DAEDALUS_ASSERT(gSaveSize <= MemoryRegionSizes[MEM_SAVE],
                  "Save size is larger than allocated memory");
#endif
  gSaveDirty = false;
  if (gSaveSize > 0) {
...
#endif
      fread(g_pMemoryBuffers[MEM_MEMPACK], MemoryRegionSizes[MEM_MEMPACK], 1,
            fp);
      fclose(fp);
      gMempackDirty = false;
    } else {
#ifdef DAEDALUS_DEBUG_CONSOLE
      DBGConsole_Msg(0, "MemPack File [C%s] cannot be found.",
                     gMempackFileName);
#endif
      InitMempackContent();
      gMempackDirty = true;
...
void Save_Fini() { Save_Flush(true); }

void Save_MarkSaveDirty() { gSaveDirty = true; }

void Save_MarkMempackDirty() { gMempackDirty = true; }
...
void Save_Flush(bool force) {
  ...
  if (gMempackDirty || force) {
    ...
    if (fp != nullptr) {
      fwrite(g_pMemoryBuffers[MEM_MEMPACK], MemoryRegionSizes[MEM_MEMPACK], 1,
             fp);
      fclose(fp);
    }
    gMempackDirty = false;
...
static const u8 gMempackInitialize[] = {
    ...
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x71, 0x00, 0x03, 0x00, 0x03, 0x00, 0x03,
    0x00, 0x03, 0x00, 0x03, 0x00, 0x03, 0x00, 0x03,
};

static void InitMempackContent() {
  for (size_t dst_off = 0; dst_off < MemoryRegionSizes[MEM_MEMPACK];
       dst_off += 32 * 1024) {
...
#ifdef DAEDALUS_ENABLE_ASSERTS
    DAEDALUS_ASSERT(dst_off + 0x8000 <= MemoryRegionSizes[MEM_MEMPACK],
                    "Buffer overflow");
    DAEDALUS_ASSERT(dst_off + sizeof(gMempackInitialize) <=
                        MemoryRegionSizes[MEM_MEMPACK],
                    "Buffer overflow");
#endif
  }
}
