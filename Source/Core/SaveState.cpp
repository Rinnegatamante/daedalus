...
//
//	SaveState code written initially by Lkb. Seems to be based about Project
//64's 	savestate format, which is partially documented here:
//http://www.hcs64.com/usf/usf.txt
//
...
public:
  SaveState_ostream_gzip(const char *filename) : mStream(filename) {}
  ...
  inline void write_memory_buffer(int buffernum, u32 size = 0) {
    write(g_pMemoryBuffers[buffernum],
          size ? Min(MemoryRegionSizes[buffernum], size)
               : MemoryRegionSizes[buffernum]);
  ...
  inline void write_memory_buffer_offset(int buffernum, u32 offset, u32 size) {
    write((u8 *)g_pMemoryBuffers[buffernum] + offset,
          size ? Min(MemoryRegionSizes[buffernum], size)
               : MemoryRegionSizes[buffernum]);
    if (size > MemoryRegionSizes[buffernum])
      skip(size - MemoryRegionSizes[buffernum]);
  ...
  bool IsValid() const { return mStream.IsOpen(); }
...
public:
  explicit SaveState_istream_gzip(const char *filename) : mStream(filename) {}

  inline bool IsValid() const { return mStream.IsOpen(); }

  template <typename T> inline SaveState_istream_gzip &operator>>(T &data) {
  ...
  inline void read_memory_buffer(int buffernum, u32 size = 0) {
    read(g_pMemoryBuffers[buffernum],
         size ? Min(MemoryRegionSizes[buffernum], size)
              : MemoryRegionSizes[buffernum]);
    if (size > MemoryRegionSizes[buffernum])
      skip(size - MemoryRegionSizes[buffernum]);
  ...
  inline void read_memory_buffer_offset(int buffernum, u32 offset, u32 size) {
    read((u8 *)g_pMemoryBuffers[buffernum] + offset,
         size ? Min(MemoryRegionSizes[buffernum], size)
              : MemoryRegionSizes[buffernum]);
    if (size > MemoryRegionSizes[buffernum])
      skip(size - MemoryRegionSizes[buffernum]);
...
bool SaveState_SaveToFile(const char *filename) {
  ...
  for (i = 0; i < 32; i++) {
    ...
    stream << (u32)(
        ((g_TLBs[i].pfne & TLBLO_V) || (g_TLBs[i].pfno & TLBLO_V)) ? 1 : 0);
...
// In revision >=715 we were byte swapping PIF RAM in a temp buffer, this broke
// compatibility with PJ64 saves Now that is fixed this been added for
// compatibility reasons for any ss created within those revs..
...
bool SaveState_LoadFromFile(const char *filename) {
  ...
  if (value != SAVESTATE_PROJECT64_MAGIC_NUMBER) {
    ...
    DBGConsole_Msg(0, "Wrong magic number - savestate could be damaged or not "
                      "in Daedalus/Project64 format");
#endif
    return false;
  }
  stream >> gRamSize;
...
// ToDo: Give Option to switch Roms to one listed in SaveState if available.
#ifdef DAEDALUS_DEBUG_CONSOLE
    DBGConsole_Msg(0,
                   "ROM name in savestate is different from the name of the "
                   "currently loaded ROM: %x-%x-%02x, %x-%x-%02x\n",

                   g_ROM.mRomID.CRC[0], g_ROM.mRomID.CRC[1],
                   g_ROM.mRomID.CountryID, new_rom_id.CRC[0], new_rom_id.CRC[1],
                   new_rom_id.CountryID);
#endif
    return false;
  }
  ...
  memcpy(g_pMemoryBuffers[MEM_DPC_REG], dpcRegData,
         MemoryRegionSizes[MEM_DPC_REG]);
  ...
  memcpy(g_pMemoryBuffers[MEM_MI_REG], miRegData,
         MemoryRegionSizes[MEM_MI_REG]);

  stream.read_memory_buffer_write_value(
      MEM_VI_REG,
      0x84400000); // call WriteValue to update global and GFX plugin data
  stream.read_memory_buffer_write_value(
      MEM_AI_REG, 0x84500000); // call WriteValue to update audio plugin data
  ...
  stream >> value;
  Memory_SI_SetRegister(SI_DRAM_ADDR_REG, value);
  stream >> value;
  Memory_SI_SetRegister(SI_PIF_ADDR_RD64B_REG, value);
  stream >> value;
  Memory_SI_SetRegister(SI_PIF_ADDR_WR64B_REG, value);
  stream >> value;
  Memory_SI_SetRegister(SI_STATUS_REG, value);
  for (i = 0; i < 32; i++) {
    stream.skip(
        4); // boolean that tells whether the entry is defined - seems redundant
