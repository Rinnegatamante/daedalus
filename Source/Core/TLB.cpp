...
void TLBEntry::UpdateValue(u32 _pagemask, u32 _hi, u32 _pfno, u32 _pfne) {
  ...
  // The G bit of the TLB is written with the logical AND of the G bits of the
  // EntryLo0 and EntryLo1 regs The TLB entry is loaded with the contents of the
  // EntryHi and EntryLo regs.

  // TLB[INDEX] <- PageMask || (EntryHi AND NOT PageMask) || EntryLo1 ||
  // EntryLo0
  DPF(DEBUG_TLB,
      "PAGEMASK: 0x%08x ENTRYHI: 0x%08x. ENTRYLO1: 0x%08x. ENTRYLO0: 0x%08x",
      _pagemask, _hi, _pfno, _pfne);
...
void TLBEntry::Reset() { UpdateValue(0x00000000, 0x80000000, 0, 0); }
