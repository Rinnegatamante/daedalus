...
// NB! This should be enabled across all builds regardless of whether
// DAEDALUS_ENABLE_ASSERTS is set! It's a compile-time assert and has no runtime
// cost!
...
EAssertResult DAEDALUS_VARARG_CALL_TYPE DaedalusAssert(const char *expression,
                                                       const char *file,
                                                       unsigned int line,
                                                       const char *msg, ...);
...
//
//	Use this api to override the default assert handler, e.g. for logging
//asserts during a batch process
//
typedef EAssertResult (*DaedalusAssertHook)(const char *expression,
