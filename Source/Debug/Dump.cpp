...
void Dump_GetSaveDirectory(char *rootdir, const char *rom_filename,
...
#ifndef DAEDALUS_PSP
      // FIXME(strmnnrmn): for OSX I generate savegames in a subdir Save, to
...
#ifdef DAEDALUS_DEBUG_CONSOLE
      if (CDebugConsole::IsAvailable()) {
        DBGConsole_Msg(0, "SaveDir is still empty - defaulting to [C%s]",
                       g_DaedalusConfig.mSaveDir);
      }
#endif
    }
  }
  ...
  // Form the filename from the file spec (i.e. strip path and replace the
  // extension)
...
//
//*****************************************************************************
void Dump_DisassembleMIPSRange(FILE *fh, u32 address_offset, const OpCode *b,
                               const OpCode *e) {
...
void Dump_Disassemble(u32 start, u32 end, const char *p_file_name) {
  ...
  DBGConsole_Msg(0, "Disassembling from 0x%08x to 0x%08x ([C%s])", start, end,
                 file_path);

  const OpCode *op_start(reinterpret_cast<const OpCode *>(p_base));
  const OpCode *op_end(
      reinterpret_cast<const OpCode *>(p_base + (end - start)));
...
//*****************************************************************************
//
//	N.B. This assumbes that b/e are 4 byte aligned (otherwise endianness is
//broken)
//
//*****************************************************************************
void Dump_MemoryRange(FILE *fh, u32 address_offset, const u32 *b,
                      const u32 *e) {
  u32 address(address_offset);
  const u32 *p(b);
  while (p < e) {
    fprintf(fh, "0x%08x: %08x %08x %08x %08x ", address, p[0], p[1], p[2],
            p[3]);
...
void Dump_DisassembleRSPRange(FILE *fh, u32 address_offset, const OpCode *b,
                              const OpCode *e) {
  u32 address(address_offset);
  const OpCode *p(b);
  while (p < e) {
    ...
    if (RomBuffer::ReadValueRaw<u8>(i ^ 0x3) >= ' ' &&
      ...
      if (ascii_count >= MIN_LENGTH) {
        ...
        for (u32 j = 0; j < ascii_count; j++) {
          fprintf(fp, "%c",
                  RomBuffer::ReadValueRaw<u8>((ascii_start + j) ^ 0x3));
        }
