...
struct SRegisterSpan {
  SRegisterSpan(EN64Reg reg, u32 start, u32 end)
      : Register(reg), SpanStart(start), SpanEnd(end) {}
...
struct SRegisterUsageInfo {
  ...
  SRegisterUsageInfo()
      : RegistersRead(0), RegistersWritten(0), RegistersAsBases(0) {}

  inline bool IsRead(EN64Reg reg) const { return (RegistersRead >> reg) & 1; }
  inline bool IsModified(EN64Reg reg) const {
    return (RegistersWritten >> reg) & 1;
  }
  inline bool IsBase(EN64Reg reg) const {
    return (RegistersAsBases >> reg) & 1;
  }
};
