...
CTraceRecorder::CTraceRecorder()
    : mTracing(false), mStartTraceAddress(0), mExpectedExitTraceAddress(0),
      mActiveBranchIdx(INVALID_IDX), mStopTraceAfterDelaySlot(false),
...
CTraceRecorder::EUpdateTraceStatus
CTraceRecorder::UpdateTrace(u32 address, bool branch_delay_slot,
                            bool branch_taken, OpCode op_code,
  ...
  // Terminate if the current instruction is in the fragment cache or the trace
  // reaches a specified size
  if (want_to_stop && (mActiveBranchIdx == INVALID_IDX)) {
#ifdef DAEDALUS_ENABLE_ASSERTS
    DAEDALUS_ASSERT(mActiveBranchIdx == INVALID_IDX,
                    "Exiting trace while in the middle of handling branch!");
#endif
...
#ifdef DAEDALUS_ENABLE_ASSERTS
    DAEDALUS_ASSERT(mActiveBranchIdx < mBranchDetails.size(),
    ...
    if (mBranchDetails[mActiveBranchIdx].SpeedHack == SHACK_POSSIBLE) {
      if (op_code._u32 == 0) {
        mBranchDetails[mActiveBranchIdx].SpeedHack = SHACK_SKIPTOEVENT;
               ...
               op_code.op == OP_ADDI ||
               op_code.op == OP_DADDIU) { // We don't handle COPYREG SPEEDHACKS
  ...
  //	We assume that if we'll exit on the next instruction (assuming this
  //isn't a branch)
  //
...
#ifdef DAEDALUS_ENABLE_ASSERTS
      DAEDALUS_ASSERT(IsBranchTypeDirect(branch_type),
                      "Not expecting an indirect branch here");
#endif
      if (branch_taken) {
...
#ifdef DAEDALUS_ENABLE_ASSERTS
    DAEDALUS_ASSERT(branch_type == BT_ERET || mActiveBranchIdx == INVALID_IDX,
                    "Exiting trace while in the middle of handling branch!");
#endif
    mTracing = false;
    return UTS_CREATE_FRAGMENT;
...
void CTraceRecorder::StopTrace(u32 exit_address) {
  ...
  DAEDALUS_ASSERT(mTracing, "We're not tracing");
  DAEDALUS_ASSERT(mActiveBranchIdx == INVALID_IDX,
                  "Stopping trace when a branch is active");
#endif
  mTracing = false;
...
CFragment *CTraceRecorder::CreateFragment(CCodeBufferManager *p_manager) {
  ...
  auto *p_frament(new CFragment(
      p_manager, mStartTraceAddress, mExpectedExitTraceAddress, mTraceBuffer,
      register_usage, mBranchDetails, mNeedIndirectExitMap));
...
void CTraceRecorder::AbortTrace() {
  ...
  if (mTracing) {
    ...
    if (fh) {
      ...
      u32 last_address(mTraceBuffer.size() > 0 ? mTraceBuffer[0].Address - 4
                                               : 0);
      for (std::vector<STraceEntry>::const_iterator it = mTraceBuffer.begin();
           it != mTraceBuffer.end(); ++it) {
...
#ifdef DAEDALUS_ENABLE_ASSERTS
          DAEDALUS_ASSERT(branch_index < mBranchDetails.size(),
...
void CTraceRecorder::Analyse(SRegisterUsageInfo &register_usage) {
  ...
  std::pair<s32, s32> invalid_span(
      std::pair<s32, s32>(mTraceBuffer.size(), -1));

  std::fill(reg_spans, reg_spans + NUM_N64_REGS,
            invalid_span); // Set the interval to an invalid range
  ...
  for (u32 i{}; i < mTraceBuffer.size(); ++i) {
    ...
    for (s32 reg = 1; reg < NUM_N64_REGS; ++reg) {
      if (all_uses & (1 << reg)) {
        if (s32(i) < reg_spans[reg].first)
          reg_spans[reg].first = i;
        if (s32(i) > reg_spans[reg].second)
          reg_spans[reg].second = i;
      }
    }
  }
