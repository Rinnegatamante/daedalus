...
struct NativePf5650 {
  ...
  static u16 Make(u8 r, u8 g, u8 b, u8 a) {
    ...
    DAEDALUS_USE(a);
#endif
    return ((r >> (8 - BitsR)) << ShiftR) | ((g >> (8 - BitsG)) << ShiftG) |
           ((b >> (8 - BitsB)) << ShiftB);
  }

  NativePf5650() = default;

  NativePf5650(u8 r, u8 g, u8 b, u8 a) : Bits(Make(r, g, b, a)) {}

  u8 GetR() const {
    u8 r(u8((Bits & MaskR) >> ShiftR));
    return (r << (8 - BitsR)) | (r >> (BitsR - (8 - BitsR)));
  }
  u8 GetG() const {
    u8 g(u8((Bits & MaskG) >> ShiftG));
    return (g << (8 - BitsG)) | (g >> (BitsG - (8 - BitsG)));
  }
  u8 GetB() const {
    u8 b(u8((Bits & MaskB) >> ShiftB));
    return (b << (8 - BitsB)) | (b >> (BitsB - (8 - BitsB)));
  }
...
struct NativePf5551 {
  ...
  NativePf5551() = default;

  NativePf5551(u8 r, u8 g, u8 b, u8 a) : Bits(Make(r, g, b, a)) {}
...
struct NativePf4444 {
  ...
  static u16 Make(u8 r, u8 g, u8 b, u8 a) {
    return ((r >> (8 - BitsR)) << ShiftR) | ((g >> (8 - BitsG)) << ShiftG) |
           ((b >> (8 - BitsB)) << ShiftB) | ((a >> (8 - BitsA)) << ShiftA);
  }

  NativePf4444() = default;

  NativePf4444(u8 r, u8 g, u8 b, u8 a) : Bits(Make(r, g, b, a)) {}
  ...
  u8 GetA() const {
    u8 a(u8((Bits & MaskA) >> ShiftA));
    return (a << (8 - BitsA)) | (a >> (BitsA - (8 - BitsA)));
  }
...
struct NativePf8888 {
  ...
  static u32 Make(u8 r, u8 g, u8 b, u8 a) {
    return (r << ShiftR) | (g << ShiftG) | (b << ShiftB) | (a << ShiftA);
  }

  template <typename T> static NativePf8888 Make(T c) {
    return NativePf8888(c.GetR(), c.GetG(), c.GetB(), c.GetA());
  }

  NativePf8888() = default;

  // Would like to remove this
  explicit NativePf8888(u32 bits) : Bits(bits) {}

  NativePf8888(u8 r, u8 g, u8 b, u8 a) : Bits(Make(r, g, b, a)) {}
...
struct NativePfCI44 // This represents 2 pixels
{
  ...
  NativePfCI44() = default;

  explicit NativePfCI44(u8 bits) : Bits(bits) {}

  inline u8 GetIdxA() const { return (Bits >> 4) & 0xf; }
  inline u8 GetIdxB() const { return (Bits)&0xf; }

  inline void SetIdxA(u8 value) {
    Bits &= MaskPixelA;
    Bits |= (value << 4) & MaskPixelB;
  }
  inline void SetIdxB(u8 value) {
    Bits &= MaskPixelB;
    Bits |= value & MaskPixelA;
  }
...
struct NativePfCI8 {
  ...
  explicit NativePfCI8(u8 bits) : Bits(bits) {}
};
