...
static void WritePngRowPal4(u8 *line, const void *src, u32 width,
  ...
  for (u32 x = 0; x < width; x++) {
    ...
    u8 color_idx((x & 1) ? colors.GetIdxB()
                         : colors.GetIdxA()); // FIXME(strmnnrmn): A/B should be
                                              // swapped? NativeTextureOSX is
                                              // broken with this ordering.
...
static void WritePngRowPal8(u8 *line, const void *src, u32 width,
                            const NativePf8888 *palette) {
  u32 i = 0;
...
static void DAEDALUS_ZLIB_CALL_TYPE PngWrite(png_structp png_ptr,
                                             png_bytep data, png_size_t len) {
...
//*****************************************************************************
void PngSaveImage(DataSink *sink, const void *data, const void *palette,
                  ETextureFormat pixelformat, s32 pitch, u32 width, u32 height,
                  bool use_alpha) {
#ifdef DAEDALUS_ENABLE_ASSERTS
  DAEDALUS_ASSERT(!IsTextureFormatPalettised(pixelformat) || palette,
  ...
  // If the pitch is negative (i.e for a screenshot), start at the last row and
  // work backwards.
  if (pitch < 0) {
    p += -pitch * (height - 1);
  }
  ...
  for (u32 y = 0; y < height; y++) {
    switch (pixelformat) {
    case TexFmt_5650:
      WritePngRow<NativePf5650>(line, p, width);
      break;
    case TexFmt_5551:
      WritePngRow<NativePf5551>(line, p, width);
      break;
    case TexFmt_4444:
      WritePngRow<NativePf4444>(line, p, width);
      break;
    case TexFmt_8888:
      WritePngRow<NativePf8888>(line, p, width);
      break;
    case TexFmt_CI4_8888:
      WritePngRowPal4(line, p, width, pal8888);
      break;
    case TexFmt_CI8_8888:
      WritePngRowPal8(line, p, width, pal8888);
      break;
    }
...
void PngSaveImage(const char *filename, const void *data, const void *palette,
                  ETextureFormat format, s32 stride, u32 width, u32 height,
                  bool use_alpha) {
  FileSink sink;
...
void PngSaveImage(DataSink *sink, const CNativeTexture *texture) {
  ...
  PngSaveImage(sink, texture->GetData(), texture->GetPalette(),
               texture->GetFormat(), texture->GetStride(), texture->GetWidth(),
               texture->GetHeight(), true);
}

// Utility function to flatten a native texture into an array of NativePf8888
// values. Should live elsewhere, but need to share WritePngRow.
void FlattenTexture(const CNativeTexture *texture, void *dst, size_t len) {
  const u8 *p = reinterpret_cast<const u8 *>(texture->GetData());
  const auto *pal8888 =
      reinterpret_cast<const NativePf8888 *>(texture->GetPalette());
...
#ifdef DAEDALUS_ENABLE_ASSERTS
  DAEDALUS_ASSERT(len == width * sizeof(NativePf8888) * height,
