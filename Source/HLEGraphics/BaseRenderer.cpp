...
//   On OSX we just use malloc, though we could use a scratch allocator to
//   simplify. On PSP we again use sceGuGetMemory.
struct TempVerts {
  TempVerts() : Verts(nullptr), Count(0) {}
...
extern "C" {
void _TnLVFPU(const Matrix4x4 *world_matrix, const Matrix4x4 *projection_matrix,
              const FiddledVtx *p_in, const DaedalusVtx4 *p_out,
              u32 num_vertices, const TnLParams *params);
void _TnLVFPU_Plight(const Matrix4x4 *world_matrix,
                     const Matrix4x4 *projection_matrix, const FiddledVtx *p_in,
                     const DaedalusVtx4 *p_out, u32 num_vertices,
                     const TnLParams *params);
void _TnLVFPUDKR(u32 num_vertices, const Matrix4x4 *projection_matrix,
                 const FiddledVtx *p_in, const DaedalusVtx4 *p_out);
void _TnLVFPUDKRB(u32 num_vertices, const Matrix4x4 *projection_matrix,
                  const FiddledVtx *p_in, const DaedalusVtx4 *p_out);
void _TnLVFPUCBFD(const Matrix4x4 *world_matrix,
                  const Matrix4x4 *projection_matrix, const FiddledVtx *p_in,
                  const DaedalusVtx4 *p_out, u32 num_vertices,
                  const TnLParams *params, const s8 *model_norm, u32 v0);
void _TnLVFPUPD(const Matrix4x4 *world_matrix,
                const Matrix4x4 *projection_matrix, const FiddledVtxPD *p_in,
                const DaedalusVtx4 *p_out, u32 num_vertices,
                const TnLParams *params, const u8 *model_norm);

void _ConvertVertice(DaedalusVtx *dest, const DaedalusVtx4 *source);
void _ConvertVerticesIndexed(DaedalusVtx *dest, const DaedalusVtx4 *source,
                             u32 num_vertices, const u16 *indices);

u32 _ClipToHyperPlane(DaedalusVtx4 *dest, const DaedalusVtx4 *source,
                      const v4 *plane, u32 num_verts);
}
...
BaseRenderer::BaseRenderer()
    : mN64ToScreenScale(2.0f, 2.0f), mN64ToScreenTranslate(0.0f, 0.0f), mMux(0)

      ,
      mTextureTile(0)

      ,
      mPrimDepth(0.0f), mPrimLODFraction(0.f)

      ,
      mFogColour(0x00ffffff) // NB top bits not set. Intentional?
      ,
      mPrimitiveColour(0xffffffff), mEnvColour(0xffffffff),
      mBlendColour(255, 255, 255, 0), mFillColour(0xffffffff)

      ,
      mModelViewTop(0), mWorldProjectValid(false), mReloadProj(true),
      mWPmodified(false)

      ,
      mScreenWidth(0.f), mScreenHeight(0.f)

      ,
      mNumIndices(0), mVtxClipFlagsUnion(0)

#ifdef DAEDALUS_DEBUG_DISPLAYLIST
      ,
      mNumTrisRendered(0), mNumTrisClipped(0), mNumRect(0), mNastyTexture(false)
#endif
{
#ifdef DAEDALUS_DEBUG_CONSOLE //&& DAEDALUS_PSP
  DAEDALUS_ASSERT(IsPointerAligned(&mTnL, 16),
                  "Oops, mTnL should be 16-byte aligned");
#endif
...
void BaseRenderer::SetVIScales() {
  ...
  // This corrects height in various games ex : Megaman 64, Cyber Tiger. 40Winks
  // need width >= ((u32)fViWidth << 1) for menus //Corn
...
void BaseRenderer::EndScene() {
  ...
  //
  //	Clear this, to ensure we're force to check for updates to it on the next
  //frame
  for (u32 i{}; i < kNumBoundTextures; i++) {
...
void BaseRenderer::InitViewport() {
  ...
  // Get the current display dimensions. This might change frame by frame e.g.
  ...
  mN64ToScreenTranslate.x =
      (f32)display_x - roundf(0.55f * (gZoomX - 1.0f) * fViWidth);
  mN64ToScreenTranslate.y =
      (f32)display_y - roundf(0.55f * (gZoomX - 1.0f) * fViHeight);
  ...
  mScreenToDevice = Matrix4x4(2.f / w, 0.f, 0.f, 0.f, 0.f, -2.f / h, 0.f, 0.f,
                              0.f, 0.f, 1.f, 0.f, -1.0f, 1.f, 0.f, 1.f);
#endif
...
void BaseRenderer::SetN64Viewport(const v2 &scale, const v2 &trans) {
  ...
  //
  if (mVpScale.x == scale.x && mVpScale.y == scale.y && mVpTrans.x == trans.x &&
      mVpTrans.y == trans.y)
    return;
...
bool BaseRenderer::AddTri(u32 v0, u32 v1, u32 v2) {
...
#ifdef DAEDALUS_ENABLE_ASSERTS
  DAEDALUS_ASSERT(v0 < kMaxN64Vertices, "Vertex index is out of bounds (%d)",
                  v0);
  DAEDALUS_ASSERT(v1 < kMaxN64Vertices, "Vertex index is out of bounds (%d)",
                  v1);
  DAEDALUS_ASSERT(v2 < kMaxN64Vertices, "Vertex index is out of bounds (%d)",
                  v2);
#endif
  ...
  if (mTnL.Flags.TriCull) {
    ...
    const f32 NSign{(((B.x * ACw - AxBC) * (C.y * ABw - AyBC) -
                      (C.x * ABw - AxBC) * (B.y * ACw - AyBC)) *
                     ABw * C.w)};
    if (NSign <= 0.0f)
#endif
    {
...
#endif
#ifdef DAEDALUS_ENABLE_ASSERTS
  DAEDALUS_ASSERT(mNumIndices + 3 < kMaxIndices,
                  "Array overflow, too many Indices");
#endif
...
*/
  ...
  // If any bit is set here it means we have to clip the trianlges since PSP HW
  // clipping sux!
...
gRDPOtherMode.z_cmp+gRDPOtherMode.z_upd > 0 )
        {
...
//
// Check for depth source, this is for Nascar games, hopefully won't mess up
// anything
#ifdef DAEDALUS_ENABLE_ASSERTS
  DAEDALUS_ASSERT(!gRDPOtherMode.depth_source,
                  " Warning : Using depth source in flushtris");
#endif
  //
  //	Render out our vertices
  RenderTriangles(temp_verts.Verts, temp_verts.Count,
                  gRDPOtherMode.depth_source ? true : false);
...
u32 clip_tri_to_frustum(DaedalusVtx4 *v0, DaedalusVtx4 *v1) {
  ...
  vOut = _ClipToHyperPlane(v1, v0, &NDCPlane[0], vOut);
  if (vOut < 3)
    return vOut; // near
  vOut = _ClipToHyperPlane(v0, v1, &NDCPlane[1], vOut);
  if (vOut < 3)
    return vOut; // far
  vOut = _ClipToHyperPlane(v1, v0, &NDCPlane[2], vOut);
  if (vOut < 3)
    return vOut; // left
  vOut = _ClipToHyperPlane(v0, v1, &NDCPlane[3], vOut);
  if (vOut < 3)
    return vOut; // right
  vOut = _ClipToHyperPlane(v1, v0, &NDCPlane[4], vOut);
  if (vOut < 3)
    return vOut;                                        // bottom
...
void DaedalusVtx4::Interpolate(const DaedalusVtx4 &lhs, const DaedalusVtx4 &rhs,
                               float factor) {
  ProjectedPos =
      lhs.ProjectedPos + (rhs.ProjectedPos - lhs.ProjectedPos) * factor;
  TransformedPos =
      lhs.TransformedPos + (rhs.TransformedPos - lhs.TransformedPos) * factor;
...
static u32 clipToHyperPlane(DaedalusVtx4 *dest, const DaedalusVtx4 *source,
                            u32 inCount, const v4 &plane) {
  u32 outCount(0);
  ...
  for (u32 i{}; i < inCount + 1; ++i) {
    ...
    if (aDotPlane <= 0.f) {
      ...
      if (bDotPlane > 0.f) {
        ...
        out->Interpolate(
            *b, *a, bDotPlane / (b->ProjectedPos - a->ProjectedPos).Dot(plane));
        out++;
        outCount++;
      }
      // copy current to out
...
u32 clip_tri_to_frustum(DaedalusVtx4 *v0, DaedalusVtx4 *v1) {
  ...
  vOut = clipToHyperPlane(v1, v0, vOut, NDCPlane[0]);
  if (vOut < 3)
    return vOut; // near
  vOut = clipToHyperPlane(v0, v1, vOut, NDCPlane[1]);
  if (vOut < 3)
    return vOut; // far
  vOut = clipToHyperPlane(v1, v0, vOut, NDCPlane[2]);
  if (vOut < 3)
    return vOut; // left
  vOut = clipToHyperPlane(v0, v1, vOut, NDCPlane[3]);
  if (vOut < 3)
    return vOut; // right
  vOut = clipToHyperPlane(v1, v0, vOut, NDCPlane[4]);
  if (vOut < 3)
    return vOut;                                      // bottom
...
void BaseRenderer::PrepareTrisClipped(TempVerts *temp_verts) const {
  ...
  //
  //	At this point all vertices are lit/projected and have both transformed
  //and projected 	vertex positions. For the best results we clip against the
  //projected vertex positions, 	but use the resulting intersections to
  //interpolate the transformed positions. 	The clipping is more efficient in
  //normalised device coordinates, but rendering these 	directly prevents the PSP
  //performing perspective correction. We could invert the projection 	matrix and
  //use this to back-project the clip planes into world coordinates, but this
  //	suffers from various precision issues. Carrying around both sets of
  //coordinates gives 	us the best of both worlds :)
  //
  //  Convert directly to PSP hardware format, that way we only copy 24 bytes
  //  instead of 64 bytes //Corn
  //
  u32 num_vertices{};
  ...
  for (u32 i{}; i < (mNumIndices - 2);) {
    ...
    // Check if any of the vertices are outside the clipbox (NDC), if so we need
    // to clip the triangle
    if (mVtxProjected[idx0].ClipFlags | mVtxProjected[idx1].ClipFlags |
        mVtxProjected[idx2].ClipFlags) {
...
// we can't make a triangle so we bail and skip rendering it.
#ifdef DAEDALUS_DEBUG_DISPLAYLIST
      DL_PF("    Clip & re-tesselate [%d,%d,%d] with %d vertices", i - 3, i - 2,
            i - 1, out);
      DL_PF("    %#5.3f, %#5.3f, %#5.3f",
            mVtxProjected[idx0].ProjectedPos.x /
                mVtxProjected[idx0].ProjectedPos.w,
            mVtxProjected[idx0].ProjectedPos.y /
                mVtxProjected[idx0].ProjectedPos.w,
            mVtxProjected[idx0].ProjectedPos.z /
                mVtxProjected[idx0].ProjectedPos.w);
      DL_PF("    %#5.3f, %#5.3f, %#5.3f",
            mVtxProjected[idx1].ProjectedPos.x /
                mVtxProjected[idx1].ProjectedPos.w,
            mVtxProjected[idx1].ProjectedPos.y /
                mVtxProjected[idx1].ProjectedPos.w,
            mVtxProjected[idx1].ProjectedPos.z /
                mVtxProjected[idx1].ProjectedPos.w);
      DL_PF("    %#5.3f, %#5.3f, %#5.3f",
            mVtxProjected[idx2].ProjectedPos.x /
                mVtxProjected[idx2].ProjectedPos.w,
            mVtxProjected[idx2].ProjectedPos.y /
                mVtxProjected[idx2].ProjectedPos.w,
            mVtxProjected[idx2].ProjectedPos.z /
                mVtxProjected[idx2].ProjectedPos.w);
#endif
...
#endif
      // Make new triangles from the vertices we got back from clipping the
      // original triangle
      for (u32 j{}; j <= out - 3; ++j) {
#ifdef DAEDALUS_PSP_USE_VFPU
    ...
    } else // Triangle is inside the clipbox so we just add it as it is.
    {
      ...
      clip_vtx[num_vertices++].Position.z =
          mVtxProjected[idx0].TransformedPos.z;
      ...
      clip_vtx[num_vertices++].Position.z =
          mVtxProjected[idx1].TransformedPos.z;
      ...
      clip_vtx[num_vertices++].Position.z =
          mVtxProjected[idx2].TransformedPos.z;
#endif
    }
  }
  ...
  if (num_vertices > 0) {
    ...
    memcpy(p_vertices, clip_vtx,
           num_vertices *
               sizeof(DaedalusVtx)); // std memcpy() is as fast as VFPU here!
  }
}
...
void BaseRenderer::PrepareTrisUnclipped(TempVerts *temp_verts) const {
...
#ifdef DAEDALUS_ENABLE_ASSERTS
  DAEDALUS_ASSERT(mNumIndices > 0,
                  "The number of indices should have been checked");
#endif
  ...
  //	quite a large performance penalty associated with using these on the
  //PSP.
  //
...
#ifdef DAEDALUS_PSP_USE_VFPU
  _ConvertVerticesIndexed(p_vertices, mVtxProjected, num_vertices,
                          mIndexBuffer);
#else
  //
  //	Now we just shuffle all the data across directly (potentially
  //duplicating verts)
  //
...
#ifdef DAEDALUS_DEBUG_DISPLAYLIST
  DL_PF("    Ambient color RGB[%f][%f][%f] Texture scale X[%f] Texture scale "
        "Y[%f]",
        mTnL.Lights[mTnL.NumLights].Colour.x,
        mTnL.Lights[mTnL.NumLights].Colour.y,
        mTnL.Lights[mTnL.NumLights].Colour.z, mTnL.TextureScaleX,
        mTnL.TextureScaleY);
  DL_PF("    Light[%d %s] Texture[%s] EnvMap[%s] Fog[%s]", mTnL.NumLights,
        (mTnL.Flags.Light) ? (mTnL.Flags.PointLight) ? "Point" : "Normal"
                           : "Off",
        (mTnL.Flags.Texture) ? "On" : "Off",
        (mTnL.Flags.TexGen) ? (mTnL.Flags.TexGenLin) ? "Linear" : "Spherical"
                            : "Off",
        (mTnL.Flags.Fog) ? "On" : "Off");
#endif

  if (!mTnL.Flags.PointLight) { // Normal rendering
    _TnLVFPU(&mat_world, &mat_world_project, pVtxBase, &mVtxProjected[v0], n,
             &mTnL);
  } else { // Point light for Zelda MM
    _TnLVFPU_Plight(&mat_world, &mat_world_project, pVtxBase,
                    &mVtxProjected[v0], n, &mTnL);
  }
}

//

/*void BaseRenderer::TestVFPUVerts( u32 v0, u32 num, const FiddledVtx * verts,
const Matrix4x4 & mat_world )
{
        bool	env_map( (mTnL.Flags._u32 & (TNL_LIGHT|TNL_TEXGEN)) ==
(TNL_LIGHT|TNL_TEXGEN) );
        ...
        for (u32 i = v0; i < vend; i++)
        {
                ...
                if (mTnL.Flags.Fog)
                {
                        ...
                        float fog_coeff = (eyespace_z * mTnL.FogMult) +
mTnL.FogOffset;
...
mVtxProjected[i].Colour.w, value );
                ...
                {
                        // Update texture coords n.b. need to divide tu/tv by
bogus scale on addition to buffer

                        // If the vert is already lit, then there is no normal
(and hence we
                        // can't generate tex coord)
                        float tx, ty;
                        if (env_map)
                        {
                                v3 vecTransformedNormal;		// Used
only when TNL_LIGHT set v3	model_normal(f32( vert.norm_x ), f32(
vert.norm_y ), f32( vert.norm_z ) );

                                vecTransformedNormal =
mat_world.TransformNormal( model_normal ); vecTransformedNormal.Normalise();
...
mVtxProjected[i].Texture.x, mVtxProjected[i].Texture.y, tx, ty, env_map ? "env"
: "scale" );
                        }
                }

                //
                //	Initialise the clipping flags (always done on the VFPU,
so skip here)
                //
...
v3 BaseRenderer::LightPointVert(const v4 &w) const {
  ...
  for (u32 l{}; l < mTnL.NumLights; l++) {
    if (mTnL.Lights[l].SkipIfZero) {
      v3 distance_vec(mTnL.Lights[l].Position.x - w.x,
                      mTnL.Lights[l].Position.y - w.y,
                      mTnL.Lights[l].Position.z - w.z);

      f32 light_qlen{distance_vec.LengthSq()};
      f32 light_llen{sqrtf(light_qlen)};

      f32 at = mTnL.Lights[l].ca + mTnL.Lights[l].la * light_llen +
               mTnL.Lights[l].qa * light_qlen;
      if (at > 0.0f) {
        f32 fCosT = 1.0f / at;
        result.x += mTnL.Lights[l].Colour.x * fCosT;
        result.y += mTnL.Lights[l].Colour.y * fCosT;
  ...
  for (u32 i = v0; i < v0 + n; i++) {
    ...
    if (mTnL.Flags.Light) {
      ...
      if (mTnL.Flags.TexGen) {
        // Update texture coords n.b. need to divide tu/tv by bogus scale on
        // addition to buffer If the vert is already lit, then there is no
        // normal (and hence we can't generate tex coord)
#if 1 // 1->Lets use mat_world_project instead of mat_world for nicer effect
      // (see SSV space ship) //Corn
        ...
        if (mTnL.Flags.TexGenLin) {
          ...
          // Cheap way to do Acos(x)/Pi (abs() fixes star in SM64, sort of)
          // //Corn
          f32 NormX{fabsf(norm.x)};
          f32 NormY{fabsf(norm.y)};
          mVtxProjected[i].Texture.x =
              0.5f - 0.25f * NormX - 0.25f * NormX * NormX * NormX;
          mVtxProjected[i].Texture.y =
              0.5f - 0.25f * NormY - 0.25f * NormY * NormY * NormY;
        }
      } else {
      ...
      // if( mTnL.Flags.Shade )
      { // FLAT shade
        mVtxProjected[i].Colour =
            v4(vert.rgba_r * (1.0f / 255.0f), vert.rgba_g * (1.0f / 255.0f),
               vert.rgba_b * (1.0f / 255.0f), vert.rgba_a * (1.0f / 255.0f));
      }
      /*else
    ...
    if (mTnL.Flags.Fog) {
      if (projected.w >
          0.0f) // checking for positive w fixes near plane fog errors //Corn
      {
...
void BaseRenderer::SetNewVertexInfoConker(u32 address, u32 v0, u32 n) {
  ...
  DL_PF("    Ambient color RGB[%f][%f][%f] Texture scale X[%f] Texture scale "
        ...
        mTnL.Lights[mTnL.NumLights].Colour.z, mTnL.TextureScaleX,
        mTnL.TextureScaleY);
  DL_PF("    Light[%s] Texture[%s] EnvMap[%s] Fog[%s]",
        (mTnL.Flags.Light) ? "On" : "Off", (mTnL.Flags.Texture) ? "On" : "Off",
        (mTnL.Flags.TexGen) ? (mTnL.Flags.TexGenLin) ? "Linear" : "Spherical"
                            : "Off",
        (mTnL.Flags.Fog) ? "On" : "Off");
#endif
  ...
  _TnLVFPUCBFD(&mat_world, &mat_project, pVtxBase, &mVtxProjected[v0], n, &mTnL,
               mn, v0 << 1);
}
...
void BaseRenderer::SetNewVertexInfoConker(u32 address, u32 v0, u32 n) {
  ...
  const FiddledVtx *const pVtxBase(
      (const FiddledVtx *)(g_pu8RamBase + address));
  const Matrix4x4 &mat_project{mProjectionMat};
  const Matrix4x4 &mat_world{mModelViewStack[mModelViewTop]};
  ...
  for (u32 i{v0}; i < v0 + n; i++) {
    ...
    mVtxProjected[i].Colour.w =
        (f32)vert.rgba_a * (1.0f / 255.0f); // Pass alpha channel unmodified

    // LIGHTING OR COLOR
    //
    if (mTnL.Flags.Light) {
      v3 model_normal(mn[((i << 1) + 0) ^ 3], mn[((i << 1) + 1) ^ 3],
                      vert.normz);
      ...
      if (mTnL.Flags.PointLight) { // POINT LIGHT
        for (l = 0; l < mTnL.NumLights - 1; l++) {
          if (mTnL.Lights[l].SkipIfZero) {
            ...
            if (fCosT > 0.0f) {
              f32 pi{mTnL.Lights[l].Iscale /
                     (Pos - mTnL.Lights[l].Position).LengthSq()};
              if (pi < 1.0f)
                fCosT *= pi;
      ...
      if (mTnL.Flags.TexGen) {
        if (mTnL.Flags.TexGenLin) {
          mVtxProjected[i].Texture.x =
              0.5f - 0.25f * norm.x -
              0.25f * norm.x * norm.x *
                  norm.x; // Cheap way to do ~Acos(x)/Pi //Corn
          mVtxProjected[i].Texture.y =
              0.5f - 0.25f * norm.y - 0.25f * norm.y * norm.y * norm.y;
        } else {
          mVtxProjected[i].Texture.x = 0.5f * (1.0f + norm.x);
          mVtxProjected[i].Texture.y = 0.5f * (1.0f + norm.y);
        }
...
void BaseRenderer::SetNewVertexInfoDKR(u32 address, u32 v0, u32 n,
  ...
  if (billboard) { // Copy vertices adding base vector and the color data
    mWPmodified = false;

#ifdef DAEDALUS_PSP_USE_VFPU
    _TnLVFPUDKRB(n, &mModelViewStack[0], (const FiddledVtx *)pVtxBase,
                 &mVtxProjected[v0]);
#else
...
#endif
  } else {             // Normal path for transform of triangles
    if (mWPmodified) { // Only reload matrix if it has been changed and no
                       // billbording //Corn
      mWPmodified = false;
      sceGuSetMatrix(GU_PROJECTION, reinterpret_cast<const ScePspFMatrix4 *>(
                                        &mat_world_project));
    }
#ifdef DAEDALUS_PSP_USE_VFPU
    _TnLVFPUDKR(n, &mat_world_project, (const FiddledVtx *)pVtxBase,
                &mVtxProjected[v0]);
#else
    for (u32 i{v0}; i < v0 + n; i++) {
      v4 &transformed(mVtxProjected[i].TransformedPos);
...
void BaseRenderer::SetNewVertexInfoPD(u32 address, u32 v0, u32 n) {
  const FiddledVtxPD *const pVtxBase{
      (const FiddledVtxPD *)(g_pu8RamBase + address)};
  ...
  _TnLVFPUPD(&mat_world, &mat_project, pVtxBase, &mVtxProjected[v0], n, &mTnL,
             mn);
}
...
void BaseRenderer::SetNewVertexInfoPD(u32 address, u32 v0, u32 n) {
  ...
  for (u32 i{v0}; i < v0 + n; i++) {
    ...
    if (mTnL.Flags.Light) {
      v3 model_normal((f32)mn[vert.cidx + 3], (f32)mn[vert.cidx + 2],
                      (f32)mn[vert.cidx + 1]);
      ...
      if (mTnL.Flags.TexGen) {
        ...
        // Env mapping
        if (mTnL.Flags.TexGenLin) { // Cheap way to do Acos(x)/Pi //Corn
          mVtxProjected[i].Texture.x =
              0.5f - 0.25f * norm.x - 0.25f * norm.x * norm.x * norm.x;
          mVtxProjected[i].Texture.y =
              0.5f - 0.25f * norm.y - 0.25f * norm.y * norm.y * norm.y;
        } else {
          mVtxProjected[i].Texture.x = 0.5f * (1.0f + norm.x);
...
#ifdef DAEDALUS_DEBUG_DISPLAYLIST
    DBGConsole_Msg(0,
                   "ModifyVtx - Setting vert data where: 0x%02x, vert: 0x%08x, "
                   "val: 0x%08x",
                   whered, vert, val);
    DL_PF("    Setting unknown value: where: 0x%02x, vert: 0x%08x, val: 0x%08x",
          whered, vert, val);
#endif
    break;
  }
...
inline void BaseRenderer::SetVtxColor(u32 vert, u32 color) {
#ifdef DAEDALUS_ENABLE_ASSERTS
  DAEDALUS_ASSERT(vert < kMaxN64Vertices, "Vertex index is out of bounds (%d)",
                  vert);
#endif
  u32 r{(color >> 24) & 0xFF};
  u32 g{(color >> 16) & 0xFF};
  u32 b{(color >> 8) & 0xFF};
  u32 a{(color)&0xFF};
  mVtxProjected[vert].Colour = v4(r * (1.0f / 255.0f), g * (1.0f / 255.0f),
                                  b * (1.0f / 255.0f), a * (1.0f / 255.0f));
}
...
{
        DAEDALUS_ASSERT( vert < kMaxN64Vertices, "Vertex index is out of bounds
(%d)", vert );
...
void BaseRenderer::UpdateTileSnapshots(u32 tile_idx) {
  ...
  if (gRDPOtherMode.cycle_type == CYCLE_2CYCLE) {
    ...
    if (!gRDPStateManager.IsTileInitialised(t1_tile)) {
      ...
      // FIXME(strmnnrmn): This is required so that Tony Hawk's text renders
      // correctly. It's odd. It calls TexRect with tile 1, and has a color
      // combiner that uses Texel 1 but not Texel 0. But tile 2 has never been
      // initialised.
      t1_tile = tile_idx;
    }
...
static void T1Hack(const TextureInfo &ti0, CNativeTexture *texture0,
  ...
  if ((ti0.GetFormat() == G_IM_FMT_RGBA) && (ti1.GetFormat() == G_IM_FMT_I) &&
...
void BaseRenderer::UpdateTileSnapshot(u32 index, u32 tile_idx) {
  ...
  // It might just be code that lazily does a texrect with Primcolour (i.e. not
  // using either T0 or T1)? DAEDALUS_ASSERT(
  // gRDPStateManager.IsTileInitialised( tile_idx ), "Tile %d hasn't been set up
  // (index %d)", tile_idx, index );
  ...
  if (mBoundTexture[index] == nullptr || mBoundTextureInfo[index] != ti) {
    ...
    // {
    // 	DAEDALUS_DL_ERROR( "Loading texture with bad width/height %dx%d in slot
    // %d", ti.GetWidth(), ti.GetHeight(), index );
    // }
    // else
    // {
    CRefPtr<CNativeTexture> texture =
        CTextureCache::Get()->GetOrCreateTexture(ti);
...
#ifdef DAEDALUS_PSP
      // If second texture is loaded try to merge two textures RGB(T0) + A(T1)
      // into one RGBA(T1) //Corn If T1 Hack is not enabled index can never be
      // other than 0
      if (index) {
        T1Hack(mBoundTextureInfo[0], mBoundTexture[0], mBoundTextureInfo[1],
               mBoundTexture[1]);
      }
#endif
      // }
  ...
  //
  u32 mode_u{(u32)((rdp_tile.clamp_s | (rdp_tile.mask_s == 0)) ? GU_CLAMP
                                                               : GU_REPEAT)};
  u32 mode_v{(u32)((rdp_tile.clamp_t | (rdp_tile.mask_t == 0)) ? GU_CLAMP
                                                               : GU_REPEAT)};

  //	In CRDPStateManager::GetTextureDescriptor, we limit the maximum
  //dimension of a 	texture to that define by the mask_s/mask_t value. 	It this
  //happens, the tile size can be larger than the truncated width/height 	as the
  //rom can set clamp_s/clamp_t to wrap up to a certain value, then clamp. 	We
  //can't support both wrapping and clamping (without manually repeating a
  //texture...) 	so we choose to prefer wrapping. 	The castle in the background of
  //the first SSB level is a good example of this behaviour. 	It sets up a
  //texture with a mask_s/t of 6/6 (64x64), but sets the tile size to 	256*128.
  //clamp_s/t are set, meaning the texture wraps 4x and 2x.
  //
  if (tile_size.GetWidth() > ti.GetWidth()) {
    // This breaks the Sun, and other textures in Zelda. Breaks Mario's hat in
    // SSB, and other textures, and foes in Kirby 64's cutscenes ToDo : Find a
    // proper workaround for this, if this disabled the castle in Link's stage
    // in SSB is broken :/ Do a hack just for Zelda for now..
    //
...
#ifdef DAEDALUS_DEBUG_DISPLAYLIST
  DL_PF("    Use Tile[%d] as Texture[%d] [%dx%d] [%s/%dbpp] [%s u, %s v] -> "
        "Adr[0x%08x] PAL[0x%x] Hash[0x%08x] Pitch[%d] TopLeft[%0.3f|%0.3f]",
        tile_idx, index, ti.GetWidth(), ti.GetHeight(), ti.GetFormatName(),
        ti.GetSizeInBits(), (mode_u == GU_CLAMP) ? "Clamp" : "Repeat",
        (mode_v == GU_CLAMP) ? "Clamp" : "Repeat", ti.GetLoadAddress(),
        ti.GetTlutAddress(), ti.GetHashCode(), ti.GetPitch(),
        mTileTopLeft[index].s / 4.f, mTileTopLeft[index].t / 4.f);
#endif
}

// This transforms UVs so that they're positive. The aim is to ensure UVs are in
// the range [(0,0),(w,h)]. If we can do this, we can specify
// GL_CLAMP_TO_EDGE/GU_CLAMP, which fixes some artifacts when rendering, such as
// bleed from wrapping at the edges of textures. E.g.
// http://imgur.com/db3Adws,dX9vOWE#1 There are two inputs into the final uvs:
// the vertex UV and the mTileTopLeft value:
//   final_uv = (vert_uv - mTileTopLeft).
// When rendering a large logo, most games set uv0=(s,t) and mTileTopLeft=(s,t)
// so that the resulting final_uv = (0,0). But some games (e.g. Automobili
// Lamborghini) set uv0=(0,0) but still have mTileTopLeft=(s,t). This results in
// a final_uv of (-s,-t). I think that the only reason this happened to work was
// because s was some multiple of the texture width, and so with GL_REPEAT the
// texrect rendered ok. Anyway the fix is to subtract mTileTopLeft from the uvs,
// zero it, then add multiples of the texture width/height until the uvs are
// positive. Then if the resulting UVs are in the range [(0,0),(w,h)] we can
// update mTexWrap to GL_CLAMP_TO_EDGE/GU_CLAMP and everything works correctly.
inline void FixUV(u32 *wrap, s16 *c0_, s16 *c1_, s16 offset, s32 size) {
  ...
  if (*wrap != GU_CLAMP && size > 0) {
    ...
    if (c0 < 0) {
      ...
      // Figure out by how much to translate so that the lowest of c0/c1 lies in
      // the range [0,size] If we do lowest%size, we run the risk of
      // implementation dependent behaviour for modulo of negative values.
      // lowest + (size<<16) just adds a large multiple of size, which
      // guarantees the result is positive.
      s32 trans{((lowest + (size << 16)) % size) - lowest};

      // NB! we have to apply the same offset to both coords, to preserve
      // direction of mapping (i.e., don't clamp each independently)
      c0 += trans;
      c1 += trans;
    }
    // If both coords are in the range [0,size], we can clamp safely.
    if ((u16)c0 <= size && (u16)c1 <= size) {
      *wrap = GU_CLAMP;
    }
  }
...
void BaseRenderer::PrepareTexRectUVs(TexCoord *puv0, TexCoord *puv1) {
  ...
  if (rdp_tile.mirror_s)
    size_x *= 2;
  if (rdp_tile.mirror_t)
    size_y *= 2;
...
CRefPtr<CNativeTexture>
BaseRenderer::LoadTextureDirectly(const TextureInfo &ti) {
  CRefPtr<CNativeTexture> texture =
      CTextureCache::Get()->GetOrCreateTexture(ti);
#ifdef DAEDALUS_ENABLE_ASSERTS
...
#if defined(DAEDALUS_PSP)
  // N.B. Think the arguments are x0,y0,x1,y1, and not x,y,w,h as the docs
  // describe
  // printf("%d %d %d %d\n",
  // s32(screen_tl.x),s32(screen_tl.y),s32(screen_br.x),s32(screen_br.y));
  sceGuScissor(l, t, r, b);
#elif defined(DAEDALUS_GL)
  // NB: OpenGL is x,y,w,h. Errors if width or height is negative, so clamp
  // this.
  s32 w{Max<s32>(r - l, 0)};
  s32 h{Max<s32>(b - t, 0)};
...
void BaseRenderer::SetProjection(const u32 address, bool bReplace) {
  ...
  mWorldProjectValid = false;
  sceGuSetMatrix(GU_PROJECTION,
                 reinterpret_cast<const ScePspFMatrix4 *>(&mProjectionMat));
#ifdef DAEDALUS_DEBUG_DISPLAYLIST
  DL_PF("	 %#+12.5f %#+12.5f %#+12.7f %#+12.5f\n"
        "    %#+12.5f %#+12.5f %#+12.7f %#+12.5f\n"
        "    %#+12.5f %#+12.5f %#+12.7f %#+12.5f\n"
        "    %#+12.5f %#+12.5f %#+12.7f %#+12.5f\n",
        mProjectionMat.m[0][0], mProjectionMat.m[0][1], mProjectionMat.m[0][2],
        mProjectionMat.m[0][3], mProjectionMat.m[1][0], mProjectionMat.m[1][1],
        mProjectionMat.m[1][2], mProjectionMat.m[1][3], mProjectionMat.m[2][0],
        mProjectionMat.m[2][1], mProjectionMat.m[2][2], mProjectionMat.m[2][3],
        mProjectionMat.m[3][0], mProjectionMat.m[3][1], mProjectionMat.m[3][2],
        mProjectionMat.m[3][3]);
#endif
}
...
void BaseRenderer::SetDKRMat(const u32 address, bool mul, u32 idx) {
  ...
  if (mul) {
    MatrixFromN64FixedPoint(mTempMat, address);
    MatrixMultiplyAligned(&mModelViewStack[idx], &mTempMat,
                          &mModelViewStack[0]);
  } else {
  ...
  DL_PF("    Mtx_DKR: Index %d %s Address 0x%08x\n"
        ...
        "    %#+12.5f %#+12.5f %#+12.5f %#+12.5f\n"
        "    %#+12.5f %#+12.5f %#+12.5f %#+12.5f\n",
        idx, mul ? "Mul" : "Load", address, mtx.m[0][0], mtx.m[0][1],
        mtx.m[0][2], mtx.m[0][3], mtx.m[1][0], mtx.m[1][1], mtx.m[1][2],
        mtx.m[1][3], mtx.m[2][0], mtx.m[2][1], mtx.m[2][2], mtx.m[2][3],
...
void BaseRenderer::SetWorldView(const u32 address, bool bPush, bool bReplace) {
  ...
  if (bPush && (mModelViewTop < mMatStackSize)) {
    ...
    if (bReplace) {
      ...
      // Hack to make GEX games work, need to multiply all elements with 2.0
      // //Corn
      if (g_ROM.GameHacks == GEX_GECKO)
        for (float &i : mModelViewStack[mModelViewTop].mRaw)
          i += i;
    } else // Multiply ModelView matrix
    {
      MatrixFromN64FixedPoint(mTempMat, address);
      MatrixMultiplyAligned(&mModelViewStack[mModelViewTop], &mTempMat,
                            &mModelViewStack[mModelViewTop - 1]);
    }
  } else // NoPush
  {
    if (bReplace) {
  ...
  DL_PF("    Level = %d\n"
        ...
        "    %#+12.5f %#+12.5f %#+12.5f %#+12.5f\n"
        "    %#+12.5f %#+12.5f %#+12.5f %#+12.5f\n",
        mModelViewTop, mModelViewStack[mModelViewTop].m[0][0],
        mModelViewStack[mModelViewTop].m[0][1],
        mModelViewStack[mModelViewTop].m[0][2],
        mModelViewStack[mModelViewTop].m[0][3],
        mModelViewStack[mModelViewTop].m[1][0],
        mModelViewStack[mModelViewTop].m[1][1],
        mModelViewStack[mModelViewTop].m[1][2],
        mModelViewStack[mModelViewTop].m[1][3],
        mModelViewStack[mModelViewTop].m[2][0],
        mModelViewStack[mModelViewTop].m[2][1],
        mModelViewStack[mModelViewTop].m[2][2],
        mModelViewStack[mModelViewTop].m[2][3],
        mModelViewStack[mModelViewTop].m[3][0],
        mModelViewStack[mModelViewTop].m[3][1],
        mModelViewStack[mModelViewTop].m[3][2],
        mModelViewStack[mModelViewTop].m[3][3]);
#endif
}
...
inline void BaseRenderer::UpdateWorldProject() {
  if (!mWorldProjectValid) {
    ...
    MatrixMultiplyAligned(&mWorldProject, &mModelViewStack[mModelViewTop],
                          &mProjectionMat);
  }
}

// If WoldProjectmatrix has been modified due to insert or force matrix (Kirby,
// SSB / Tarzan, Rayman2, Donald duck, SW racer, Robot on wheels) we need to
// update sceGU projmtx //Corn
inline void BaseRenderer::PokeWorldProject() {
  if (mWPmodified) {
    mWPmodified = false;
    mReloadProj = true;
    if (gGlobalPreferences.ViewportType ==
        VT_FULLSCREEN_HD) { // proper 16:9 scale
    ...
    sceGuSetMatrix(GU_PROJECTION,
                   reinterpret_cast<const ScePspFMatrix4 *>(&mWorldProject));
...
void BaseRenderer::InsertMatrix(u32 w0, u32 w1) {
  ...
  if (w0 & 0x20) {
    ...
    mWorldProject.m[y][x] =
        (f32)(s32)mWorldProject.m[y][x] + ((f32)(w1 >> 16) / 65536.0f);
    mWorldProject.m[y][x + 1] =
        (f32)(s32)mWorldProject.m[y][x + 1] + ((f32)(w1 & 0xFFFF) / 65536.0f);
  } else {
    // Change integer part
  ...
  DL_PF("    %#+12.5f %#+12.5f %#+12.5f %#+12.5f\n"
        ...
        "    %#+12.5f %#+12.5f %#+12.5f %#+12.5f\n"
        "    %#+12.5f %#+12.5f %#+12.5f %#+12.5f\n",
        mWorldProject.m[0][0], mWorldProject.m[0][1], mWorldProject.m[0][2],
        mWorldProject.m[0][3], mWorldProject.m[1][0], mWorldProject.m[1][1],
        mWorldProject.m[1][2], mWorldProject.m[1][3], mWorldProject.m[2][0],
        mWorldProject.m[2][1], mWorldProject.m[2][2], mWorldProject.m[2][3],
        mWorldProject.m[3][0], mWorldProject.m[3][1], mWorldProject.m[3][2],
        mWorldProject.m[3][3]);
#endif
}
