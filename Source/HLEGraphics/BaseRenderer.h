...
DAEDALUS_STATIC_ASSERT(
    sizeof(DaedalusLight) ==
    64); // Size=64 bytes and order is important or VFPU ASM for PSP will fail
...
// +++---
// zyxzyx
// NB: These are ordered such that the VFPU can generate them easily - make sure
// you keep the VFPU code up to date if changing these.
...
public:
...
#ifdef DAEDALUS_PSP
  inline void SetTnLMode(u32 mode) {
    mTnL.Flags.Modes = mode; /*UpdateFogEnable(); UpdateShadeModel();*/
  }
#else
  inline void SetTnLMode(u32 mode) {
    mTnL.Flags.Modes = mode;
    UpdateFogEnable(); /*UpdateShadeModel();*/
  }
#endif
  ...
  inline void SetCullMode(bool enable, bool mode) {
    mTnL.Flags.TriCull = enable;
    mTnL.Flags.CullBack = mode;
  }

  // Fog stuff
  inline void SetFogMultOffs(f32 Mult, f32 Offs) {
    mTnL.FogMult = Mult / 255.0f;
    mTnL.FogOffs = Offs / 255.0f;
  }
  inline void SetFogMinMax(f32 fog_near, f32 fog_far) {
    sceGuFog(fog_near, fog_far, mFogColour.GetColour());
  }
  inline void SetFogColour(c32 colour) { mFogColour = colour; }

  // PrimDepth will replace the z value if depth_source=1 (z range 32767-0 while
  // PSP depthbuffer range 0-65535)//Corn
#ifdef DAEDALUS_PSP
  inline void SetPrimitiveDepth(u32 z) {
    mPrimDepth = (f32)(((32767 - z) << 1) + 1);
  }
#else
  inline void SetPrimitiveDepth(u32 z) {
    mPrimDepth = (f32)(z - 0x4000) / (f32)0x4000;
  }
#endif
  ...
  inline void SetLightCol(u32 l, u8 r, u8 g, u8 b) {
    mTnL.Lights[l].SkipIfZero = (r + g + b);
    mTnL.Lights[l].Colour.x = r / 255.0f;
    mTnL.Lights[l].Colour.y = g / 255.0f;
    mTnL.Lights[l].Colour.z = b / 255.0f;
  }
  inline void SetLightDirection(u32 l, f32 x, f32 y, f32 z) {
    v3 n(x, y, z);
    n.Normalise();
    mTnL.Lights[l].Direction.x = n.x;
    mTnL.Lights[l].Direction.y = n.y;
    mTnL.Lights[l].Direction.z = n.z;
  }
  inline void SetLightPosition(u32 l, f32 x, f32 y, f32 z, f32 w) {
    mTnL.Lights[l].Position.x = x;
    mTnL.Lights[l].Position.y = y;
    mTnL.Lights[l].Position.z = z;
    mTnL.Lights[l].Position.w = w;
  }
  inline void SetLightCBFD(u32 l, u8 nonzero) {
    mTnL.Lights[l].Iscale = (f32)(nonzero << 12);
    mTnL.Lights[l].SkipIfZero = mTnL.Lights[l].SkipIfZero && nonzero;
  }
  inline void SetLightEx(u32 l, f32 ca, f32 la, f32 qa) {
    mTnL.Lights[l].ca = ca / 16.0f;
    mTnL.Lights[l].la = la / 65535.0f;
    mTnL.Lights[l].qa = qa / (8.0f * 65535.0f);
  }
  ...
  inline void SetTextureScale(float fScaleX, float fScaleY) {
    mTnL.TextureScaleX = fScaleX;
    mTnL.TextureScaleY = fScaleY;
  }

  // TextRect stuff
  virtual void TexRect(u32 tile_idx, const v2 &xy0, const v2 &xy1, TexCoord st0,
                       TexCoord st1) = 0;
  virtual void TexRectFlip(u32 tile_idx, const v2 &xy0, const v2 &xy1,
                           TexCoord st0, TexCoord st1) = 0;
  virtual void FillRect(const v2 &xy0, const v2 &xy1, u32 color) = 0;

  // Texture stuff
  virtual void Draw2DTexture(f32 x0, f32 y0, f32 x1, f32 y1, f32 u0, f32 v0,
                             f32 u1, f32 v1, const CNativeTexture *texture) = 0;
  virtual void Draw2DTextureR(f32 x0, f32 y0, f32 x1, f32 y1, f32 x2, f32 y2,
                              f32 x3, f32 y3, f32 s, f32 t) = 0;
  ...
  // inline void			PopProjection() {if (mProjectionTop > 0)
  // --mProjectionTop;	mWorldProjectValid = false;}
  inline void PopWorldView(u32 num = 1) {
    if (mModelViewTop > (num - 1))
      mModelViewTop -= num;
    mWorldProjectValid = false;
  }
  void InsertMatrix(u32 w0, u32 w1);
  void ForceMatrix(const u32 address);
  inline void DKRMtxChanged(u32 idx) {
    mWPmodified = true;
    mDKRMatIdx = idx;
  }

  // Vertex stuff
  void SetNewVertexInfo(u32 address, u32 v0,
                        u32 n); // Assumes dwAddress has already been checked!
  void SetNewVertexInfoConker(u32 address, u32 v0, u32 n); // For conker..
  void SetNewVertexInfoDKR(
      u32 address, u32 v0, u32 n,
      bool billboard); // Assumes dwAddress has already been checked!
  void SetNewVertexInfoPD(u32 address, u32 v0,
                          u32 n); // Assumes dwAddress has already been checked!
  void ModifyVertexInfo(u32 whered, u32 vert, u32 val);
  void SetVtxColor(u32 vert, u32 color);
  inline void SetVtxTextureCoord(u32 vert, s16 tu, s16 tv) {
  ...
  inline void CopyVtx(u32 vert_src, u32 vert_dst) {
    mVtxProjected[vert_dst] = mVtxProjected[vert_src];
  }
  ...
  inline bool TestVerts(u32 v0, u32 vn) const {
    u32 f = mVtxProjected[v0].ClipFlags;
    for (u32 i = v0 + 1; i <= vn; i++)
      f &= mVtxProjected[i].ClipFlags;
    return f == 0;
  }
  inline s32 GetVtxDepth(u32 i) const {
    return (s32)mVtxProjected[i].ProjectedPos.z;
  }
  inline v4 GetTransformedVtxPos(u32 i) const {
    return mVtxProjected[i].TransformedPos;
  }
  inline v4 GetProjectedVtxPos(u32 i) const {
    return mVtxProjected[i].ProjectedPos;
  }
  ...
  inline float N64ToScreenX(float x) const {
    return x * mN64ToScreenScale.x + mN64ToScreenTranslate.x;
  }
  inline float N64ToScreenY(float y) const {
    return y * mN64ToScreenScale.y + mN64ToScreenTranslate.y;
  }
...
protected:
  ...
  inline void UpdateFogEnable() {
    if (gFogEnabled)
      mTnL.Flags.Fog ? sceGuEnable(GU_FOG) : sceGuDisable(GU_FOG);
  }
  inline void UpdateShadeModel() {
    sceGuShadeModel(mTnL.Flags.Shade ? GU_SMOOTH : GU_FLAT);
  }
#else
  inline void UpdateFogEnable() {
    if (gFogEnabled)
      mTnL.Flags.Fog ? glEnable(GL_FOG) : glDisable(GL_FOG);
  }
  inline void UpdateShadeModel() {
    glShadeModel(mTnL.Flags.Shade ? GL_SMOOTH : GL_FLAT);
  }
#endif
  ...
  //*****************************************************************************
  // We round these value here, so that when we scale up the coords to our
  // screen coords we don't get any gaps.
  //*****************************************************************************
  ...
  virtual void RenderTriangles(DaedalusVtx *p_vertices, u32 num_vertices,
                               bool disable_zbuffer) = 0;

  void TestVFPUVerts(u32 v0, u32 num, const FiddledVtx *verts,
                     const Matrix4x4 &mat_world);
  template <bool FogEnable, int TextureMode>
  void ProcessVerts(u32 v0, u32 num, const FiddledVtx *verts,
                    const Matrix4x4 &mat_world);
...
protected:
  ...
  u32 mFillColour; // RDP. NB u32 not c32 as this is typically 2 16-bit colour
                   // values.
  // Texturing
  struct TextureWrap {
    u32 u;
  ...
  Matrix4x4 mModelViewStack[MATRIX_STACK_SIZE]{}; // DKR reuses these and need
                                                  // at least 4 //Corn
  u32 mModelViewTop;
  u32 mMatStackSize{};
  ...
  Matrix4x4 mScreenToDevice; // Used by OSX renderer - scales screen coords
                             // (0..640 etc) to device coords (-1..+1)
#endif

  static const u32 kMaxIndices =
      320; // We need at least 80 verts * 3 = 240? But Flying Dragon uses more
           // than 256 //Corn
  ...
  DaedalusVtx4
      mVtxProjected[kMaxN64Vertices]; // Transformed and projected vertices
                                      // (suitable for clipping etc)
  u32 mVtxClipFlagsUnion; // Bitwise OR of all the vertex flags added to the
                          // current batch. If this is 0, we can trivially
                          // accept everything without clipping
