...
static bool GenerateTexels(void **p_texels, void **p_palette,
                           const TextureInfo &ti, ETextureFormat texture_format,
                           u32 pitch, u32 buffer_size) {
  if (gTexelBuffer.size() <
      buffer_size) //|| gTexelBuffer.size() > (128 * 1024))//Cut off for
                   //downsizing may need to be adjusted to prevent some
                   //thrashing
  {
  ...
  NativePf8888 *palette =
      IsTextureFormatPalettised(texture_format) ? gPaletteBuffer : nullptr;

#ifdef DAEDALUS_ACCURATE_TMEM
  // NB: if line is 0, it implies this is a direct load from ram (e.g.
  // DLParser_Sprite2DDraw etc) This check isn't robust enough, SSV set ti.Line
  // == 0 in game without calling Sprite2D
  if (ti.GetLine() > 0) {
...
static void UpdateTexture(const TextureInfo &ti, CNativeTexture *texture) {
  ...
  if (texture != nullptr && texture->HasData()) {
    ...
    void *texels;
    void *palette;
    if (GenerateTexels(&texels, &palette, ti, format, stride,
                       texture->GetBytesRequired())) {
      //
      //	Recolour the texels
      //
      if (ti.GetWhite()) {
        Recolour(texels, palette, ti.GetWidth(), ti.GetHeight(), stride, format,
                 c32::White);
      }

      //
      //	Clamp edges. We do this so that non power-of-2 textures whose
      //whose width/height 	is less than the mask value clamp correctly. It still
      //doesn't fix those 	textures with a width which is greater than the
      //power-of-2 size.
      //
      ClampTexels(texels, ti.GetWidth(), ti.GetHeight(),
                  texture->GetCorrectedWidth(), texture->GetCorrectedHeight(),
                  stride, format);
      ...
      if (mirror_s || mirror_t) {
        MirrorTexels(mirror_s, mirror_t, texels, stride, texels, stride, format,
                     ti.GetWidth(), ti.GetHeight());
      }
...
CachedTexture::CachedTexture(const TextureInfo &ti)
    : mTextureInfo(ti), mpTexture(nullptr), mTextureContentsHash(0),
      mFrameLastUpToDate(gRDPFrame), mFrameLastUsed(gRDPFrame) {}

CachedTexture::~CachedTexture() = default;
...
bool CachedTexture::Initialise() {
  ...
  if (mTextureInfo.GetEmulateMirrorS())
    width *= 2;
  if (mTextureInfo.GetEmulateMirrorT())
    height *= 2;

  mpTexture =
      CNativeTexture::Create(width, height, SelectNativeFormat(mTextureInfo));
  if (mpTexture != nullptr) {
    ...
    // created on the same frame we update them over a nice distribution of
    // frames.

    if (gCheckTextureHashFrequency > 0) {
      mFrameLastUpToDate =
          gRDPFrame + (FastRand() & (gCheckTextureHashFrequency - 1));
    }
    UpdateTextureHash();
...
// Update the hash of the texture. Returns true if the texture should be
// updated.
...
void CachedTexture::UpdateIfNecessary() {
  if (!IsFresh()) {
    ...
    // FIXME(strmnrmn): should probably recreate mpWhiteTexture if it exists,
    // else it may have stale data.
...
bool CachedTexture::HasExpired() const {
  if (!kUpdateTexturesEveryFrame) {
    if (!IsFresh()) {
      // Hack to make WONDER PROJECT J2 work (need to reload some textures every
      // frame!) //Corn
      if ((g_ROM.GameHacks == WONDER_PROJECTJ2) &&
          (mTextureInfo.GetTLutFormat() == kTT_RGBA16) &&
          (mTextureInfo.GetSize() == G_IM_SIZ_8b))
        return true;

      // Hack for Worms Armageddon
      if ((g_ROM.GameHacks == WORMS_ARMAGEDDON) &&
          (mTextureInfo.GetSize() == G_IM_SIZ_8b) &&
          (mTextureContentsHash != mTextureInfo.GenerateHashValue()))
        return true;

      // Hack for Zelda OOT & MM text (only needed if there is not a general
      // hash check) //Corn
      if (g_ROM.ZELDA_HACK && (mTextureInfo.GetSize() == G_IM_SIZ_4b) &&
          mTextureContentsHash != mTextureInfo.GenerateHashValue())
        return true;

      // Check if texture has changed
      // if( mTextureContentsHash != mTextureInfo.GenerateHashValue() ) return
      // true;
    }
  }

  // Otherwise we wait 20+random(0-3) frames before trashing the texture if
  // unused Spread trashing them over time so not all get killed at once (lower
  // value uses less VRAM) //Corn
  return gRDPFrame - mFrameLastUsed > (20 + (FastRand() & 0x3));
}

#ifdef DAEDALUS_DEBUG_DISPLAYLIST
void CachedTexture::DumpTexture(const TextureInfo &ti,
  ...
  if (texture != nullptr && texture->HasData()) {
    ...
    sprintf(filename, "%08x-%s_%dbpp-%dx%d-%dx%d.png", ti.GetLoadAddress(),
            ti.GetFormatName(), ti.GetSizeInBits(), 0, 0, // Left/Top
            ti.GetWidth(), ti.GetHeight());
    ...
    // Note that we re-convert the texels because those in the native texture
    // may well already be swizzle. Maybe we should just have an unswizzle
    // routine?
    if (GenerateTexels(&texels, &palette, ti, texture->GetFormat(),
                       texture->GetStride(), texture->GetBytesRequired())) {
      // NB - this does not include the mirrored texels

      // NB we use the palette from the native texture. This is a total hack.
      // We have to do this because the palette texels come from emulated tmem,
      // rather than ram. This means that when we dump out the texture here,
      // tmem won't necessarily contain our pixels.
      const void *native_palette = texture->GetPalette();

      PngSaveImage(filepath, texels, native_palette, texture->GetFormat(),
                   texture->GetStride(), ti.GetWidth(), ti.GetHeight(), true);
    }
  }
}
