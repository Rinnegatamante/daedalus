...
struct TextureDestInfo {
  explicit TextureDestInfo(ETextureFormat tex_fmt)
      : Format(tex_fmt), Width(0), Height(0), Pitch(0), Data(nullptr),
        Palette(nullptr) {}

  ETextureFormat Format;
  u32 Width;  // Describes the width of the locked area. Use lPitch to move
              // between successive lines
  u32 Height; // Describes the height of the locked area
  s32 Pitch;  // Specifies the number of bytes on each row (not necessarily
              // bitdepth*width/8)
...
static const u8 FourToEight[16] = {0x00, 0x11, 0x22, 0x33, 0x44, 0x55,
                                   0x66, 0x77, 0x88, 0x99, 0xaa, 0xbb,
                                   0xcc, 0xdd, 0xee, 0xff};

template <u32 Size> struct SByteswapInfo;
...
template <u32 Size> struct SSwizzleInfo;
...
template <typename OutT> struct SConvertGeneric {
  typedef void (*ConvertRowFunction)(OutT *dst, const u8 *src, u32 src_offset,
                                     u32 width);

  static void ConvertGeneric(const TextureDestInfo &dsti, const TextureInfo &ti,
...
typedef void (*ConvertPalettisedRowFunction)(NativePf8888 *dst, const u8 *src,
                                             u32 src_offset, u32 width,
                                             const NativePf8888 *palette);

static void ConvertPalettisedTo8888(const TextureDestInfo &dsti,
...
static void ConvertPalettisedToCI(
    const TextureDestInfo &dsti, const TextureInfo &ti,
...
template <typename InT> struct SConvert {
  enum { Fiddle = SByteswapInfo<sizeof(InT)>::Fiddle };
  enum { Swizzle = SSwizzleInfo<sizeof(InT)>::Swizzle };

  //
  //	This routine converts from any format which is > 1 byte to any Psp
  //format.
  //
  template <typename OutT, u32 InFiddle, u32 OutFiddle>
  static inline void ConvertRow(OutT *dst, const u8 *src, u32 src_offset,
                                u32 width) {
#ifdef DAEDALUS_ENABLE_ASSERTS
    DAEDALUS_DL_ASSERT(IsAligned(src_offset, sizeof(InT)),
                       "Offset should be correctly aligned");
#endif
    //
    //	Need to be careful of this - ensure that it's doing the right thing in
    //all cases and not overflowing rows. 	This is to ensure that we correctly
    //convert all the texels in a row, even when we're fiddling. 	If we have a
    //fiddle of 2 for instance, and the row is not a multiple of the fiddle
    //amount 	then we don't convert enough pixels (we actually poke some values
    //in past the end of the row) 	and get some random noise at the end instead.
    //
    //	There may well be an easier (and less gross)  way of doing this if we
    //move the OutFiddle calculation 	into the source pixel lookup, and just have
    //dst[x] = ...
    //
  ...
  template <typename OutT>
  static inline void ConvertTextureT(const TextureDestInfo &dsti,
                                     const TextureInfo &ti) {
  ...
  static void ConvertTexture(const TextureDestInfo &dsti,
                             const TextureInfo &ti) {
    switch (dsti.Format) {
    case TexFmt_5650:
      ConvertTextureT<NativePf5650>(dsti, ti);
      return;
    case TexFmt_5551:
      ConvertTextureT<NativePf5551>(dsti, ti);
      return;
    case TexFmt_4444:
      ConvertTextureT<NativePf4444>(dsti, ti);
      return;
    case TexFmt_8888:
      ConvertTextureT<NativePf8888>(dsti, ti);
      return;
...
struct SConvertIA4 {
  ...
  static inline void ConvertRow(OutT *dst, const u8 *src, u32 src_offset,
    ...
    for (u32 x{}; x < width; x += 2) {
      ...
      // Even
      dst[x + 0] =
          OutT(ThreeToEight[(b & 0xE0) >> 5], ThreeToEight[(b & 0xE0) >> 5],
               ThreeToEight[(b & 0xE0) >> 5], OneToEight[(b & 0x10) >> 4]);
      // Odd
      dst[x + 1] =
          OutT(ThreeToEight[(b & 0x0E) >> 1], ThreeToEight[(b & 0x0E) >> 1],
               ThreeToEight[(b & 0x0E) >> 1], OneToEight[(b & 0x01)]);
      src_offset++;
    }
  ...
  static inline void ConvertTextureT(const TextureDestInfo &dsti,
                                     const TextureInfo &ti) {
    SConvertGeneric<OutT>::ConvertGeneric(
        dsti, ti, ConvertRow<OutT, 0x4 | Fiddle>, ConvertRow<OutT, Fiddle>);
  }
...
struct SConvertI4 {
  ...
  static inline void ConvertRow(OutT *dst, const u8 *src, u32 src_offset,
    ...
    for (u32 x{}; x + 1 < width; x += 2) {
      ...
      // Even
      dst[x + 0] =
          OutT(FourToEight[(b & 0xF0) >> 4], FourToEight[(b & 0xF0) >> 4],
               FourToEight[(b & 0xF0) >> 4], FourToEight[(b & 0xF0) >> 4]);
      // Odd
      dst[x + 1] = OutT(FourToEight[(b & 0x0F)], FourToEight[(b & 0x0F)],
                        FourToEight[(b & 0x0F)], FourToEight[(b & 0x0F)]);
...
static void ConvertPalette(ETLutFmt tlut_format, NativePf8888 *dst,
                           const void *src, u32 count) {
...
template <u32 F>
static void ConvertCI4_Row(NativePfCI44 *dst, const u8 *src, u32 src_offset,
                           u32 width) {
  for (u32 x{}; x + 1 < width; x += 2) {
    u8 b = src[src_offset ^ F];
...
template <u32 F>
static void ConvertCI4_Row_To_8888(NativePf8888 *dst, const u8 *src,
                                   u32 src_offset, u32 width,
                                   const NativePf8888 *palette) {
#ifdef DAEDALUS_ENABLE_ASSERTS
...
template <u32 F>
static void ConvertCI8_Row(NativePfCI8 *dst, const u8 *src, u32 src_offset,
                           u32 width) {
  for (u32 x{}; x < width; x++) {
...
template <u32 F>
static void ConvertCI8_Row_To_8888(NativePf8888 *dst, const u8 *src,
                                   u32 src_offset, u32 width,
                                   const NativePf8888 *palette) {
#ifdef DAEDALUS_ENABLE_ASSERTS
  DAEDALUS_ASSERT(palette, "No palette");
...
static void ConvertCI8(const TextureDestInfo &dsti, const TextureInfo &ti) {
  ...
  NativePf8888 *dst_palette =
      dsti.Palette ? reinterpret_cast<NativePf8888 *>(dsti.Palette)
                   : temp_palette;
  ...
  case TexFmt_CI8_8888:
    ConvertPalettisedToCI(dsti, ti, ConvertCI8_Row<0x4 | 0x3>,
                          ConvertCI8_Row<0x3>);
    break;
...
static void ConvertCI4(const TextureDestInfo &dsti, const TextureInfo &ti) {
  ...
  case TexFmt_CI4_8888:
    ConvertPalettisedToCI(dsti, ti, ConvertCI4_Row<0x4 | 0x3>,
                          ConvertCI4_Row<0x3>);
    break;
...
typedef void (*ConvertFunction)(const TextureDestInfo &dsti,
                                const TextureInfo &ti);
...
bool ConvertTexture(const TextureInfo &ti, void *texels, NativePf8888 *palette,
  ...
  // Do nothing if palette address is nullptr or close to nullptr in a palette
  // texture //Corn Loading a SaveState (OOT -> SSV) dont bring back our TMEM
  // data which causes issues for the first rendered frame. Checking if the
  // palette pointer is less than 0x1000 (rather than just nullptr) fixes it.
  // Seems to happen on the first frame of Goldeneye too?
  if ((ti.GetFormat() == G_IM_FMT_CI) && (ti.GetTlutAddress() < 0x1000))
    return false;
  ...
  const ConvertFunction fn =
      gConvertFunctions[(ti.GetFormat() << 2) | ti.GetSize()];
  if (fn) {
    fn(dsti, ti);
    return true;
  }
