...
#define DL_UNIMPLEMENTED_ERROR(msg)                                            \
  {                                                                            \
    ...
    if (!shown) {                                                              \
      ...
      DAEDALUS_DL_ERROR("%s: %08x %08x", (msg), command.inst.cmd0,             \
                        command.inst.cmd1);                                    \
      shown = true;                                                            \
    }                                                                          \
  }
...
#define RDPSegAddr(seg)                                                        \
  ((gSegments[((seg) >> 24) & 0x0F] & 0x00ffffff) + ((seg)&0x00FFFFFF))
...
void DLParser_InitMicrocode(u32 code_base, u32 code_size, u32 data_base,
                            u32 data_size);
...
inline void DLParser_FetchNextCommand(MicroCodeCommand *p_command) {
  ...
  DAEDALUS_ASSERT(pc < MAX_RAM_ADDRESS,
                  "Display list PC is out of range: 0x%08x", pc);
#endif
...
static const char *const gFormatNames[8] = {"RGBA", "YUV", "CI", "IA",
                                            "I",    "?1",  "?2", "?3"};
...
void DLParser_DumpVtxInfo(u32 address, u32 v0_idx, u32 num_verts) {
  if (DLDebug_IsActive()) {
    ...
    for (u32 idx = v0_idx; idx < v0_idx + num_verts; idx++) {
      ...
      DL_PF("    #%02d Flags: 0x%04x Pos:{% 0.1f,% 0.1f,% 0.1f} Tex:{% 7.2f,% "
            "7.2f} Extra: %02x %02x %02x %02x Tran:{% 0.3f,% 0.3f,% 0.3f,% "
            "0.3f} Proj:{% 6f,% 6f,% 6f,% 6f}",
            idx, wFlags, x, y, z, tu, tv, a, b, c, d, t.x, t.y, t.z, t.w,
...
//*************************************************************************************
// This is called from Microcode.cpp after a custom ucode has been detected and
// cached This function is only called once per custom ucode set Main resaon for
// this function is to save memory since custom ucodes share a common table
//	ucode:			custom ucode (ucode>= MAX_UCODE)
//	offset:			offset to normal ucode this custom ucode is
//based of ex GBI0
//*************************************************************************************
static void DLParser_SetCustom(u32 ucode, u32 offset) {
  memcpy(&gCustomInstruction, &gNormalInstruction[offset],
         1024); // sizeof(gNormalInstruction)/MAX_UCODE
...
void DLParser_InitMicrocode(u32 code_base, u32 code_size, u32 data_base,
                            u32 data_size) {
  u32 ucode{GBIMicrocode_DetectVersion(code_base, code_size, data_base,
                                       data_size, &DLParser_SetCustom)};

  gVertexStride = ucode_stride[ucode];
  gLastUcodeBase = code_base;
  gUcodeFunc =
      IS_CUSTOM_UCODE(ucode) ? gCustomInstruction : gNormalInstruction[ucode];

  // Used for fetching ucode names (Debug Only)
#if defined(DAEDALUS_DEBUG_DISPLAYLIST) || defined(DAEDALUS_ENABLE_PROFILING)
  gUcodeName = IS_CUSTOM_UCODE(ucode) ? gCustomInstructionName
                                      : gNormalInstructionName[ucode];
#endif
}
...
#define PROFILE_DL_CMD(cmd)                                                    \
  if (gpProfileItemHandles[(cmd)] == nullptr) {                                \
    gpProfileItemHandles[(cmd)] =                                              \
        new SProfileItemHandle(CProfiler::Get()->AddItem(gUcodeName[cmd]));    \
  }                                                                            \
...
static u32 DLParser_ProcessDList(u32 instruction_limit) {
  ...
  while (gDlistStackPointer >= 0) {
    ...
    DL_BEGIN_INSTR(current_instruction_count, command.inst.cmd0,
                   command.inst.cmd1, gDlistStackPointer,
                   gUcodeName[command.inst.cmd]);
...
u32 DLParser_Process(u32 instruction_limit, DLDebugOutput *debug_output) {
  ...
  // Update Screen only when something is drawn, otherwise several games ex Army
  // Men will flash or shake.
...
void MatrixFromN64FixedPoint(Matrix4x4 &mat, u32 address) {
#ifdef DAEDALUS_ENABLE_ASSERTS
  DAEDALUS_ASSERT(address + 64 < MAX_RAM_ADDRESS,
                  "Mtx: Address invalid (0x%08x)", address);
#endif
...
#endif
#ifdef DAEDALUS_DEBUG_DISPLAYLIST
  DL_PF("    Light[%d] RGB[%d, %d, %d] x[%d] y[%d] z[%d]", light_idx, r, g, b,
        dir_x, dir_y, dir_z);
...
// vscale is the scale applied to the normalized homogeneous coordinates after
// 4x4 projection transformation vtrans is the offset added to the scaled number

void RDP_MoveMemViewport(u32 address) {
#ifdef DAEDALUS_ENABLE_ASSERTS
  DAEDALUS_ASSERT(address + 16 < MAX_RAM_ADDRESS,
                  "MoveMem Viewport, invalid memory");
#endif
...
#endif
  // Terminate!
  //	DBGConsole_Msg(0, "Warning, DL cut short with unknown command: 0x%08x
  //0x%08x", command.inst.cmd0, command.inst.cmd1);
  DLParser_PopDL();
}
...
#ifdef DAEDALUS_DEBUG_DISPLAYLIST
  DL_PF("    SetPrimDepth z[0x%04x] dz[0x%04x]", command.primdepth.z,
        command.primdepth.dz);
#endif
...
#ifdef DAEDALUS_DEBUG_DISPLAYLIST
  DL_PF("    RDPSetOtherMode: 0x%08x 0x%08x", command.inst.cmd0,
        command.inst.cmd1);
#endif
...
//
//*****************************************************************************
void DLParser_RDPLoadSync(
    MicroCodeCommand command) { /*DL_PF("    LoadSync: (Ignored)");*/
}
void DLParser_RDPPipeSync(
    MicroCodeCommand command) { /*DL_PF("    PipeSync: (Ignored)");*/
}
void DLParser_RDPTileSync(
    MicroCodeCommand command) { /*DL_PF("    TileSync: (Ignored)");*/
}
...
#ifdef DAEDALUS_DEBUG_DISPLAYLIST
  DL_PF("    x0=%d y0=%d x1=%d y1=%d mode=%d", scissors.left, scissors.top,
        scissors.right, scissors.bottom, command.scissor.mode);
#endif
  // Set the cliprect now...
  if (scissors.left < scissors.right && scissors.top < scissors.bottom) {
    gRenderer->SetScissor(scissors.left, scissors.top, scissors.right,
                          scissors.bottom);
  }
}
//*****************************************************************************
...
#ifdef DAEDALUS_DEBUG_DISPLAYLIST
  DL_PF("    Tile[%d] Format[%s/%s] Line[%d] TMEM[0x%03x] Palette[%d]",
        tile.tile_idx, gFormatNames[tile.format], gSizeNames[tile.size],
        tile.line, tile.tmem, tile.palette);
  DL_PF("      S: Clamp[%s] Mirror[%s] Mask[0x%x] Shift[0x%x]",
        gOnOffNames[tile.clamp_s], gOnOffNames[tile.mirror_s], tile.mask_s,
        tile.shift_s);
  DL_PF("      T: Clamp[%s] Mirror[%s] Mask[0x%x] Shift[0x%x]",
        gOnOffNames[tile.clamp_t], gOnOffNames[tile.mirror_t], tile.mask_t,
        tile.shift_t);
#endif
}
...
void DLParser_SetTileSize(MicroCodeCommand command) {
  ...
  tile.cmd1 = command.inst.cmd1;
#ifdef DAEDALUS_DEBUG_DISPLAYLIST
  DL_PF("    Tile[%d] (%d,%d) -> (%d,%d) [%d x %d]", tile.tile_idx,
        tile.left / 4, tile.top / 4, tile.right / 4, tile.bottom / 4,
...
void DLParser_SetTImg(MicroCodeCommand command) {
  ...
  DL_PF("    TImg Adr[0x%08x] Format[%s/%s] Width[%d] Pitch[%d] Bytes/line[%d]",
        g_TI.Address, gFormatNames[g_TI.Format], gSizeNames[g_TI.Size],
        g_TI.Width, g_TI.GetPitch(), g_TI.Width << g_TI.Size >> 1);
#endif
}
...
void DLParser_TexRect(MicroCodeCommand command) {
...
#ifdef DAEDALUS_DEBUG_DISPLAYLIST
  DAEDALUS_DL_ASSERT(
      gRDPOtherMode.cycle_type != CYCLE_COPY || tex_rect.dsdx == (4 << 10),
      "Expecting dsdx of 4<<10 in copy mode, got %d", tex_rect.dsdx);
#endif
  // NB: In FILL and COPY mode, rectangles are scissored to the nearest four
  // pixel boundary. This isn't currently handled, but I don't know of any games
  // that depend on it.

  // Keep integers for as long as possible //Corn

  // X for upper left corner should be less than X for lower right corner else
  // skip rendering it, seems to happen in Rayman 2 and Star Soldier
  ...
  // Fixes black box in SSB when moving far way from the screen and offscreen in
  // Conker
  ...
  // Removes offscreen texrect, also fixes several glitches like in John
  // Romero's Daikatana
  if (tex_rect.x0 >= (scissors.right << 2) ||
      tex_rect.y0 >= (scissors.bottom << 2) ||
      tex_rect.x1 < (scissors.left << 2) || tex_rect.y1 < (scissors.top << 2)) {
#ifdef DAEDALUS_DEBUG_DISPLAYLIST
    ++gNumRectsClipped;
#endif
    return;
  ...
  rect_s0 += (((u32)rect_dsdx >> 31)
              << 5); // Fixes California Speed, if(rect_dsdx<0) rect_s0 += 32;
  ...
  s16 rect_s1{
      (s16)(rect_s0 + (rect_dsdx * (tex_rect.x1 - tex_rect.x0) >>
                       7))}; // 7 = (>>10)=1/1024, (>>2)=1/4 and (<<5)=32
...
#ifdef DAEDALUS_DEBUG_DISPLAYLIST
  DL_PF("    Screen(%.1f,%.1f) -> (%.1f,%.1f) Tile[%d]", xy0.x, xy0.y, xy1.x,
        xy1.y, tex_rect.tile_idx);
  DL_PF("    Tex:(%#5.3f,%#5.3f) -> (%#5.3f,%#5.3f) (DSDX:%#5f DTDY:%#5f)",
        rect_s0 / 32.f, rect_t0 / 32.f, rect_s1 / 32.f, rect_t1 / 32.f,
        rect_dsdx / 1024.f, rect_dtdy / 1024.f);
#endif
...
void DLParser_TexRectFlip(MicroCodeCommand command) {
  ...
  s16 rect_s1{(s16)(rect_s0 + (rect_dsdx * (tex_rect.y1 - tex_rect.y0) >>
                               7))}; // Flip - use y
  s16 rect_t1{(s16)(rect_t0 + (rect_dtdy * (tex_rect.x1 - tex_rect.x0) >>
                               7))}; // Flip - use x
  ...
  DL_PF("    Screen(%.1f,%.1f) -> (%.1f,%.1f) Tile[%d]", xy0.x, xy0.y, xy1.x,
        xy1.y, tex_rect.tile_idx);
  DL_PF("    FlipTex:(%#5.3f,%#5.3f) -> (%#5.3f,%#5.3f) (DSDX:%#5f DTDY:%#5f)",
        rect_s0 / 32.f, rect_t0 / 32.f, rect_s1 / 32.f, rect_t1 / 32.f,
        rect_dsdx / 1024.f, rect_dtdy / 1024.f);
#endif
...
// Clear framebuffer, thanks Gonetz!
...
void DLParser_FillRect(MicroCodeCommand command) {
  //
  // Removes annoying rect that appears in Conker and fillrects that cover
  // screen in banjo tooie
...
#else
    if (true) // This always enabled for PC, this should be optional once we
              // have a GUI to disable it!
#endif
    {
  ...
  // Should we init with Prim or Blend colour? Doesn't work well see Mk64
  // transition before a race
  c32 colour{};
...
#ifdef DAEDALUS_DEBUG_DISPLAYLIST
  DL_PF("    Filling Rectangle (%d,%d)->(%d,%d)", command.fillrect.x0,
        command.fillrect.y0, command.fillrect.x1, command.fillrect.y1);
#endif
...
#endif
  // No need check for (MAX_RAM_ADDRESS-1) here, since g_DI.Address is never
  // used to reference a RAM location
...
#ifdef DAEDALUS_DEBUG_DISPLAYLIST
  DL_PF("    CImg Adr[0x%08x] Format[%s] Size[%s] Width[%d]",
        RDPSegAddr(command.inst.cmd1), gFormatNames[g_CI.Format],
        gSizeNames[g_CI.Size], g_CI.Width);
#endif
}
...
#ifdef DAEDALUS_DEBUG_DISPLAYLIST
  DL_PF("    RGBA: %d %d %d %d", command.color.r, command.color.g,
        command.color.b, command.color.a);
#endif
  // c32	fog_colour( command.color.r, command.color.g, command.color.b,
  // command.color.a );
  c32 fog_colour(command.color.r, command.color.g, command.color.b,
                 0); // alpha is always 0
...
void DLParser_SetBlendColor(MicroCodeCommand command) {
  ...
  DL_PF("    RGBA: %d %d %d %d", command.color.r, command.color.g,
        command.color.b, command.color.a);
#endif
  c32 blend_colour(command.color.r, command.color.g, command.color.b,
                   command.color.a);
...
#ifdef DAEDALUS_DEBUG_DISPLAYLIST
  DL_PF("    M:%d L:%d RGBA: %d %d %d %d", command.color.prim_min_level,
        command.color.prim_level, command.color.r, command.color.g,
        command.color.b, command.color.a);
#endif
  c32 prim_colour(command.color.r, command.color.g, command.color.b,
                  command.color.a);
...
void DLParser_SetEnvColor(MicroCodeCommand command) {
  ...
  DL_PF("    RGBA: %d %d %d %d", command.color.r, command.color.g,
        command.color.b, command.color.a);
#endif
  c32 env_colour(command.color.r, command.color.g, command.color.b,
                 command.color.a);
