...
// F3DLX: Compatible with F3DEX, GBI, but not sub-pixel accurate. Clipping can
// be explicitly enabled/disabled F3DLX.Rej: No clipping, rejection instead.
// Vertex cache is 64 F3FLP.Rej: Like F3DLX.Rej. Vertex cache is 80 L3DEX: Line
// processing, Vertex cache is 32.
...
static bool GBIMicrocode_DetectVersionString(u32 data_base, u32 data_size,
                                             char *str, u32 str_len) {
#ifdef DAEDALUS_ENABLE_ASSERTS
  DAEDALUS_ASSERT(data_base < MAX_RAM_ADDRESS + 0x1000,
                  "GBIMicrocode out of bound %08X", data_base);
#endif
  ...
  for (u32 i{}; i + 2 < data_size; i++) {
    if (ram[(data_base + i + 0) ^ 3] == 'R' &&
      ...
      // Loop while we haven't filled our buffer, and there's space for our
      // terminator
      while (p + 1 < e) {
...
static u32 GBIMicrocode_MicrocodeHash(u32 code_base, u32 code_size) {
  ...
  for (u32 i = 0; i < code_size; ++i) {
    hash = (hash << 4) + hash +
           ram[(code_base + i) ^ U8_TWIDDLE]; // Best hash ever!
  }
  return hash;
}

void GBIMicrocode_Reset() { memset(&gUcodeInfo, 0, sizeof(gUcodeInfo)); }
...
static const MicrocodeData gMicrocodeData[] = {
    //
    //	The only games that need defining are custom ucodes and incorrectly
    //detected ones 	If you believe a title should be here post the line for it
    //from ucodes.txt @ http://www.daedalusx64.com 	Note - Games are in
    //alphabetical order by game title
    //
    {GBI_CONKER, GBI_2,
     0x60256efc}, //"RSP Gfx ucode F3DEXBG.NoN fifo 2.08  Yoshitaka Yasumoto
                  //1999 Nintendo.", "Conker's Bad Fur Day"},
...
u32 GBIMicrocode_DetectVersion(u32 code_base, u32 code_size, u32 data_base,
  ...
  // Cheap way to cache ucodes, don't check for strings (too slow!) but check
  // last used ucode entries which is alot faster than string comparison. This
  // only needed for GBI1/2/SDEX ucodes that use LoadUcode, else we only check
  // when code_base changes, which usually never happens
  //
  u32 i;
  ...
  //
  //	Try to find the version string in the microcode data. This is faster
  //than calculating a crc of the code
  //
  char str[256] = "";
  GBIMicrocode_DetectVersionString(data_base, data_size, str, 256);

  // It wasn't the same as the last time around, we'll hash it and check if is a
  // custom ucode.
  //
  ...
  for (auto i : gMicrocodeData) {
    if (code_hash == i.hash) {
      // DBGConsole_Msg(0, "Ucode has been Detected in Array :[M\"%s\", Ucode
      // %d]", str, gMicrocodeData[ i ].ucode);
  ...
  if (ucode_version != GBI_0) {
    ...
    // See if we can identify it by string, if no match was found set default
    // for Fast3D ucode
    //
  ...
  DBGConsole_Msg(0,
                 "Detected %s Ucode is: [M Ucode %d, 0x%08x, \"%s\", \"%s\"]",
                 ucode_offset == u32(~0) ? "" : "Custom", ucode_version,
                 code_hash, str, g_ROM.settings.GameName.c_str());
#endif
  // This is no longer needed as we now have an auto ucode detector, I'll leave
  // it as reference ~Salvy
  //
  /*
