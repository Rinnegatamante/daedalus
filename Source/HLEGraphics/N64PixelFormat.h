...
inline PixelFormatA ConvertPixelFormat(PixelFormatB colour) {
  return PixelFormatA(colour.GetR(), colour.GetG(), colour.GetB(),
                      colour.GetA());
}
...
struct N64Pf5551 {
  ...
  explicit N64Pf5551(u16 bits) : Bits(bits) {}

  N64Pf5551(u8 r, u8 g, u8 b, u8 a) : Bits(Make(r, g, b, a)) {}
...
struct N64Pf8888 {
  ...
  template <typename T> static N64Pf8888 Make(T c) {
    return N64Pf8888(c.GetR(), c.GetG(), c.GetB(), c.GetA());
  }

  N64Pf8888() = default;

  explicit N64Pf8888(u32 bits) : Bits(bits) {}

  N64Pf8888(u8 r, u8 g, u8 b, u8 a) : Bits(Make(r, g, b, a)) {}
...
struct N64PfIA8 {
  ...
  u8 GetI() const {
    u8 i((Bits & MaskI) >> ShiftI);
    return (i << (8 - BitsI)) | (i >> (BitsI - (8 - BitsI)));
  }
  u8 GetA() const {
    u8 a((Bits & MaskA) >> ShiftA);
