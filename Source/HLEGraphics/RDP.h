...
inline u32 bytes2pixels(u32 bytes, u32 size) { return (bytes << 1) >> size; }
...
struct SImageDescriptor {
  ...
  inline u32 GetPitch() const { return ((Width << Size) >> 1); }
  ...
  inline u32 GetPitch16bpp() const { return Width << 1; }
...
struct RDP_TileSize {
  ...
  u16 GetWidth() const { return ((right - left) / 4) + 1; }

  u16 GetHeight() const { return ((bottom - top) / 4) + 1; }
