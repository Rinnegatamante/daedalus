...
// Granularity down to 24bytes is good enuff also only need to address the upper
// half of TMEM for palettes//Corn
...
// CopyLineQwords** passes in an offset in 32bit words, (i.e. does
// mem_address/4) So it's aligned by definition, we have asserts in place just
// in case!

// Due to how TMEM is organized, erg the last 3 bits in the address are always
// "0" It should be safe to assume copies will always be in qwords
...
static inline void CopyLineSwap(void *dst, const void *src, u32 bytes) {
...
#ifdef FAST_TMEM_COPY
  DAEDALUS_ASSERT((bytes & 0x7) == 0, "CopyLineSwap: Remaning bytes! (%d)",
                  bytes);
...
#endif
  {
    ...
    while (bytes--) {
      ...
      *(u8 *)((uintptr_t)dst8++ ^ 0x4) =
          *(u8 *)((uintptr_t)src8++ ^ U8_TWIDDLE);
    }
  }
}
...
#ifdef FAST_TMEM_COPY
  DAEDALUS_ASSERT((bytes & 0x7) == 0, "CopyLineSwap32: Remaning bytes! (%d)",
                  bytes);
...
CRDPStateManager::CRDPStateManager() : EmulateMirror(true) {
  ClearAllEntries();
  InvalidateAllTileTextureInfo();
}

CRDPStateManager::~CRDPStateManager() = default;
...
void CRDPStateManager::LoadBlock(const SetLoadTile &load) {
  ...
  DL_PF("    Tile[%d] (%d,%d - %d) DXT[0x%04x] = [%d]Bytes/line => "
        "[%d]Pixels/line Address[0x%08x]",
        tile_idx, uls, ult, load.sh, dxt, (g_TI.Width << g_TI.Size >> 1),
        bytes2pixels((g_TI.Width << g_TI.Size >> 1), g_TI.Size), address);
#endif

  InvalidateAllTileTextureInfo(); // Can potentially invalidate all texture
                                  // infos
  ...
  // Invalidate load info after current TMEM address to the end of TMEM (fixes
  // Fzero and SSV) //Corn
  ...
  DAEDALUS_DL_ASSERT(bytes <= 4096, "Suspiciously large loadblock: %d bytes",
                     bytes);
  ...
  if (((address + bytes) > MAX_RAM_ADDRESS) ||
      (tmem_offset + bytes) > MAX_TMEM_ADDRESS) {
  ...
  if (dxt == 0) {
    ...
    DAEDALUS_ASSERT(qwords_per_line == (u32)ceilf(2048.f / (float)dxt),
                    "Broken DXT calc");
...
void CRDPStateManager::LoadTile(const SetLoadTile &load) {
...
#ifdef DAEDALUS_DEBUG_DISPLAYLIST
  DL_PF("    Tile[%d] (%d,%d)->(%d,%d) [%d x %d] Address[0x%08x]", tile_idx,
        load.sl / 4, load.tl / 4, load.sh / 4 + 1, load.th / 4 + 1,
        (load.sh - load.sl) / 4 + 1, (load.th - load.tl) / 4 + 1, address);
#endif
  InvalidateAllTileTextureInfo(); // Can potentially invalidate all texture
                                  // infos
...
#ifdef DAEDALUS_DEBUG_DISPLAYLIST
  DAEDALUS_DL_ASSERT((rdp_tile.size > 0) || (uls & 4) == 0,
                     "Expecting an even Left for 4bpp formats (left is %f)",
                     uls / 4.f);
#endif
  u32 tmem_lookup{(u32)(rdp_tile.tmem >> 4)};
  SetValidEntry(tmem_lookup);
  ...
  u32 ram_address {
    ...
    DAEDALUS_USE(bytes);
    DAEDALUS_DL_ASSERT(
        bytes <= MAX_TMEM_ADDRESS,
        "Suspiciously large texture load: %d bytes (%dx%d, %dbpp)", bytes, w, h,
        (1 << (g_TI.Size + 2)));
#endif
    ...
    if ((address + bytes_to_copy) > MAX_RAM_ADDRESS ||
        (tmem_offset + bytes_to_copy) > MAX_TMEM_ADDRESS) {
  ...
  void CRDPStateManager::LoadTlut(const SetLoadTile &load) {
    ...
    // DAEDALUS_DL_ASSERT(g_TI.Size == G_IM_SIZ_16b, "Crazy tlut load - not
    // 16bpp");
    ...
    // Store address of PAL (assuming PAL is only stored in upper half of TMEM)
    // //Corn
    gTlutLoadAddresses[(rdp_tile.tmem >> 2) & 0x3F] = (u32 *)address;
#ifdef DAEDALUS_DEBUG_DISPLAYLIST
    DL_PF("    TLut Addr[0x%08x] TMEM[0x%03x] Tile[%d] Count[%d] Format[%s] "
          "(%d,%d)->(%d,%d)",
          address, rdp_tile.tmem, tile_idx, count,
          kTLUTTypeName[gRDPOtherMode.text_tlut], uls >> 2, ult >> 2, lrs >> 2,
          lrt >> 2);
#endif
#ifdef DAEDALUS_ACCURATE_TMEM
    DAEDALUS_DL_ASSERT((rdp_tile.tmem + count) <= (MAX_TMEM_ADDRESS / 8),
                       "LoadTlut address is invalid");
  ...
  // See the detailed noted in BaseRenderer::UpdateTileSnapshots for issues
  // relating to this.
  ...
  const TextureInfo &CRDPStateManager::GetUpdatedTextureDescriptor(u32 idx) {
    ...
    DAEDALUS_ASSERT(idx < ARRAYSIZE(mTileTextureInfoValid), "Invalid index %d",
                    idx);
#endif
    if (!mTileTextureInfoValid[idx]) {
      ...
      // Check if tmem_lookup has a valid entry, if not we assume load was done
      // on TMEM[0] and we add the offset //Corn Games that uses this is
      // Fzero/Space station Silicon Valley/Animal crossing.
      ...
      //	Limit the tile's width/height to the number of bits specified by
      //mask_s/t. 	See the detailed notes in BaseRenderer::UpdateTileSnapshots
      //for issues relating to this.
      //
      u16 tile_width{GetTextureDimension(rdp_tilesize.GetWidth(),
                                         rdp_tile.mask_s, rdp_tile.clamp_s)};
      u16 tile_height{GetTextureDimension(rdp_tilesize.GetHeight(),
                                          rdp_tile.mask_t, rdp_tile.clamp_t)};

#ifdef DAEDALUS_ACCURATE_TMEM
      ti.SetTlutAddress(TLUT_BASE);
#else
    //
    // If indexed TMEM PAL address is nullptr then assume that the base address
    // is stored in TMEM address 0x100 (gTlutLoadAddresses[ 0 ]) and calculate
    // offset from there with TLutIndex(palette index) This trick saves us from
    // the need to copy the real palette to TMEM and we just pass the pointer
    // //Corn
    //
    u32 tlut{TLUT_BASE};
    if (rdp_tile.size == G_IM_SIZ_4b) {
      ...
      // If pointer == nullptr(=invalid entry) add offset to base address
      // (TMEM[0] + offset)
      if (tlut_idx1 == 0) {
    ...
    ti.SetTlutAddress(tlut);
      ...
      // NB: ACCURATE_TMEM doesn't care about pitch - it's already been loaded
      // into tmem. We only care about line.
#endif
