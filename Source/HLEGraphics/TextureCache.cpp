...
CTextureCache::~CTextureCache() { DropTextures(); }

inline u32 CTextureCache::MakeHashIdxA(const TextureInfo &ti) {
  u32 address(ti.GetLoadAddress());
  u32 hash((address >> (HASH_TABLE_BITS * 2)) ^ (address >> HASH_TABLE_BITS) ^
           address);
...
static void TextureCacheStat(u32 l1_hit, u32 l2_hit, u32 size) {
  ...
  if (total_lookups == 1000) {
    printf("L1 hits[%d] L2 hits[%d] Miss[%d] (%d entries)\n", total_l1_hits,
           total_l2_hits, total_lookups - (total_l1_hits + total_l2_hits),
           size);
...
CachedTexture *CTextureCache::GetOrCreateCachedTexture(const TextureInfo &ti) {
  ...
  auto it = std::lower_bound(mTextures.begin(), mTextures.end(), ti,
                             SSortTextureEntries());
...
CTextureCache::GetOrCreateTexture(const TextureInfo &ti) {
...
#ifdef DAEDALUS_DEBUG_DISPLAYLIST
void CTextureCache::Snapshot(
    const MutexLock &lock, std::vector<STextureInfoSnapshot> &snapshot) const {
  DAEDALUS_ASSERT(lock.HasLock(mDebugMutex), "No debug lock");

  snapshot.erase(snapshot.begin(), snapshot.end());

  for (TextureVec::const_iterator it = mTextures.begin(); it != mTextures.end();
       ++it) {
