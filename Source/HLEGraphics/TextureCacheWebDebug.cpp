  ...
  // If we delete textures on this thread, we'll crash OpenGL.
  {
    ...
    for (size_t i{}; i < textures.size(); ++i) {
      ...
      connection->WriteF("<tr>"
                         ...
                         "<td>%d</td>"
                         "<td>%d</td>"
                         "<td><a href=\"/texture?ptr=%p\"><img "
                         "src=\"/texture?ptr=%p\" width=%d height=%d></a></td>"
                         "</tr>"
                         "\n",
                         ti.GetFormatName(), ti.GetSizeInBits(), ti.GetPitch(),
                         ti.GetWidth(), ti.GetHeight(),
                         (CNativeTexture *)snap.Texture,
                         (CNativeTexture *)snap.Texture,
                         snap.Texture->GetWidth(), snap.Texture->GetHeight());
    }
  }
