...
static const char *const gImageFormatNames[8]{"RGBA", "YUV", "CI", "IA",
                                              "I",    "?1",  "?2", "?3"};
...
u32 TextureInfo::GetSizeInBits() const { return gImageSizesInBits[Size]; }
...
u32 TextureInfo::GenerateHashValue() const {
  ...
  // DAEDALUS_ASSERT( (GetLoadAddress() + Height * Pitch) < 4*1024*1024,
  // "Address of texture is out of bounds" );
  ...
  if (g_ROM.GameHacks == YOSHI) {
    ...
    if (GetFormat() == G_IM_FMT_CI) {
      ...
      for (u32 z{}; z < 8; z++)
        hash_value = ((hash_value << 1) | (hash_value >> 0x1F)) ^ *ptr_u32++;
    }
  ...
  u32 step{(u32)(Height *
                 Pitch)}; // Get size in bytes, seems to be more accurate
                          // (alternative -> Height * Width * (1<<Size) >> 1;)
  ...
  // Might not be needed but it would catch if only the colors are changed in a
  // palette texture It is a bit expensive CPU wise so better leave out unless
  // really needed It assumes 4 byte alignment so we use u32 (faster than u8)
  // Used in OOT for the sky, really minor so is not worth the CPU time to
  // always check for it
  /*if (GetFormat() == G_IM_FMT_CI)
  {
          const u32* ptr_u32 = (u32*)GetTlutAddress();
          for (u32 z = 0; z < ((GetSize() == G_IM_SIZ_4b)? 8 : 128); z++)
  hash_value ^= *ptr_u32++;
  }*/

  // printf("%08X %d S%d P%d H%d W%d B%d\n", hash_value, step, Size, Pitch,
  // Height, Width, Height * Pitch);
  return hash_value;
}
