...
public:
  ...
  TextureInfo(const TextureInfo &rhs) {
    memcpy(this, &rhs, sizeof(TextureInfo));
  }
  ...
  // the hash output should match the number of bits used in the texture cache
  // array
  inline u32 GetHashCode() const {
    u8 *ptr((u8 *)this);
    u8 *end_ptr(ptr + sizeof(TextureInfo));
    u32 hash(0);
    while (ptr < end_ptr)
      hash = ((hash << 1) | (hash >> 0x9)) ^ *ptr++;
    return hash;
  }
  // inline u32				GetHashCode() const				{ return
  // murmur2_neutral_hash( reinterpret_cast< const u8 * >( this ), sizeof(
  // TextureInfo ), 0 ); }

  // Compute a hash of the contents of the texture data. Not to be confused with
  // GetHashCode() that hashes the Textureinfo!
  ...
  inline int Compare(const TextureInfo &rhs) const {
    return memcmp(this, &rhs, sizeof(TextureInfo));
  }
