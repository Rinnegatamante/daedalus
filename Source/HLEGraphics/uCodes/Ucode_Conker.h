...
void DLParser_Vtx_Conker( MicroCodeCommand command )
{
	...
	auto len    {((command.inst.cmd0 >> 1 )& 0x7F)};
	auto n      {((command.inst.cmd0 >> 12)& 0xFF)};
	auto v0		{len - n};
#ifdef DAEDALUS_DEBUG_DISPLAYLIST
...
void DLParser_MoveMem_Conker( MicroCodeCommand command )
{
	...
	case G_GBI2_MV_LIGHT:
		{
			auto offset2 {(command.inst.cmd0 >> 5) & 0x3FFF};
			auto light_idx {(offset2 / 48)};
			if (light_idx < 2)
			{
				#ifdef DAEDALUS_DEBUG_DISPLAYLIST
				DL_PF("    G_MV_LOOKAT" );
...
{
#if 1
	auto index {(u8)(( command.inst.cmd0 >> 16) & 0xFF)};
	switch (index)
	{
	case G_MW_NUMLIGHT:
		{
			auto num_lights = command.inst.cmd1 / 48;
				#ifdef DAEDALUS_DEBUG_DISPLAYLIST
	...
	case G_MW_SEGMENT:
		{
			auto offset {(u16)( command.inst.cmd0 & 0xFFFF)};
	...
	case 0x10:  // moveword coord mod
	{
		...
		{
			auto idx {(command.inst.cmd0 >> 1) & 3};
			auto pos {command.inst.cmd0 & 0x30};
