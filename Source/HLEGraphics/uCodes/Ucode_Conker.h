...
void DLParser_Tri1_Conker(MicroCodeCommand command) {
  ...
  // If Off screen rendering is true then just skip the whole list of tris
  // //Corn Skip shadow as well
  if (g_CI.Format != G_IM_FMT_RGBA || (gRDPOtherMode.L == CONKER_SHADOW)) {
    do {
#ifdef DAEDALUS_DEBUG_DISPLAYLIST
...
void DLParser_Tri4_Conker(MicroCodeCommand command) {
  u32 pc =
      gDlistStack
          .address[gDlistStackPointer]; // This points to the next instruction
  ...
  do {
    ...
    // Tri #4
    idx[9] =
        (((command.inst.cmd0 >> 15) & 0x7) << 2) | (command.inst.cmd1 >> 30);
...
void DLParser_MoveMem_Conker(MicroCodeCommand command) {
  ...
  case G_GBI2_MV_LIGHT: {
    ...
    gRenderer->SetLightPosition(light_idx, light->x, light->y, light->z,
                                light->w);
  ...
  case 0x10: // moveword coord mod
  {
    ...
    if ((command.inst.cmd0 & 8) == 0) {
      ...
      case 0x10:
        ...
        gRenderer->SetCoordMod(5 + idx,
                               (command.inst.cmd1 & 0xFFFF) / 65536.0f);
        gRenderer->SetCoordMod(12 + idx, gRenderer->GetCoordMod(0 + idx) +
                                             gRenderer->GetCoordMod(4 + idx));
        gRenderer->SetCoordMod(13 + idx, gRenderer->GetCoordMod(1 + idx) +
                                             gRenderer->GetCoordMod(5 + idx));
        break;
      case 0x20:
