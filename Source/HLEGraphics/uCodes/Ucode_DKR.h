...
void DLParser_DumpVtxInfoDKR(u32 address, u32 v0_idx, u32 num_verts)
{
	if (DLDebug_IsActive())
	{
		auto psSrc {(u32)(g_pu8RamBase + address)};

		for ( auto idx {v0_idx}; idx < v0_idx + num_verts; idx++ )
		{
			auto x {*(s16*)((psSrc + 0) ^ 2)};
			auto y {*(s16*)((psSrc + 2) ^ 2)};
			auto z {*(s16*)((psSrc + 4) ^ 2)};

			//u16 wFlags = gRenderer->GetVtxFlags( idx ); //(u16)psSrc[3^0x1];

			auto a {*(u8*)((psSrc + 6) ^ 3)};	//R
			auto b {*(u8*)((psSrc + 7) ^ 3)};	//G
		 	auto c {*(u8*)((psSrc + 8) ^ 3)};	//B
			auto d {*(u8*)((psSrc + 9) ^ 3)};	//A
...
{
	auto address	{command.inst.cmd1 + gAuxAddr};
	auto num_verts {((command.inst.cmd0 >> 19) & 0x1F)};
	auto v0_idx		{0};
...
{
	auto address	{command.inst.cmd1 + RDPSegAddr(gDKRMatrixAddr)};
	auto mtx_command {(command.inst.cmd0 >> 16) & 0x3};
...
void DLParser_MoveWord_DKR( MicroCodeCommand command )
{
	...
	case G_MW_LIGHTCOL:
		{
		auto idx {(command.inst.cmd1 >> 6) & 0x3};
...
void DLParser_DMA_Tri_DKR( MicroCodeCommand command )
{
	...
	auto count {(command.inst.cmd0 >> 4) & 0x1F};	//Count should never exceed 16
	...
	for (auto i {0}; i < count; i++)
	{
		...
		const auto new_v0_idx {i * 3 + 32};
		const auto new_v1_idx {i * 3 + 33};
		const auto new_v2_idx {i * 3 + 34};
...
{
	auto tile  {command.texture.tile};
