...
static inline void DrawFrameBuffer(u32 origin, const CNativeTexture * texture)
{
	...
	auto *pixels {(u16*)malloc(FB_WIDTH*FB_HEIGHT * sizeof(u16))};	// TODO: should cache this, but at some point we'll need to deal with variable framebuffer size, so do this later.
	auto src_offset {0};

	for (auto y {0}; y < FB_HEIGHT; ++y)
	{
		auto dst_row_offset {y * FB_WIDTH};
