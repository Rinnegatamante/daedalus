...
void DLParser_GBI0_Vtx(MicroCodeCommand command) {
  ...
  DL_PF("    Address[0x%08x] v0[%d] Num[%d] Len[0x%04x]", addr, v0, n,
        command.vtx0.len);
#endif
#ifdef DAEDALUS_ENABLE_ASSERTS
  // Never seen or should happen, but just in case..
  DAEDALUS_ASSERT((v0 + n) < 80,
                  "Warning, attempting to load into invalid vertex positions");
  DAEDALUS_ASSERT((addr + (n * 16)) < MAX_RAM_ADDRESS,
                  "Address out of range (0x%08x)", addr);
#endif
...
// Maybe not the best idea to inline nor have an extra jump or and extra branch
// in Addtri..
...
void DLParser_GBI0_Tri4(MicroCodeCommand command) {
  ...
  do {
    // DL_PF("    0x%08x: %08x %08x Flag: 0x%02x %-10s", pc-8,
    // command.inst.cmd0, command.inst.cmd1, (command.inst.cmd0 >> 16) & 0xFF,
    // "G_GBI1_TRI4");
