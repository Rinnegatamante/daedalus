...
void DLParser_GBI1_Vtx( MicroCodeCommand command )
{
	...
	auto addr {RDPSegAddr(command.vtx1.addr)};
	auto v0   {command.vtx1.v0};
	auto n    {command.vtx1.n};
...
{
	auto offset {command.modifyvtx.offset};
	auto vert   {command.modifyvtx.vtx};
	auto value  {command.modifyvtx.value};
...
{
	auto address {RDPSegAddr(command.mtx1.addr)};
#ifdef DAEDALUS_DEBUG_DISPLAYLIST
...
{
	auto type     {(command.inst.cmd0>>16) &0xFF};
	auto address {RDPSegAddr(command.inst.cmd1)};
...
void DLParser_GBI1_MoveWord( MicroCodeCommand command )
{
	...
	auto value  {command.mw1.value};
	auto offset {command.mw1.offset};
	...
	case G_MW_NUMLIGHT:
		{
			auto num_lights {((value - 0x80000000) >> 5) - 1};
			#ifdef DAEDALUS_DEBUG_DISPLAYLIST
...
*/
	...
	case G_MW_FOG:	// WIP, only works for the PSP
		{
#ifdef DAEDALUS_PSP
			auto mul {(f32)(s16)(value >> 16)};	//Fog mult
			auto offs {(f32)(s16)(value & 0xFFFF)};	//Fog Offset
...
#if 0
			auto a {f32(value >> 16)};
			auto b {f32(value & 0xFFFF)};

			auto fog_near {a / 256.0f};
			auto fog_far {b / 6.0f};
	...
	case G_MW_LIGHTCOL:
		{
			...
			auto field_offset {(offset & 0x7)};
			auto light_idx {offset >> 5};
#ifdef DAEDALUS_DEBUG_DISPLAYLIST
...
{
	auto first {command.culldl.first};
	auto last {command.culldl.end};
#ifdef DAEDALUS_DEBUG_DISPLAYLIST
...
void DLParser_GBI1_DL( MicroCodeCommand command )
{
	...
	auto addr {RDPSegAddr(command.dlist.addr)};
...
void DLParser_GBI1_BranchZ( MicroCodeCommand command )
{
	...
	{
		auto address {RDPSegAddr(gRDPHalf1)};
#ifdef DAEDALUS_DEBUG_DISPLAYLIST
...
{
	auto code_base {(command.inst.cmd1 & 0x1fffffff)};
	auto code_size {0x1000};
	auto data_base {gRDPHalf1 & 0x1fffffff};         // Preceeding RDP_HALF1 sets this up
	auto data_size {(command.inst.cmd0 & 0xFFFF) + 1};
...
{
	const auto mask {command.inst.cmd1};
...
{
	const auto mask {((1 << command.othermode.len) - 1) << command.othermode.sft};
...
void DLParser_GBI1_Texture( MicroCodeCommand command )
{
	...
	auto scale_s {f32(command.texture.scaleS)  / (65535.0f * 32.0f)};
	auto scale_t {f32(command.texture.scaleT)  / (65535.0f * 32.0f)};
#ifdef DAEDALUS_DEBUG_DISPLAYLIST
...
void DLParser_GBI1_Tri2( MicroCodeCommand command )
{
	...
	do
	{
		...
		auto v0_idx {command.gbi1tri2.v0 >> 1};
		auto v1_idx {command.gbi1tri2.v1 >> 1};
		auto v2_idx {command.gbi1tri2.v2 >> 1};

		tris_added |= gRenderer->AddTri(v0_idx, v1_idx, v2_idx);

		auto v3_idx {command.gbi1tri2.v3 >> 1};
		auto v4_idx {command.gbi1tri2.v4 >> 1};
		auto v5_idx {command.gbi1tri2.v5 >> 1};
...
void DLParser_GBI1_Line3D( MicroCodeCommand command )
{
	...
	do
	{
		...
		auto v0_idx   {command.gbi1line3d.v0 / stride};
		auto v1_idx   {command.gbi1line3d.v1 / stride};
		auto v2_idx   {command.gbi1line3d.v2 / stride};
		auto v3_idx   {command.gbi1line3d.v3 / stride};
...
void DLParser_GBI1_Tri1( MicroCodeCommand command )
{
	...
	do
	{
		...
		auto v0_idx {command.gbi1tri1.v0 / stride};
		auto v1_idx {command.gbi1tri1.v1 / stride};
		auto v2_idx {command.gbi1tri1.v2 / stride};
