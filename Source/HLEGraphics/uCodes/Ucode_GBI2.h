...
void DLParser_GBI2_Vtx(MicroCodeCommand command) {
  ...
  if (vend > 64) {
    ...
    DBGConsole_Msg(0,
                   "DLParser_GBI2_Vtx: Warning, attempting to load into "
                   "invalid vertex positions: %d -> %d",
                   v0, v0 + n);
#endif
    return;
  }
...
// Only games I seen that set this are Mario Golf/Tennis, but it looks like is
// caused by a dynarec issue, anyways they crash eventually
#ifdef DAEDALUS_ENABLE_ASSERTS
  DAEDALUS_ASSERT((address + (n * 16)) < MAX_RAM_ADDRESS,
                  "Address out of range (0x%08x)", address);
#endif
...
void DLParser_GBI2_Mtx(MicroCodeCommand command) {
  ...
  DL_PF("    Command: %s %s %s Length %d Address 0x%08x",
        ...
        command.mtx2.nopush == 0 ? "Push" : "No Push", command.mtx2.len,
        address);
#endif
  ...
  if (command.mtx2.projection) {
    ...
    gRenderer->SetWorldView(address, command.mtx2.nopush == 0,
                            command.mtx2.load);
  }
}
//*****************************************************************************
...
//
//*****************************************************************************
// 0016A710: DB020000 00000018 CMD Zelda_MOVEWORD  Mem[2][00]=00000018
// Lightnum=0 001889F0: DB020000 00000030 CMD Zelda_MOVEWORD  Mem[2][00]=00000030
// Lightnum=2
...
#endif
    // DL_PF(" G_MW_FOG. Mult = 0x%04x (%f), Off = 0x%04x (%f)", wMult, 255.0f *
    // fMult, wOff, 255.0f * fOff ); printf("1Fog %.0f | %.0f || %.0f | %.0f\n",
    // min, max, a, b);
#endif
  } break;
...
#ifdef DAEDALUS_DEBUG_DISPLAYLIST
    DL_PF("    G_MW_LIGHTCOL/0x%08x: 0x%08x", command.mw2.offset,
          command.mw2.value);
#endif
    if (field_offset == 0) {
...
!light 0 color 0.12 0.16 0.35 dir 0.01 0.00 0.00 0.00 (2 lights) [ 1E285A00
1E285A00 01000000 00000000 ] data(00188708): 1E285A00 1E285A00 01000000 00000000
00188A00: DC08090A 80188718 CMD Zelda_MOVEMEM  Movemem[0809] <- 80188718
!light 1 color 0.23 0.25 0.30 dir 0.01 0.00 0.00 0.00 (2 lights) [ 3C404E00
3C404E00 01000000 00000000 ] data(00188718): 3C404E00 3C404E00 01000000 00000000
00188A08: DC080C0A 80188700 CMD Zelda_MOVEMEM  Movemem[080C] <- 80188700
!light 2 color 0.17 0.16 0.26 dir 0.23 0.31 0.70 0.00 (2 lights) [ 2C294300
2C294300 1E285A00 1E285A00 ]
*/
/*
...
0xa4001120: <0x0c000487> JAL       0x121c        Seg2Addr(t8)
dram 0xa4001124: <0x332100fe> ANDI      at = t9 & 0x00fe 0xa4001128:
<0x937309c1> LBU       s3 <- 0x09c1(k1)
len 0xa400112c: <0x943402f0> LHU       s4 <- 0x02f0(at)
dmem 0xa4001130: <0x00191142> SRL       v0 = t9 >> 0x0005 0xa4001134:
<0x959f0336> LHU       ra <- 0x0336(t4) 0xa4001138: <0x080007f6> J 0x1fd8
SpMemXfer 0xa400113c: <0x0282a020> ADD       s4 = s4 + v0
dmem
...
void DLParser_GBI2_MoveMem(MicroCodeCommand command) {
  ...
  case G_GBI2_MV_LIGHT: {
    ...
    gRenderer->SetLightPosition(light_idx, light->x1, light->y1, light->z1,
                                1.0f);
  ...
  default:
#ifdef DAEDALUS_DEBUG_DISPLAYLIST
    DBGConsole_Msg(0, "GBI2 MoveMem: Unknown Type. 0x%08x 0x%08x",
...
void DLParser_GBI2_DL_Count(MicroCodeCommand command) {
  ...
  DL_PF("    Address=0x%08x %s", address,
        (command.dlist.param == G_DL_NOPUSH)
            ? "Jump"
            : (command.dlist.param == G_DL_PUSH) ? "Push" : "?");
  DL_PF("    \\/ \\/ \\/ \\/ \\/ \\/ \\/ \\/ \\/ \\/ \\/ \\/ \\/ \\/ \\/");
  DL_PF("    ############################################");
#endif
}
...
void DLParser_GBI2_GeometryMode(MicroCodeCommand command) {
...
#ifdef DAEDALUS_DEBUG_DISPLAYLIST
  DL_PF("    0x%08x 0x%08x =(x & 0x%08x) | 0x%08x", command.inst.cmd0,
        command.inst.cmd1, command.inst.arg0, command.inst.arg1);
  DL_PF("    ZBuffer %s", (gGeometryMode.GBI2_Zbuffer) ? "On" : "Off");
  DL_PF("    Culling %s",
        (gGeometryMode.GBI2_CullBack)
            ? "Back face"
            : (gGeometryMode.GBI2_CullFront) ? "Front face" : "Off");
  DL_PF("    Smooth Shading %s",
        (gGeometryMode.GBI2_ShadingSmooth) ? "On" : "Off");
  DL_PF("    Lighting %s", (gGeometryMode.GBI2_Lighting) ? "On" : "Off");
  DL_PF("    Texture Gen %s", (gGeometryMode.GBI2_TexGen) ? "On" : "Off");
  DL_PF("    Texture Gen Linear %s",
        (gGeometryMode.GBI2_TexGenLin) ? "On" : "Off");
  ...
  TnL.Fog =
      gGeometryMode.GBI2_Fog &
      gFogEnabled; // && (gRDPOtherMode.c1_m1a==3 || gRDPOtherMode.c1_m2a==3 ||
                   // gRDPOtherMode.c2_m1a==3 || gRDPOtherMode.c2_m2a==3);
  TnL.Shade =
      !(gGeometryMode
            .GBI2_TexGenLin /* & (g_ROM.GameHacks != TIGERS_HONEY_HUNT)*/);
...
void DLParser_GBI2_SetOtherModeL(MicroCodeCommand command) {
  ...
  const u32 mask = (u32)((s32)(0x80000000) >> command.othermode.len) >>
                   command.othermode.sft;
...
#ifdef DAEDALUS_DEBUG_DISPLAYLIST
  DL_PF("    Level[%d] Tile[%d] %s", command.texture.level,
        command.texture.tile,
        command.texture.enable_gbi2 ? "enable" : "disable");
#endif
...
void DLParser_GBI2_Quad(MicroCodeCommand command) {
  ...
  do {
    // DL_PF("    0x%08x: %08x %08x %-10s", pc-8, command.inst.cmd0,
    // command.inst.cmd1, "G_GBI2_QUAD");
    ...
    // printf("Q 0x%08x: %08x %08x %d\n", pc-8, command.inst.cmd0,
    // command.inst.cmd1, tris_added);
...
void DLParser_GBI2_Line3D(MicroCodeCommand command) {
  ...
  do {
    // DL_PF("    0x%08x: %08x %08x %-10s", pc-8, command.inst.cmd0,
    // command.inst.cmd1, "G_GBI2_LINE3D");
...
{
        if( (command.inst.arg0 == 0x2F && ((command.inst.cmd1)&0xFF000000) ==
0x80000000 )
        {
                // V-Rally 64
