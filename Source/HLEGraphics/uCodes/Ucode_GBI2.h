...
{
	auto address {RDPSegAddr(command.vtx2.addr)};

	auto vend   {command.vtx2.vend >> 1};
	auto n      {command.vtx2.n};
	auto v0	   {vend - n};
...
{
	auto address {RDPSegAddr(command.mtx2.addr)};
#ifdef DAEDALUS_DEBUG_DISPLAYLIST
	...
	case G_MW_NUMLIGHT:
		{
			...
			auto num_lights {command.mw2.value / 24};
			#ifdef DAEDALUS_DEBUG_DISPLAYLIST
...
*/
	case G_MW_SEGMENT:
		{
			auto segment {command.mw2.offset >> 2};
			auto address	{command.mw2.value};
#ifdef DAEDALUS_DEBUG_DISPLAYLIST
	...
	case G_MW_FOG: // WIP, only works for the PSP
		{
#ifdef DAEDALUS_PSP
			auto mul {(f32)(s16)(command.mw2.value >> 16)};	//Fog mult
			auto offs {(f32)(s16)(command.mw2.value & 0xFFFF)};	//Fog Offset

			gRenderer->SetFogMultOffs(mul, offs);

			// HW fog, only works for a few games
#if 0
			auto a {(f32)(command.mw2.value >> 16)};
			auto b {(f32)(command.mw2.value & 0xFFFF)};
	...
	case G_MW_LIGHTCOL:
		{
			auto light_idx {command.mw2.offset / 0x18};
			auto field_offset {(command.mw2.offset & 0x7)};
#ifdef DAEDALUS_DEBUG_DISPLAYLIST
			DL_PF("    G_MW_LIGHTCOL/0x%08x: 0x%08x", command.mw2.offset, command.mw2.value);
#endif
			if (field_offset == 0)
			{
				auto r {((command.mw2.value>>24)&0xFF)};
				auto g {((command.mw2.value>>16)&0xFF)};
				auto b {((command.mw2.value>>8)&0xFF)};
...
void DLParser_GBI2_MoveMem( MicroCodeCommand command )
{
	...
	auto type	 {(command.inst.cmd0     ) & 0xFE};
	...
	case G_GBI2_MV_LIGHT:
		{
			auto offset2 {(command.inst.cmd0 >> 5) & 0x7F8};
			auto light_idx {offset2 / 24};
			if (light_idx < 2)
			{
				#ifdef DAEDALUS_DEBUG_DISPLAYLIST
				DL_PF("    G_MV_LOOKAT" );
...
void DLParser_GBI2_SetOtherModeL( MicroCodeCommand command )
{
	...
	const auto mask {(u32)((s32)(0x80000000) >> command.othermode.len) >> command.othermode.sft};
...
void DLParser_GBI2_Quad( MicroCodeCommand command )
{
	...
	do
	{
		...
		auto v0_idx {command.gbi2line3d.v0 >> 1};
		auto v1_idx {command.gbi2line3d.v1 >> 1};
		auto v2_idx {command.gbi2line3d.v2 >> 1};

		tris_added |= gRenderer->AddTri(v0_idx, v1_idx, v2_idx);

		auto v3_idx {command.gbi2line3d.v3 >> 1};
		auto v4_idx {command.gbi2line3d.v4 >> 1};
		auto v5_idx {command.gbi2line3d.v5 >> 1};
...
void DLParser_GBI2_Tri1( MicroCodeCommand command )
{
	...
	do
	{
		...
		auto v0_idx {command.gbi2tri1.v0 >> 1};
		auto v1_idx {command.gbi2tri1.v1 >> 1};
		auto v2_idx {command.gbi2tri1.v2 >> 1};
...
void DLParser_GBI2_Tri2( MicroCodeCommand command )
{
	...
	do
	{
		...
		auto v0_idx {command.gbi2tri2.v0};
		auto v1_idx {command.gbi2tri2.v1};
		auto v2_idx {command.gbi2tri2.v2};

		tris_added |= gRenderer->AddTri(v0_idx, v1_idx, v2_idx);

		auto v3_idx {command.gbi2tri2.v3};
		auto v4_idx {command.gbi2tri2.v4};
		auto v5_idx {command.gbi2tri2.v5};
