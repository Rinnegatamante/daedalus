...
void DLParser_Sprite2DDraw( MicroCodeCommand command, const Sprite2DInfo &info, Sprite2DStruct *sprite )
{
	...
	auto address {RDPSegAddr(sprite->address)};
	...
	auto px {(s16)((command.inst.cmd1>>16)&0xFFFF)/4};
	auto py {(s16)(command.inst.cmd1 &0xFFFF)/4};
	auto pw {(u16)(sprite->width / info.scaleX)};
	auto ph {(u16)(sprite->height / info.scaleY)};
...
void DLParser_GBI1_Sprite2DBase( MicroCodeCommand command )
{
	...
	do
	{
	auto	address = RDPSegAddr(command.inst.cmd1) & (MAX_RAM_ADDRESS-1);
