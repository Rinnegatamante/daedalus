...
//
//*****************************************************************************
void DLParser_Sprite2DDraw(MicroCodeCommand command, const Sprite2DInfo &info,
  ...
  SImageDescriptor desc = {sprite->format, sprite->size, sprite->stride,
                           address};
...
void DLParser_GBI1_Sprite2DBase(MicroCodeCommand command) {
  ...
  // Try to execute as many sprite2d ucodes as possible, I seen chains over 200!
  // in FB NB Glover calls RDP Sync before draw for the sky.. so checks were
  // added
  do {
