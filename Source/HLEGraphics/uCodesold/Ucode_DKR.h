...
void DLParser_DumpVtxInfoDKR(u32 address, u32 v0_idx, u32 num_verts) {
  if (DLDebug_IsActive()) {
    ...
    for (u32 idx = v0_idx; idx < v0_idx + num_verts; idx++) {
      ...
      DL_PF("    #%02d Pos:{% 0.1f,% 0.1f,% 0.1f}->{% 0.1f,% 0.1f,% 0.1f} "
            "Proj:{% 6f,% 6f,% 6f,% 6f} RGBA:{%02x%02x%02x%02x}",
            idx, x, y, z, t.x, t.y, t.z, p.x / p.w, p.y / p.w, p.z / p.w, p.w,
            a, b, c, d);
...
void DLParser_DMA_Tri_DKR(MicroCodeCommand command) {
  ...
  for (u32 i = 0; i < count; i++) {
    ...
    DL_PF("    Index[%d %d %d] Cull[%s] uv_TexCoord[%0.2f|%0.2f] [%0.2f|%0.2f] "
          "[%0.2f|%0.2f]",
          v0_idx, v1_idx, v2_idx, !(tri->flag & 0x40) ? "On" : "Off",
          (f32)tri->s0 / 32.0f, (f32)tri->t0 / 32.0f, (f32)tri->s1 / 32.0f,
          (f32)tri->t1 / 32.0f, (f32)tri->s2 / 32.0f, (f32)tri->t2 / 32.0f);

#if 1 // 1->Fixes texture scaling, 0->Render as is and get some texture scaling
      // errors
      //
      // This will create problem since some verts will get re-used and
      // over-write new texture coords before previous has been rendered To fix
      // it we copy all verts to a new location where we can have individual
      // texture coordinates for each triangle//Corn
...
void DLParser_GBI1_Texture_DKR(MicroCodeCommand command) {
  ...
  DL_PF("    Level[%d] Tile[%d] %s", command.texture.level, tile,
