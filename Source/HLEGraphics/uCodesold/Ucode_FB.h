...
#define FB_HEIGHT 240
  ...
  DAEDALUS_ASSERT(g_CI.Size == G_IM_SIZ_16b,
                  "32b frame buffer is not supported");
  // DAEDALUS_ASSERT((uViWidth+1) == FB_WIDTH,"Variable width is not handled");
  // DAEDALUS_ASSERT((uViHeight+1) == FB_HEIGHT,"Variable height is not
  // handled");
...
static inline void DrawFrameBuffer(u32 origin, const CNativeTexture *texture) {
  ...
  u16 *pixels = (u16 *)malloc(
      FB_WIDTH * FB_HEIGHT *
      sizeof(u16)); // TODO: should cache this, but at some point we'll need to
                    // deal with variable framebuffer size, so do this later.
  u32 src_offset = 0;
  ...
  for (u32 y = 0; y < FB_HEIGHT; ++y) {
    ...
    for (u32 x = 0; x < FB_WIDTH; ++x) {
      pixels[dst_offset] =
          (g_pu8RamBase[(origin + src_offset) ^ U8_TWIDDLE] << 8) |
          g_pu8RamBase[(origin + src_offset + 1) ^ U8_TWIDDLE] |
          1; // NB: or 1 to ensure we have alpha
      dst_offset += 1;
      src_offset += 2;
    }
  }
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, FB_WIDTH, FB_HEIGHT, 0, GL_RGBA,
               GL_UNSIGNED_SHORT_5_5_5_1, pixels);

  // ToDO: Implement me PSP
  // Doesn't work
  // sceGuTexMode( GU_PSM_5551, 0, 0, 1 );		// maxmips/a2/swizzle =
  // 0 sceGuTexImage(0, texture->GetCorrectedWidth(),
  // texture->GetCorrectedHeight(), texture->GetBlockWidth(), pixels);
