...
void DLParser_GBI1_Vtx(MicroCodeCommand command) {
  ...
  DL_PF("    Address 0x%08x, v0: %d, Num: %d, Length: 0x%04x", addr, v0, n,
        command.vtx1.len);
  DAEDALUS_ASSERT((v0 + n) <= 64,
...
void DLParser_GBI1_Mtx(MicroCodeCommand command) {
  ...
  DL_PF("    Command: %s %s %s Length %d Address 0x%08x",
        ...
        command.mtx1.push == 1 ? "Push" : "NoPush", command.mtx1.len, address);
...
void DLParser_GBI1_MoveMem(MicroCodeCommand command) {
  ...
  // Next 3 MATRIX commands should not appear, since they were in the previous
  // command.
  // case G_MV_MATRIX_2:	/*IGNORED*/	DL_PF("     G_MV_MATRIX_2");
  // break; case G_MV_MATRIX_3:	/*IGNORED*/	DL_PF("     G_MV_MATRIX_3");
  // break; case G_MV_MATRIX_4:	/*IGNORED*/	DL_PF("     G_MV_MATRIX_4");
  // break;
  /*
...
void DLParser_GBI1_DL(MicroCodeCommand command) {
  ...
  gDlistStack.address[gDlistStackPointer] =
      RDPSegAddr(command.dlist.addr) & (MAX_RAM_ADDRESS - 1);
}
...
void DLParser_GBI1_BranchZ(MicroCodeCommand command) {
  ...
  // Seems to work differently for non Zelda games as if Z axis is inverted...
  // //Corn

  // printf("VtxDepth[%d] Zval[%d] Vtx[%d]\n",
  // gRenderer->GetVtxDepth(command.branchz.vtx), (s32)command.branchz.value,
  // command.branchz.vtx); DL_PF("BranchZ VtxDepth[%d] Zval[%d] Vtx[%d]",
  // gRenderer->GetVtxDepth(command.branchz.vtx), (s32)command.branchz.value,
  // command.branchz.vtx);

  if (gRenderer->GetVtxDepth(command.branchz.vtx) <=
      (s32)command.branchz.value) {
...
void DLParser_GBI1_GeometryMode(MicroCodeCommand command) {
  ...
  DL_PF("    Culling %s",
        (gGeometryMode.GBI1_CullBack)
            ? "Back face"
            : (gGeometryMode.GBI1_CullFront) ? "Front face" : "Off");
  DL_PF("    Shade %s", (gGeometryMode.GBI1_Shade) ? "On" : "Off");
  DL_PF("    Smooth Shading %s",
        (gGeometryMode.GBI1_ShadingSmooth) ? "On" : "Off");
  ...
  DL_PF("    Texture Gen Linear %s",
        (gGeometryMode.GBI1_TexGenLin) ? "On" : "Off");
...
void DLParser_GBI1_RDPHalf_Cont(MicroCodeCommand command) {
  // DBGConsole_Msg( 0, "Unexpected RDPHalf_Cont: %08x %08x", command.inst.cmd0,
  // command.inst.cmd1 );
}
...
void DLParser_GBI1_RDPHalf_2(MicroCodeCommand command) {
  //	DBGConsole_Msg( 0, "Unexpected RDPHalf_2: %08x %08x", command.inst.cmd0,
  //command.inst.cmd1 );
}
...
void DLParser_GBI1_Line3D(MicroCodeCommand command) {
  ...
  do {
    // DL_PF("    0x%08x: %08x %08x %-10s", pc-8, command.inst.cmd0,
    // command.inst.cmd1, "G_GBI1_LINE3D");
