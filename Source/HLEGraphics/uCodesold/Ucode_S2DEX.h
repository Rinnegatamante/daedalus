...
//
//*****************************************************************************
enum ESpriteMode { FULL_ROTATION, PARTIAL_ROTATION, NO_ROTATION };
...
//
//*****************************************************************************
static inline CRefPtr<CNativeTexture> Load_ObjSprite(const uObjSprite *sprite,
                                                     const uObjTxtr *txtr) {
  TextureInfo ti;

  // When txtr is NULL, it means TLUT was loaded from ObjLoadTxtr ucode
  if (txtr == NULL) {
    // Get ti info from TextureDescriptor since there's no txtr for tile or
    // block (txtr = NULL)
    ti = gRDPStateManager.GetUpdatedTextureDescriptor(
        gRenderer->GetTextureTile());
#ifdef DAEDALUS_ACCURATE_TMEM
    // TLUT is loaded from ObjLoadTxtr ucode, so this is a direct load from ram
    // (line = 0)
    ti.SetLine(0);
#endif
  } else {
    ...
    case S2DEX_OBJLT_TXTRBLOCK:
      ...
      ti.SetWidth(((sprite->imageW >= 0x8000) ? (0x10000 - sprite->imageW)
                                              : sprite->imageW) /
                  32);
      ti.SetHeight(((sprite->imageH >= 0x8000) ? (0x10000 - sprite->imageH)
                                               : sprite->imageH) /
                   32);
    ...
    case S2DEX_OBJLT_TXTRTILE:
      ...
      ti.SetPitch((sprite->imageSiz == G_IM_SIZ_4b)
                      ? (ti.GetWidth() >> 1)
                      : (ti.GetWidth() << (sprite->imageSiz - 1)));
      break;
    default:
...
//
//*****************************************************************************
static inline void Draw_ObjSprite(const uObjSprite *sprite, ESpriteMode mode,
...
void DLParser_S2DEX_ObjSprite(MicroCodeCommand command) {
  uObjSprite *sprite =
      (uObjSprite *)(g_pu8RamBase + RDPSegAddr(command.inst.cmd1));
...
//
//*****************************************************************************
// Nintendo logo, shade, items, enemies & foes, sun, and pretty much everything
// in Yoshi
void DLParser_S2DEX_ObjLdtxSprite(MicroCodeCommand command) {
  uObjTxSprite *sprite =
      (uObjTxSprite *)(g_pu8RamBase + RDPSegAddr(command.inst.cmd1));
...
void DLParser_S2DEX_ObjLoadTxtr(MicroCodeCommand command) {
  uObjTxtr *ObjTxtr =
      (uObjTxtr *)(g_pu8RamBase + RDPSegAddr(command.inst.cmd1));
  if (ObjTxtr->block.type == S2DEX_OBJLT_TLUT) {
    uObjTxtrTLUT *ObjTlut = (uObjTxtrTLUT *)ObjTxtr;

    // Store TLUT pointer
    gTlutLoadAddresses[(ObjTxtr->tlut.phead >> 2) & 0x3F] =
        (u32 *)(g_pu8RamBase + RDPSegAddr(ObjTlut->image));
    gObjTxtr = NULL;
...
inline void DLParser_Yoshi_MemRect(MicroCodeCommand command) {
  ...
  DL_PF("    MemRect->Addr[0x%08x] (%d, %d -> %d, %d) Width[%d]", tile_addr, x0,
        y0, mem_rect.x1, y1, g_CI.Width);

#if 1 // 1->Optimized, 0->Generic
      // This assumes Yoshi always copy 16 bytes per line and dst is aligned and
      // we force alignment on src!!! //Corn
  u32 tex_width = rdp_tile.line << 3;
  u32 texaddr = ((u32)g_pu8RamBase + tile_addr + tex_width * (mem_rect.s >> 5) +
                 (mem_rect.t >> 5) + 3) &
                ~3;
  ...
  u32 width = x1 - x0;
  u32 tex_width = rdp_tile.line << 3;
  u8 *texaddr = g_pu8RamBase + tile_addr + tex_width * (mem_rect.s >> 5) +
                (mem_rect.t >> 5);
...
void DLParser_OB_YUV(const uObjSprite *sprite) {
  ...
  u32 *mb =
      (u32 *)(g_pu8RamBase + g_TI.Address); // pointer to the first macro block
  u16 *dst = (u16 *)(g_pu8RamBase + g_CI.Address);
  dst += ul_x + ul_y * ci_width;

  // yuv macro block contains 16x16 texture. we need to put it in the proper
  // place inside cimg
  for (u16 h = 0; h < 16; h++) {
    for (u16 w = 0; w < 16; w += 2) {
      u32 t = *(mb++); // each u32 contains 2 pixels
      if ((h < height) &&
          (w < width)) // clipping. texture image may be larger than color image
      {
        u8 y0 = (u8)t & 0xFF;
        u8 v = (u8)(t >> 8) & 0xFF;
        u8 y1 = (u8)(t >> 16) & 0xFF;
...
void DLParser_S2DEX_BgCopy(MicroCodeCommand command) {
  ...
  ti.SetWidth(imageW);
  ti.SetHeight(imageH);
  ti.SetPitch(((imageW << objBg->imageSiz >> 1) >> 3)
              << 3); // force 8-bit alignment
  ...
  gRenderer->Draw2DTexture((float)frameX, (float)frameY, (float)frameW,
                           (float)frameH, (float)imageX, (float)imageY,
                           (float)imageW, (float)imageH, texture);
}
...
void DLParser_S2DEX_Bg1cyc(MicroCodeCommand command) {
  ...
  uObjScaleBg *objBg =
      (uObjScaleBg *)(g_pu8RamBase + RDPSegAddr(command.inst.cmd1));
  ...
  ti.SetPitch(
      ((objBg->imageW / 4 << ti.GetSize() >> 1) >> 3)
      << 3); // force 8-bit alignment, this what sets our correct viewport.
  ...
  if (g_ROM.GameHacks != YOSHI) {
    ...
    gRenderer->Draw2DTexture(frameX, frameY, frameW, frameH, imageX, imageY, s1,
                             t1, texture);
  } else {
    ...
    gRenderer->Draw2DTexture(frameX, frameY, x2, y2, imageX, imageY, imageW,
                             imageH, texture);
