...
static const u64 ROMDB_MAGIC_NO{
    0x42444D5244454144LL}; // DAEDRMDB		// 44 41 45 44 52 4D 44 42
...
public:
  ...
  bool QueryByFilename(const char *filename, RomID *id, u32 *rom_size,
                       ECicType *cic_type) override;
  bool QueryByID(const RomID &id, u32 *rom_size,
                 ECicType *cic_type) const override;
  const char *QueryFilenameFromID(const RomID &id) const override;

private:
  void AddRomFile(const char *filename);

  void AddRomEntry(const char *filename, const RomID &id, u32 rom_size,
                   ECicType cic_type);
...
private:
  ...
  struct RomFilesKeyValue {
    RomFilesKeyValue() { memset(FileName, 0, sizeof(FileName)); }
    ...
    // This is actually IO::Path::kMaxPathLen+1, but we need to ensure that it
    // doesn't change if we ever change the kMaxPathLen constant.
  ...
  struct SSortByFilename {
    bool operator()(const RomFilesKeyValue &a,
                    const RomFilesKeyValue &b) const {
  ...
  struct RomDetails {
    RomDetails() : ID(), RomSize(0), CicType(CIC_UNKNOWN) {}

    RomDetails(const RomID &id, u32 rom_size, ECicType cic_type)
        : ID(id), RomSize(rom_size), CicType(cic_type) {}
...
IRomDB::IRomDB() : mDirty(false) { mRomDBFileName[0] = '\0'; }

IRomDB::~IRomDB() { Commit(); }
...
bool IRomDB::OpenDB(const char *filename) {
...
#ifdef DAEDALUS_DEBUG_CONSOLE
    DBGConsole_Msg(0, "RomDB has unexpectedly large number of files (%d).",
                   num_files);
#endif
    goto fail;
  }
  ...
  if (fread(&mRomFiles[0], sizeof(RomFilesKeyValue), num_files, fh) !=
      num_files) {
    goto fail;
  }
  // Redundant?
...
#ifdef DAEDALUS_DEBUG_CONSOLE
    DBGConsole_Msg(0, "RomDB has unexpectedly large number of details (%d).",
                   num_details);
#endif
    goto fail;
  }
  ...
  if (fread(&mRomDetails[0], sizeof(RomDetails), num_details, fh) !=
      num_details) {
    goto fail;
  }
  // Redundant?
...
#ifdef DAEDALUS_DEBUG_CONSOLE
  DBGConsole_Msg(0, "RomDB initialised with %d files and %d details.",
                 mRomFiles.size(), mRomDetails.size());
#endif
  fclose(fh);
  return true;
...
void IRomDB::AddRomEntry(const char *filename, const RomID &id, u32 rom_size,
                         ECicType cic_type) {
  // Update filename/id map
  auto fit(std::lower_bound(mRomFiles.begin(), mRomFiles.end(), filename,
                            SSortByFilename()));
  ...
  auto dit(std::lower_bound(mRomDetails.begin(), mRomDetails.end(), id,
                            SSortDetailsByID()));
...
static bool GenerateRomDetails(const char *filename, RomID *id, u32 *rom_size,
                               ECicType *cic_type) {
  //
...
bool IRomDB::QueryByFilename(const char *filename, RomID *id, u32 *rom_size,
                             ECicType *cic_type) {
  //
  // First of all, check if we have these details cached in the rom database
  //
  FilenameVec::const_iterator fit(std::lower_bound(
      mRomFiles.begin(), mRomFiles.end(), filename, SSortByFilename()));
  if (fit != mRomFiles.end() && strcmp(fit->FileName, filename) == 0) {
...
bool IRomDB::QueryByID(const RomID &id, u32 *rom_size,
                       ECicType *cic_type) const {
  auto it(std::lower_bound(mRomDetails.begin(), mRomDetails.end(), id,
                           SSortDetailsByID()));
