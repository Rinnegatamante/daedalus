...
public:
  ...
  virtual bool
  OpenDB(const char *filename) = 0; // Open the specified rom db, or creates a
                                    // new one if it does not currently exist
  ...
  virtual bool
  QueryByFilename(const char *filename, RomID *id, u32 *rom_size,
                  ECicType *cic_type) = 0; // Query a rom from the database
  virtual bool
  QueryByID(const RomID &id, u32 *rom_size,
            ECicType *cic_type) const = 0; // Query a rom from the database
