...
// Sign of Z coord from cross product normal, used for triangle front/back face
// culling //Corn Note that we pass s32 even if it is a f32! The check for <=
// 0.0f is valid also with signed integers(bit31 in f32 is sign bit)
...
inline float vfpu_dot_4Dvec(const float x, const float y, const float z,
  ...
  __asm__ volatile("mtv   %1, S000\n"
                   ...
                   "mfv   %0, S020\n"
                   : "=r"(result)
                   : "r"(x), "r"(y), "r"(z), "r"(w), "r"(a), "r"(b), "r"(c),
                     "r"(d));
  return result;
}

inline float vfpu_dot_3Dvec(const float x, const float y, const float z,
...
#else
inline float vfpu_invSqrt(
    float x) // Trick using int/float to get 1/SQRT() fast on FPU/CPU //Corn
{
  union {
    int itg;
    float flt;
...
inline void vfpu_N64_2_PSP(float *Pcoord, const float *Ncoord,
                           const float *Pscale, const float *Ptrans) {
...
inline float pspFpuSqrt(float fs) { return (__builtin_allegrex_sqrt_s(fs)); }
#else
...
inline int pspFpuFloor(float fs) { return (__builtin_allegrex_floor_w_s(fs)); }

inline int pspFpuCeil(float fs) { return (__builtin_allegrex_ceil_w_s(fs)); }

inline int pspFpuTrunc(float fs) { return (__builtin_allegrex_trunc_w_s(fs)); }

inline int pspFpuRound(float fs) { return (__builtin_allegrex_round_w_s(fs)); }

// I'm not sure if the vfpu_fmaxf and vfpu_fminf it's slower or faster than fpu
// version Let's do regardless to avoid overhead.
/*
...
inline int pspFpuIsNaN(float f) {
  ...
  asm(".set push\n"
      ...
      "sll $8, $8, 9\n"        // t0 = t0<<9
      "sltiu $9, $9, 0x00FF\n" // t1 = (t1<0xFF)
      "movz %0, $0, $8\n"      // v  = (t0==0) ? 0 : v		if (frac==0) is not
                               // NaN
      "movn %0, $0, $9\n"      // v  = (t1!=0) ? 0 : v		if (exp!=0xFF) is not
                               // NAN
      ".set pop\n"
      : "=r"(v)
      : "f"(f)
...
inline s32 Double2Int(f64 *d) { return (s32)*d; }
...
// We map these because the compiler doesn't use the fpu math... we have to do
// it manually
//
#define isnanf(x) pspFpuIsNaN((x))
#define sqrtf(x) pspFpuSqrt((x))
#define roundf(x)                                                              \
  pspFpuRound((x)) // FIXME(strmnnrmn): results in an int! Alternate version
                   // below results in a float!
