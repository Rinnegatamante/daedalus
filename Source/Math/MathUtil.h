...
inline float Interpolate(float a, float b, float r) { return a + r * (b - a); }
...
template <typename T> inline T Saturate(s32 x);

template <> inline s16 Saturate(s32 x) {
  return s16(Clamp<s32>(x, -32768, 32767));
}

template <> inline s8 Saturate(s32 x) { return s8(Clamp<s32>(x, -128, 127)); }
...
template <typename T> inline const T *RoundPointerUp(const T *p, uintptr_t r) {
  const uintptr_t mask = r - 1;
  return reinterpret_cast<const T *>((reinterpret_cast<uintptr_t>(p) + mask) &
                                     ~mask);
}
...
inline u32 AlignPow2(u32 x, u32 r) { return (x + (r - 1)) & ~(r - 1); }

inline bool IsAligned(u32 x, u32 r) { return (x & (r - 1)) == 0; }
