...
typedef struct OSMesgQueue_s {
        OSThread	*mtqueue;	// Queue to store threads blocked on
empty mailboxes (receive) OSThread	*fullqueue;	// Queue to store
threads blocked on full mailboxes (send) s32			validCount;
...
class COSMesgQueue {
public:
  inline COSMesgQueue(u32 addr) { p_base = (u8 *)ReadAddress(addr); }
