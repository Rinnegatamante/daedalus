...
static const char *const gEventStrings[23] = {"OS_EVENT_SW1",
                                              "OS_EVENT_SW2",
                                              "OS_EVENT_CART",
                                              "OS_EVENT_COUNTER",
                                              "OS_EVENT_SP",
...
#define PATCH_RET_NOT_PROCESSED0(name)                                         \
  RET_NOT_PROCESSED(&PATCH_SYMBOL_FUNCTION_ENTRY(name))
...
static bool Patch_VerifyLocation_CheckSignature(PatchSymbol *ps,
                                                PatchSignature *psig,
                                                u32 index);
...
void Patch_ApplyPatches() {
  ...
  if (!Patch_GetCache()) {
    ...
    // Tip : Disable this when working on oshle funcs, you save the time to
    // delete hle cache everyttime you need to test :p
    Patch_FlushCache();
  }
  ...
  for (u32 i = 0; i < nPatchSymbols; i++) {
    if (g_PatchSymbols[i]->Found) {
      ...
      Dump_Disassemble(PHYS_TO_K0(ps->Location),
                       PHYS_TO_K0(ps->Location) +
                           ps->Signatures->NumOps * sizeof(OpCode),
                       buf);
...
void Patch_ApplyPatch(u32 i) {
  ...
  auto *frag =
      new CFragment(gFragmentCache.GetCodeBufferManager(), PHYS_TO_K0(pc),
...
void Patch_DumpOsThreadInfo() {
  ...
  DBGConsole_Msg(0, "");
  DBGConsole_Msg(
      0, "Threads:      Pri   Queue       State   Flags   ID          FP Used");
  // DBGConsole_Msg(0, "  0x01234567, xxxx, 0x01234567, 0x0123, 0x0123,
  ...
  while (dwThread) {
    ...
    if (dwThread == dwCurrentThread) {
      DBGConsole_Msg(0,
                     "->0x%08x, % 4d, 0x%08x, 0x%04x, 0x%04x, 0x%08x, 0x%08x",
...
void Patch_DumpOsQueueInfo() {
  ...
  // DBGConsole_Msg(0, "01234567, 01234567, 01234567, xxxx, xxxx, xxxx,
  // 01234567",
  for (i = 0; i < g_MessageQueues.size(); i++) {
    ...
    if (strlen(type_buffer) == 0 && VAR_FOUND(osEventMesgArray)) {
      for (u32 j = 0; j < 23; j++) {
        if (dwQueue ==
            Read32Bits(VAR_ADDRESS(osEventMesgArray) + (j * 8) + 0x0)) {
          sprintf(type_buffer, "<- %s", gEventStrings[j]);
          break;
        }
      }
    }
    DBGConsole_Msg(0, "%08x, %s, %s, % 4d, % 4d, % 4d, %08x %s", dwQueue,
                   emptyqueue_buffer, fullqueue_buffer, dwValidCount, dwFirst,
                   dwMsgCount, dwMsg, type_buffer);
  }
#endif
}
...
void Patch_DumpOsEventInfo() {
  ...
  for (u32 i = 0; i < 23; i++) {
    ...
    DBGConsole_Msg(0, "  %-26s 0x%08x 0x%08x", gEventStrings[i], dwQueue,
                   dwMsg);
  }
}
...
bool Patch_Hacks(PatchSymbol *ps) {
  ...
  // This alot cheaper than adding a check on the func itself, this is only
  // checked once -Salvy Eventually we should fix them though
  //
...
void Patch_RecurseAndFind() {
  ...
  DBGConsole_Msg(
      0, "Searching for os functions. This may take several seconds...");
#endif
  ...
  intraFont *ltn8 =
      intraFontLoad("flash0:/font/ltn8.pgf", INTRAFONT_CACHE_ASCII);
  intraFontSetStyle(ltn8, 1.0f, 0xFFFFFFFF, 0, 0.f, INTRAFONT_ALIGN_CENTER);
#endif
#endif

  // Loops through all symbols, until name is nullptr
  for (u32 i = 0; i < nPatchSymbols && !gCPUState.IsJobSet(CPU_STOP_RUNNING);
       i++) {
    ...
    // intraFontPrintf( ltn8, 480/2, (272>>1)-50, "Searching for os functions.
    // This may take several seconds...");
    intraFontPrintf(ltn8, 480 / 2, (272 >> 1), "OS HLE Patching: %d%%",
                    i * 100 / (nPatchSymbols - 1));
    intraFontPrintf(ltn8, 480 / 2, (272 >> 1) - 50, "Searching for %s",
                    g_PatchSymbols[i]->Name);
  ...
  for (u32 i = 0; i < nPatchSymbols; i++) {
    if (!g_PatchSymbols[i]->Found) {
      ...
      // Find duplicates! (to avoid showing the same clash twice, only scan up
      // to the first symbol)
...
#ifdef DAEDALUS_DEBUG_CONSOLE
          DBGConsole_Msg(0, "Warning [C%s==%s]", g_PatchSymbols[i]->Name,
    ...
    CGraphicsContext::Get()->BeginFrame();
    CGraphicsContext::Get()->ClearToBlack();
    intraFontPrintf(ltn8, 480 / 2, (272 >> 1), "Symbols Identified: %d%%",
                    100 * nFound / (nPatchSymbols - 1));
    intraFontPrintf(ltn8, 480 / 2, (272 >> 1) + 50, "Range 0x%08x -> 0x%08x",
                    first, last);
    CGraphicsContext::Get()->EndFrame();
    CGraphicsContext::Get()->UpdateFrame(true);
#endif
  ...
  for (u32 i = 0; i < nPatchVariables; i++) {
...
#ifdef DAEDALUS_DEBUG_CONSOLE
          DBGConsole_Msg(0, "Warning [C%s==%s]", g_PatchVariables[i]->Name,
    ...
    CGraphicsContext::Get()->BeginFrame();
    CGraphicsContext::Get()->ClearToBlack();
    intraFontPrintf(ltn8, 480 / 2, 272 >> 1, "Variables Identified: %d%%",
                    100 * nFound / (nPatchVariables - 1));
    CGraphicsContext::Get()->EndFrame();
    CGraphicsContext::Get()->UpdateFrame(true);
#endif
#endif
...
bool Patch_VerifyLocation_CheckSignature(PatchSymbol *ps, PatchSignature *psig,
                                         u32 index) {
  OpCode op{};
      ...
      case PX_VARIABLE_HI: {
        ...
        if (pcr->Variable->FoundHi) {
          ...
          if (pcr->Variable->FoundLo)
            pcr->Variable->Location =
                (pcr->Variable->HiWord << 16) + (short)(pcr->Variable->LoWord);
        }
  ...
  if (fp != nullptr) {
    ...
    for (u32 i = 0; i < nPatchSymbols; i++) {
      ...
      if (data != 0) {
        ...
        fread(&data, 1, sizeof(data), fp);
        g_PatchSymbols[i]->Function =
            g_PatchSymbols[i]->Signatures[data].Function;
      } else
        g_PatchSymbols[i]->Found = false;
    }
...
static u32 RET_NOT_PROCESSED(PatchSymbol *ps) {
  ...
  // DBGConsole_Msg(0, "%s RET_NOT_PROCESSED PC=0x%08x RA=0x%08x", ps->Name,
  // gCPUState.TargetPC, gGPR[REG_ra]._u32_0);

  gCPUState.Delay = NO_DELAY;
  gCPUState.TargetPC = gCPUState.CurrentPC;

  // Simulate the first op then return to dynarec. so we still can leverage
  // dynarec.
  OpCode op_code{};
  ...
  DAEDALUS_ASSERT(gCPUState.Delay == NO_DELAY,
                  "OS functions' first op is a JUMP??");
#endif
  INCREMENT_PC();
  gCPUState.TargetPC = gCPUState.CurrentPC;
...
#ifdef DAEDALUS_ENABLE_ASSERTS
  DAEDALUS_ASSERT(IS_K0K1(addr) == (IS_KSEG0(addr) | IS_KSEG1(addr)),
                  "IS_K0K1 is inconsistent");
#endif
  if (IS_K0K1(addr)) {
...
u32 Patch___osPackRequestData() { return PATCH_RET_NOT_PROCESSED; }
...
u32 Patch___osEepromRead_Prepare() { return PATCH_RET_NOT_PROCESSED; }
u32 Patch___osEepromWrite_Prepare() { return PATCH_RET_NOT_PROCESSED; }
