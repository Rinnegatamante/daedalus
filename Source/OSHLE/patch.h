...
enum PATCH_CROSSREF_TYPE { PX_JUMP, PX_VARIABLE_HI, PX_VARIABLE_LO };
...
typedef struct _PatchSymbol {
  ...
  PatchFunction Function; // The installed patch Emulated function call
...
#define BEGIN_PATCH_XREFS(label) static PatchCrossRef g_##label##_x[] = {
...
#define BEGIN_PATCH_SYMBOL_TABLE(name) PatchSymbol *name[] = {

#define PATCH_FUNCTION_ENTRY(name) &g_##name##_s,
...
#define BEGIN_PATCH_VARIABLE_TABLE(name) PatchVariable *name[] = {

#define PATCH_VARIABLE_ENTRY(name) &g_##name##_v,
