...
CNativeTexture::CNativeTexture(u32 w, u32 h, ETextureFormat texture_format)
    : mTextureFormat(texture_format), mWidth(w), mHeight(h),
      ...
      mpData(NULL), mpPalette(NULL), mTextureId(0) {
...
bool CNativeTexture::HasData() const { return mTextureId != 0; }
...
CRefPtr<CNativeTexture> LoadPng(const char *p_filename,
  ...
  if (!buffer) {
    ...
    case TexFmt_5650:
      ReadPngData<NativePf5650>(width, height, stride, row_pointers, color_type,
                                reinterpret_cast<NativePf5650 *>(buffer));
      break;
    case TexFmt_5551:
      ReadPngData<NativePf5551>(width, height, stride, row_pointers, color_type,
                                reinterpret_cast<NativePf5551 *>(buffer));
      break;
    case TexFmt_4444:
      ReadPngData<NativePf4444>(width, height, stride, row_pointers, color_type,
                                reinterpret_cast<NativePf4444 *>(buffer));
      break;
    case TexFmt_8888:
      ReadPngData<NativePf8888>(width, height, stride, row_pointers, color_type,
                                reinterpret_cast<NativePf8888 *>(buffer));
      break;
...
void CNativeTexture::SetData(void *data, void *palette) {
  // It's pretty gross that we don't pass this in, or better yet, provide a way
  // for the caller to write directly to our buffers instead of setting the
  // data.
  size_t data_len = GetBytesRequired();
  ...
  if (HasData()) {
    ...
    case TexFmt_5650:
      glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, mCorrectedWidth, mCorrectedHeight,
    ...
    case TexFmt_8888:
      ...
      break;
    case TexFmt_CI4_8888: {
      // Convert palletised texture to non-palletised. This is wsteful - we
      // should avoid generating these updated for OSX.
      ...
      NativePf8888 *out = static_cast<NativePf8888 *>(
          malloc(mCorrectedWidth * mCorrectedHeight * sizeof(NativePf8888)));
      ...
      for (u32 y = 0; y < mCorrectedHeight; ++y) {
        ...
        pix_ptr = reinterpret_cast<const NativePfCI44 *>(
            reinterpret_cast<const u8 *>(pix_ptr) + pitch);
      }
    ...
    case TexFmt_CI8_8888: {
      ...
      for (u32 y = 0; y < mCorrectedHeight; ++y) {
        ...
        pix_ptr = reinterpret_cast<const NativePfCI8 *>(
            reinterpret_cast<const u8 *>(pix_ptr) + pitch);
      }
