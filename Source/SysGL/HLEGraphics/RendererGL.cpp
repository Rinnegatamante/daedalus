...
const float kShiftScales[] = {
    1.f / (float)(1 << 0), 1.f / (float)(1 << 1),  1.f / (float)(1 << 2),
    1.f / (float)(1 << 3), 1.f / (float)(1 << 4),  1.f / (float)(1 << 5),
    1.f / (float)(1 << 6), 1.f / (float)(1 << 7),  1.f / (float)(1 << 8),
    1.f / (float)(1 << 9), 1.f / (float)(1 << 10), (float)(1 << 5),
    (float)(1 << 4),       (float)(1 << 3),        (float)(1 << 2),
    (float)(1 << 1),
};
...
bool initgl() {
  ...
  // Only do software emulation of mirror_s/mirror_t if we're not doing accurate
  // UV handling
  gRDPStateManager.SetEmulateMirror(!gAccurateUVPipe);

  // FIXME(strmnnrmn): we shouldn't need these with GLEW, but they don't seem to
  // resolve on OSX.
  GLboolean status = GL_TRUE;
  RESOLVE_GL_FCN(PFN_glGenVertexArrays, pglGenVertexArrays,
                 "glGenVertexArrays");
  RESOLVE_GL_FCN(PFN_glDeleteVertexArrays, pglDeleteVertexArrays,
                 "glDeleteVertexArrays");
  RESOLVE_GL_FCN(PFN_glBindVertexArray, pglBindVertexArray,
                 "glBindVertexArray");
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(gPositionBuffer), gPositionBuffer,
               GL_DYNAMIC_DRAW);

  glBindBuffer(GL_ARRAY_BUFFER, gVBOs[kTexCoordBuffer]);
  glBufferData(GL_ARRAY_BUFFER, sizeof(gTexCoordBuffer), gTexCoordBuffer,
               GL_DYNAMIC_DRAW);

  glBindBuffer(GL_ARRAY_BUFFER, gVBOs[kColorBuffer]);
  glBufferData(GL_ARRAY_BUFFER, sizeof(gColorBuffer), gColorBuffer,
               GL_DYNAMIC_DRAW);
  return true;
}
...
inline bool operator==(const ShaderConfiguration &a,
                       const ShaderConfiguration &b) {
  return a.Mux == b.Mux && a.CycleType == b.CycleType &&
         a.BilerpFilter == b.BilerpFilter && a.ClampS0 == b.ClampS0 &&
         a.ClampT0 == b.ClampT0 && a.ClampS1 == b.ClampS1 &&
         a.ClampT1 == b.ClampT1 && a.AlphaThreshold == b.AlphaThreshold;
}
 ...
 */
static GLuint make_shader_program(const char **vertex_lines,
                                  size_t num_vertex_lines,
                                  const char **fragment_lines,
                                  size_t num_fragment_lines) {
  GLuint program = 0u;
  GLint program_ok;
  ...
  if (vertex_shader != 0u) {
    fragment_shader =
        make_shader(GL_FRAGMENT_SHADER, fragment_lines, num_fragment_lines);
...
static const char *kRGBParams16[] = {
    "combined.rgb", "tex0.rgb", "tex1.rgb",      "prim.rgb",
    "shade.rgb",    "env.rgb",  "one.rgb",       "combined.a",
    "tex0.a",       "tex1.a",   "prim.a",        "shade.a",
    "env.a",        "lod_frac", "prim_lod_frac", "zero.rgb",
};

static const char *kRGBParams8[8] = {
    "combined.rgb", "tex0.rgb", "tex1.rgb", "prim.rgb",
    "shade.rgb",    "env.rgb",  "one.rgb",  "zero.rgb",
};

static const char *kAlphaParams8[8] = {"combined.a", "tex0.a",  "tex1.a",
                                       "prim.a",     "shade.a", "env.a",
                                       "one.a",      "zero.a"};
...
static inline const char *GetFilter(bool bilerp, bool clamp_s, bool clamp_t) {
  if (bilerp) {
    if (clamp_s && clamp_t)
      return "fetchBilinearClampedST";
    else if (clamp_s)
      return "fetchBilinearClampedS";
    else if (clamp_t)
      return "fetchBilinearClampedT";
    else
...
static void SprintShader(char (&frag_shader)[2048],
  ...
  if (cycle_type == CYCLE_FILL) {
    ...
    strcpy(body,
           "\tcol = fetchCopy(sti, uTileShift0, uTileMirror0, uTileMask0, "
           "uTileTL0, uTileBR0, uTileClampEnable0, uTexture0, uTexScale0);\n");
  } else if (cycle_type == CYCLE_1CYCLE) {
    const char *filter0 =
        GetFilter(config.BilerpFilter, config.ClampS0, config.ClampT0);
    const char *filter1 =
        GetFilter(config.BilerpFilter, config.ClampS1, config.ClampT1);

    sprintf(body,
            "\tvec4 tex0 = %s(sti, uTileShift0, uTileMirror0, uTileMask0, "
            "uTileTL0, uTileBR0, uTileClampEnable0, uTexture0, uTexScale0);\n"
            "\tvec4 tex1 = %s(sti, uTileShift1, uTileMirror1, uTileMask1, "
            "uTileTL1, uTileBR1, uTileClampEnable1, uTexture1, uTexScale1);\n"
            "\tcol.rgb = (%s - %s) * %s + %s;\n"
            "\tcol.a   = (%s - %s) * %s + %s;\n",
            filter0, filter1, kRGBParams16[aRGB0], kRGBParams16[bRGB0],
            kRGBParams32[cRGB0], kRGBParams8[dRGB0], kAlphaParams8[aA0],
            kAlphaParams8[bA0], kAlphaParams8[cA0], kAlphaParams8[dA0]);
  } else {
    const char *filter0 =
        GetFilter(config.BilerpFilter, config.ClampS0, config.ClampT0);
    const char *filter1 =
    ...
    sprintf(body,
            ...
            "\tcol.a   = (%s - %s) * %s + %s;\n"
            "\tcombined = col;\n"
            "\ttex0 = tex1;\n" // NB: tex0 becomes tex1 on the second cycle -
                               // see mame.
            "\tcol.rgb = (%s - %s) * %s + %s;\n"
            "\tcol.a   = (%s - %s) * %s + %s;\n",
            filter0, filter1, kRGBParams16[aRGB0], kRGBParams16[bRGB0],
            kRGBParams32[cRGB0], kRGBParams8[dRGB0], kAlphaParams8[aA0],
            kAlphaParams8[bA0], kAlphaParams8[cA0], kAlphaParams8[dA0],
            kRGBParams16[aRGB1], kRGBParams16[bRGB1], kRGBParams32[cRGB1],
            kRGBParams8[dRGB1], kAlphaParams8[aA1], kAlphaParams8[bA1],
            kAlphaParams8[cA1], kAlphaParams8[dA1]);
  }
...
static void InitShaderProgram(ShaderProgram *program,
                              const ShaderConfiguration &config,
  ...
  program->uloc_primlodfrac =
      glGetUniformLocation(shader_program, "uPrimLODFrac");

  program->uloc_foo = glGetUniformLocation(shader_program, "uFoo");

  program->uloc_tileclamp[0] =
      glGetUniformLocation(shader_program, "uTileClampEnable0");
  ...
  program->uloc_tileshift[0] =
      glGetUniformLocation(shader_program, "uTileShift0");
  program->uloc_tilemask[0] =
      glGetUniformLocation(shader_program, "uTileMask0");
  program->uloc_tilemirror[0] =
      glGetUniformLocation(shader_program, "uTileMirror0");
  program->uloc_texscale[0] =
      glGetUniformLocation(shader_program, "uTexScale0");
  program->uloc_texture[0] = glGetUniformLocation(shader_program, "uTexture0");

  program->uloc_tileclamp[1] =
      glGetUniformLocation(shader_program, "uTileClampEnable1");
  ...
  program->uloc_tileshift[1] =
      glGetUniformLocation(shader_program, "uTileShift1");
  program->uloc_tilemask[1] =
      glGetUniformLocation(shader_program, "uTileMask1");
  program->uloc_tilemirror[1] =
      glGetUniformLocation(shader_program, "uTileMirror1");
  program->uloc_texscale[1] =
      glGetUniformLocation(shader_program, "uTexScale1");
...
void RendererGL::MakeShaderConfigFromCurrentState(
    ShaderConfiguration *config) const {
  config->Mux = mMux;
  ...
  if ((gRDPOtherMode.alpha_compare == G_AC_THRESHOLD) &&
    ...
    // FIXME(strmnnrmn): alpha func: (mAlphaThreshold | g_ROM.ALPHA_HACK) ?
    // GL_GEQUAL : GL_GREATER
    config->AlphaThreshold = mBlendColour.GetA();
  } else if (gRDPOtherMode.cvg_x_alpha) {
    // Going over 0x70 brakes OOT, but going lesser than that makes lines on
  ...
  // In fill/cycle modes, we ignore the mux. Set it to zero so we don't create
  // unecessary shaders.
  u32 cycle_type = config->CycleType;
  ...
  config->BilerpFilter = (gRDPOtherMode.text_filt != G_TF_POINT) ||
...
static ShaderProgram *GetShaderForConfig(const ShaderConfiguration &config) {
  DAEDALUS_ASSERT(gN64FramentLibrary != NULL,
                  "Haven't initialised the n64 fragment library");
  ...
  GLuint shader_program =
      make_shader_program(vertex_lines, ARRAYSIZE(vertex_lines), fragment_lines,
                          ARRAYSIZE(fragment_lines));
  if (shader_program == 0) {
...
void RendererGL::RenderDaedalusVtx(int prim, const DaedalusVtx *vertices,
                                   int count) {
  ...
  const f32 scale =
      (g_ROM.ZELDA_HACK && (gRDPOtherMode.L == 0x0c184241)) ? 16.f : 32.f;
  ...
  RenderDaedalusVtxStreams(prim, &gPositionBuffer[0][0], &gTexCoordBuffer[0],
                           &gColorBuffer[0], count);
}

void RendererGL::RenderDaedalusVtxStreams(int prim, const float *positions,
                                          const TexCoord *uvs,
...
static inline void DebugBlender(u32 cycle_type, u32 blender, u32 alpha_cvg_sel,
                                u32 cvg_x_alpha) {
  ...
  if (last_blender != blender) {
    ...
    printf("0x%04x: // %s * %s + %s * %s", blender,
           kBlendCl[(blender >> 14) & 0x3], kBlendA1[(blender >> 10) & 0x3],
           kBlendCl[(blender >> 6) & 0x3], kBlendA2[(blender >> 2) & 0x3]);

    if (cycle_type == CYCLE_2CYCLE) {
      printf(" | %s * %s + %s * %s", kBlendCl[(blender >> 12) & 0x3],
             kBlendA1[(blender >> 8) & 0x3], kBlendCl[(blender >> 4) & 0x3],
...
static void InitBlenderMode() {
  ...
  u32 active_mode =
      (cycle_type == CYCLE_2CYCLE) ? blendmode : (blendmode & 0xcccc);
  ...
  default:
    ...
    DL_PF("		 Blend: SRCALPHA/INVSRCALPHA (default: 0x%04x)",
          active_mode);
#endif
    break;
  }

  // NB: we only have alpha in the blender is alpha_cvg_sel is 0 or cvg_x_alpha
  // is 1.
...
inline u32 MakeMask(u32 m) { return m ? ((1 << m) - 1) : 0xffffffff; }

inline u32 MakeMirror(u32 mirror, u32 m) {
  return (mirror && m) ? (1 << m) : 0;
}

void RendererGL::PrepareRenderState(const float (&mat_project)[16],
  ...
  if (program == NULL) {
    ...
    DBGConsole_Msg(
        0, "Couldn't generate a shader for mux %llx, cycle %d, alpha %d\n",
  ...
  glUniform4f(program->uloc_primcol, mPrimitiveColour.GetRf(),
              mPrimitiveColour.GetGf(), mPrimitiveColour.GetBf(),
              mPrimitiveColour.GetAf());
  glUniform4f(program->uloc_envcol, mEnvColour.GetRf(), mEnvColour.GetGf(),
              mEnvColour.GetBf(), mEnvColour.GetAf());
  ...
  for (u32 i = 0; i < kNumTextures; ++i) {
    ...
    if (texture != NULL) {
      ...
      glUniform2f(program->uloc_tileshift[i], kShiftScales[rdp_tile.shift_s],
      ...
      glUniform2i(program->uloc_tiletl[i], mTileTopLeft[i].s,
                  mTileTopLeft[i].t);
      glUniform2i(program->uloc_tilebr[i], tile_size.right, tile_size.bottom);

      glUniform2f(program->uloc_texscale[i], 1.f / texture->GetCorrectedWidth(),
...
// It ends up copying colour/uv coords when not needed, and can use a shader
// uniform for the fill colour.
void RendererGL::RenderTriangles(DaedalusVtx *p_vertices, u32 num_vertices,
                                 bool disable_zbuffer) {
  if (mTnL.Flags.Texture) {
    UpdateTileSnapshots(mTextureTile);

    // FIXME: this should be applied in SetNewVertexInfo, and use
    // TextureScaleX/Y to set the scale
    if (mTnL.Flags.Light && mTnL.Flags.TexGen) {
      if (CNativeTexture *texture = mBoundTexture[0]) {
        ...
        // It implies that the RSP code is checking RDP tile state, which seems
        // wrong. gsDPSetHilite1Tile might set up some RSP state?
...
void RendererGL::TexRect(u32 tile_idx, const v2 &xy0, const v2 &xy1,
                         TexCoord st0, TexCoord st1) {
  // FIXME(strmnnrmn): in copy mode, depth buffer is always disabled. Might not
  ...
  // etc. We have to do it before PrepareRenderState, because those values are
  // applied to the graphics state.
  PrepareTexRectUVs(&st0, &st1);

  PrepareRenderState(mScreenToDevice.mRaw,
  ...
  DL_PF("    Screen:  %.1f,%.1f -> %.1f,%.1f", screen0.x, screen0.y, screen1.x,
        screen1.y);
  DL_PF("    Texture: %.1f,%.1f -> %.1f,%.1f", st0.s / 32.f, st0.t / 32.f,
        st1.s / 32.f, st1.t / 32.f);

  const f32 depth = gRDPOtherMode.depth_source ? mPrimDepth : 0.0f;

  float positions[] = {
      screen0.x, screen0.y, depth, screen1.x, screen0.y, depth,
      screen0.x, screen1.y, depth, screen1.x, screen1.y, depth,
  };
...
void RendererGL::TexRectFlip(u32 tile_idx, const v2 &xy0, const v2 &xy1,
                             TexCoord st0, TexCoord st1) {
  UpdateTileSnapshots(tile_idx);
...
void RendererGL::Draw2DTexture(f32 x0, f32 y0, f32 x1, f32 y1, f32 u0, f32 v0,
  ...
  float positions[] = {
      sx0, sy0, depth, sx1, sy0, depth, sx0, sy1, depth, sx1, sy1, depth,
  };
...
void RendererGL::Draw2DTextureR(f32 x0, f32 y0, f32 x1, f32 y1, f32 x2, f32 y2,
