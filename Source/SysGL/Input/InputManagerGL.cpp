...
IInputManager::IInputManager() : mGamePadAvailable(false) {}
...
void IInputManager::GetJoyPad(OSContPad *pPad) {
  ...
  // ToDo: Different gamepads will need different configuration, this is for
  // PS3/PS2 controller
  if (buttons[11])
    pPad->button |= START_BUTTON;
  if (buttons[9])
    pPad->button |= START_BUTTON;
  if (buttons[2])
    pPad->button |= A_BUTTON;
  if (buttons[3])
    pPad->button |= B_BUTTON;
  if (buttons[6])
    pPad->button |= Z_TRIG;
  if (buttons[4])
    pPad->button |= L_TRIG;
  ...
  // We could use the second analog stick to map them, but will require to
  // translate asix >=2
  if (buttons[1]) {
    if (buttons[20])
...
void IInputManager::GetState(OSContPad pPad[4]) {
  ...
  if (mGamePadAvailable) {
    ...
    if (glfwGetKey(window, 'X'))
      pPad[0].button |= A_BUTTON;
    if (glfwGetKey(window, 'C'))
      pPad[0].button |= B_BUTTON;
    if (glfwGetKey(window, 'Z'))
      pPad[0].button |= Z_TRIG;
    if (glfwGetKey(window, 'Y'))
      pPad[0].button |= Z_TRIG; // For German keyboards :)
    if (glfwGetKey(window, 'A'))
      pPad[0].button |= L_TRIG;
    if (glfwGetKey(window, 'S'))
      pPad[0].button |= R_TRIG;

    if (glfwGetKey(window, GLFW_KEY_ENTER))
      pPad[0].button |= START_BUTTON;

    if (glfwGetKey(window, GLFW_KEY_KP_8))
      pPad[0].button |= U_JPAD;
    if (glfwGetKey(window, GLFW_KEY_KP_2))
      pPad[0].button |= D_JPAD;
    if (glfwGetKey(window, GLFW_KEY_KP_4))
      pPad[0].button |= L_JPAD;
    if (glfwGetKey(window, GLFW_KEY_KP_6))
      pPad[0].button |= R_JPAD;

    if (glfwGetKey(window, GLFW_KEY_HOME))
      pPad[0].button |= U_CBUTTONS;
    if (glfwGetKey(window, GLFW_KEY_END))
      pPad[0].button |= D_CBUTTONS;
    if (glfwGetKey(window, GLFW_KEY_DELETE))
      pPad[0].button |= L_CBUTTONS;
    if (glfwGetKey(window, GLFW_KEY_PAGE_DOWN))
      pPad[0].button |= R_CBUTTONS;
...
u32 IInputManager::GetNumConfigurations() const { return 0; }
