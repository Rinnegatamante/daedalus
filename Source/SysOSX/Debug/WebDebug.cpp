...
WebDebugConnection::WebDebugConnection(WebbyConnection *connection)
    : mConnection(connection), mState(kUnresponded), mBytesExpected(0),
      mBytesWritten(0) {
...
void WebDebugConnection::BeginResponse(int code, int content_length,
  ...
  WebbyBeginResponse(mConnection, code, content_length, headers,
                     ARRAYSIZE(headers));
  mState = kResponding;
}
...
void WebDebugConnection::EndResponse() {
  ...
  DAEDALUS_ASSERT(mBytesExpected == 0 || mBytesWritten == mBytesExpected,
                  "Promised %d bytes, but only wrote %d", mBytesExpected,
                  mBytesWritten);
...
static int WebDebugDispatch(struct WebbyConnection *connection) {
  ...
  for (size_t i = 0; i < gHandlers.size(); ++i) {
    ...
    if (strcmp(connection->request.uri, entry.Request) == 0) {
      ...
      DAEDALUS_ASSERT(dbg_connection.GetState() ==
                          WebDebugConnection::kResponded,
...
static int test_ws_connect(struct WebbyConnection *connection) {
  ...
  if (0 == strcmp(connection->request.uri, "/wstest") &&
      ws_connection_count < MAX_WSCONN)
    return 0;
  else
    return 1;
}
...
static void test_ws_closed(struct WebbyConnection *connection) {
  ...
  for (i = 0; i < ws_connection_count; i++) {
    if (ws_connections[i] == connection) {
      ...
      memmove(ws_connections + i, ws_connections + i + 1,
              remain * sizeof(struct WebbyConnection *));
      --ws_connection_count;
      break;
    }
  }
...
static int test_ws_frame(struct WebbyConnection *connection,
                         const struct WebbyWsFrame *frame) {
  int i = 0;
...
static u32 DAEDALUS_THREAD_CALL_TYPE WebDebugThread(void *arg) {
  ...
  while (gKeepRunning) {
    ...
    // 	{
    // 		WebbyBeginSocketFrame(ws_connections[i],
    // WEBBY_WS_OP_TEXT_FRAME); 		WebbyPrintf(ws_connections[i], "Hello world over
  ...
  if (IO::FindFileOpen(dir, &find_handle, find_data)) {
    do {
      ...
      if (IO::Directory::IsDirectory(full_path)) {
        ...
        DBGConsole_Msg(0, " adding [M%s] -> [C%s]", resource.Resource.c_str(),
                       resource.FullPath.c_str());
...
bool WebDebug_Init() {
  ...
  config.connection_max =
      16; // Chrome and Firefox open lots of connections simultaneously.
