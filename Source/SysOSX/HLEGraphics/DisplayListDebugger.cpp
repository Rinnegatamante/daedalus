...
// These coordinate requests between the web threads and the main thread (only
// the main thread can call OpenGL functions)
...
void DLDebugger_RequestDebug() { gDebugging = true; }
  ...
  if (WebDebugConnection *connection = gActiveConnection) {
    ...
    case kTaskScrub: {
      ...
      for (u32 i = 0; i < 8; ++i) {
        ...
        if ((tile.format != 0 || tile.size != 0) && tile_size.GetWidth() > 0 &&
            tile_size.GetHeight() > 0) {
          const TextureInfo &ti =
              gRDPStateManager.GetUpdatedTextureDescriptor(i);
    ...
    case kTaskTakeScreenshot: {
      ...
      PngSaveImage(connection, pixels, NULL, TexFmt_8888, pitch, width, height,
                   false);
...
bool DLDebugger_Process() {
  ...
  while (gDebugging) {
    ...
    // FIXME: shouldn't need to do this, just wake up when there's incoming
    // WebDebug request.
    ThreadSleepMs(10);
  }
