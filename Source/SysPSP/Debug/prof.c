...
void gprof_cleanup();
static SceUInt timer_handler(SceUID uid, SceKernelSysClock *c1,
                             SceKernelSysClock *c2, void *common);
...
static void initialize() {
  ...
  gp.samples =
      (unsigned short int *)malloc(sizeof(unsigned short int) * gp.nsamples);
  if (gp.samples == NULL) {
    free(gp.arcs);
    gp.arcs = 0;
    gp.state = GMON_PROF_ERROR;
 ...
 */
static SceUInt timer_handler(SceUID uid, SceKernelSysClock *requested,
                             SceKernelSysClock *actual, void *common) {
