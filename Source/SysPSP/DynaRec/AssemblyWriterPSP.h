...
public:
  CAssemblyWriterPSP(CAssemblyBuffer *p_buffer_a, CAssemblyBuffer *p_buffer_b)
      : mpCurrentBuffer(p_buffer_a), mpAssemblyBufferA(p_buffer_a),
...
public:
  ...
  void SetBufferA() { mpCurrentBuffer = mpAssemblyBufferA; }
  void SetBufferB() { mpCurrentBuffer = mpAssemblyBufferB; }
  bool IsBufferA() const { return mpCurrentBuffer == mpAssemblyBufferA; }
...
public:
  ...
  void LoadRegister(EPspReg reg_dst, OpCodeValue load_op, EPspReg reg_base,
                    s16 offset);
  void StoreRegister(EPspReg reg_src, OpCodeValue store_op, EPspReg reg_base,
                     s16 offset);
  ...
  CJumpLocation BNEL(EPspReg a, EPspReg b, CCodeLabel target,
                     bool insert_delay);
  CJumpLocation BEQL(EPspReg a, EPspReg b, CCodeLabel target,
                     bool insert_delay);
  ...
  void Cop1Op(ECop1Op cop1_op, EPspFloatReg fd, EPspFloatReg fs,
              ECop1OpFunction cop1_funct, EPspFloatReg ft);
  void Cop1Op(ECop1Op cop1_op, EPspFloatReg fd, EPspFloatReg fs,
              ECop1OpFunction cop1_funct);
  ...
  CJumpLocation BranchCop1(ECop1BCOp bc_op, CCodeLabel target,
                           bool insert_delay);
  CJumpLocation BC1F(CCodeLabel target, bool insert_delay);
  CJumpLocation BC1T(CCodeLabel target, bool insert_delay);

  static void GetLoadConstantOps(EPspReg reg, s32 value, PspOpCode *p_op1,
                                 PspOpCode *p_op2);

  inline void AppendOp(PspOpCode op) { mpCurrentBuffer->EmitDWORD(op._u32); }

private:
  CJumpLocation BranchOp(EPspReg a, OpCodeValue op, EPspReg b,
                         CCodeLabel target, bool insert_delay);
  CJumpLocation BranchRegImmOp(EPspReg a, ERegImmOp op, CCodeLabel target,
                               bool insert_delay);
