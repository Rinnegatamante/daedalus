...
struct SCodeBuffer {
  ...
  SCodeBuffer() : mpBuffer(nullptr), mBufferPtr(0), mBufferSize(0) {}
...
// this value can be enlarged
    ...
    DAEDALUS_ASSERT(mBufferPtr + 32768 <= mBufferSize,
                    "Out of memory for dynamic recompiler");
#endif
    return mpBuffer + mBufferPtr;
  }

  void Consume(const u8 *p_base, u32 used_size) {
    //	DAEDALUS_ASSERT( AlignPow2( (u32)p_base, 64 ) == (u32)p_base, "Base ptr
    //is not aligned" );
...
bool CCodeBufferManagerPSP::Initialise() {
  ...
  // Use 2MB for each code buffer. When debugging dynarec, additional spare
  // memory is needed for storing info from dynarec.
