...
extern "C" {
const void *g_MemoryLookupTableReadForDynarec = g_MemoryLookupTableRead;
} // Important pointer for Dynarec see DynaRecStubs.s

extern "C" {
void _DDIV(s64 Num, s32 Div);
} // signed 64bit division  //Corn
extern "C" {
void _DDIVU(u64 Num, u32 Div);
} // unsigned 64bit division  //Corn
extern "C" {
void _DMULTU(u64 A, u64 B);
} // unsigned 64bit multiply  //Corn
extern "C" {
void _DMULT(s64 A, s64 B);
} // signed 64bit multiply (result is 64bit not 128bit!)  //Corn

extern "C" {
u64 _FloatToDouble(u32 _float);
} // Uses CPU to pass f64/32 thats why its maskerading as u64/32 //Corn
extern "C" {
u32 _DoubleToFloat(u64 _double);
} // Uses CPU to pass f64/32 thats why its maskerading as u64/32 //Corn

extern "C" {
void _ReturnFromDynaRec();
}
extern "C" {
void _DirectExitCheckNoDelay(u32 instructions_executed, u32 exit_pc);
}
extern "C" {
void _DirectExitCheckDelay(u32 instructions_executed, u32 exit_pc,
                           u32 target_pc);
}
extern "C" {
void _IndirectExitCheck(u32 instructions_executed, CIndirectExitMap *p_map,
                        u32 target_pc);
}
...
void _WriteBitsDirect_u16(u32 address, u32 value,
                          u32 current_pc); // Value in low 16 bits
void _WriteBitsDirect_u8(u32 address, u32 value,
                         u32 current_pc); // Value in low 8 bits

void _WriteBitsDirectBD_u32(u32 address, u32 value, u32 current_pc);
void _WriteBitsDirectBD_u16(u32 address, u32 value,
                            u32 current_pc); // Value in low 16 bits
void _WriteBitsDirectBD_u8(u32 address, u32 value,
                           u32 current_pc); // Value in low 8 bits
}
...
// Used to print value from ASM just add these two OPs (all caller saved regs
// will to be saved including RA and A0) //Corn 	jal		_printf_asm 	lw
//$a0, _Delay($fp)	#ex: prints Delay value
// OR ADD
//	JAL( CCodeLabel( (void*)_printf_asm ), false );
//	OR( PspReg_A0, dst_reg, PspReg_R0 );
extern "C" {
void _printf_asm(u32 val);
}
extern "C" {
void output_extern(u32 val) { printf("%d\n", val); }
}

extern "C" {
void _ReturnFromDynaRecIfStuffToDo(u32 register_mask);
}
extern "C" {
...
void Dynarec_ClearedCPUStuffToDo() {
  ...
  PspOpCode *p_function_address =
      reinterpret_cast<PspOpCode *>(MAKE_UNCACHED_PTR(p_void_function));
...
const EPspReg gRegistersToUseForCaching[] = {
    ...
    PspReg_AT, PspReg_T0, PspReg_T1, PspReg_T2, PspReg_T3, PspReg_T4,
    PspReg_T5, PspReg_T6, PspReg_T7, PspReg_T8, PspReg_T9, PspReg_S0,
    PspReg_S1, PspReg_S2, PspReg_S3, PspReg_S4, PspReg_S5,
    //	PspReg_S6,		// Memory upper bound
    //	PspReg_S7,		// Used for g_pu8RamBase - 0x80000000
    //	PspReg_S8,		// Used for base register (&gCPUState)
    //	PspReg_K0,		//Used as load base register. Normally it is reserved for
    //Kernel but seems to work if we borrow it...(could come back and bite us if
    //we use kernel stuff?) //Corn
    //	PspReg_K1,		//Used as store base register. Normally it is reserved for
    //Kernel but seems to work if we borrow it...(could come back and bite us if
    //we use kernel stuff?)
...
CCodeGeneratorPSP::CCodeGeneratorPSP(CAssemblyBuffer *p_buffer_a,
                                     CAssemblyBuffer *p_buffer_b)
    : CCodeGenerator(), CAssemblyWriterPSP(p_buffer_a, p_buffer_b),
      mpBasePointer(nullptr), mBaseRegister(PspReg_S8) // TODO
      ,
      mEntryAddress(0), mLoopTop(nullptr), mUseFixedRegisterAllocation(false) {}

//

void CCodeGeneratorPSP::Initialise(u32 entry_address, u32 exit_address,
                                   u32 *hit_counter, const void *p_base,
...
void CCodeGeneratorPSP::Finalise(
    ExceptionHandlerFn p_exception_handler_fn,
    const std::vector<CJumpLocation> &exception_handler_jumps) {
// We handle exceptions directly with _ReturnFromDynaRecIfStuffToDo - we should
// never get here on the psp
#ifdef DAEDALUS_ENABLE_ASSERTS
  DAEDALUS_ASSERT(
      exception_handler_jumps.empty(),
      "Not expecting to have any exception handler jumps to process");
#endif
...
void CCodeGeneratorPSP::SetRegisterSpanList(
    const SRegisterUsageInfo &register_usage, bool loops_to_self) {
  mRegisterSpanList = register_usage.SpanList;

  // Sort in order of increasing start point
  std::sort(mRegisterSpanList.begin(), mRegisterSpanList.end(),
            SAscendingSpanStartSort());

  const u32 NUM_CACHE_REGS(sizeof(gRegistersToUseForCaching) /
                           sizeof(gRegistersToUseForCaching[0]));

  // Push all the available registers in reverse order (i.e. use temporaries
  // later) Use temporaries first so we can avoid flushing them in case of a
  // funcion call //Corn
#ifdef DAEDALUS_ENABLE_ASSERTS
  DAEDALUS_ASSERT(mAvailableRegisters.empty(),
                  "Why isn't the available register list empty?");
#endif
  ...
  if (gDynarecLoopOptimisation & loops_to_self) {
    ...
    u32 HiLo{0};
    while (HiLo <
           2) // If there are still unused registers, assign to high part of reg
    {
...
void CCodeGeneratorPSP::ExpireOldIntervals(u32 instruction_idx) {
  ...
  for (auto span_it = mActiveIntervals.begin();
       span_it < mActiveIntervals.end(); ++span_it) {
    const SRegisterSpan &span(*span_it);
    ...
    // This interval is no longer active - flush the register and return it to
    // the list of available regs
...
void CCodeGeneratorPSP::SpillAtInterval(const SRegisterSpan &live_span) {
...
#endif
  const SRegisterSpan &last_span(
      mActiveIntervals.back()); // Spill the last active interval (it has the
                                // greatest end point)
  ...
  if (last_span.SpanEnd > live_span.SpanEnd) {
    ...
    mActiveIntervals.push_back(
        live_span); // Insert in order of increasing end point

    std::sort(
        mActiveIntervals.begin(), mActiveIntervals.end(),
        SAscendingSpanEndSort()); // XXXX - will be quicker to insert in the
                                  // correct place rather than sorting each time
  } else {
    // There is no space for this register - we just don't update the register
    // cache info, so we save/restore it from memory as needed
  }
}
...
void CCodeGeneratorPSP::UpdateRegisterCaching(u32 instruction_idx) {
  if (!mUseFixedRegisterAllocation) {
    ...
    for (RegisterSpanList::const_iterator span_it = mRegisterSpanList.begin();
         span_it < mRegisterSpanList.end(); ++span_it) {
      const SRegisterSpan &span(*span_it);

      // As we keep the intervals sorted in order of SpanStart, we can exit as
      // soon as we encounter a SpanStart in the future
      ...
      if ((instruction_idx >= span.SpanStart) &
          (instruction_idx <= span.SpanEnd)) {
        if (!mRegisterCache.IsCached(span.Register, 0)) {
          if (mAvailableRegisters.empty()) {
            SpillAtInterval(span);
          } else {
            // Use this register for caching
            mRegisterCache.SetCachedReg(span.Register, 0,
                                        mAvailableRegisters.top());

            // Pop this register from the available list
            mAvailableRegisters.pop();
            mActiveIntervals.push_back(
                span); // Insert in order of increasing end point

            std::sort(mActiveIntervals.begin(), mActiveIntervals.end(),
                      SAscendingSpanEndSort()); // XXXX - will be quicker to
                                                // insert in the correct place
                                                // rather than sorting each time
          }
        }
      }
    }
...
// Get a (cached) N64 register mapped to a PSP register(usefull for dst
// register)

EPspReg CCodeGeneratorPSP::GetRegisterNoLoad(EN64Reg n64_reg, u32 lo_hi_idx,
                                             EPspReg scratch_reg) {
  if (mRegisterCache.IsCached(n64_reg, lo_hi_idx)) {
...
void CCodeGeneratorPSP::GetRegisterValue(EPspReg psp_reg, EN64Reg n64_reg,
                                         u32 lo_hi_idx) {
  if (mRegisterCache.IsKnownValue(n64_reg, lo_hi_idx)) {
    // printf( "Loading %s[%d] <- %08x\n", RegNames[ n64_reg ], lo_hi_idx,
    // mRegisterCache.GetKnownValue( n64_reg, lo_hi_idx ) );
    LoadConstant(psp_reg,
                 mRegisterCache.GetKnownValue(n64_reg, lo_hi_idx)._s32);
    if (mRegisterCache.IsCached(n64_reg, lo_hi_idx)) {
...
EPspReg CCodeGeneratorPSP::GetRegisterAndLoad(EN64Reg n64_reg, u32 lo_hi_idx,
                                              EPspReg scratch_reg) {
  EPspReg reg;
...
//	Similar to GetRegisterAndLoad, but ALWAYS loads into the specified psp
//register

void CCodeGeneratorPSP::LoadRegister(EPspReg psp_reg, EN64Reg n64_reg,
                                     u32 lo_hi_idx) {
  if (mRegisterCache.IsCached(n64_reg, lo_hi_idx)) {
...
//	This function pulls in a cached register so that it can be used at a
//later point. 	This is usally done when we have a branching instruction - it
//guarantees that 	the register is valid regardless of whether or not the branch
//is taken.
...
void CCodeGeneratorPSP::StoreRegister(EN64Reg n64_reg, u32 lo_hi_idx,
                                      EPspReg psp_reg) {
  mRegisterCache.ClearKnownValue(n64_reg, lo_hi_idx);
...
void CCodeGeneratorPSP::SetRegister64(EN64Reg n64_reg, s32 lo_value,
                                      s32 hi_value) {
...
inline void CCodeGeneratorPSP::SetRegister(EN64Reg n64_reg, u32 lo_hi_idx,
                                           u32 value) {
  mRegisterCache.SetKnownValue(n64_reg, lo_hi_idx, value);
  mRegisterCache.MarkAsDirty(n64_reg, lo_hi_idx, true);
  if (mRegisterCache.IsCached(n64_reg, lo_hi_idx)) {
    mRegisterCache.MarkAsValid(n64_reg, lo_hi_idx,
                               false); // The actual cache is invalid though!
  }
}

//

void CCodeGeneratorPSP::UpdateRegister(EN64Reg n64_reg, EPspReg psp_reg,
                                       bool options) {
...
EPspFloatReg CCodeGeneratorPSP::GetFloatRegisterAndLoad(EN64FloatReg n64_reg) {
  auto psp_reg = EPspFloatReg(n64_reg); // 1:1 mapping
...
// Get Double or SimDouble and add code to convert to simulated double if needed
// //Corn

#define SIMULATESIG 0x1234 // Reduce signature to load value with one OP
EPspFloatReg
CCodeGeneratorPSP::GetSimFloatRegisterAndLoad(EN64FloatReg n64_reg) {
  auto psp_reg_sig = EPspFloatReg(n64_reg); // 1:1 mapping
  auto psp_reg = EPspFloatReg(n64_reg + 1);
  ...
  // If register is not SimDouble yet or unknown add check and conversion
  // routine
  if (!mRegisterCache.IsFPSim(n64_reg)) {
    ...
    CJumpLocation test_reg(BEQ(PspReg_A0, PspReg_A1, CCodeLabel(nullptr),
                               false)); // compare float to signature
    MTC1(psp_reg_sig, PspReg_A1); // Always write back signature to float reg

    JAL(CCodeLabel(reinterpret_cast<const void *>(_DoubleToFloat)),
...
const CN64RegisterCachePSP &CCodeGeneratorPSP::GetRegisterCacheFromHandle(
    RegisterSnapshotHandle snapshot) const {
#ifdef DAEDALUS_ENABLE_ASSERTS
  DAEDALUS_ASSERT(snapshot.Handle < mRegisterSnapshots.size(),
...
void CCodeGeneratorPSP::FlushRegister(CN64RegisterCachePSP &cache,
                                      EN64Reg n64_reg, u32 lo_hi_idx,
                                      bool invalidate) {
  if (cache.IsDirty(n64_reg, lo_hi_idx)) {
    if (cache.IsKnownValue(n64_reg, lo_hi_idx)) {
      ...
      SetVar(lo_hi_idx ? &gGPR[n64_reg]._u32_1 : &gGPR[n64_reg]._u32_0,
             known_value);
    } else if (cache.IsCached(n64_reg, lo_hi_idx)) {
#ifdef DAEDALUS_ENABLE_ASSERTS
      DAEDALUS_ASSERT(cache.IsValid(n64_reg, lo_hi_idx),
                      "Register is dirty but not valid?");
#endif
  ...
  // Invalidate the register, so we pick up any values the function might have
  // changed
  if (invalidate) {
...
//	If the invalidate flag is set this also invalidates the known
//value/cached 	register. This is primarily to ensure that we keep the register
//set 	in a consistent set across calls to generic functions. Ideally we need 	to
//reimplement generic functions with specialised code to avoid the flush.

void CCodeGeneratorPSP::FlushAllRegisters(CN64RegisterCachePSP &cache,
                                          bool invalidate) {
...
void CCodeGeneratorPSP::FlushAllFloatingPointRegisters(
    CN64RegisterCachePSP &cache, bool invalidate) {
  for (u32 i{0}; i < NUM_N64_FP_REGS; i++) {
    auto n64_reg = EN64FloatReg(i);
    if (cache.IsFPDirty(n64_reg)) {
#ifdef DAEDALUS_ENABLE_ASSERTS
      DAEDALUS_ASSERT(cache.IsFPValid(n64_reg),
                      "Register is dirty but not valid?");
#endif
      auto psp_reg = EPspFloatReg(n64_reg);
    ...
    if (invalidate) {
      // Invalidate the register, so we pick up any values the function might
      // have changed
...
//	This function flushes all dirty *temporary* registers back to memory,
//and 	marks them as invalid.

void CCodeGeneratorPSP::FlushAllTemporaryRegisters(CN64RegisterCachePSP &cache,
                                                   bool invalidate) {
  // Skip r0
  for (u32 i{1}; i < NUM_N64_REGS; i++) {
    auto n64_reg = EN64Reg(i);
...
void CCodeGeneratorPSP::RestoreAllRegisters(CN64RegisterCachePSP &current_cache,
                                            CN64RegisterCachePSP &new_cache) {
  // Skip r0
  for (u32 i{1}; i < NUM_N64_REGS; i++) {
    auto n64_reg = EN64Reg(i);
...
CJumpLocation CCodeGeneratorPSP::GenerateExitCode(u32 exit_address,
                                                  u32 jump_address,
                                                  u32 num_instructions,
                                                  CCodeLabel next_fragment) {
#ifdef DAEDALUS_ENABLE_ASSERTS
  // DAEDALUS_ASSERT( exit_address != u32( ~0 ), "Invalid exit address" );
  DAEDALUS_ASSERT(
      !next_fragment.IsSet() || jump_address == 0,
      "Shouldn't be specifying a jump address if we have a next fragment?");
#endif
  if ((exit_address == mEntryAddress) & mLoopTop.IsSet()) {
#ifdef DAEDALUS_ENABLE_ASSERTS
    DAEDALUS_ASSERT(mUseFixedRegisterAllocation,
                    "Have mLoopTop but unfixed register allocation?");
#endif
    ...
    for (u32 i{1}; i < NUM_N64_REGS; i++) {
      ...
      // if( mRegisterCache.IsDirty( n64_reg, 0 ) & mRegisterCache.IsKnownValue(
      // n64_reg, 0 ) )
      //{
      //	FlushRegister( mRegisterCache, n64_reg, 0, false );
      //}
      // if( mRegisterCache.IsDirty( n64_reg, 1 ) & mRegisterCache.IsKnownValue(
      // n64_reg, 1 ) )
      //{
    ...
    // Assuming we don't need to set CurrentPC/Delay flags before we branch to
    // the top..
    //
    ...
    //
    //	If the event counter is still positive, just jump directly to the top of
    //our loop
    //
    ADDIU(PspReg_A0, PspReg_A0, -s16(num_instructions));
    BGTZ(PspReg_A0, mLoopTop, false);
    SetVar((u32 *)&gCPUState.Events[0].mCount,
           PspReg_A0); // ASSUMES store is done in just a single op.

    FlushAllRegisters(mRegisterCache, true);

    SetVar(&gCPUState.CurrentPC, exit_address);
    JAL(CCodeLabel(reinterpret_cast<const void *>(CPU_HANDLE_COUNT_INTERRUPT)),
        false);
    SetVar(&gCPUState.Delay,
           NO_DELAY); // ASSUMES store is done in just a single op.

    J(CCodeLabel(reinterpret_cast<const void *>(_ReturnFromDynaRec)), true);

    //
    //	Return an invalid jump location to indicate we've handled our own
    //linking.
    //
  ...
  if (jump_address != 0) {
    ...
    exit_routine =
        CCodeLabel(reinterpret_cast<const void *>(_DirectExitCheckDelay));
  } else {
    LoadConstant(PspReg_A0, num_instructions);
    GetLoadConstantOps(PspReg_A1, exit_address, &op1, &op2);

    exit_routine =
        CCodeLabel(reinterpret_cast<const void *>(_DirectExitCheckNoDelay));
  }
  ...
  // This gets patched with a jump to the next fragment if the target is later
  // found
  ...
  if (!next_fragment.IsSet()) {
    PatchJumpLong(
        ...
        CCodeLabel(reinterpret_cast<const void *>(_ReturnFromDynaRec)));
  }
...
void CCodeGeneratorPSP::GenerateEretExitCode(u32 num_instructions,
                                             CIndirectExitMap *p_map) {
...
void CCodeGeneratorPSP::GenerateIndirectExitCode(u32 num_instructions,
                                                 CIndirectExitMap *p_map) {
  FlushAllRegisters(mRegisterCache, false);
...
void CCodeGeneratorPSP::GenerateBranchHandler(CJumpLocation branch_handler_jump,
                                              RegisterSnapshotHandle snapshot) {
#ifdef DAEDALUS_ENABLE_ASSERTS
  DAEDALUS_ASSERT(branch_handler_jump.IsSet(),
                  "Why is the branch handler jump not set?");
#endif
...
CJumpLocation CCodeGeneratorPSP::GenerateBranchIfSet(const u32 *p_var,
                                                     CCodeLabel target) {
...
CJumpLocation CCodeGeneratorPSP::GenerateBranchIfNotSet(const u32 *p_var,
                                                        CCodeLabel target) {
  GetVar(PspReg_V0, p_var);
...
CJumpLocation CCodeGeneratorPSP::GenerateBranchIfEqual(const u32 *p_var,
                                                       u32 value,
                                                       CCodeLabel target) {
  GetVar(PspReg_V0, p_var);
...
CJumpLocation CCodeGeneratorPSP::GenerateBranchIfNotEqual(const u32 *p_var,
                                                          u32 value,
                                                          CCodeLabel target) {
  GetVar(PspReg_V0, p_var);
  LoadConstant(PspReg_A0, value);
...
CJumpLocation CCodeGeneratorPSP::GenerateBranchIfNotEqual(EPspReg reg_a,
                                                          u32 value,
                                                          CCodeLabel target) {
  EPspReg reg_b(reg_a == PspReg_V0
                    ? PspReg_A0
                    : PspReg_V0); // Make sure we don't use the same reg
...
void CCodeGeneratorPSP::SetVar(u32 *p_var, EPspReg reg_src) {
  // DAEDALUS_ASSERT( reg_src != PspReg_AT, "Whoops, splattering source
  // register" );
...
void CCodeGeneratorPSP::GetBaseRegisterAndOffset(const void *p_address,
                                                 EPspReg *p_reg,
                                                 s16 *p_offset) {
  s32 base_pointer_offset(reinterpret_cast<const u8 *>(p_address) -
                          mpBasePointer);
...
#ifdef DAEDALUS_ENABLE_ASSERTS
    DAEDALUS_ASSERT(long_offset >= SHRT_MIN && long_offset <= SHRT_MAX,
                    "Offset is out of range!");
#endif
    s16 offset((s16)long_offset);
#ifdef DAEDALUS_ENABLE_ASSERTS
    DAEDALUS_ASSERT(((s32)hi_bits << 16) + offset == (s32)address,
...
/*void	CCodeGeneratorPSP::UpdateAddressAndDelay( u32 address, bool
set_branch_delay )
{
        DAEDALUS_ASSERT( !set_branch_delay || (address != 0), "Have branch delay
and no address?" );
...
CJumpLocation CCodeGeneratorPSP::GenerateOpCode(const STraceEntry &ti,
                                                bool branch_delay_slot,
                                                const SBranchDetails *p_branch,
                                                CJumpLocation *p_branch_jump) {
  ...
  if (branch_delay_slot)
    mPreviousStoreBase = mPreviousLoadBase = N64Reg_R0; // Invalidate

  mQuickLoad = ti.Usage.Access8000;

  const auto rs = EN64Reg(op_code.rs);
  const auto rt = EN64Reg(op_code.rt);
  const auto rd = EN64Reg(op_code.rd);
  const auto base = EN64Reg(op_code.base);
  const u32 ft = op_code.ft;
  const u32 sa = op_code.sa;
  // const u32		jump_target( (address&0xF0000000) | (op_code.target<<2)
  // );
  // const u32		branch_target( address + ( ((s32)(s16)op_code.immediate)<<2
  // ) + 4);
  ...
  case OP_SPECOP:
    ...
    case SpecOp_SLL:
      GenerateSLL(rd, rt, sa);
      handled = true;
      break;
    case SpecOp_SRA:
      GenerateSRA(rd, rt, sa);
      handled = true;
      break;
    case SpecOp_SRL:
      GenerateSRL(rd, rt, sa);
      handled = true;
      break;
    case SpecOp_SLLV:
      GenerateSLLV(rd, rs, rt);
      handled = true;
      break;
    case SpecOp_SRAV:
      GenerateSRAV(rd, rs, rt);
      handled = true;
      break;
    case SpecOp_SRLV:
      GenerateSRLV(rd, rs, rt);
      handled = true;
      break;

    case SpecOp_JR:
      GenerateJR(rs, p_branch, p_branch_jump);
      handled = true;
      break;
    case SpecOp_JALR:
      GenerateJALR(rs, rd, address, p_branch, p_branch_jump);
      handled = true;
      break;
    ...
    case SpecOp_OR:
      ...
      break;
    case SpecOp_XOR:
      GenerateXOR(rd, rs, rt);
      handled = true;
      break;
    case SpecOp_NOR:
      GenerateNOR(rd, rs, rt);
      handled = true;
      break;

    case SpecOp_SLT:
      GenerateSLT(rd, rs, rt);
      handled = true;
      break;
    case SpecOp_SLTU:
      GenerateSLTU(rd, rs, rt);
      handled = true;
      break;

    case SpecOp_DADD:
      GenerateDADDU(rd, rs, rt);
      handled = true;
      break;
    case SpecOp_DADDU:
      GenerateDADDU(rd, rs, rt);
    ...
    case SpecOp_DSRA32:
      GenerateDSRA32(rd, rt, sa);
      handled = true;
      break;
    case SpecOp_DSRA:
      GenerateDSRA(rd, rt, sa);
      handled = true;
      break;
    case SpecOp_DSLL32:
      GenerateDSLL32(rd, rt, sa);
      handled = true;
      break;
    case SpecOp_DSLL:
      GenerateDSLL(rd, rt, sa);
      handled = true;
      break;
    default:
      break;
  ...
  case OP_REGIMM:
    switch (op_code.regimm_op) {
      // These can be handled by the same Generate function, as the 'likely' bit
      // is handled elsewhere
    case RegImmOp_BLTZ:
    case RegImmOp_BLTZL:
      GenerateBLTZ(rs, p_branch, p_branch_jump);
      handled = true;
      break;

    case RegImmOp_BGEZ:
    case RegImmOp_BGEZL:
      GenerateBGEZ(rs, p_branch, p_branch_jump);
      handled = true;
      break;
    }
    break;

  case OP_J: /* nothing to do */
    handled = true;
    break;
  case OP_JAL:
    GenerateJAL(address);
  ...
  case OP_BEQ:
    GenerateBEQ(rs, rt, p_branch, p_branch_jump);
    handled = true;
    break;
  case OP_BEQL:
    GenerateBEQ(rs, rt, p_branch, p_branch_jump);
    ...
    break;
  case OP_BNE:
    GenerateBNE(rs, rt, p_branch, p_branch_jump);
    handled = true;
    break;
  case OP_BNEL:
    GenerateBNE(rs, rt, p_branch, p_branch_jump);
    ...
    break;
  case OP_BLEZ:
    GenerateBLEZ(rs, p_branch, p_branch_jump);
    handled = true;
    break;
  case OP_BLEZL:
    GenerateBLEZ(rs, p_branch, p_branch_jump);
    ...
    break;
  case OP_BGTZ:
    GenerateBGTZ(rs, p_branch, p_branch_jump);
    handled = true;
    break;
  case OP_BGTZL:
    GenerateBGTZ(rs, p_branch, p_branch_jump);
  ...
  case OP_ADDI:
    GenerateADDIU(rt, rs, s16(op_code.immediate));
    handled = true;
    break;
  case OP_ADDIU:
    GenerateADDIU(rt, rs, s16(op_code.immediate));
    ...
    break;
  case OP_SLTI:
    GenerateSLTI(rt, rs, s16(op_code.immediate));
    handled = true;
    break;
  case OP_SLTIU:
    GenerateSLTIU(rt, rs, s16(op_code.immediate));
    handled = true;
    break;

  case OP_DADDI:
    GenerateDADDIU(rt, rs, s16(op_code.immediate));
    handled = true;
    break;
  case OP_DADDIU:
    GenerateDADDIU(rt, rs, s16(op_code.immediate));
  ...
  case OP_LB:
    GenerateLB(address, branch_delay_slot, rt, base, s16(op_code.immediate));
    handled = true;
    break;
  case OP_LBU:
    GenerateLBU(address, branch_delay_slot, rt, base, s16(op_code.immediate));
    handled = true;
    break;
  case OP_LH:
    GenerateLH(address, branch_delay_slot, rt, base, s16(op_code.immediate));
    handled = true;
    break;
  case OP_LHU:
    GenerateLHU(address, branch_delay_slot, rt, base, s16(op_code.immediate));
    handled = true;
    break;
  case OP_LW:
    GenerateLW(address, branch_delay_slot, rt, base, s16(op_code.immediate));
    handled = true;
    break;
  case OP_LD:
    GenerateLD(address, branch_delay_slot, rt, base, s16(op_code.immediate));
    handled = true;
    break;
  case OP_LWC1:
    GenerateLWC1(address, branch_delay_slot, ft, base, s16(op_code.immediate));
    handled = true;
    break;
#ifdef ENABLE_LDC1
  case OP_LDC1:
    if (!branch_delay_slot & gMemoryAccessOptimisation & mQuickLoad) {
      GenerateLDC1(address, branch_delay_slot, ft, base,
                   s16(op_code.immediate));
      handled = true;
    }
    break;
#endif
  ...
  case OP_LWL:
    GenerateLWL(address, branch_delay_slot, rt, base, s16(op_code.immediate));
    handled = true;
    break;
  case OP_LWR:
    GenerateLWR(address, branch_delay_slot, rt, base, s16(op_code.immediate));
    handled = true;
    break;
#endif

  case OP_SB:
    GenerateSB(address, branch_delay_slot, rt, base, s16(op_code.immediate));
    handled = true;
    break;
  case OP_SH:
    GenerateSH(address, branch_delay_slot, rt, base, s16(op_code.immediate));
    handled = true;
    break;
  case OP_SW:
    GenerateSW(address, branch_delay_slot, rt, base, s16(op_code.immediate));
    handled = true;
    break;
  case OP_SD:
    GenerateSD(address, branch_delay_slot, rt, base, s16(op_code.immediate));
    handled = true;
    break;
  case OP_SWC1:
    GenerateSWC1(address, branch_delay_slot, ft, base, s16(op_code.immediate));
    handled = true;
    break;
#ifdef ENABLE_SDC1
  case OP_SDC1:
    if (!branch_delay_slot & gMemoryAccessOptimisation & mQuickLoad) {
      GenerateSDC1(address, branch_delay_slot, ft, base,
                   s16(op_code.immediate));
      handled = true;
    }
    break;
  ...
  case OP_SWL:
    GenerateSWL(address, branch_delay_slot, rt, base, s16(op_code.immediate));
    handled = true;
    break;
  case OP_SWR:
    GenerateSWR(address, branch_delay_slot, rt, base, s16(op_code.immediate));
    handled = true;
    break;
#endif

  case OP_CACHE:
    GenerateCACHE(base, op_code.immediate, rt);
    handled = true;
    break;
  ...
  case OP_COPRO0:
    ...
    case Cop0Op_MFC0:
      GenerateMFC0(rt, op_code.fs);
      handled = true;
      break;
    // case Cop0Op_MTC0:	GenerateMTC0( rt, op_code.fs ); handled = true; break;
    // //1080 deg has issues with this
    default:
      break;
    }
    break;
  ...
  case OP_COPRO1:
    ...
    case Cop1Op_MFC1:
      GenerateMFC1(rt, op_code.fs);
      handled = true;
      break;
    case Cop1Op_MTC1:
      GenerateMTC1(op_code.fs, rt);
      handled = true;
      break;

    case Cop1Op_CFC1:
      GenerateCFC1(rt, op_code.fs);
      handled = true;
      break;
    case Cop1Op_CTC1:
      GenerateCTC1(op_code.fs, rt);
      handled = true;
      break;
    ...
    case Cop1Op_DInstr:
      if (gDynarecDoublesOptimisation) {
        ...
        case Cop1OpFunc_ADD:
          GenerateADD_D_Sim(op_code.fd, op_code.fs, op_code.ft);
          handled = true;
          break;
        case Cop1OpFunc_SUB:
          GenerateSUB_D_Sim(op_code.fd, op_code.fs, op_code.ft);
          handled = true;
          break;
        case Cop1OpFunc_MUL:
          GenerateMUL_D_Sim(op_code.fd, op_code.fs, op_code.ft);
          handled = true;
          break;
        case Cop1OpFunc_DIV:
          GenerateDIV_D_Sim(op_code.fd, op_code.fs, op_code.ft);
          handled = true;
          break;
        case Cop1OpFunc_SQRT:
          GenerateSQRT_D_Sim(op_code.fd, op_code.fs);
          handled = true;
          break;
        case Cop1OpFunc_ABS:
          GenerateABS_D_Sim(op_code.fd, op_code.fs);
          handled = true;
          break;
        case Cop1OpFunc_MOV:
          GenerateMOV_D_Sim(op_code.fd, op_code.fs);
          handled = true;
          break;
        case Cop1OpFunc_NEG:
          GenerateNEG_D_Sim(op_code.fd, op_code.fs);
          handled = true;
          break;

        case Cop1OpFunc_TRUNC_W:
          GenerateTRUNC_W_D_Sim(op_code.fd, op_code.fs);
          handled = true;
          break;

        case Cop1OpFunc_CVT_W:
          GenerateCVT_W_D_Sim(op_code.fd, op_code.fs);
          handled = true;
          break;
        case Cop1OpFunc_CVT_S:
          GenerateCVT_S_D_Sim(op_code.fd, op_code.fs);
          handled = true;
          break;

        case Cop1OpFunc_CMP_F:
          GenerateCMP_D_Sim(op_code.fs, Cop1OpFunc_CMP_F, op_code.ft);
          handled = true;
          break;
        case Cop1OpFunc_CMP_UN:
          GenerateCMP_D_Sim(op_code.fs, Cop1OpFunc_CMP_UN, op_code.ft);
          handled = true;
          break;
        case Cop1OpFunc_CMP_EQ:
          GenerateCMP_D_Sim(op_code.fs, Cop1OpFunc_CMP_EQ, op_code.ft);
          handled = true;
          break; // Conker has issues with this
        case Cop1OpFunc_CMP_UEQ:
          GenerateCMP_D_Sim(op_code.fs, Cop1OpFunc_CMP_UEQ, op_code.ft);
          handled = true;
          break;
        case Cop1OpFunc_CMP_OLT:
          GenerateCMP_D_Sim(op_code.fs, Cop1OpFunc_CMP_OLT, op_code.ft);
          handled = true;
          break;
        case Cop1OpFunc_CMP_ULT:
          GenerateCMP_D_Sim(op_code.fs, Cop1OpFunc_CMP_ULT, op_code.ft);
          handled = true;
          break;
        case Cop1OpFunc_CMP_OLE:
          GenerateCMP_D_Sim(op_code.fs, Cop1OpFunc_CMP_OLE, op_code.ft);
          handled = true;
          break;
        case Cop1OpFunc_CMP_ULE:
          GenerateCMP_D_Sim(op_code.fs, Cop1OpFunc_CMP_ULE, op_code.ft);
          handled = true;
          break;

        case Cop1OpFunc_CMP_SF:
          GenerateCMP_D_Sim(op_code.fs, Cop1OpFunc_CMP_SF, op_code.ft);
          handled = true;
          break;
        case Cop1OpFunc_CMP_NGLE:
          GenerateCMP_D_Sim(op_code.fs, Cop1OpFunc_CMP_NGLE, op_code.ft);
          handled = true;
          break;
        case Cop1OpFunc_CMP_SEQ:
          GenerateCMP_D_Sim(op_code.fs, Cop1OpFunc_CMP_SEQ, op_code.ft);
          handled = true;
          break;
        case Cop1OpFunc_CMP_NGL:
          GenerateCMP_D_Sim(op_code.fs, Cop1OpFunc_CMP_NGL, op_code.ft);
          handled = true;
          break;
        case Cop1OpFunc_CMP_LT:
          GenerateCMP_D_Sim(op_code.fs, Cop1OpFunc_CMP_LT, op_code.ft);
          handled = true;
          break;
        case Cop1OpFunc_CMP_NGE:
          GenerateCMP_D_Sim(op_code.fs, Cop1OpFunc_CMP_NGE, op_code.ft);
          handled = true;
          break;
        case Cop1OpFunc_CMP_LE:
          GenerateCMP_D_Sim(op_code.fs, Cop1OpFunc_CMP_LE, op_code.ft);
          handled = true;
          break;
        case Cop1OpFunc_CMP_NGT:
          GenerateCMP_D_Sim(op_code.fs, Cop1OpFunc_CMP_NGT, op_code.ft);
          handled = true;
          break;
        default:
    ...
    case Cop1Op_SInstr:
      switch (op_code.cop1_funct) {
      case Cop1OpFunc_ADD:
        GenerateADD_S(op_code.fd, op_code.fs, op_code.ft);
        handled = true;
        break;
      case Cop1OpFunc_SUB:
        GenerateSUB_S(op_code.fd, op_code.fs, op_code.ft);
        handled = true;
        break;
      case Cop1OpFunc_MUL:
        GenerateMUL_S(op_code.fd, op_code.fs, op_code.ft);
        handled = true;
        break;
      case Cop1OpFunc_DIV:
        GenerateDIV_S(op_code.fd, op_code.fs, op_code.ft);
        handled = true;
        break;
      case Cop1OpFunc_SQRT:
        GenerateSQRT_S(op_code.fd, op_code.fs);
        handled = true;
        break;
      case Cop1OpFunc_ABS:
        GenerateABS_S(op_code.fd, op_code.fs);
        handled = true;
        break;
      case Cop1OpFunc_MOV:
        GenerateMOV_S(op_code.fd, op_code.fs);
        handled = true;
        break;
      case Cop1OpFunc_NEG:
        GenerateNEG_S(op_code.fd, op_code.fs);
        handled = true;
        break;

      case Cop1OpFunc_TRUNC_W:
        GenerateTRUNC_W_S(op_code.fd, op_code.fs);
        handled = true;
        break;
      case Cop1OpFunc_FLOOR_W:
        GenerateFLOOR_W_S(op_code.fd, op_code.fs);
        handled = true;
        break;

      case Cop1OpFunc_CVT_W:
        GenerateCVT_W_S(op_code.fd, op_code.fs);
        handled = true;
        break;
      case Cop1OpFunc_CVT_D:
        if (gDynarecDoublesOptimisation & !g_ROM.DISABLE_SIM_CVT_D_S)
          GenerateCVT_D_S_Sim(
              op_code.fd,
              op_code.fs); // Sim has issues with EWJ/Tom&Jerry/PowerPuffGirls
        else
          GenerateCVT_D_S(op_code.fd, op_code.fs);
        handled = true;
        break;

      case Cop1OpFunc_CMP_F:
        GenerateCMP_S(op_code.fs, Cop1OpFunc_CMP_F, op_code.ft);
        handled = true;
        break;
      case Cop1OpFunc_CMP_UN:
        GenerateCMP_S(op_code.fs, Cop1OpFunc_CMP_UN, op_code.ft);
        handled = true;
        break;
      case Cop1OpFunc_CMP_EQ:
        GenerateCMP_S(op_code.fs, Cop1OpFunc_CMP_EQ, op_code.ft);
        handled = true;
        break;
      case Cop1OpFunc_CMP_UEQ:
        GenerateCMP_S(op_code.fs, Cop1OpFunc_CMP_UEQ, op_code.ft);
        handled = true;
        break;
      case Cop1OpFunc_CMP_OLT:
        GenerateCMP_S(op_code.fs, Cop1OpFunc_CMP_OLT, op_code.ft);
        handled = true;
        break;
      case Cop1OpFunc_CMP_ULT:
        GenerateCMP_S(op_code.fs, Cop1OpFunc_CMP_ULT, op_code.ft);
        handled = true;
        break;
      case Cop1OpFunc_CMP_OLE:
        GenerateCMP_S(op_code.fs, Cop1OpFunc_CMP_OLE, op_code.ft);
        handled = true;
        break;
      case Cop1OpFunc_CMP_ULE:
        GenerateCMP_S(op_code.fs, Cop1OpFunc_CMP_ULE, op_code.ft);
        handled = true;
        break;

      case Cop1OpFunc_CMP_SF:
        GenerateCMP_S(op_code.fs, Cop1OpFunc_CMP_SF, op_code.ft);
        handled = true;
        break;
      case Cop1OpFunc_CMP_NGLE:
        GenerateCMP_S(op_code.fs, Cop1OpFunc_CMP_NGLE, op_code.ft);
        handled = true;
        break;
      case Cop1OpFunc_CMP_SEQ:
        GenerateCMP_S(op_code.fs, Cop1OpFunc_CMP_SEQ, op_code.ft);
        handled = true;
        break;
      case Cop1OpFunc_CMP_NGL:
        GenerateCMP_S(op_code.fs, Cop1OpFunc_CMP_NGL, op_code.ft);
        handled = true;
        break;
      case Cop1OpFunc_CMP_LT:
        GenerateCMP_S(op_code.fs, Cop1OpFunc_CMP_LT, op_code.ft);
        handled = true;
        break;
      case Cop1OpFunc_CMP_NGE:
        GenerateCMP_S(op_code.fs, Cop1OpFunc_CMP_NGE, op_code.ft);
        handled = true;
        break;
      case Cop1OpFunc_CMP_LE:
        GenerateCMP_S(op_code.fs, Cop1OpFunc_CMP_LE, op_code.ft);
        handled = true;
        break;
      // This breaks D64, but I think is a bug somewhere else since interpreter
      // trows fp nan exception in Cop1_S_NGT
      case Cop1OpFunc_CMP_NGT:
        if (g_ROM.GameHacks != DK64) {
          GenerateCMP_S(op_code.fs, Cop1OpFunc_CMP_NGT, op_code.ft);
          handled = true;
        }
        break;
      }
    ...
    case Cop1Op_WInstr:
      switch (op_code.cop1_funct) {
      case Cop1OpFunc_CVT_S:
        GenerateCVT_S_W(op_code.fd, op_code.fs);
        handled = true;
        break;
      case Cop1OpFunc_CVT_D:
        if (gDynarecDoublesOptimisation) {
    ...
    case Cop1Op_BCInstr:
      switch (op_code.cop1_bc) {
        // These can be handled by the same Generate function, as the 'likely'
        // bit is handled elsewhere
      case Cop1BCOp_BC1F:
      case Cop1BCOp_BC1FL:
        GenerateBC1F(p_branch, p_branch_jump);
        handled = true;
        break;
      case Cop1BCOp_BC1T:
      case Cop1BCOp_BC1TL:
        GenerateBC1T(p_branch, p_branch_jump);
        handled = true;
        break;
      }
      break;
  ...
  // Invalidate load/store base regs if they has been modified with current
  // OPcode //Corn
  if ((ti.Usage.RegWrites >> mPreviousLoadBase) & 1)
    mPreviousLoadBase = N64Reg_R0; // Invalidate
  if ((ti.Usage.RegWrites >> mPreviousStoreBase) & 1)
    mPreviousStoreBase = N64Reg_R0; // Invalidate
...
//1->Show not handled OP codes (Require that DAEDALUS_SILENT flag is undefined)
    ...
    if (R4300_InstructionHandlerNeedsPC(op_code)) {
      ...
      // Make sure all dirty registers are flushed. NB - we don't invalidate
      // them to avoid reloading the contents if no exception was thrown.
      FlushAllRegisters(mRegisterCache, false);

      JAL(CCodeLabel(
              reinterpret_cast<const void *>(_ReturnFromDynaRecIfStuffToDo)),
          false);
    ...
    if (p_branch != nullptr) {
      ...
      if (p_branch->Direct) {
        if (p_branch->ConditionalBranchTaken) {
          *p_branch_jump =
              GenerateBranchIfNotEqual(&gCPUState.Delay, DO_DELAY, no_target);
        } else {
          *p_branch_jump =
              GenerateBranchIfEqual(&gCPUState.Delay, DO_DELAY, no_target);
        }
      } else {
        ...
        if (p_branch->Eret) {
          ...
          *p_branch_jump = GenerateBranchIfNotEqual(
              &gCPUState.TargetPC, p_branch->TargetAddress, no_target);
        }
      }
    }
...
//
//	NB - This assumes that the address and branch delay have been set up
//before 	calling this function.
//

void CCodeGeneratorPSP::GenerateGenericR4300(OpCode op_code,
                                             CPU_Instruction p_instruction) {
  mPreviousLoadBase = N64Reg_R0;  // Invalidate
  mPreviousStoreBase = N64Reg_R0; // Invalidate
  ...
  if (op2._u32 == 0) {
    JAL(CCodeLabel(reinterpret_cast<const void *>(p_instruction)),
        false); // No branch delay
    AppendOp(op1);
  } else {
    AppendOp(op1);
    JAL(CCodeLabel(reinterpret_cast<const void *>(p_instruction)),
...
CJumpLocation CCodeGeneratorPSP::ExecuteNativeFunction(CCodeLabel speed_hack,
                                                       bool check_return) {
  JAL(speed_hack, true);
...
inline bool CCodeGeneratorPSP::GenerateDirectLoad(EPspReg psp_dst, EN64Reg base,
                                                  s16 offset,
                                                  OpCodeValue load_op,
                                                  u32 swizzle) {
  if (mRegisterCache.IsKnownValue(base, 0)) {
    ...
    if (m.pRead) {
      ...
      // base_address, u16(offset), address, load_op );
      ...
      // base ], base_address, u16(offset), address );
    }
  }
...
void CCodeGeneratorPSP::GenerateSlowLoad(u32 current_pc, EPspReg psp_dst,
                                         EPspReg reg_address,
                                         ReadMemoryFunction p_read_memory) {
  // We need to flush all the regular registers AND invalidate all the temp
  // regs. NB: When we can flush registers through some kind of handle (i.e.
  // when the exception handling branch is taken), this won't be necessary
...
void CCodeGeneratorPSP::GenerateLoad(u32 current_pc, EPspReg psp_dst,
                                     EN64Reg n64_base, s16 offset,
                                     OpCodeValue load_op, u32 swizzle,
                                     ReadMemoryFunction p_read_memory) {
  //
  //	Check if the base pointer is a known value, in which case we can load
  //directly.
  //
  ...
  if (load_op == OP_LWL) // We handle both LWL & LWR here
  {
    ...
    ANDI(PspReg_A3, reg_address,
         3); // copy two LSB bits to A3, used later for mask
    XOR(PspReg_A0, reg_address, PspReg_A3); // zero two LSB bits in address

    // Dont cache the current pointer to K0 reg because the address is mangled
    // by the XOR
    ...
    if ((n64_base == N64Reg_SP) | (gMemoryAccessOptimisation & mQuickLoad)) {
      ...
      // Re use old base register if consegutive accesses from same base
      // register //Corn
  ...
  //
  //	If the fast load failed, we need to perform a 'slow' load going through
  //a handler routine 	We may already be performing this instruction in the
  //second buffer, in which 	case we don't bother with the transitions
  //
  if (CAssemblyWriterPSP::IsBufferA()) {
    ...
    CJumpLocation branch(BEQ(PspReg_V1, PspReg_R0, CCodeLabel(nullptr),
                             false)); // branch to jump to handler
    ...
    //
    //	Keep track of the details we need to generate the jump to the distant
    //handler function
    //
    ...
    CJumpLocation branch(
        BNEL(PspReg_V1, PspReg_R0, CCodeLabel(nullptr), false));
    CAssemblyWriterPSP::LoadRegister(psp_dst, load_op, PspReg_A1, offset);
...
inline void
CCodeGeneratorPSP::GenerateAddressCheckFixup(const SAddressCheckFixup &fixup) {
  CAssemblyWriterPSP::SetBufferA();

  //
  //	Fixup the branch we emitted so that it jumps to the handler routine.
  //	If possible, we try to branch directly.
  //	If that fails, we try to emit a stub function which jumps to the
  //handler. Hopefully that's a bit closer 	If *that* fails, we have to revert to
  //an unconditional jump to the handler routine. Expensive!!
  //
  /*
          ...
          {
                  if( PatchJumpLong( fixup.BranchToJump,
     GetAssemblyBuffer()->GetLabel() ) )
                  {
                          //
                  ...
                  {
                          printf( "Warning, jump is too far for address handler
     function\n" ); ReplaceBranchWithJump( fixup.BranchToJump,
     fixup.HandlerAddress );
                  }
          }
  */
  // Optimized version of above code - Salvy
  // First condition which is the fastest, never happens
  // Second condition which still fast, always happens (we have an assert if
  // otherwise) Third condition (slowest) never happens and thus is ignored
...
inline bool CCodeGeneratorPSP::GenerateDirectStore(EPspReg psp_src,
                                                   EN64Reg base, s16 offset,
  ...
  if (mRegisterCache.IsKnownValue(base, 0)) {
    ...
    if (m.pWrite) {
      ...
      // base_address, u16(offset), address, store_op );

      EPspReg reg_base;
      s16 base_offset;
      GetBaseRegisterAndOffset(p_memory, &reg_base, &base_offset);

      CAssemblyWriterPSP::StoreRegister(psp_src, store_op, reg_base,
                                        base_offset);
...
inline void
CCodeGeneratorPSP::GenerateSlowStore(u32 current_pc, EPspReg psp_src,
                                     EPspReg reg_address,
  ...
  if (op2._u32 == 0) {
    JAL(CCodeLabel(reinterpret_cast<const void *>(p_write_memory)),
        false); // Don't set branch delay slot
    AppendOp(op1);
  } else {
    AppendOp(op1);
    JAL(CCodeLabel(reinterpret_cast<const void *>(p_write_memory)),
...
void CCodeGeneratorPSP::GenerateStore(u32 current_pc, EPspReg psp_src,
                                      EN64Reg n64_base, s32 offset,
                                      OpCodeValue store_op, u32 swizzle,
                                      WriteMemoryFunction p_write_memory) {
  //
  //	Check if the base pointer is a known value, in which case we can store
  //directly.
  //
  ...
  if ((n64_base == N64Reg_SP) | (gMemoryAccessOptimisation & mQuickLoad)) {
    ...
    // Re use old base register if consegutive accesses from same base register
    // //Corn
...
#ifdef ENABLE_SWC1
    if (store_op == OP_SWC1) { // Move value from FPU to CPU
...
inline void CCodeGeneratorPSP::GenerateCACHE(EN64Reg base, s16 offset,
                                             u32 cache_op) {
  // u32 cache_op  = op_code.rt;
  // u32 address = (u32)( gGPR[op_code.base]._s32_0 +
  // (s32)(s16)op_code.immediate );

  u32 cache = cache_op & 0x3;
  u32 action = (cache_op >> 2) & 0x7;

  // For instruction cache invalidation, make sure we let the CPU know so the
  // whole dynarec system can be invalidated
  // if(!(cache_op & 0x1F))
  if (cache == 0 && (action == 0 || action == 4)) {
    FlushAllRegisters(mRegisterCache, true);
    LoadRegister(PspReg_A0, base, 0);
    ADDI(PspReg_A0, PspReg_A0, offset);
    JAL(CCodeLabel(reinterpret_cast<const void *>(CPU_InvalidateICacheRange)),
        false);
...
inline void CCodeGeneratorPSP::GenerateJAL(u32 address) {
  // gGPR[REG_ra]._s64 = (s64)(s32)(gCPUState.CurrentPC + 8);		// Store
  // return address u32	new_pc( (gCPUState.CurrentPC & 0xF0000000) |
  // (op_code.target<<2) ); CPU_TakeBranch( new_pc, CPU_BRANCH_DIRECT );
...
inline void CCodeGeneratorPSP::GenerateJR(EN64Reg rs,
                                          const SBranchDetails *p_branch,
                                          CJumpLocation *p_branch_jump) {
  ...
  // Necessary? Could just directly compare reg_lo and constant
  // p_branch->TargetAddress??
  // SetVar( &gCPUState.TargetPC, reg_lo );
  // SetVar( &gCPUState.Delay, DO_DELAY );
  //*p_branch_jump = GenerateBranchIfNotEqual( &gCPUState.TargetPC,
  //p_branch->TargetAddress, CCodeLabel() );

  SetVar(&gCPUState.TargetPC, reg_lo);
  // SetVar( &gCPUState.Delay, DO_DELAY );
  *p_branch_jump =
      GenerateBranchIfNotEqual(reg_lo, p_branch->TargetAddress, CCodeLabel());
}

//

inline void CCodeGeneratorPSP::GenerateJALR(EN64Reg rs, EN64Reg rd, u32 address,
                                            const SBranchDetails *p_branch,
                                            CJumpLocation *p_branch_jump) {
  // Jump And Link
  // u32	new_pc( gGPR[ op_code.rs ]._u32_0 );
  // gGPR[ op_code.rd ]._s64 = (s64)(s32)(gCPUState.CurrentPC + 8);		//
  // Store return address EPspReg	savereg_lo( GetRegisterNoLoadLo( rd,
  // PspReg_V0 ) );
  ...
  // SetVar( &gCPUState.Delay, DO_DELAY );
  //*p_branch_jump = GenerateBranchIfNotEqual( &gCPUState.TargetPC,
  //p_branch->TargetAddress, CCodeLabel() ); LoadConstant( savereg_lo, address +
  // 8 ); UpdateRegister( rd, savereg_lo, URO_HI_SIGN_EXTEND, PspReg_V0 );
...
inline void CCodeGeneratorPSP::GenerateMULT(EN64Reg rs, EN64Reg rt) {
  ...
  // s64 dwResult = (s64)gGPR[ op_code.rs ]._s32_0 * (s64)gGPR[ op_code.rt
  // ]._s32_0; gCPUState.MultLo = (s64)(s32)(dwResult); gCPUState.MultHi =
  // (s64)(s32)(dwResult >> 32);
...
inline void CCodeGeneratorPSP::GenerateMULTU(EN64Reg rs, EN64Reg rt) {
  ...
  // u64 dwResult = (u64)gGPR[ op_code.rs ]._u32_0 * (u64)gGPR[ op_code.rt
  // ]._u32_0; gCPUState.MultLo = (s64)(s32)(dwResult); gCPUState.MultHi =
  // (s64)(s32)(dwResult >> 32);
...
inline void CCodeGeneratorPSP::GenerateDIV(EN64Reg rs, EN64Reg rt) {
  ...
  CJumpLocation branch(BEQ(reg_lo_rt, PspReg_R0, CCodeLabel(nullptr),
                           true)); // Can use branch delay for something?
...
inline void CCodeGeneratorPSP::GenerateADDU(EN64Reg rd, EN64Reg rs,
                                            EN64Reg rt) {
  ...
  if (rs == N64Reg_R0) {
    ...
    // gGPR[ op_code.rd ]._s64 = (s64)(s32)( gGPR[ op_code.rs ]._s32_0 + gGPR[
    // op_code.rt ]._s32_0 );
    EPspReg reg_lo_d(GetRegisterNoLoadLo(rd, PspReg_V0));
    EPspReg reg_lo_a(GetRegisterAndLoadLo(rs, PspReg_V0));
    EPspReg reg_lo_b(GetRegisterAndLoadLo(rt, PspReg_A0));
...
inline void CCodeGeneratorPSP::GenerateSUBU(EN64Reg rd, EN64Reg rs,
                                            EN64Reg rt) {
  if (mRegisterCache.IsKnownValue(rs, 0) & mRegisterCache.IsKnownValue(rt, 0)) {
    SetRegister32s(rd, mRegisterCache.GetKnownValue(rs, 0)._s32 -
                           mRegisterCache.GetKnownValue(rt, 0)._s32);
...
inline void CCodeGeneratorPSP::GenerateDDIVU(EN64Reg rs, EN64Reg rt) {
  ...
  if (reg_lo_a != PspReg_A0)
    OR(PspReg_A0, PspReg_R0, reg_lo_a);
  if (reg_hi_a != PspReg_A1)
    OR(PspReg_A1, PspReg_R0, reg_hi_a);
  if (reg_lo_b != PspReg_A2)
    OR(PspReg_A2, PspReg_R0, reg_lo_b);
...
inline void CCodeGeneratorPSP::GenerateDMULTU(EN64Reg rs, EN64Reg rt) {
  ...
  if (reg_lo_b != PspReg_A2)
    OR(PspReg_A2, PspReg_R0, reg_lo_b);
  if (reg_hi_b != PspReg_A3)
    OR(PspReg_A3, PspReg_R0, reg_hi_b);
...
inline void CCodeGeneratorPSP::GenerateDADDIU(EN64Reg rt, EN64Reg rs,
                                              s16 immediate) {
  if (rs == N64Reg_R0) {
...
inline void CCodeGeneratorPSP::GenerateDADDU(EN64Reg rd, EN64Reg rs,
                                             EN64Reg rt) {
  ...
  SLTU(PspReg_V1, reg_lo_d,
       reg_lo_d == reg_lo_a ? reg_lo_b : reg_lo_a); // Overflowed?
...
inline void CCodeGeneratorPSP::GenerateAND(EN64Reg rd, EN64Reg rs, EN64Reg rt) {
  // gGPR[ op_code.rd ]._u64 = gGPR[ op_code.rs ]._u64 & gGPR[ op_code.rt
  // ]._u64;
  ...
  if (mRegisterCache.IsKnownValue(rs, 1) & mRegisterCache.IsKnownValue(rt, 1)) {
    SetRegister(rd, 1,
                mRegisterCache.GetKnownValue(rs, 1)._u32 &
                    mRegisterCache.GetKnownValue(rt, 1)._u32);
    HiIsDone = true;
  } else if ((mRegisterCache.IsKnownValue(rs, 1) &
              (mRegisterCache.GetKnownValue(rs, 1)._u32 == 0)) |
             (mRegisterCache.IsKnownValue(rt, 1) &
              (mRegisterCache.GetKnownValue(rt, 1)._u32 == 0))) {
    SetRegister(rd, 1, 0);
    HiIsDone = true;
  } else if (mRegisterCache.IsKnownValue(rt, 1) &
             (mRegisterCache.GetKnownValue(rt, 1)._s32 == -1)) {
    EPspReg reg_hi_d(GetRegisterNoLoadHi(rd, PspReg_V0));
  ...
  if (mRegisterCache.IsKnownValue(rs, 0) & mRegisterCache.IsKnownValue(rt, 0)) {
    SetRegister(rd, 0,
                mRegisterCache.GetKnownValue(rs, 0)._u32 &
                    mRegisterCache.GetKnownValue(rt, 0)._u32);
...
void CCodeGeneratorPSP::GenerateOR(EN64Reg rd, EN64Reg rs, EN64Reg rt) {
  ...
  if (mRegisterCache.IsKnownValue(rs, 1) & mRegisterCache.IsKnownValue(rt, 1)) {
    ...
    HiIsDone = true;
              ...
              (mRegisterCache.GetKnownValue(rs, 1)._s32 == -1)) |
             (mRegisterCache.IsKnownValue(rt, 1) &
              (mRegisterCache.GetKnownValue(rt, 1)._s32 == -1))) {
    SetRegister(rd, 1, ~0);
    HiIsDone = true;
  ...
  if (rs == N64Reg_R0) {
    ...
    */
    if (mRegisterCache.IsKnownValue(rt, 0)) {
      SetRegister64(rd, mRegisterCache.GetKnownValue(rt, 0)._u32,
                    mRegisterCache.GetKnownValue(rt, 1)._u32);
      return;
    }
...
inline void CCodeGeneratorPSP::GenerateSLT(EN64Reg rd, EN64Reg rs, EN64Reg rt) {
#ifdef ENABLE_64BIT
  // Because we have a branch here, we need to make sure that we have a
  // consistent view of the registers regardless of whether we take it or not.
  // We pull in the lo halves of the registers here so that they're Valid
  // regardless of whether we take the branch or not
  PrepareCachedRegisterLo(rs);
  PrepareCachedRegisterLo(rt);

  // If possible, we write directly into the destination register. We have to be
  // careful though - if the destination register is the same as either of the
  // source registers we have to use a temporary instead, to avoid overwriting
  // the contents.
  EPspReg reg_lo_d(GetRegisterNoLoadLo(rd, PspReg_V0));
  ...
  // If the branch was not taken, it means that the high part of the registers
  // was equal, so compare bottom half
  EPspReg reg_lo_a(GetRegisterAndLoadLo(rs, PspReg_V0));
  EPspReg reg_lo_b(GetRegisterAndLoadLo(rt, PspReg_A0));
  ...
  {
          SetRegister32s(rd, (mRegisterCache.GetKnownValue(rs, 0)._s32 <
  mRegisterCache.GetKnownValue(rt, 0)._s32) ); return;
  }
  */
  EPspReg reg_lo_d(GetRegisterNoLoadLo(rd, PspReg_V0));
  EPspReg reg_lo_a(GetRegisterAndLoadLo(rs, PspReg_V0));
...
inline void CCodeGeneratorPSP::GenerateSLTU(EN64Reg rd, EN64Reg rs,
                                            EN64Reg rt) {
#ifdef ENABLE_64BIT
  // Because we have a branch here, we need to make sure that we have a
  // consistant view of the registers regardless of whether we take it or not.
  // We pull in the lo halves of the registers here so that they're Valid
  // regardless of whether we take the branch or not
  PrepareCachedRegisterLo(rs);
  PrepareCachedRegisterLo(rt);
  ...
  if (mRegisterCache.IsKnownValue(rs, 0) & mRegisterCache.IsKnownValue(rt, 0))
  {
          SetRegister32s(rd, (mRegisterCache.GetKnownValue(rs, 0)._u32 <
  mRegisterCache.GetKnownValue(rt, 0)._u32) ); return;
  }
  */
  EPspReg reg_lo_d(GetRegisterNoLoadLo(rd, PspReg_V0));
  EPspReg reg_lo_a(GetRegisterAndLoadLo(rs, PspReg_V0));
...
inline void CCodeGeneratorPSP::GenerateADDIU(EN64Reg rt, EN64Reg rs,
                                             s16 immediate) {
  // gGPR[op_code.rt]._s64 = (s64)(s32)(gGPR[op_code.rs]._s32_0 +
  // (s32)(s16)op_code.immediate);
...
inline void CCodeGeneratorPSP::GenerateANDI(EN64Reg rt, EN64Reg rs,
                                            u16 immediate) {
  // gGPR[op_code.rt]._u64 = gGPR[op_code.rs]._u64 &
  // (u64)(u16)op_code.immediate;

  if (mRegisterCache.IsKnownValue(rs, 0)) {
    s32 known_value_lo(mRegisterCache.GetKnownValue(rs, 0)._u32 &
                       (u32)immediate);
...
inline void CCodeGeneratorPSP::GenerateORI(EN64Reg rt, EN64Reg rs,
                                           u16 immediate) {
  // gGPR[op_code.rt]._u64 = gGPR[op_code.rs]._u64 |
  // (u64)(u16)op_code.immediate;
...
#ifdef ENABLE_64BIT // This is out of spec but seems we can always ignore
                    // copying the hi part
    // If the source/dest regs are different we need to copy the high bits
    // across
    if (rt != rs) {
...
inline void CCodeGeneratorPSP::GenerateXORI(EN64Reg rt, EN64Reg rs,
                                            u16 immediate) {
  // gGPR[op_code.rt]._u64 = gGPR[op_code.rs]._u64 ^
  // (u64)(u16)op_code.immediate;
  ...
  if (mRegisterCache.IsKnownValue(rs, 0)) {
    ...
    StoreRegisterLo(rt, dst_reg);
                    ...
                    // copying the hi part
    // If the source/dest regs are different we need to copy the high bits
    // across (if they are the same, we're xoring 0 to the top half which is
    // essentially a NOP)
    if (rt != rs) {
      EPspReg dst_reg_hi(GetRegisterNoLoadHi(rt, PspReg_V0));
      LoadRegisterHi(dst_reg_hi, rs);
...
inline void CCodeGeneratorPSP::GenerateSLTI(EN64Reg rt, EN64Reg rs,
                                            s16 immediate) {
#ifdef ENABLE_64BIT
  // Because we have a branch here, we need to make sure that we have a
  // consistant view of the register regardless of whether we take it or not. We
  // pull in the lo halves of the register here so that it's Valid regardless of
  ...
  // Potential bug!!!
  // If using 64bit mode this might need to be SLTIU since sign is already
  // checked in hi word //Strmn
  ...
  if (mRegisterCache.IsKnownValue(rs, 0))
  {
          SetRegister32s( rt, (mRegisterCache.GetKnownValue(rs, 0)._s32 <
  (s32)immediate) ); return;
  }
  */
  EPspReg reg_lo_d(GetRegisterNoLoadLo(rt, PspReg_V0));
  EPspReg reg_lo_a(GetRegisterAndLoadLo(rs, PspReg_V0));
...
inline void CCodeGeneratorPSP::GenerateSLTIU(EN64Reg rt, EN64Reg rs,
                                             s16 immediate) {
#ifdef ENABLE_64BIT
  // Because we have a branch here, we need to make sure that we have a
  // consistent view of the register regardless of whether we take it or not. We
  // pull in the lo halves of the register here so that it's Valid regardless of
  // whether we take the branch or not
  PrepareCachedRegisterLo(rs);
  ...
  if (mRegisterCache.IsKnownValue(rs, 0))
  {
          SetRegister32s( rt, (mRegisterCache.GetKnownValue(rs, 0)._u32 <
  (u32)immediate) ); return;
  }
  */
  EPspReg reg_lo_d(GetRegisterNoLoadLo(rt, PspReg_V0));
  EPspReg reg_lo_a(GetRegisterAndLoadLo(rs, PspReg_V0));
...
inline void CCodeGeneratorPSP::GenerateSLL(EN64Reg rd, EN64Reg rt, u32 sa) {
  // gGPR[ op_code.rd ]._s64 = (s64)(s32)( (gGPR[ op_code.rt ]._u32_0 <<
  // op_code.sa) & 0xFFFFFFFF );
  if (mRegisterCache.IsKnownValue(rt, 0)) {
...
inline void CCodeGeneratorPSP::GenerateDSLL(EN64Reg rd, EN64Reg rt, u32 sa) {
  // gGPR[ op_code.rd ]._s64 = (s64)(s32)( (gGPR[ op_code.rt ]._u32_0 <<
  // op_code.sa) & 0xFFFFFFFF ); if (mRegisterCache.IsKnownValue(rt, 0))
  //{
...
inline void CCodeGeneratorPSP::GenerateDSRA(EN64Reg rd, EN64Reg rt, u32 sa) {
  // gGPR[ op_code.rd ]._s64 = (s64)(s32)( gGPR[ op_code.rt ]._s32_0 >>
  // op_code.sa ); if (mRegisterCache.IsKnownValue(rt, 0))
  //{
...
inline void CCodeGeneratorPSP::GenerateSLLV(EN64Reg rd, EN64Reg rs,
                                            EN64Reg rt) {
  // gGPR[ op_code.rd ]._s64 = (s64)(s32)( (gGPR[ op_code.rt ]._u32_0 << ( gGPR[
  // op_code.rs ]._u32_0 & 0x1F ) ) & 0xFFFFFFFF );
  if (mRegisterCache.IsKnownValue(rs, 0) & mRegisterCache.IsKnownValue(rt, 0)) {
    SetRegister32s(rd,
                   (s32)(mRegisterCache.GetKnownValue(rt, 0)._u32
...
inline void CCodeGeneratorPSP::GenerateSRLV(EN64Reg rd, EN64Reg rs,
                                            EN64Reg rt) {
  // gGPR[ op_code.rd ]._s64 = (s64)(s32)( gGPR[ op_code.rt ]._u32_0 >> ( gGPR[
  // op_code.rs ]._u32_0 & 0x1F ) );
  if (mRegisterCache.IsKnownValue(rs, 0) & mRegisterCache.IsKnownValue(rt, 0)) {
    SetRegister32s(rd,
                   (s32)(mRegisterCache.GetKnownValue(rt, 0)._u32 >>
                         (mRegisterCache.GetKnownValue(rs, 0)._u32 & 0x1F)));
...
inline void CCodeGeneratorPSP::GenerateSRAV(EN64Reg rd, EN64Reg rs,
                                            EN64Reg rt) {
  // gGPR[ op_code.rd ]._s64 = (s64)(s32)( gGPR[ op_code.rt ]._s32_0 >> ( gGPR[
  // op_code.rs ]._u32_0 & 0x1F ) );
  if (mRegisterCache.IsKnownValue(rs, 0) & mRegisterCache.IsKnownValue(rt, 0)) {
    SetRegister32s(rd,
                   (s32)(mRegisterCache.GetKnownValue(rt, 0)._s32 >>
                         (mRegisterCache.GetKnownValue(rs, 0)._u32 & 0x1F)));
    return;
  }
...
inline void CCodeGeneratorPSP::GenerateLB(u32 address, bool set_branch_delay,
                                          EN64Reg rt, EN64Reg base,
                                          s32 offset) {
  EPspReg reg_dst(GetRegisterNoLoadLo(
      rt,
      PspReg_V0)); // Use V0 to avoid copying return value if reg is not cached

  GenerateLoad(address, reg_dst, base, offset, OP_LB, 3,
               set_branch_delay
                   ? ReadBitsDirectBD_s8
                   : ReadBitsDirect_s8); // NB this fills the whole of reg_dst
...
inline void CCodeGeneratorPSP::GenerateLBU(u32 address, bool set_branch_delay,
                                           EN64Reg rt, EN64Reg base,
                                           s32 offset) {
  EPspReg reg_dst(GetRegisterNoLoadLo(
      rt,
  ...
  GenerateLoad(address, reg_dst, base, offset, OP_LBU, 3,
               set_branch_delay
                   ? ReadBitsDirectBD_u8
                   : ReadBitsDirect_u8); // NB this fills the whole of reg_dst
...
inline void CCodeGeneratorPSP::GenerateLH(u32 address, bool set_branch_delay,
                                          EN64Reg rt, EN64Reg base,
                                          s32 offset) {
  EPspReg reg_dst(GetRegisterNoLoadLo(
      rt,
  ...
  GenerateLoad(address, reg_dst, base, offset, OP_LH, 2,
               set_branch_delay
                   ? ReadBitsDirect_s16
                   : ReadBitsDirect_s16); // NB this fills the whole of reg_dst
...
inline void CCodeGeneratorPSP::GenerateLHU(u32 address, bool set_branch_delay,
                                           EN64Reg rt, EN64Reg base,
                                           s32 offset) {
  EPspReg reg_dst(GetRegisterNoLoadLo(
      rt,
  ...
  GenerateLoad(address, reg_dst, base, offset, OP_LHU, 2,
               set_branch_delay
                   ? ReadBitsDirectBD_u16
                   : ReadBitsDirect_u16); // NB this fills the whole of reg_dst
...
inline void CCodeGeneratorPSP::GenerateLW(u32 address, bool set_branch_delay,
                                          EN64Reg rt, EN64Reg base,
                                          s16 offset) {
  EPspReg reg_dst(GetRegisterNoLoadLo(
      rt,
  ...
  // This is for San Francisco 2049, otherwise it crashes when the race is about
  // to start.
  if (rt == N64Reg_R0) {
#ifdef DAEDALUS_DEBUG_CONSOLE
  ...
  GenerateLoad(address, reg_dst, base, offset, OP_LW, 0,
               set_branch_delay ? ReadBitsDirectBD_u32 : ReadBitsDirect_u32);
...
inline void CCodeGeneratorPSP::GenerateLD(u32 address, bool set_branch_delay,
                                          EN64Reg rt, EN64Reg base,
                                          s16 offset) {
  EPspReg reg_dst_hi(GetRegisterNoLoadHi(
      rt,
      PspReg_V0)); // Use V0 to avoid copying return value if reg is not cached
  GenerateLoad(address, reg_dst_hi, base, offset, OP_LW, 0,
               set_branch_delay ? ReadBitsDirectBD_u32 : ReadBitsDirect_u32);
  StoreRegisterHi(rt, reg_dst_hi);

  // Adding 4 to offset should be quite safe //Corn
  EPspReg reg_dst_lo(GetRegisterNoLoadLo(
      rt,
      PspReg_V0)); // Use V0 to avoid copying return value if reg is not cached
  GenerateLoad(address, reg_dst_lo, base, offset + 4, OP_LW, 0,
               set_branch_delay ? ReadBitsDirectBD_u32 : ReadBitsDirect_u32);
...
inline void CCodeGeneratorPSP::GenerateLWC1(u32 address, bool set_branch_delay,
                                            u32 ft, EN64Reg base, s32 offset) {
  auto n64_ft = EN64FloatReg(ft);
  auto psp_ft = EPspFloatReg(n64_ft); // 1:1 Mapping

  // u32 address = (u32)( gGPR[op_code.base]._s32_0 +
  // (s32)(s16)op_code.immediate ); value = Read32Bits(address);
  // gCPUState.FPU[op_code.ft]._s32_0 = value;

#ifdef ENABLE_LWC1
  // Since LW and LWC1 have same format we do a small hack that let us load
  // directly to the float reg without passing to the CPU first that saves us the
  // MFC1 instruction //Corn
  GenerateLoad(address, (EPspReg)psp_ft, base, offset, OP_LWC1, 0,
               set_branch_delay ? ReadBitsDirectBD_u32 : ReadBitsDirect_u32);
  UpdateFloatRegister(n64_ft);
#else
  // TODO: Actually perform LWC1 here
  EPspReg reg_dst(
      PspReg_V0); // GenerateLoad is slightly more efficient when using V0
  GenerateLoad(address, reg_dst, base, offset, OP_LW, 0,
               set_branch_delay ? ReadBitsDirectBD_u32 : ReadBitsDirect_u32);
...
#ifdef ENABLE_LDC1
inline void CCodeGeneratorPSP::GenerateLDC1(u32 address, bool set_branch_delay,
                                            u32 ft, EN64Reg base, s32 offset) {
  ...
  // GenerateLoad( address, (EPspReg)psp_ft, base, offset + 4, OP_LWC1, 0,
  // set_branch_delay ? ReadBitsDirectBD_u32 : ReadBitsDirect_u32 );
...
inline void CCodeGeneratorPSP::GenerateLWL(u32 address, bool set_branch_delay,
                                           EN64Reg rt, EN64Reg base,
                                           s16 offset) {

  // Will return the value in PspReg_V0 and the shift in PspReg_A3
  GenerateLoad(address, PspReg_V0, base, offset, OP_LWL, 0,
               set_branch_delay ? ReadBitsDirectBD_u32 : ReadBitsDirect_u32);
...
inline void CCodeGeneratorPSP::GenerateLWR(u32 address, bool set_branch_delay,
                                           EN64Reg rt, EN64Reg base,
                                           s16 offset) {
  // Will return the value in PspReg_V0 and the shift in PspReg_A3
  GenerateLoad(address, PspReg_V0, base, offset, OP_LWL, 0,
...
inline void CCodeGeneratorPSP::GenerateSB(u32 current_pc, bool set_branch_delay,
                                          EN64Reg rt, EN64Reg base,
                                          s32 offset) {
  EPspReg reg_value(GetRegisterAndLoadLo(rt, PspReg_A1));

  GenerateStore(current_pc, reg_value, base, offset, OP_SB, 3,
                set_branch_delay ? WriteBitsDirectBD_u8 : WriteBitsDirect_u8);
}

//

inline void CCodeGeneratorPSP::GenerateSH(u32 current_pc, bool set_branch_delay,
                                          EN64Reg rt, EN64Reg base,
                                          s32 offset) {
  EPspReg reg_value(GetRegisterAndLoadLo(rt, PspReg_A1));

  GenerateStore(current_pc, reg_value, base, offset, OP_SH, 2,
                set_branch_delay ? WriteBitsDirectBD_u16 : WriteBitsDirect_u16);
}

//

inline void CCodeGeneratorPSP::GenerateSW(u32 current_pc, bool set_branch_delay,
                                          EN64Reg rt, EN64Reg base,
                                          s32 offset) {
  EPspReg reg_value(GetRegisterAndLoadLo(rt, PspReg_A1));

  GenerateStore(current_pc, reg_value, base, offset, OP_SW, 0,
                set_branch_delay ? WriteBitsDirectBD_u32 : WriteBitsDirect_u32);
}

//

inline void CCodeGeneratorPSP::GenerateSD(u32 current_pc, bool set_branch_delay,
                                          EN64Reg rt, EN64Reg base,
                                          s32 offset) {
  EPspReg reg_value_hi(GetRegisterAndLoadHi(rt, PspReg_A1));
  GenerateStore(current_pc, reg_value_hi, base, offset, OP_SW, 0,
                set_branch_delay ? WriteBitsDirectBD_u32 : WriteBitsDirect_u32);

  // Adding 4 to offset should be quite safe //Corn
  EPspReg reg_value_lo(GetRegisterAndLoadLo(rt, PspReg_A1));
  GenerateStore(current_pc, reg_value_lo, base, offset + 4, OP_SW, 0,
                set_branch_delay ? WriteBitsDirectBD_u32 : WriteBitsDirect_u32);
}

//

inline void CCodeGeneratorPSP::GenerateSWC1(u32 current_pc,
                                            bool set_branch_delay, u32 ft,
                                            EN64Reg base, s32 offset) {
  auto n64_ft = EN64FloatReg(ft);
  EPspFloatReg psp_ft(GetFloatRegisterAndLoad(n64_ft));

#ifdef ENABLE_SWC1
  // Since SW and SWC1 have same format we do a small hack that let us save
  // directly from the float reg without passing to the CPU first that saves us
  // the MFC1 instruction //Corn
  GenerateStore(current_pc, (EPspReg)psp_ft, base, offset, OP_SWC1, 0,
                set_branch_delay ? WriteBitsDirectBD_u32 : WriteBitsDirect_u32);
#else
  MFC1(PspReg_A1, psp_ft);
  GenerateStore(current_pc, PspReg_A1, base, offset, OP_SW, 0,
                set_branch_delay ? WriteBitsDirectBD_u32 : WriteBitsDirect_u32);
#endif
}
...
#ifdef ENABLE_SDC1
inline void CCodeGeneratorPSP::GenerateSDC1(u32 current_pc,
                                            bool set_branch_delay, u32 ft,
                                            EN64Reg base, s32 offset) {
  ...
  // GenerateStore( current_pc, (EPspReg)psp_ft, base, offset, OP_SWC1, 0,
  // set_branch_delay ? WriteBitsDirectBD_u32 : WriteBitsDirect_u32 );
  // GenerateStore( current_pc, (EPspReg)psp_ft_sig, base, offset + 4, OP_SWC1,
  // 0, set_branch_delay ? WriteBitsDirectBD_u32 : WriteBitsDirect_u32 );
  EPspReg reg_base(GetRegisterAndLoadLo(base, PspReg_A0));
  ADDU(PspReg_A0, reg_base, gMemoryBaseReg);
...
#ifdef ENABLE_SWR_SWL
inline void CCodeGeneratorPSP::GenerateSWL(u32 current_pc,
                                           bool set_branch_delay, EN64Reg rt,
                                           EN64Reg base, s32 offset) {
  EPspReg reg_value(GetRegisterAndLoadLo(rt, PspReg_A1));

  GenerateStore(current_pc, reg_value, base, offset, OP_SWL, 3,
                set_branch_delay ? WriteBitsDirectBD_u32 : WriteBitsDirect_u32);
}

//

inline void CCodeGeneratorPSP::GenerateSWR(u32 current_pc,
                                           bool set_branch_delay, EN64Reg rt,
                                           EN64Reg base, s32 offset) {
  EPspReg reg_value(GetRegisterAndLoadLo(rt, PspReg_A1));

  GenerateStore(current_pc, reg_value, base, offset, OP_SWR, 3,
                set_branch_delay ? WriteBitsDirectBD_u32 : WriteBitsDirect_u32);
}
#endif
...
inline void CCodeGeneratorPSP::GenerateMTC1(u32 fs, EN64Reg rt) {
  ...
  auto n64_fs = EN64FloatReg(fs);
  auto psp_fs = EPspFloatReg(n64_fs); // 1:1 Mapping
...
// This breaks BombreMan Hero, not sure how to fix it though, maybe because we
// don't set the rounding mode?
...
inline void CCodeGeneratorPSP::GenerateBEQ(EN64Reg rs, EN64Reg rt,
                                           const SBranchDetails *p_branch,
                                           CJumpLocation *p_branch_jump) {
#ifdef DAEDALUS_ENABLE_ASSERTS
...
#endif
  // One or other of these may be r0 - we don't really care for optimisation
  // purposes though as ultimately the register load regs factored out
  EPspReg reg_a(GetRegisterAndLoadLo(rs, PspReg_V0));
  EPspReg reg_b(GetRegisterAndLoadLo(rt, PspReg_A0));

  // XXXX This may actually need to be a 64 bit compare, but this is what
  // R4300.cpp does
...
inline void CCodeGeneratorPSP::GenerateBNE(EN64Reg rs, EN64Reg rt,
                                           const SBranchDetails *p_branch,
                                           CJumpLocation *p_branch_jump) {
#ifdef DAEDALUS_ENABLE_ASSERTS
  DAEDALUS_ASSERT(p_branch != nullptr, "No branch details?");
...
inline void CCodeGeneratorPSP::GenerateBLEZ(EN64Reg rs,
                                            const SBranchDetails *p_branch,
                                            CJumpLocation *p_branch_jump) {
#ifdef DAEDALUS_ENABLE_ASSERTS
  DAEDALUS_ASSERT(p_branch != nullptr, "No branch details?");
...
inline void CCodeGeneratorPSP::GenerateBGTZ(EN64Reg rs,
                                            const SBranchDetails *p_branch,
                                            CJumpLocation *p_branch_jump) {
#ifdef DAEDALUS_ENABLE_ASSERTS
  DAEDALUS_ASSERT(p_branch != nullptr, "No branch details?");
...
inline void CCodeGeneratorPSP::GenerateBLTZ(EN64Reg rs,
                                            const SBranchDetails *p_branch,
                                            CJumpLocation *p_branch_jump) {
#ifdef DAEDALUS_ENABLE_ASSERTS
  DAEDALUS_ASSERT(p_branch != nullptr, "No branch details?");
...
inline void CCodeGeneratorPSP::GenerateBGEZ(EN64Reg rs,
                                            const SBranchDetails *p_branch,
                                            CJumpLocation *p_branch_jump) {
#ifdef DAEDALUS_ENABLE_ASSERTS
  DAEDALUS_ASSERT(p_branch != nullptr, "No branch details?");
...
inline void CCodeGeneratorPSP::GenerateBC1F(const SBranchDetails *p_branch,
                                            CJumpLocation *p_branch_jump) {
#ifdef DAEDALUS_ENABLE_ASSERTS
  DAEDALUS_ASSERT(p_branch != nullptr, "No branch details?");
  DAEDALUS_ASSERT(p_branch->Direct, "Indirect branch for BC1F?");
#endif
  // If compare was done in current fragment then use BC1T or BC1F directly
  // //Corn
  if (mFloatCMPIsValid) {
    if (p_branch->ConditionalBranchTaken) {
      // Flip the sign of the test -
...
inline void CCodeGeneratorPSP::GenerateBC1T(const SBranchDetails *p_branch,
                                            CJumpLocation *p_branch_jump) {
#ifdef DAEDALUS_ENABLE_ASSERTS
  DAEDALUS_ASSERT(p_branch != nullptr, "No branch details?");
...
inline void CCodeGeneratorPSP::GenerateADD_D_Sim(u32 fd, u32 fs, u32 ft) {
  auto n64_fs = EN64FloatReg(fs);
  auto n64_ft = EN64FloatReg(ft);
  auto n64_fd = EN64FloatReg(fd);

  auto psp_fd_sig = EPspFloatReg(n64_fd); // 1:1 Mapping
  auto psp_fd = EPspFloatReg(n64_fd + 1); // 1:1 Mapping
  ...
  if (!mRegisterCache.IsFPSim(n64_fd))
    MOV_S(psp_fd_sig, EPspFloatReg(fs)); // Copy signature as well if needed
...
inline void CCodeGeneratorPSP::GenerateMOV_D_Sim(u32 fd, u32 fs) {
  ...
  auto psp_fd_sig = EPspFloatReg(n64_fd); // 1:1 Mapping
  auto psp_fd = EPspFloatReg(n64_fd + 1); // 1:1 Mapping
  auto psp_fs_sig = EPspFloatReg(n64_fs); // 1:1 Mapping
...
inline void CCodeGeneratorPSP::GenerateCMP_D_Sim(u32 fs, ECop1OpFunction cmp_op,
                                                 u32 ft) {
...
inline void CCodeGeneratorPSP::GenerateADD_S(u32 fd, u32 fs, u32 ft) {
  ...
  auto psp_fd = EPspFloatReg(n64_fd); // 1:1 Mapping
  EPspFloatReg psp_fs(GetFloatRegisterAndLoad(n64_fs));
  EPspFloatReg psp_ft(GetFloatRegisterAndLoad(n64_ft));
...
inline void CCodeGeneratorPSP::GenerateMUL_S(u32 fd, u32 fs, u32 ft) {
  ...
  auto psp_fd = EPspFloatReg(n64_fd);
  EPspFloatReg psp_fs(GetFloatRegisterAndLoad(n64_fs));
  EPspFloatReg psp_ft(GetFloatRegisterAndLoad(n64_ft));
...
inline void CCodeGeneratorPSP::GenerateCVT_D_W_Sim(u32 fd, u32 fs) {
  ...
  auto psp_fd_sig = EPspFloatReg(n64_fd);
  auto psp_fd = EPspFloatReg(n64_fd + 1);
  EPspFloatReg psp_fs(GetFloatRegisterAndLoad(n64_fs));
...
inline void CCodeGeneratorPSP::GenerateCVT_D_S(u32 fd, u32 fs) {
  ...
  JAL(CCodeLabel(reinterpret_cast<const void *>(_FloatToDouble)),
      false);              // Convert Float to Double
  ...
  mRegisterCache.MarkFPAsSim(
      n64_fd, false); // Dont flag as simulated its a real Double!!!
...
inline void CCodeGeneratorPSP::GenerateCMP_S(u32 fs, ECop1OpFunction cmp_op,
                                             u32 ft) {
  mFloatCMPIsValid = true;
