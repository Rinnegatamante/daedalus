...
public:
  ...
  inline bool IsValid(EN64Reg reg, u32 lo_hi_idx) const {
    ...
    DAEDALUS_ASSERT(!mRegisterCacheInfo[reg][lo_hi_idx].Valid ||
                        IsCached(reg, lo_hi_idx),
                    "Checking register is valid but uncached?");
#endif
  ...
  inline bool IsDirty(EN64Reg reg, u32 lo_hi_idx) const {
    ...
    if (is_dirty) {
      DAEDALUS_ASSERT(IsKnownValue(reg, lo_hi_idx) || IsCached(reg, lo_hi_idx),
                      "Checking dirty flag on unknown/uncached register?");
    }
...
#ifdef DAEDALUS_ENABLE_ASSERTS
    DAEDALUS_ASSERT(IsCached(reg, lo_hi_idx),
                    "Trying to retreive an uncached register");
#endif
...
#ifdef DAEDALUS_ENABLE_ASSERTS
    DAEDALUS_ASSERT(IsCached(reg, lo_hi_idx),
                    "Changing valid flag on uncached register?");
#endif
  ...
  inline void MarkAsDirty(EN64Reg reg, u32 lo_hi_idx, bool dirty) {
    ...
    if (dirty) {
      DAEDALUS_ASSERT(IsKnownValue(reg, lo_hi_idx) || IsCached(reg, lo_hi_idx),
                      "Setting dirty flag on unknown/uncached register?");
    }
#endif
...
private:
  struct RegisterCacheInfoPSP {
    ...
    // bool			SignExtended;		// Is this (high) register just sign extension of
    // low reg?
  };
