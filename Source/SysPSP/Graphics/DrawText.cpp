...
void CDrawText::Initialise() {
  ...
  gFonts[F_REGULAR] = intraFontLoad(
      "flash0:/font/ltn8.pgf",
      INTRAFONT_CACHE_ALL | INTRAFONT_STRING_UTF8); // Regular/sans-serif
  gFonts[F_LARGE_BOLD] = intraFontLoad(
      "flash0:/font/ltn4.pgf",
      INTRAFONT_CACHE_ALL | INTRAFONT_STRING_UTF8); // Large/sans-serif/bold
...
//
//*************************************************************************************
u32 CDrawText::Render(EFont font, s32 x, s32 y, float scale, const char *p_str,
...
//
//*************************************************************************************
u32 CDrawText::Render(EFont font_type, s32 x, s32 y, float scale,
                      const char *p_str, u32 length, c32 colour,
                      c32 drop_colour) {
#ifdef DAEDALUS_ENABLE_ASSERTS
  ...
  if (font) {
    ...
    return s32(intraFontPrintEx(font, x, y, Translate(p_str, length), length)) -
           x;
  }
...
s32 CDrawText::GetTextWidth(EFont font_type, const char *p_str, u32 length) {
  ...
  return strlen(p_str) *
         16; // Return a reasonable value. Better off just returning 0?
}
...
void WrapText(CDrawText::EFont font, s32 width, const char *p_str, u32 length,
  ...
  while (p_line_str < p_str_end) {
    ...
    s32 chunk_width(
        CDrawText::GetTextWidth(font, p_line_str, length_remaining));
    ...
    if (chunk_width <= width) {
      ...
      DAEDALUS_ASSERT(
          found_chunk,
          "Didn't find chunk while splitting string for rendering?");
#endif
    }
  }
}
