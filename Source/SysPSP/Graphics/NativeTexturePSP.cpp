...
EPspTextureFormat GetPspTextureFormat(ETextureFormat texture_format) {
  ...
  case TexFmt_5650:
    return PspTexFmt_5650;
  case TexFmt_5551:
    return PspTexFmt_5551;
  case TexFmt_4444:
    return PspTexFmt_4444;
  case TexFmt_8888:
...
inline void swizzle_fast(u8 *out, const u8 *in, u32 width, u32 height) {
  ...
  DAEDALUS_ASSERT((width & 15) == 0, "Width is not a multiple of 16 - is %d",
                  width);
  DAEDALUS_ASSERT((height & 7) == 0, "Height is not a multiple of 8 - is %d",
                  height);
#endif
...
//*****************************************************************************
//	Swizzle the texture, explicitly setting the texture colour to the
//specified value (alpha is left alone!)
//*****************************************************************************
/*void swizzle_fast_setcolour(u8* out, const u8* in, u32 width, u32 height, u32
mask, u32 colour)
{
        u32 blockx, blocky;
        u32 j;
...
#ifdef DAEDALUS_ENABLE_ASSERTS
  DAEDALUS_ASSERT(GetNextPowerOf2(dimension) == dimension,
...
CNativeTexture::CNativeTexture(u32 w, u32 h, ETextureFormat texture_format)
    : mTextureFormat(texture_format), mWidth(w), mHeight(h),
      ...
      mpPalette(nullptr), mIsPaletteVidMem(false), mIsSwizzled(true)
#ifdef DAEDALUS_ENABLE_ASSERTS
      ,
      mPaletteSet(false)
#endif
{
  ...
  if (!CVideoMemoryManager::Get()->Alloc(bytes_required, &mpData,
                                         &mIsDataVidMem)) {
#ifdef DAEDALUS_DEBUG_CONSOLE
  ...
  switch (texture_format) {
  case TexFmt_CI4_8888:
    if (!CVideoMemoryManager::Get()->Alloc(kPalette4BytesRequired, &mpPalette,
                                           &mIsPaletteVidMem)) {
#ifdef DAEDALUS_DEBUG_CONSOLE
      DAEDALUS_ERROR("Out of memory for 4-bit palette, %d bytes",
                     kPalette4BytesRequired);
#endif
    }
    break;

  case TexFmt_CI8_8888:
    if (!CVideoMemoryManager::Get()->Alloc(kPalette8BytesRequired, &mpPalette,
                                           &mIsPaletteVidMem)) {
#ifdef DAEDALUS_DEBUG_CONSOLE
      DAEDALUS_ERROR("Out of memory for 8-bit palette, %d bytes",
                     kPalette8BytesRequired);
#endif
    }
    break;
#ifdef DAEDALUS_ENABLE_ASSERTS
  default:
    DAEDALUS_ASSERT(!IsTextureFormatPalettised(texture_format),
                    "Unhandled palette texture");
    break;
#endif
  }
}
...
bool CNativeTexture::HasData() const {
  return mpData != nullptr &&
         (!IsTextureFormatPalettised(mTextureFormat) || mpPalette != nullptr);
}
...
void CNativeTexture::InstallTexture() const {
  ...
  if (!HasData()) {
    ...
    // sceGuTexImage( 0, gPlaceholderTextureWidth, gPlaceholderTextureHeight,
    // gPlaceholderTextureWidth, gWhiteTexture );
  } else {
    EPspTextureFormat psp_texture_format(GetPspTextureFormat(mTextureFormat));
    sceGuTexMode(psp_texture_format, 0, 0,
                 mIsSwizzled ? 1 : 0); // maxmips/a2/swizzle = 0
    sceGuTexImage(0, mCorrectedWidth, mCorrectedHeight, mTextureBlockWidth,
                  mpData);

#ifdef DAEDALUS_ENABLE_ASSERTS
    DAEDALUS_ASSERT(!IsTextureFormatPalettised(mTextureFormat) || mPaletteSet,
                    "The palette has not been set");
...
#ifdef DAEDALUS_ENABLE_ASSERTS
    default:
      DAEDALUS_ASSERT(!IsTextureFormatPalettised(mTextureFormat),
                      "Unhandled palette texture");
      break;
#endif
    }
...
namespace {
template <typename T>
void ReadPngData(u32 width, u32 height, u32 stride, u8 **p_row_table,
                 int color_type, T *p_dest) {
  u8 r = 0, g = 0, b = 0, a = 0;
...
//	correct dimensions and format) else a new texture is created and
//returned.
//*****************************************************************************
CRefPtr<CNativeTexture> LoadPng(const char *p_filename,
  ...
  png_read_png(p_png_struct, p_png_info,
               PNG_TRANSFORM_STRIP_16 | PNG_TRANSFORM_PACKING |
                   PNG_TRANSFORM_EXPAND | PNG_TRANSFORM_BGR,
               nullptr);

  png_uint_32 width =
      png_get_image_width(p_png_struct, p_png_info); // p_png_info->width;
  png_uint_32 height =
      png_get_image_height(p_png_struct, p_png_info); // p_png_info->height;

  CRefPtr<CNativeTexture> texture =
      CNativeTexture::Create(width, height, texture_format);

#ifdef DAEDALUS_ENABLE_ASSERTS
  DAEDALUS_ASSERT(texture->GetWidth() >= width, "Width is unexpectedly small");
  DAEDALUS_ASSERT(texture->GetHeight() >= height,
                  "Height is unexpectedly small");
  DAEDALUS_ASSERT(texture_format == texture->GetFormat(),
                  "Texture format doesn't match");
#endif
  ...
  if (!p_dest) {
    ...
    switch (texture_format) {
    case TexFmt_5650:
      ReadPngData<NativePf5650>(width, height, stride,
                                png_get_rows(p_png_struct, p_png_info),
                                png_get_color_type(p_png_struct, p_png_info),
                                reinterpret_cast<NativePf5650 *>(p_dest));
      break;
    case TexFmt_5551:
      ReadPngData<NativePf5551>(width, height, stride,
                                png_get_rows(p_png_struct, p_png_info),
                                png_get_color_type(p_png_struct, p_png_info),
                                reinterpret_cast<NativePf5551 *>(p_dest));
      break;
    case TexFmt_4444:
      ReadPngData<NativePf4444>(width, height, stride,
                                png_get_rows(p_png_struct, p_png_info),
                                png_get_color_type(p_png_struct, p_png_info),
                                reinterpret_cast<NativePf4444 *>(p_dest));
      break;
    case TexFmt_8888:
      ReadPngData<NativePf8888>(width, height, stride,
                                png_get_rows(p_png_struct, p_png_info),
                                png_get_color_type(p_png_struct, p_png_info),
                                reinterpret_cast<NativePf8888 *>(p_dest));
      break;
...
CNativeTexture::CreateFromPng(const char *p_filename,
                              ETextureFormat texture_format) {
...
void CNativeTexture::SetData(void *data, void *palette) {
  ...
  if (HasData()) {
    mIsSwizzled =
        swizzle(reinterpret_cast<u8 *>(mpData), reinterpret_cast<u8 *>(data),
                bytes_per_row, mCorrectedHeight);
