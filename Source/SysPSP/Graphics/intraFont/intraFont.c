/*
 * intraFont.c
 * This file is used to display the PSP's internal font (pgf and bwfon firmware
 * files) intraFont Version 0.31 by BenHur -
...
unsigned long intraFontGetV(unsigned long n, unsigned char *p,
                            unsigned long *b) {
  unsigned long i, v = 0;
  for (i = 0; i < n; i++) {
    v += ((p[(*b) / 8] >> ((*b) % 8)) & 1) << i;
...
unsigned long *intraFontGetTableMem(const char *font_buffer,
                                    long int *buffer_pos, unsigned int filesize,
                                    unsigned long n_elements,
                                    unsigned long bp_element) {
  unsigned long len_table = ((n_elements * bp_element + 31) / 32) * 4;
  unsigned char *raw_table =
      (unsigned char *)malloc(len_table * sizeof(unsigned char));
  if (raw_table == NULL)
    return NULL;

  memcpy(raw_table, font_buffer + *buffer_pos,
         len_table * sizeof(unsigned char));
  *buffer_pos += len_table * sizeof(unsigned char);

  unsigned long *table =
      (unsigned long *)malloc(n_elements * sizeof(unsigned long));
  if (table == NULL) {
    free(raw_table);
    return NULL;
  }
...
unsigned long *intraFontGetTable(FILE *file, unsigned long n_elements,
                                 unsigned long bp_element) {
  unsigned long len_table = ((n_elements * bp_element + 31) / 32) * 4;
  unsigned char *raw_table =
      (unsigned char *)malloc(len_table * sizeof(unsigned char));
...
int intraFontGetBMP(intraFont *font, unsigned short id,
  ...
  if (glyph->width > 0 && glyph->height > 0) {
    if (!(glyph->flags & PGF_BMP_H_ROWS) !=
        !(glyph->flags &
          PGF_BMP_V_ROWS)) { // H_ROWS xor V_ROWS (real glyph, no overlay)
      ...
      if (font->fileType == FILETYPE_PGF) { // for compressed pgf format
        while (i < (glyph->width * glyph->height)) {
          ...
          for (j = 0; (j <= ((nibble < 8) ? (nibble) : (15 - nibble))) &&
                      (i < (glyph->width * glyph->height));
               j++) {
            if (nibble >= 8)
              value = intraFontGetV(4, font->fontdata, &b);
            if (glyph->flags & PGF_BMP_H_ROWS) {
            ...
            if ((font->texX + xx) & 1) {
              font->texture[((font->texX + xx) +
                             (font->texY + yy) * font->texWidth) >>
                            1] &= 0x0F;
              font->texture[((font->texX + xx) +
                             (font->texY + yy) * font->texWidth) >>
                            1] |= (value << 4);
      ...
      } else { // for uncompressed bwfon format (could use some
               // optimizations...)
        for (yy = 0; yy < glyph->height; yy++) {
          for (xx = 0; xx < glyph->width; xx++) {
            if (glyphtype & PGF_CHARGLYPH) {
              value = intraFontGetV(1, font->fontdata, &b) *
                      0x0f; // scale 1 bit/pix to 4 bit/pix
              /* Simple anti-aliasing/blur for black pixels. Unfortunately, does
              not improve the result... if ((value == 0) && (xx > 0) && (yy > 0)
              && (xx < (glyph->width-1)) && (yy < (glyph->height-1))) { b -= 19;
                value += intraFontGetV(1,font->fontdata,&b);
                value += intraFontGetV(1,font->fontdata,&b);
                value += intraFontGetV(1,font->fontdata,&b);
                b += 13;
              ...
              if ((font->texX + (7 - (xx & 7) + (xx & 248))) & 1) {
                font->texture[((font->texX + (7 - (xx & 7) + (xx & 248))) +
                               (font->texY + yy) * font->texWidth) >>
                              1] &= 0x0F;
                font->texture[((font->texX + (7 - (xx & 7) + (xx & 248))) +
                               (font->texY + yy) * font->texWidth) >>
      ...
      for (i = font->texX / 2; i < (font->texX + glyph->width + 1) / 2; i++) {
        ...
        font->texture[i + (font->texY + glyph->height) * font->texWidth / 2] =
            0;
      }
      for (i = font->texY - 1; i < (font->texY + glyph->height + 1); i++) {
        font->texture[((font->texX - 1) + (i * font->texWidth)) >> 1] &=
            (font->texX & 1) ? 0xF0 : 0x0F;
        font->texture[((font->texX + glyph->width) + (i * font->texWidth)) >>
                      1] &= ((font->texX + glyph->width) & 1) ? 0x0F : 0xF0;
      }
      font->texX +=
          glyph->width +
          1; // add empty gap to prevent interpolation artifacts from showing

      // mark dirty glyphs as uncached
      if (font->fileType == FILETYPE_PGF) { //...for PGF glyphs
        for (i = 0; i < font->n_chars; i++) {
          if ((font->glyph[i].flags & PGF_CACHED) &&
              (font->glyph[i].y == glyph->y)) {
            if ((font->glyph[i].x + font->glyph[i].width + 1) > glyph->x &&
                font->glyph[i].x < (glyph->x + glyph->width + 1)) {
        ...
        for (i = 0; i < font->n_chars; i++) {
          if ((font->glyphBW[i].flags & PGF_CACHED) &&
              (font->glyphBW[i].y == glyph->y)) {
            if ((font->glyphBW[i].x + font->glyph[0].width + 1) > glyph->x &&
                font->glyphBW[i].x < (glyph->x + glyph->width + 1)) {
      ...
      for (i = 0; i < font->n_shadows; i++) {
        if ((font->shadowGlyph[i].flags & PGF_CACHED) &&
            (font->shadowGlyph[i].y == glyph->y)) {
          if ((font->shadowGlyph[i].x + font->shadowGlyph[i].width + 1) >
                  glyph->x &&
              font->shadowGlyph[i].x < (glyph->x + glyph->width + 1)) {
  ...
  if ((font->fileType == FILETYPE_BWFON) &&
      (glyphtype & PGF_CHARGLYPH)) { // write properties back to BW-glyphs
...
int intraFontGetGlyph(unsigned char *data, unsigned long *b,
                      unsigned char glyphtype, signed long *advancemap,
                      Glyph *glyph) {
  if (glyphtype & PGF_CHARGLYPH) {
  ...
  if (glyph->flags & PGF_CHARGLYPH) {
    ...
    (*b) += 24 + ((glyph->flags & PGF_NO_EXTRA1) ? 0 : 56) +
            ((glyph->flags & PGF_NO_EXTRA2) ? 0 : 56) +
            ((glyph->flags & PGF_NO_EXTRA3)
                 ? 0
                 : 56); // skip to 6bits before end of header
...
unsigned short intraFontGetID(intraFont *font, cccUCS2 ucs) {
  ...
  for (j = 0; j < font->charmap_compr_len && !found; j++) {
    if ((ucs >= font->charmap_compr[j * 2]) &&
        (ucs < (font->charmap_compr[j * 2] + font->charmap_compr[j * 2 + 1]))) {
  ...
  // if (font->glyph[id].width == 0 || font->glyph[id].height == 0) return
  // 65535; //char has no valid glyph
  return id;
}
...
int intraFontPreCache(intraFont *font, unsigned int options) {
  ...
  for (i = 0; i < font->n_chars; i++) {
    ...
    if (font->texY > y || font->texYSize < font->glyph[i].height)
      font->texYSize =
          font->glyph[i].height; // minimize ysize after newline in cache(only
                                 // valid for precached glyphs)
    if (font->texY < y)
      return 0; // char did not fit into cache -> abort precache (should reset
                // cache and glyph.flags)
  }
  for (i = 0; i < font->n_shadows; i++) {
    y = font->texY;
  ...
  int index = 0, j;
  for (i = 0; i < font->n_chars; i++) {
    if ((font->glyph[i].flags & PGF_BMP_H_ROWS) &&
        (font->glyph[i].flags & PGF_BMP_V_ROWS)) {
      for (j = 0; j < 6; j++, index++) {
  ...
  if (index == 0) {
    ...
    font->fontdata =
        (unsigned char *)realloc(font->fontdata, index * sizeof(unsigned char));
  }
...
intraFont *intraFontLoadMem(const char *filename, const char *font_buffer,
                            unsigned int filesize, unsigned int options) {
  unsigned long i, j;
  ...
  static const unsigned short bw_charmap_compr[] = {
      0x00a4, 1,  0x00a7, 2,  0x00b0, 2,    0x00b7, 1,     0x00d7, 1,
      0x00e0, 2,  0x00e8, 3,  0x00ec, 2,    0x00f2, 2,     0x00f7, 1,
      0x00f9, 2,  0x00fc, 1,  0x0101, 1,    0x0113, 1,     0x011b, 1,
      0x012b, 1,  0x0144, 1,  0x0148, 1,    0x014d, 1,     0x016b, 1,
      0x01ce, 1,  0x01d0, 1,  0x01d2, 1,    0x01d4, 1,     0x01d6, 1,
      0x01d8, 1,  0x01da, 1,  0x01dc, 1,    0x0251, 1,     0x0261, 1,
      0x02c7, 1,  0x02c9, 3,  0x02d9, 1,    0x0391, 17,    0x03a3, 7,
      0x03b1, 17, 0x03c3, 7,  0x0401, 1,    0x0410, 64,    0x0451, 1,
      0x2010, 1,  0x2013, 4,  0x2018, 2,    0x201c, 2,     0x2025, 2,
      0x2030, 1,  0x2032, 2,  0x2035, 1,    0x203b, 1,     0x20ac, 1,
      0x2103, 1,  0x2105, 1,  0x2109, 1,    0x2116, 1,     0x2121, 1,
      0x2160, 12, 0x2170, 10, 0x2190, 4,    0x2196, 4,     0x2208, 1,
      0x220f, 1,  0x2211, 1,  0x2215, 1,    0x221a, 1,     0x221d, 4,
      0x2223, 1,  0x2225, 1,  0x2227, 5,    0x222e, 1,     0x2234, 4,
      0x223d, 1,  0x2248, 1,  0x224c, 1,    0x2252, 1,     0x2260, 2,
      0x2264, 4,  0x226e, 2,  0x2295, 1,    0x2299, 1,     0x22a5, 1,
      0x22bf, 1,  0x2312, 1,  0x2460, 10,   0x2474, 40,    0x2500, 76,
      0x2550, 36, 0x2581, 15, 0x2593, 3,    0x25a0, 2,     0x25b2, 2,
      0x25bc, 2,  0x25c6, 2,  0x25cb, 1,    0x25ce, 2,     0x25e2, 4,
      0x2605, 2,  0x2609, 1,  0x2640, 1,    0x2642, 1,     0x2e81, 1,
      0x2e84, 1,  0x2e88, 1,  0x2e8b, 2,    0x2e97, 1,     0x2ea7, 1,
      0x2eaa, 1,  0x2eae, 1,  0x2eb3, 1,    0x2eb6, 2,     0x2ebb, 1,
      0x2eca, 1,  0x2ff0, 12, 0x3000, 4,    0x3005, 19,    0x301d, 2,
      0x3021, 9,  0x303e, 1,  0x3041, 83,   0x309b, 4,     0x30a1, 86,
      0x30fc, 3,  0x3105, 37, 0x3220, 10,   0x3231, 1,     0x32a3, 1,
      0x338e, 2,  0x339c, 3,  0x33a1, 1,    0x33c4, 1,     0x33ce, 1,
      0x33d1, 2,  0x33d5, 1,  0x3400, 6582, 0x4e00, 20902, 0xe78d, 10,
      0xe7c7, 2,  0xe816, 3,  0xe81e, 1,    0xe826, 1,     0xe82b, 2,
      0xe831, 2,  0xe83b, 1,  0xe843, 1,    0xe854, 2,     0xe864, 1,
      0xf92c, 1,  0xf979, 1,  0xf995, 1,    0xf9e7, 1,     0xf9f1, 1,
      0xfa0c, 4,  0xfa11, 1,  0xfa13, 2,    0xfa18, 1,     0xfa1f, 3,
      0xfa23, 2,  0xfa27, 3,  0xfe30, 2,    0xfe33, 18,    0xfe49, 10,
      0xfe54, 4,  0xfe59, 14, 0xfe68, 4,    0xff01, 94,    0xffe0, 6};
  static const unsigned char bw_shadow[] = {
      0x10, 0x11, 0x11, 0x01, 0x10, 0x22, 0x22, 0x01, 0x21, 0x43,
      0x34, 0x12, 0x31, 0x75, 0x57, 0x13, 0x31, 0x86, 0x68, 0x13,
      0x31, 0x86, 0x68, 0x13, 0x31, 0x75, 0x57, 0x13, 0x21, 0x43,
      0x34, 0x12, 0x10, 0x22, 0x22, 0x01, 0x10, 0x11, 0x11, 0x01};
  ...
  if ((strncmp(header.pgf_id, "PGF0", 4) == 0) && (header.version == 6) &&
      (header.revision == 2 || header.revision == 3) &&
      (header.charmap_len <= 65535) && (header.charptr_len <= 65535) &&
      (header.charmap_bpe <= 32) && (header.charptr_bpe <= 32) &&
      (header.charmap_min <= header.charmap_max) &&
      (header.shadowmap_len <= 511) && (header.shadowmap_bpe <= 16)) {
    font->fileType = FILETYPE_PGF;
  } else if (filesize == 1023372) { // only known size for bwfon -> refuse to
                                    // work for modified sizes
  ...
  if (font->fileType == FILETYPE_PGF) {
    ...
    font->charmap_compr = (unsigned short *)malloc(font->charmap_compr_len *
                                                   sizeof(unsigned short) * 2);
    font->charmap =
        (unsigned short *)malloc(header.charmap_len * sizeof(unsigned short));
    if (!font->glyph || !font->shadowGlyph || !font->charmap_compr ||
        !font->charmap) {
    ...
    memset(font->glyph, 0,
           font->n_chars *
               sizeof(Glyph)); // make sure glyphs are initialized with 0
    ...
    font->charmap_compr = (unsigned short *)bw_charmap_compr; // static for
                                                              // bwfon
    ...
    memset(font->glyphBW, 0,
           font->n_chars *
               sizeof(GlyphBW)); // make sure glyphBWs are initialized with 0
  }
  ...
  if ((options & INTRAFONT_CACHE_ASCII))
    font->options -= INTRAFONT_CACHE_ASCII; // pre-cached texture enabled at the
                                            // end of font-load
  ...
  font->texture =
      (unsigned char *)memalign(16, font->texWidth * font->texHeight >> 1);
  ...
  if (font->fileType == FILETYPE_PGF) {
    ...
    buffer_pos =
        header.header_len +
        (header.table1_len + header.table2_len + header.table3_len) * 8;
    signed long *advancemap =
        (signed long *)malloc(header.advance_len * sizeof(signed long) * 2);
    if (!advancemap) {
      intraFontUnload(font);
      return NULL;
    }
    memcpy(advancemap, font_buffer + buffer_pos,
           header.advance_len * sizeof(signed long) * 2);
    buffer_pos += header.advance_len * sizeof(signed long) * 2;

    // read shadowmap
    unsigned long *ucs_shadowmap =
        intraFontGetTableMem(font_buffer, &buffer_pos, filesize,
    ...
    if (header.revision == 3) {
      memcpy(font->charmap_compr, font_buffer + buffer_pos,
             font->charmap_compr_len * sizeof(unsigned short) * 2);
    ...
    if (header.charmap_bpe == 16) { // read directly from file...
      memcpy(font->charmap, font_buffer + buffer_pos,
             header.charmap_len * sizeof(unsigned short));
      buffer_pos += header.charmap_len * sizeof(unsigned short);
    } else {
      unsigned long *id_charmap =
          intraFontGetTableMem(font_buffer, &buffer_pos, filesize,
                               header.charmap_len, header.charmap_bpe);
      if (id_charmap == NULL) {
        free(advancemap);
        free(ucs_shadowmap);
        intraFontUnload(font);
      ...
      for (i = 0; i < header.charmap_len; i++) {
        font->charmap[i] =
            (unsigned short)((id_charmap[i] < font->n_chars) ? id_charmap[i]
                                                             : 65535);
      }
      free(id_charmap);
    }
    ...
    unsigned long *charptr =
        intraFontGetTableMem(font_buffer, &buffer_pos, filesize,
                             header.charptr_len, header.charptr_bpe);
    if (charptr == NULL) {
      free(advancemap);
      free(ucs_shadowmap);
      intraFontUnload(font);
    ...
    font->fontdata =
        (unsigned char *)malloc(len_fontdata * sizeof(unsigned char));
    ...
    memcpy(font->fontdata, font_buffer + buffer_pos,
           len_fontdata * sizeof(unsigned char));
    ...
    if ((options & PGF_CACHE_MASK) == INTRAFONT_CACHE_ASCII) {
      font->n_chars = 1; // assume there's at least one char (needed for
                         // intraFontGetID to work properly)
      for (i = 0; i < 128; i++) {
      ...
      font->glyph =
          (Glyph *)realloc(font->glyph, font->n_chars * sizeof(Glyph));
      font->charmap_compr[1] = 128 - font->charmap_compr[0];
      font->charmap = (unsigned short *)realloc(
          font->charmap, font->charmap_compr[1] * sizeof(unsigned short));
    }
    ...
    for (i = 0; i < font->n_chars; i++) {
      j = charptr[i] * 4 * 8;
      if (!intraFontGetGlyph(font->fontdata, &j, PGF_CHARGLYPH, advancemap,
                             &(font->glyph[i]))) {
        free(advancemap);
        free(ucs_shadowmap);
        free(charptr);
      ...
      if (!(font->glyph[i].flags & PGF_BMP_H_ROWS) !=
          !(font->glyph[i].flags &
            PGF_BMP_V_ROWS)) { // H_ROWS xor V_ROWS (real glyph, not overlay)
    ...
    for (i = 0; i < font->n_chars; i++) {
      ...
      if (char_id < font->n_chars &&
          font->shadowGlyph[shadow_id].shadowID ==
              0) { // valid char and shadow glyph not yet loaded
        j = charptr[char_id] * 4 * 8;
        if (!intraFontGetGlyph(font->fontdata, &j, PGF_SHADOWGLYPH, NULL,
                               &(font->shadowGlyph[shadow_id]))) {
          free(advancemap);
          free(ucs_shadowmap);
          free(charptr);
          intraFontUnload(font);
        ...
        if (!(font->shadowGlyph[shadow_id].flags & PGF_BMP_H_ROWS) !=
            !(font->shadowGlyph[shadow_id].flags &
              PGF_BMP_V_ROWS)) { // H_ROWS xor V_ROWS (real glyph, not overlay)
          if (font->shadowGlyph[shadow_id].height > font->texYSize)
            font->texYSize =
                font->shadowGlyph[shadow_id].height; // find max glyph height
    ...
    // cache chars, swizzle texture and free unneeded stuff (if
    // INTRAFONT_CACHE_ALL or _ASCII)
    sceKernelDcacheWritebackAll();
    if ((options & INTRAFONT_CACHE_ASCII) &&
        ((y + ysize + 1) <=
         font->texHeight)) { // cache all and does it fit into cache?
    ...
    buffer_pos = 0;
    font->fontdata =
        (unsigned char *)malloc((filesize + 40) * sizeof(unsigned char));
    if (font->fontdata == NULL) {
      intraFontUnload(font);
      return NULL;
    }
    ...
    // count ascii chars and reduce mem required: no ascii chars in bwfon ->
    // abort
    if ((options & PGF_CACHE_MASK) == INTRAFONT_CACHE_ASCII) {
      intraFontUnload(font);
      return NULL;
    ...
    // populate chars and count space used in cache to prebuffer all chars: not
    // needed/available -> skip populate shadows and count space (only for chars
    // needed): not needed/available -> skip free temp stuff: not needed -> skip
    // cache chars, swizzle texture and free unneeded stuff (if
    // INTRAFONT_CACHE_ALL or _ASCII): not available ->skip
  }
...
intraFont *intraFontLoad(const char *filename, unsigned int options) {
  ...
  if (font->fileType == FILETYPE_PGF) {
    ...
    if (fread(advancemap, header.advance_len * sizeof(signed long) * 2, 1,
              file) != 1) {
      free(advancemap);
      fclose(file);
      intraFontUnload(font);
    ...
    // read shadowmap
    unsigned long *ucs_shadowmap =
        intraFontGetTable(file, header.shadowmap_len, header.shadowmap_bpe);
    if (ucs_shadowmap == NULL) {
      free(advancemap);
      fclose(file);
      intraFontUnload(font);
    ...
    // version 6.3 charmap compression
    if (header.revision == 3) {
      if (fread(font->charmap_compr,
                font->charmap_compr_len * sizeof(unsigned short) * 2, 1,
                file) != 1) {
        free(advancemap);
        free(ucs_shadowmap);
        fclose(file);
    ...
    // read charmap
    if (header.charmap_bpe == 16) { // read directly from file...
      if (fread(font->charmap, header.charmap_len * sizeof(unsigned short), 1,
                file) != 1) {
        free(advancemap);
        free(ucs_shadowmap);
        fclose(file);
      ...
      unsigned long *id_charmap =
          intraFontGetTable(file, header.charmap_len, header.charmap_bpe);
      if (id_charmap == NULL) {
        free(advancemap);
        free(ucs_shadowmap);
        fclose(file);
    ...
    // read charptr
    unsigned long *charptr =
        intraFontGetTable(file, header.charptr_len, header.charptr_bpe);
    if (charptr == NULL) {
      free(advancemap);
      free(ucs_shadowmap);
      fclose(file);
    ...
    if (fread(font->fontdata, len_fontdata * sizeof(unsigned char), 1, file) !=
        1) {
      free(advancemap);
      free(ucs_shadowmap);
      free(charptr);
...
void intraFontActivate(intraFont *font) {
  ...
  sceGuTexMode(GU_PSM_T4, 0, 0,
               (font->options & INTRAFONT_CACHE_ASCII) ? 1 : 0);
  sceGuTexImage(0, font->texWidth, font->texWidth, font->texWidth,
                font->texture);
...
void intraFontSetStyle(intraFont *font, float size, unsigned int color,
                       unsigned int shadowColor, float angle,
  ...
  font->options = (options & PGF_OPTIONS_MASK) |
                  (font->options & PGF_STRING_MASK) |
                  (font->options & PGF_CACHE_MASK);
...
void intraFontSetEncoding(intraFont *font, unsigned int options) {
  if (!font)
    return;
  font->options = (font->options & PGF_OPTIONS_MASK) |
                  (options & PGF_STRING_MASK) |
                  (font->options & PGF_CACHE_MASK);
}
...
float intraFontPrintEx(intraFont *font, float x, float y, const char *text,
                       int length) {
  return intraFontPrintColumnEx(font, x, y, 0.f, text, length);
}

float intraFontPrintColumn(intraFont *font, float x, float y, float column,
                           const char *text) {
  if (!font)
    return x;
  int length = cccStrlenCode((cccCode *)text, font->options / 0x00010000);
...
float intraFontPrintColumnEx(intraFont *font, float x, float y, float column,
...
#define LOCAL_BUFFER_LENGTH 256
  ...
  //->UCS2 conversion
  length = cccCodetoUCS2(ucs2_text, length, (cccCode *)text,
                         font->options / 0x00010000);
  ...
  if (column >= 0) {
    ...
    x = intraFontMeasureTextUCS2Ex(
        font, ucs2_text, length); //(hack to share local buffer between
                                  //intraFontPrint and intraFontMeasure)
  }
...
float intraFontPrintUCS2(intraFont *font, float x, float y,
                         const cccUCS2 *text) {
  return intraFontPrintColumnUCS2Ex(font, x, y, 0.f, text, cccStrlenUCS2(text));
}

float intraFontPrintUCS2Ex(intraFont *font, float x, float y,
                           const cccUCS2 *text, int length) {
  return intraFontPrintColumnUCS2Ex(font, x, y, 0.f, text, length);
}

float intraFontPrintColumnUCS2(intraFont *font, float x, float y, float column,
                               const cccUCS2 *text) {
  return intraFontPrintColumnUCS2Ex(font, x, y, 0.f, text, cccStrlenUCS2(text));
}

float intraFontPrintColumnUCS2Ex(intraFont *font, float x, float y,
                                 float column, const cccUCS2 *text,
                                 int length) {
  if (!text || length <= 0 || !font)
    return x;

  // for scrolling: if text contains '\n', replace with spaces and call
  // intraFontColumnUCS2Ex again
  int i;
  if (font->options & INTRAFONT_SCROLL_LEFT) {
    for (i = 0; i < length; i++) {
      if (text[i] == '\n') {
    ...
    for (i = 0; i < length; i++) {
      ...
      if (char_id < font->n_chars) {
        if (font->fileType == FILETYPE_PGF) { // PGF-file
          if ((font->glyph[char_id].flags & PGF_BMP_OVERLAY) ==
              PGF_BMP_OVERLAY) { // overlay glyph?
            for (j = 0; j < 3; j++) {
              subucs2 =
                  font->fontdata[(font->glyph[char_id].ptr) + j * 2] +
                  font->fontdata[(font->glyph[char_id].ptr) + j * 2 + 1] * 256;
              if (subucs2) {
          ...
          if (n_glyphs > last_n_glyphs) {
            n_sglyphs++; // shadow
            if (!(font->shadowGlyph[font->glyph[char_id].shadowID].flags &
                  PGF_CACHED)) {
              if (intraFontGetBMP(font, font->glyph[char_id].shadowID,
                                  PGF_SHADOWGLYPH))
                changed = 1;
            }
  ...
  if (changed)
    return x; // not all chars fit into texture -> abort (better solution: split
              // up string and call intraFontPrintUCS2 twice)

  // reserve memory in displaylist (switch between GU_TRIANGLES and GU_SPRITES)
  v = sceGuGetMemory((font->isRotated ? 6 : 2) * (n_glyphs + n_sglyphs) *
                     sizeof(fontVertex));

  int s_index = 0, c_index = n_sglyphs,
      last_c_index = n_sglyphs; // index for shadow and character/overlay glyphs
  for (i = 0; i < length; i++) {
    ...
    if ((i == 0) || (text[i] == '\n') ||
      ...
      if (column > 0.f) {
        if (font->options &
            INTRAFONT_SCROLL_LEFT) { // scrolling (any direction) requested
          eol = length;
          scroll = 1;
          left = ((float)((int)x));
          union {
          ...
          count = ux.i - uleft.i;
          textwidth =
              intraFontMeasureTextUCS2Ex(font, text + i, length - i) + 1;
          if (textwidth > column) { // scrolling really required

            switch (font->options & PGF_SCROLL_MASK) {

            case INTRAFONT_SCROLL_LEFT: // scroll left
              sceGuScissor(left - 2, 0, left + column + 4,
                           272); // limit to column width
              if (count < 60) {
                // show initial text for 1s
              } else if (count < (textwidth + 90)) {
                left -= (count - 60); // scroll left for (textwidth/60) s (speed
                                      // = 1 pix/frame) and disappear for 0.5s
              } else if (count < (textwidth + 120)) {
                color = (color & 0x00FFFFFF) |
                        ((((color >> 24) * (count - textwidth - 90)) / 30)
                         << 24); // fade-in in 0.5s
                shadowColor =
                    (shadowColor & 0x00FFFFFF) |
                    ((((shadowColor >> 24) * (count - textwidth - 90)) / 30)
                     << 24);
              } else {
            ...
            case INTRAFONT_SCROLL_SEESAW: // scroll left and right
              sceGuScissor(left - column / 2 - 2, 0, left + column / 2 + 4,
                           272); // limit to column width
              textwidth -= column;
              if (count < 60) {
                ...
                left -= column / 2 + 2 * textwidth - count + 120; // scroll
                                                                  // right
              } else {
                ux.f = left; // reset counter
                left -= column / 2;
            ...
            case INTRAFONT_SCROLL_RIGHT: // scroll right
              sceGuScissor(left - column - 2, 0, left + 4,
                           272); // limit to column width
              if (count < 60) {
                left -= textwidth; // show initial text for 1s
              } else if (count < (textwidth + 90)) {
                left -= textwidth - count +
                        60; // scroll right for (textwidth/60) s (speed = 1
                            // pix/frame) and disappear for 0.5s
              } else if (count < (textwidth + 120)) {
                left -= textwidth;
                color = (color & 0x00FFFFFF) |
                        ((((color >> 24) * (count - textwidth - 90)) / 30)
          ...
          for (j = i; j < length; j++) {
            ...
            if (intraFontMeasureTextUCS2Ex(font, text + i, j + 1 - i) >
                column) { // line too long -> line-break
              if (eol < 0)
                eol = j; // break in the middle of the word
              if (n_spaces > 0)
                fill = (column -
                        intraFontMeasureTextUCS2Ex(font, text + i, eol - i)) /
                       ((float)n_spaces);
              break;
            }
          }
        ...
        if (text[i] == '\n') {
          if ((font->options & PGF_ALIGN_MASK) == INTRAFONT_ALIGN_RIGHT)
            left -=
                intraFontMeasureTextUCS2Ex(font, text + i + 1, length - i - 1);
          if ((font->options & PGF_ALIGN_MASK) == INTRAFONT_ALIGN_CENTER)
            left -=
                intraFontMeasureTextUCS2Ex(font, text + i + 1, length - i - 1) /
                2.0;
          ...
          if ((font->options & PGF_ALIGN_MASK) == INTRAFONT_ALIGN_CENTER)
            left -=
                intraFontMeasureTextUCS2Ex(font, text + i, length - i) / 2.0;
        }
      }
    ...
    if (char_id < font->n_chars) {
      ...
      glyph_ptr = (font->fileType == FILETYPE_PGF)
                      ? char_id
                      : 0; // for fields not covered in GlyphBW (advance,...)
      shadowGlyph_ptr = (font->fileType == FILETYPE_PGF)
                            ? font->glyph[glyph_ptr].shadowID
                            : 0;

      // center glyphs for monospace
      if (font->options & INTRAFONT_WIDTH_FIX) {
        width += (((float)(font->options & PGF_WIDTH_MASK)) / 2.0f -
                  ((float)font->glyph[glyph_ptr].advance) / 8.0f) *
                 glyphscale;
      }
      ...
      if (c_index > last_c_index) {
        ...
        xl =
            left + width +
            shadowGlyph->left * glyphscale * 64.0f / ((float)font->shadowscale);
        xr = xl + shadowGlyph->width * glyphscale * 64.0f /
                      ((float)font->shadowscale);
        yu = top + height -
             shadowGlyph->top * glyphscale * 64.0f / ((float)font->shadowscale);
        yd = yu + shadowGlyph->height * glyphscale * 64.0f /
                      ((float)font->shadowscale);
        // Tex coords
      ...
      if ((text[i] == 32) &&
          ((font->options & INTRAFONT_ALIGN_FULL) == INTRAFONT_ALIGN_FULL))
        width += fill;

    } else {
      if (font->altFont) {
        unsigned int altOptions = (font->altFont)->options;
        (font->altFont)->options =
            altOptions & (PGF_WIDTH_MASK + PGF_CACHE_MASK);
        width += intraFontPrintColumnUCS2Ex(font->altFont, left + width,
                                            top + height, 0.f, text + i, 1) -
                 (left + width);
  ...
  sceGuDrawArray((font->isRotated ? GU_TRIANGLES : GU_SPRITES),
                 GU_TEXTURE_32BITF | GU_COLOR_8888 | GU_VERTEX_32BITF |
                     GU_TRANSFORM_2D,
                 (n_glyphs + n_sglyphs) * (font->isRotated ? 6 : 2), 0, v);
  if (font->fileType == FILETYPE_BWFON) // draw chars again without shadows for
                                        // improved readability
    sceGuDrawArray((font->isRotated ? GU_TRIANGLES : GU_SPRITES),
                   GU_TEXTURE_32BITF | GU_COLOR_8888 | GU_VERTEX_32BITF |
                       GU_TRANSFORM_2D,
                   n_glyphs * (font->isRotated ? 6 : 2), 0,
  ...
  if (scroll == 1) {
    sceGuScissor(
        0, 0, 480,
        272); // reset window to whole screen (test was previously enabled)
...
float intraFontMeasureTextEx(intraFont *font, const char *text, int length) {
  return intraFontPrintColumnEx(
      font, 0.f, 0.f, -1.0f, text,
      length); // explanation: intraFontPrintColumnEx does the String -> UCS2
               // conversation, but a negative column width triggers measurement
               // without drawing
}
...
float intraFontMeasureTextUCS2Ex(intraFont *font, const cccUCS2 *text,
                                 int length) {
  if (!text || length <= 0 || !font)
    return 0.f;
  ...
  for (i = 0; (i < length) && (text[i] != '\n');
    ...
    if (char_id < font->n_chars) {
      unsigned short glyph_ptr =
          (font->fileType == FILETYPE_PGF)
              ? char_id
              : 0; // for fields not covered in GlyphBW (advance,...)
      x += (font->options & INTRAFONT_WIDTH_FIX)
               ? (font->options & PGF_WIDTH_MASK) * font->size
               : (font->glyph[glyph_ptr].advance) * font->size * 0.25f;
    } else {
      x += intraFontMeasureTextUCS2Ex(
          font->altFont, text + i,
          1); // try alternative font if char does not exist in current font
    }
  }
