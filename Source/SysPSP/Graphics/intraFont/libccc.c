...
/* the following code is adapted from libLZR 0.11 (see
 * http://www.psp-programming.com/benhur) */

void cccLZRFillBuffer(unsigned int *test_mask, unsigned int *mask,
                      unsigned int *buffer, unsigned char **next_in) {
...
char cccLZRNextBit(unsigned char *buf_ptr1, int *number,
                   unsigned int *test_mask, unsigned int *mask,
                   unsigned int *buffer, unsigned char **next_in) {
  /* extract and return next bit of information from in stream, update buffer
   * and mask */
...
int cccLZRGetNumber(signed char n_bits, unsigned char *buf_ptr, char inc,
                    char *flag, unsigned int *mask, unsigned int *buffer,
...
int cccLZRDecompress(void *out, unsigned int out_capacity, void *in,
                     void *in_end) {
  unsigned char **next_in, *tmp, *next_out, *out_end, *next_seq, *seq_end,
      *buf_ptr1, *buf_ptr2;
  ...
  while (1) {
    ...
    if (!cccLZRNextBit(buf_ptr1, 0, &mask, &mask, &buffer, next_in)) {
      ...
      if (next_out == out_end)
        return CCC_ERROR_BUFFER_SIZE;
      buf_ptr1 = buf +
                 (((((((int)(next_out - (unsigned char *)out)) & 0x07) << 8) +
                    last_char) >>
                   type) &
                  0x07) *
                     0xFF -
      ...
      *next_out++ = j;
      ...
      if ((flag != 0) || (n_bits >= 0)) {
        buf_ptr1 =
            buf + (n_bits << 5) +
            (((((int)(next_out - (unsigned char *)out)) << n_bits) & 0x03)
             << 3) +
            buf_off + 2552;
        seq_len = cccLZRGetNumber(n_bits, buf_ptr1, 8, &flag, &mask, &buffer,
                                  next_in);
        if (seq_len == 0xFF)
          return next_out - (unsigned char *)out; // end of data stream
        if ((flag != 0) || (n_bits > 0)) {
          buf_ptr2 += 56;
          j = 352;
        }
      ...
      do {
        n_bits = (i << 4) - j;
        flag = cccLZRNextBit(buf_ptr2 + (i << 3), &i, &mask, &mask, &buffer,
                             next_in);
      } while (n_bits < 0);
      ...
      if (flag || (n_bits > 0)) {
        if (!flag)
          n_bits -= 8;
        seq_off = cccLZRGetNumber(n_bits / 8, buf + n_bits + 2344, 1, &flag,
                                  &mask, &buffer, next_in);
      } else {
        seq_off = 1;
      }
      ...
      if (next_seq < (unsigned char *)out)
        return CCC_ERROR_INPUT_STREAM;
...
/* end of code adapted from libLZR 0.11 (see
 * http://www.psp-programming.com/benhur) */
...
int cccLoadTable(const char *filename, unsigned char cp) {
  ...
  while (header[0]) {
    if ((cp == 0) || (cp == header[0])) {
      ...
      int ret =
          cccLZRDecompress(table, header[4], table_data + header[2], NULL);
      if (ret < 0) {
        free(table_data);
        free(table);
        return ret;
...
void cccInit(void) {
  if (!cccInitialized) {
    ...
    // cccLoadTable("flash0:/vsh/etc/cptbl.dat", 0); //this would load all
    // tables available, but it's done on demand
    cccInitialized = 1;
  }
}
...
int cccStrlenSJIS(cccCode const *str) {
  ...
  while (str[i]) {
    length++;
    i +=
        (str[i] <= 0x80 || (str[i] >= 0xA0 && str[i] <= 0xDF) || str[i] >= 0xFD)
            ? 1
            : 2; // single or double byte
  }
  return length;
}
...
int cccStrlenKOR(cccCode const *str) { return cccStrlenGBK(str); }

int cccStrlenBIG5(cccCode const *str) { return cccStrlenGBK(str); }
...
int cccStrlenUTF8(cccCode const *str) {
  ...
  while (str[i]) {
    if (str[i] <= 0x7F) {
      ...
      i++;
    } // part of multi-byte or overlong encoding ->ignore
    else if (str[i] <= 0xDF) {
      i += 2;
      length++;
    } // 2-byte
      ...
      i++;
    } // 4-byte, restricted or invalid range ->ignore
  }
  return length;
}
...
int cccStrlenCode(cccCode const *str, unsigned char cp) {
  ...
  case CCC_CP932:
    length = cccStrlenSJIS(str);
    break;
  case CCC_CP936:
    length = cccStrlenGBK(str);
    break;
  case CCC_CP949:
    length = cccStrlenKOR(str);
    break;
  case CCC_CP950:
    length = cccStrlenBIG5(str);
    break;
  case CCC_CPUTF8:
    length = cccStrlenUTF8(str);
    break;
  /* single byte character sets */
  default:
    length = cccStrlen(str);
    break;
  }
  return length;
}
...
int cccSJIStoUCS2(cccUCS2 *dst, size_t count, cccCode const *str) {
  ...
  if (!(__table_ptr__[CCC_CP932]))
    cccLoadTable("flash0:/vsh/etc/cptbl.dat", CCC_CP932);
    ...
    while (str[i] && length < count) {
      ...
      for (j = 1; (j <= header[2]) && (id < 0); j++) {
        if ((code >= header[j * 3]) && (code <= header[j * 3 + 1])) {
          ...
          if (j == 2)
            code =
                0x0200 * str[i] - 0xE100 - ((str[i] >= 0xE0) ? 0x8000 : 0) +
                str[i + 1] +
                ((str[i + 1] <= 0x7E) ? -0x1F
                                      : ((str[i + 1] >= 0x9F) ? 0x82 : -0x20));
        }
      }
      dst[length++] = (id < 0) ? __error_char_ucs2__ : SJIStoUCS2[id];
      i += (str[i] <= 0x80 || (str[i] >= 0xA0 && str[i] <= 0xDF) ||
            str[i] >= 0xFD)
               ? 1
               : 2; // single or double byte
    }
...
int cccGBKtoUCS2(cccUCS2 *dst, size_t count, cccCode const *str) {
  ...
  if (!cccInitialized)
    cccInit();
  if (!(__table_ptr__[CCC_CP936]))
    cccLoadTable("flash0:/vsh/etc/cptbl.dat", CCC_CP936);
  ...
  while (str[i] && length < count) {
    if (str[i] <= 0x7f) {
      ...
      if (__table_ptr__[CCC_CP936]) { // table is present
        ...
        for (entry = (unsigned char *)(__table_ptr__[CCC_CP936]);
             (entry < (unsigned char *)(__table_end__[CCC_CP936])) &&
             ((entry[0] + 0x100 * entry[1] + entry[4]) <= code);
             entry += 5)
          ;
        if ((entry >= (unsigned char *)(__table_end__[CCC_CP936])) ||
            (code < entry[0] + 0x100 * entry[1])) {
          dst[length] = __error_char_ucs2__;
        } else {
          dst[length] = entry[2] + 0x100 * entry[3] +
                        (code - entry[0] - 0x100 * entry[1]);
        }
      } else {
        dst[length] = __error_char_ucs2__;
...
int cccKORtoUCS2(cccUCS2 *dst, size_t count, cccCode const *str) {
  ...
  if (!cccInitialized)
    cccInit();
  if (!(__table_ptr__[CCC_CP949]))
    cccLoadTable("flash0:/vsh/etc/cptbl.dat", CCC_CP949);
  ...
  while (str[i] && length < count) {
    if (str[i] <= 0x7f) {
      ...
      if (__table_ptr__[CCC_CP949]) { // table is present
        code = 0x0100 * str[i] + str[i + 1];
        for (entry = (unsigned char *)(__table_ptr__[CCC_CP949]);
             (entry < (unsigned char *)(__table_end__[CCC_CP949])) &&
             ((entry[0] + 0x100 * entry[1] + entry[4]) <= code);
             entry += 5)
          ;
        if ((entry >= (unsigned char *)(__table_end__[CCC_CP949])) ||
            (code < entry[0] + 0x100 * entry[1])) {
          dst[length] = __error_char_ucs2__;
        } else {
          dst[length] = entry[2] + 0x100 * entry[3] +
...
int cccBIG5toUCS2(cccUCS2 *dst, size_t count, cccCode const *str) {
  ...
  if (!cccInitialized)
    cccInit();
  if (!(__table_ptr__[CCC_CP950]))
    cccLoadTable("flash0:/vsh/etc/cptbl.dat", CCC_CP950);
  ...
  while (str[i] && length < count) {
    if (str[i] <= 0x7f) {
      ...
      if (__table_ptr__[CCC_CP950]) { // table is present
        code = 0x0100 * str[i] + str[i + 1];
        for (entry = (cccTblEntry *)(__table_ptr__[CCC_CP950]);
             (entry < (cccTblEntry *)(__table_end__[CCC_CP950])) &&
             (entry->code < code);
             entry++)
          ;
        if ((entry >= (cccTblEntry *)(__table_end__[CCC_CP950])) ||
            (code < entry->code)) {
          dst[length] = __error_char_ucs2__;
        } else {
  ...
  while (str[i] && length < count) {
    if (str[i] <= 0x7FU) { // ASCII
      ...
      length++;
               ...
               0xC1U) { // part of multi-byte or overlong encoding ->ignore
      i++;
      ...
      dst[length] = ((str[i] & 0x001fu) << 12) | ((str[i + 1] & 0x003fu) << 6) |
                    (str[i + 2] & 0x003fu);
      i += 3;
      length++;
    } else
...
int cccCodetoUCS2(cccUCS2 *dst, size_t count, cccCode const *str,
                  unsigned char cp) {
  if (!str || !dst)
    return 0;
  ...
  // multi-byte character sets
  case CCC_CP932:
    length = cccSJIStoUCS2(dst, count, str);
    break;
  case CCC_CP936:
    length = cccGBKtoUCS2(dst, count, str);
    break;
  case CCC_CP949:
    length = cccKORtoUCS2(dst, count, str);
    break;
  case CCC_CP950:
    length = cccBIG5toUCS2(dst, count, str);
    break;
  case CCC_CPUTF8:
    length = cccUTF8toUCS2(dst, count, str);
    break;
  // single-byte character sets
  default:
    if (cp < CCC_N_CP) { // codepage in range?
      if (!cccInitialized)
        cccInit();
      if (!(__table_ptr__[cp]) && (cp > 0))
        cccLoadTable("flash0:/vsh/etc/cptbl.dat", cp);
      while (str[length] &&
             length <
                 count) { // conversion: ASCII (if ASCII) or LUT-value (if LUT
                          // exists) or error_char (if LUT doesn't exist)
        if (str[length] < 0x80) {
          dst[length] = (cccUCS2)str[length];
        } else {
          dst[length] =
              ((__table_ptr__[cp])
                   ? ((cccUCS2 *)(__table_ptr__[cp]))[str[length] - 0x80]
                   : __error_char_ucs2__);
          if (!dst[length])
            dst[length] = __error_char_ucs2__;
        }
