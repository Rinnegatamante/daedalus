...
pgeFont *pgeFontLoad(const char *filename, unsigned int fontSize,
  ...
  if ((textureSize < PGE_FONT_TEXTURE_MIN_SIZE) ||
      (textureSize > PGE_FONT_TEXTURE_MAX_SIZE))
    return NULL;
  ...
  for (n = 0; n < charCount; n++) {
    ...
    for (yy = 0; yy < gi[n].glyph.height; yy++) {
      xx = 0;
      if (x & 1) {
        font->texture[(x + (y + yy) * textureSize) >> 1] |=
            (slot->bitmap.buffer[yy * slot->bitmap.width] & 0xF0);
        xx++;
      }
      for (; xx < gi[n].glyph.width; xx += 2) {
        if (xx + 1 < gi[n].glyph.width)
          font->texture[((x + xx) + (y + yy) * textureSize) >> 1] =
              (slot->bitmap.buffer[yy * slot->bitmap.width + xx] >> 4) |
              (slot->bitmap.buffer[yy * slot->bitmap.width + xx + 1] & 0xF0);
        else
          font->texture[((x + xx) + (y + yy) * textureSize) >> 1] =
              (slot->bitmap.buffer[yy * slot->bitmap.width + xx] >> 4);
      }
