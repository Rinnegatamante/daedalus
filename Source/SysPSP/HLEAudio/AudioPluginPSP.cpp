...
static const u32 kAudioBufferSize{
    1024 * 2}; // OSX uses a circular buffer length, 1024 * 1024
...
public:
  SAddSamplesJob(CAudioBuffer *buffer, const Sample *samples, u32 num_samples,
      ...
      : SJob(), mBuffer(buffer), mSamples(samples), mNumSamples(num_samples),
...
void AudioPluginPSP::AddBuffer(u8 *start, u32 length) {
  ...
  case APM_ENABLED_SYNC: {
    mAudioBufferUncached->AddSamples(reinterpret_cast<const Sample *>(start),
                                     num_samples, mFrequency, kOutputFrequency);
  } break;
  }
  ...
  num_samples, mFrequency, ms, mBufferLenMs); #endif
  */
}
...
CAudioPlugin *CreateAudioPlugin() { return new AudioPluginPSP(); }
