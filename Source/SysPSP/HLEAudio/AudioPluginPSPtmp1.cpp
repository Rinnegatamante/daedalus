...
static const u32 kAudioBufferSize{
    1024 * 2}; // gpSP has a 4096kb sized audio buffer maybe we can use that?
static const u32 kNumChannels{2};
static const u32 kMaxBufferLengthMs{
    30}; // The maximum input we keep buffered in sync code, if too low we may
         // skip otherwise too high, lag will persist.
...
AudioPluginPSP::~AudioPluginPSP() { sceKernelDeleteSema(mSemaphore); }
