...
static s16 __attribute__((aligned(16)))
pcmout1[PSP_NUM_AUDIO_SAMPLES]; // # of stereo samples
...
static int fillBuffer(SceSize args, void *argp) {
  ...
  while (sound_status != 0xDEADBEEF) {
    ...
    ac->mAudioBufferUncached->Drain(reinterpret_cast<Sample *>(fillbuf),
                                    PSP_NUM_SAMPLES);
  }
...
static void AudioInit() {
  ...
  sound_channel = sceAudioChReserve(sound_channel, PSP_NUM_SAMPLES,
                                    PSP_AUDIO_FORMAT_STEREO);

  sound_status = 0; // threads running

  // create audio playback thread to provide timing
  int audioThid = sceKernelCreateThread("audioOutput", audioOutput, 0x15,
                                        0x1800, PSP_THREAD_ATTR_USER, NULL);
  if (audioThid < 0) {
  ...
  int bufferThid = sceKernelCreateThread("bufferFilling", fillBuffer, 0x14,
                                         0x1800, PSP_THREAD_ATTR_USER, NULL);
  if (bufferThid < 0) {
...
static void AudioExit() {
  ...
  if (audio_open) {
    ...
    sceKernelSignalSema(bufferEmpty,
                        1); // fillbuffer thread is probably waiting.
    sceKernelDelayThread(100 * 1000);
    sceAudioChRelease(sound_channel);
    sound_channel = PSP_AUDIO_NEXT_CHANNEL;
  }
...
AudioOutput::AudioOutput() : mAudioPlaying(false), mFrequency(44100) {
  ...
  dcache_wbinv_range_unaligned(mAudioBuffer,
                               mAudioBuffer + sizeof(CAudioBuffer));
}
...
struct SAddSamplesJob : public SJob {
  ...
  SAddSamplesJob(CAudioBuffer *buffer, const Sample *samples, u32 num_samples,
                 u32 frequency, u32 output_freq)
      : mBuffer(buffer), mSamples(samples), mNumSamples(num_samples),
        mFrequency(frequency), mOutputFreq(output_freq) {
    InitJob = NULL;
...
*/
  ...
  case APM_ENABLED_ASYNC: {
    SAddSamplesJob job(mAudioBufferUncached,
                       reinterpret_cast<const Sample *>(start), num_samples,
                       mFrequency, 44100);
  ...
  case APM_ENABLED_SYNC: {
    mAudioBufferUncached->AddSamples(reinterpret_cast<const Sample *>(start),
                                     num_samples, mFrequency, 44100);
  } break;
  }
}
