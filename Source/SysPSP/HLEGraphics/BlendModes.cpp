...
//
inline void BlendMode_Generic(BLEND_MODE_ARGS) {
...
void BlendMode_0x0040fe8155fef97cLL(BLEND_MODE_ARGS) {
  details.ColourAdjuster.SetRGB(details.EnvColour);
  sceGuTexFunc(GU_TFX_ADD,
               GU_TCC_RGB); // ADD looks better than BLEND for the sky
}
...
void BlendMode_0x0030fe045ffefdf8LL(BLEND_MODE_ARGS) {
  sceGuTexFunc(GU_TFX_MODULATE, GU_TCC_RGB);
  // Below blend fixes the ground on the first level, but breaks many other
  // things. Reverted. details.ColourAdjuster.SetRGB(details.EnvColour);
  // sceGuTexFunc(GU_TFX_BLEND,GU_TCC_RGB);
}
...
{
        //Makes Mario & his partner appear as black boxes.( This game has this
...
void BlendMode_0x00127eacf0fff238LL(BLEND_MODE_ARGS) {
  ...
  details.ColourAdjuster
      .SetAOpaque(); // Opaque still isn't doing much on the Alpha..?
  details.ColourAdjuster.SetRGB(details.EnvColour);
  sceGuTexFunc(GU_TFX_BLEND,
               GU_TCC_RGBA); // Not sure if BLEND is doing anything?
}
...
void BlendMode_0x005196a3112cfe7fLL(BLEND_MODE_ARGS) {
  // ADD fixes the text when the game starts, but breaks the main menu errrg, we
  // prefer to render the menu right instead..
  // sceGuTexFunc(GU_TFX_ADD,GU_TCC_RGBA);
  sceGuTexFunc(GU_TFX_REPLACE, GU_TCC_RGBA);
}
...
OverrideBlendModeFn LookupOverrideBlendModeForced(u64 mux) {
  ...
  if (!gGlobalPreferences.CustomBlendModes)
    return nullptr;
#endif
  switch (mux) {
#define BLEND_MODE(x)                                                          \
  ...
  case (0x00327e64fffff9fcLL):
    return g_ROM.GameHacks == SIN_PUNISHMENT
               ? BlendMode_0x00327e64fffff9fcLL
               : nullptr; // Sin and Punishment - blinds the screen (breaks
                          // splash screen race flag in MK64)
...
#define BLEND_MODE(x)                                                          \
  case (x):                                                                    \
    return BlendMode_##x;
    BLEND_MODE(
        0x001114a7f3fffef8LL); // Sin and Punishment - Sky <----- Needs work
    BLEND_MODE(0x001147fffffffe38LL); // Command & Conquer - Water
    // BLEND_MODE(0x00117e80f5fff438LL); // Paper Mario block texture partial
    // fix
    BLEND_MODE(0x00117ffffffefc38LL); // Quake 64 - Walls and ground
    BLEND_MODE(0x0011fe2344fe7339LL); // Mortal Kombat 4 - Text
    BLEND_MODE(0x0011fe2355fefd7eLL); // Mortal Kombat 4 -Character Selection
                                      // screen background / Tower
    ...
    BLEND_MODE(
        0x0015fea3f00ff23fLL); // WWF Wreslemania 2000 - Wrestlers Main Menu
    ...
    BLEND_MODE(0x003432685566ff7fLL); // Ogre Battle - Intro Dust / Conker -
                                      // Chainsaw Smoke
    ...
    // BLEND_MODE(0x0040fe8155fefd7eLL); // Kirby Far Terrain -> Breaks
    ...
    BLEND_MODE(
        0x00551aaa1134fe7fLL); // Sin and Punishment - Particles and Explosions
