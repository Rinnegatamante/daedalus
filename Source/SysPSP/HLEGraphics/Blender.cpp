...
To Devs:
 ...
 Incomplete blenders shouldn't be summitted, also please test and make sure the
XX blender actually works ! also please check common games (Mario, Zelda,
Majora's Mask before commiting) Note : Please edo not create extra brakes and
such, only do so if is really needed. For example adding a hack on XXX blender.

 Deafult case, should work most of the time, ignore most unknown blenders, if
the XXX looks correct of course.
...
: Blender: c800 - First:  Fog * AShade + In * 1-A 0000 - Second:  In * AIn + In
* 1-A
...
#define	G_RM_FOG_SHADE_A		GBL_c1(G_BL_CLR_FOG, G_BL_A_SHADE,
G_BL_CLR_IN, G_BL_1MA)
#define	G_RM_FOG_PRIM_A			GBL_c1(G_BL_CLR_FOG, G_BL_A_FOG,
G_BL_CLR_IN, G_BL_1MA)
#define	G_RM_PASS				GBL_c1(G_BL_CLR_IN, G_BL_0,
G_BL_CLR_IN, G_BL_1) #define	RM_AA_ZB_OPA_SURF(clk)	GBL_c##clk(G_BL_CLR_IN,
G_BL_A_IN, G_BL_CLR_MEM, G_BL_A_MEM)
...
inline void DebugBlender(u32 blender) {
  ...
  if (mBlender != blender) {
    ...
    printf("Unknown Blender: %04x - %s * %s + %s * %s || %s * %s + %s * %s\n",
           blender, sc_szBlClr[(blender >> 14) & 0x3],
           sc_szBlA1[(blender >> 10) & 0x3], sc_szBlClr[(blender >> 6) & 0x3],
           sc_szBlA2[(blender >> 2) & 0x3], sc_szBlClr[(blender >> 12) & 0x3],
           sc_szBlA1[(blender >> 8) & 0x3], sc_szBlClr[(blender >> 4) & 0x3],
...
//*****************************************************************************
// This version uses a 16bit hash, which is around 4X times faster than the old
// 64bit version
//*****************************************************************************
void InitBlenderMode(u32 blendmode) // Set Alpha Blender mode
{
  ...
  // case 0x0044:					// ?
  // case 0x0055:					// ?
  case 0x0c08: // In * 0 + In * 1 || :In * AIn + In * 1-A
               // Tarzan - Medalion in bottom part of the screen
  // case 0x0c19:					// ?
  case 0x0f0a: // In * 0 + In * 1 || :In * 0 + In * 1
               // SSV - ??? and MM - Walls, Wipeout - Mountains
  case 0x0fa5: // In * 0 + Bl * AMem || :In * 0 + Bl * AMem			OOT
               // Menu
  // case 0x5f50:					// ?
  case 0x8410: // Bl * AFog + In * 1-A || :In * AIn + Mem * 1-A		Paper
               // Mario Menu
  case 0xc302: // Fog * AIn + In * 1-A || :In * 0 + In * 1
               // ISS64 - Ground
  case 0xc702: // Fog * AFog + In * 1-A || :In * 0 + In * 1
               // Donald Duck - Sky
  // case 0xc811:					// ?
  case 0xfa00: // Fog * AShade + In * 1-A || :Fog * AShade + In * 1-A F-Zero -
               // Power Roads
    // case 0x050: 					// In * AIn + Mem * 1-A || In * AIn + Mem * 1-A Power
    // Pro Yakkau 2000
    ...
    //
    // Add here blenders which work fine with default case but causes too much
    // spam, this disabled in release mode
    //
#ifdef DAEDALUS_DEBUG_DISPLAYLIST
  // case 0x55f0:					// Mem * AFog + Fog * 1-A || :Mem * AFog + Fog *
  // 1-A	Bust a Move 3 - ???
  case 0x0150: // In * AIn + Mem * 1-A || :In * AFog + Mem * 1-A
               // Spiderman - Waterfall Intro
  case 0x0f5a: // In * 0 + Mem * 1 || :In * 0 + Mem * 1
               // Starwars Racer
  case 0x0010: // In * AIn + In * 1-A || :In * AIn + Mem * 1-A
               // Hey You Pikachu - Shadow
  case 0x0040: // In * AIn + Mem * 1-A || :In * AIn + In * 1-A
               // Mario - Princess peach text
  // case 0x0050:					// In * AIn + Mem * 1-A || :In * AIn + Mem *
  // 1-A:		SSV - TV Screen and SM64 text
  case 0x04d0: // In * AFog + Fog * 1-A || In * AIn + Mem * 1-A Conker's Eyes
  case 0x0c18: // In * 0 + In * 1 || :In * AIn + Mem * 1-A:			SSV
               // - WaterFall and dust
  case 0xc410: // Fog * AFog + In * 1-A || :In * AIn + Mem * 1-A
               // Donald Duck - Stars
  case 0xc810: // Fog * AShade + In * 1-A || :In * AIn + Mem * 1-A
               // SSV - Fog? and MM - Shadows
  case 0xcb02: // Fog * AShade + In * 1-A || :In * 0 + In * 1
               // Doom 64 - Weapons
...
#endif
  //
  // Default case should handle most blenders, ignore most unknown blenders
