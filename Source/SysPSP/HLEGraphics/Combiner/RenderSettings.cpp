 ...
 *   - GU_TFX_MODULATE -		Cv=Ct*Cf				TCC_RGB:
 *Av=Af				TCC_RGBA: Av=At*Af
 *   - GU_TFX_DECAL -
 *TCC_RGB: Cv=Ct,Av=Af		TCC_RGBA: Cv=Cf*(1-At)+Ct*At Av=Af
 *   - GU_TFX_BLEND -			Cv=(Cf*(1-Ct))+(Cc*Ct)	TCC_RGB: Av=Af
 *TCC_RGBA: Av=At*Af
 *   - GU_TFX_REPLACE -		Cv=Ct					TCC_RGB:
 *Av=Af				TCC_RGBA: Av=At
 *   - GU_TFX_ADD -			Cv=Cf+Ct				TCC_RGB:
 *Av=Af				TCC_RGBA: Av=At*Af
 *
 * The fields TCC_RGB and TCC_RGBA specify components that differ between
 * the two different component modes.
 *
 *   - GU_TFX_MODULATE - The texture is multiplied with the current diffuse
 *fragment
...
CAlphaRenderSettings::CAlphaRenderSettings(const char *description)
    : mDescription(description), mInexact(false), mUsesTexel0(false),
      mUsesTexel1(false), mConstantExpression(NULL) {}

//*****************************************************************************
//
//*****************************************************************************
CAlphaRenderSettings::~CAlphaRenderSettings() { delete mConstantExpression; }
...
//
//*****************************************************************************
void CAlphaRenderSettings::AddTermConstant(
    const CBlendConstantExpression *constant_expression) {
  if (mConstantExpression != NULL) {
    mConstantExpression = new CBlendConstantExpressionMul(mConstantExpression,
                                                          constant_expression);
  } else {
...
//
//*****************************************************************************
void CAlphaRenderSettings::Apply(bool texture_installed,
                                 const SRenderState &state,
                                 SRenderStateOut &out) const {
  out.VertexExpressionA = mConstantExpression;

  if (mUsesTexel0 || mUsesTexel1) {
#ifdef DAEDALUS_ENABLE_ASSERTS
    DAEDALUS_ASSERT(texture_installed,
                    "We have a texture, but it's not installed?");
#endif
...
CRenderSettingsModulate::CRenderSettingsModulate(const char *description)
    : CRenderSettings(description), mConstantExpression(NULL),
      mUsesTexel0(false), mUsesTexel1(false), mInexact(false) {}
...
//
//*****************************************************************************
void CRenderSettingsModulate::AddTermConstant(
    const CBlendConstantExpression *constant_expression) {
  if (mConstantExpression != NULL) {
    mConstantExpression = new CBlendConstantExpressionMul(mConstantExpression,
                                                          constant_expression);
...
//
//*****************************************************************************
void CRenderSettingsModulate::Apply(bool texture_installed,
                                    const SRenderState &state,
                                    SRenderStateOut &out) const {
...
//
//*****************************************************************************
CRenderSettingsBlend::CRenderSettingsBlend(const char *description,
                                           const CBlendConstantExpression *a,
                                           const CBlendConstantExpression *b)
    : CRenderSettings(description), mConstantExpressionA(a),
      mConstantExpressionB(b), mInexact(false) {}
...
//
//*****************************************************************************
void CRenderSettingsBlend::Apply(bool texture_installed,
                                 const SRenderState &state,
                                 SRenderStateOut &out) const {
#ifdef DAEDALUS_ENABLE_ASSERTS
  ...
  out.TextureFactor = mConstantExpressionB->EvaluateConstant(
      state.PrimitiveColour, state.EnvironmentColour);
...
//
//*****************************************************************************
CBlendStates::CBlendStates() : mAlphaSettings(NULL) {}
...
void CBlendStates::Print() const {
  ...
  for (u32 i = 0; i < mColourSettings.size(); ++i) {
    ...
    bool install_texture0(settings->UsesTexture0() ||
                          mAlphaSettings->UsesTexture0());
    bool install_texture1(settings->UsesTexture1() ||
                          mAlphaSettings->UsesTexture1());
