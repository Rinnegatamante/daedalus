...
// We should call DLParser_Process(kUnlimitedInstructionCount) when we enter the
// debugger, and that will return a count. T
...
bool DLDebugger_IsDebugging() { return gDebugDisplayList; }

void DLDebugger_RequestDebug() { gDebugDisplayList = true; }
...
CDebugMenuOption::CDebugMenuOption() : mRefreshDisplay(true) {}
...
void CCombinerExplorerDebugMenuOption::Display() const {
  const std::set<u64> &combiner_states(
      gRendererPSP->GetRecordedCombinerStates());
  ...
  u32 idx(0);
  u64 selected_mux(0);
  for (std::set<u64>::const_iterator it = combiner_states.begin();
       it != combiner_states.end(); ++it) {
    u64 state(*it);
    ...
    printf("  %s%c%08x%08x\n", text_col, selected ? '*' : ' ', u32(state >> 32),
           u32(state));
  ...
  if (selected_mux != 0) {
    ...
    RendererPSP::SBlendStateEntry entry1(
        gRendererPSP->LookupBlendState(selected_mux, false));
    ...
    RendererPSP::SBlendStateEntry entry2(
        gRendererPSP->LookupBlendState(selected_mux, true));
...
void CCombinerExplorerDebugMenuOption::Update(const SPspPadState &pad_state,
                                              float elapsed_time) {
  const std::set<u64> &combiner_states(
      gRendererPSP->GetRecordedCombinerStates());
  ...
  if (pad_state.OldButtons != pad_state.NewButtons) {
    ...
    if (pad_state.NewButtons & PSP_CTRL_DOWN) {
      mSelectedIdx =
          (mSelectedIdx < state_count - 1) ? mSelectedIdx + 1 : mSelectedIdx;
      InvalidateDisplay();
    }
...
CBlendDebugMenuOption::CBlendDebugMenuOption()
    : mIdx(0), mSel(0), modify(false) {}

void CBlendDebugMenuOption::Display() const {
  const char *const ForceColor[8] = {
      "( OFF )",        "( c32::White )", "( c32::Black )",   "( c32::Red )",
      "( c32::Green )", "( c32::Blue )",  "( c32::Magenta )", "( c32::Gold )"};

  const char *const PSPtxtFunc[10] = {
      "( GU_TFX_MODULATE, GU_TCC_RGB )", "( GU_TFX_MODULATE, GU_TCC_RGBA )",
      "( GU_TFX_BLEND, GU_TCC_RGB )",    "( GU_TFX_BLEND, GU_TCC_RGBA )",
      "( GU_TFX_ADD, GU_TCC_RGB )",      "( GU_TFX_ADD, GU_TCC_RGBA )",
      "( GU_TFX_REPLACE, GU_TCC_RGB )",  "( GU_TFX_REPLACE, GU_TCC_RGBA )",
      "( GU_TFX_DECAL, GU_TCC_RGB )",    "( GU_TFX_DECAL, GU_TCC_RGBA )"};

  const char *const CAdj[5] = {"( OFF )", "( details.PrimColour )",
                               "( details.PrimColour.ReplicateAlpha() )",
                               "( details.EnvColour )",
                               "( details.EnvColour.ReplicateAlpha() )"};

  const char *const ENVAdj[3] = {"( OFF )", "( details.EnvColour.GetColour() )",
                                 "( details.PrimColour.GetColour() )"};

  if (modify) {
    switch (mSel) {
    case 0:
      gDBlend.SetRGB = mIdx % 5;
      break;
    case 1:
      gDBlend.SetA = mIdx % 5;
      break;
    case 2:
      gDBlend.SetRGBA = mIdx % 5;
      break;
    case 3:
      gDBlend.ModRGB = mIdx % 5;
      break;
    case 4:
      gDBlend.ModA = mIdx % 5;
      break;
    case 5:
      gDBlend.ModRGBA = mIdx % 5;
      break;
    case 6:
      gDBlend.SubRGB = mIdx % 5;
      break;
    case 7:
      gDBlend.SubA = mIdx % 5;
      break;
    case 8:
      gDBlend.SubRGBA = mIdx % 5;
      break;
    case 9:
      gDBlend.AOpaque = mIdx & 1;
      break;
    case 10:
      gDBlend.sceENV = mIdx % 3;
      break;
    case 11:
      gDBlend.TXTFUNC = mIdx % 10;
      break;
    case 12:
      gDBlend.TexInstall = mIdx & 1;
      break;
    case 13:
      gDBlend.ForceRGB = mIdx % 8;
      break;
    }
  }

#define BLEND_SELECTION(x)                                                     \
  (mSel == x && modify) ? TERMINAL_GREEN : TERMINAL_WHITE, mSel == x ? '*' : ' '
...
namespace {
bool OrderTextures(const CTextureCache::STextureInfoSnapshot &lhs,
                   const CTextureCache::STextureInfoSnapshot &rhs) {
...
u16 gCheckTexture[gCheckTextureWidth * gCheckTextureHeight] = {
    0xfbbb, 0xfbbb, 0xfbbb, 0xfbbb, 0xfbbb, 0xfbbb, 0xfbbb, 0xfbbb, 0xffff,
    0xffff, 0xffff, 0xffff, 0xffff, 0xffff, 0xffff, 0xffff, 0xfbbb, 0xfbbb,
    0xfbbb, 0xfbbb, 0xfbbb, 0xfbbb, 0xfbbb, 0xfbbb, 0xffff, 0xffff, 0xffff,
    0xffff, 0xffff, 0xffff, 0xffff, 0xffff, 0xfbbb, 0xfbbb, 0xfbbb, 0xfbbb,
    0xfbbb, 0xfbbb, 0xfbbb, 0xfbbb, 0xffff, 0xffff, 0xffff, 0xffff, 0xffff,
    0xffff, 0xffff, 0xffff, 0xfbbb, 0xfbbb, 0xfbbb, 0xfbbb, 0xfbbb, 0xfbbb,
    0xfbbb, 0xfbbb, 0xffff, 0xffff, 0xffff, 0xffff, 0xffff, 0xffff, 0xffff,
    0xffff, 0xfbbb, 0xfbbb, 0xfbbb, 0xfbbb, 0xfbbb, 0xfbbb, 0xfbbb, 0xfbbb,
    0xffff, 0xffff, 0xffff, 0xffff, 0xffff, 0xffff, 0xffff, 0xffff, 0xfbbb,
    0xfbbb, 0xfbbb, 0xfbbb, 0xfbbb, 0xfbbb, 0xfbbb, 0xfbbb, 0xffff, 0xffff,
    0xffff, 0xffff, 0xffff, 0xffff, 0xffff, 0xffff, 0xfbbb, 0xfbbb, 0xfbbb,
    0xfbbb, 0xfbbb, 0xfbbb, 0xfbbb, 0xfbbb, 0xffff, 0xffff, 0xffff, 0xffff,
    0xffff, 0xffff, 0xffff, 0xffff, 0xfbbb, 0xfbbb, 0xfbbb, 0xfbbb, 0xfbbb,
    0xfbbb, 0xfbbb, 0xfbbb, 0xffff, 0xffff, 0xffff, 0xffff, 0xffff, 0xffff,
    ...
    0xffff, 0xffff, 0xffff, 0xffff, 0xfbbb, 0xfbbb, 0xfbbb, 0xfbbb, 0xfbbb,
    0xfbbb, 0xfbbb, 0xfbbb, 0xffff, 0xffff, 0xffff, 0xffff, 0xffff, 0xffff,
    0xffff, 0xffff, 0xfbbb, 0xfbbb, 0xfbbb, 0xfbbb, 0xfbbb, 0xfbbb, 0xfbbb,
    0xfbbb, 0xffff, 0xffff, 0xffff, 0xffff, 0xffff, 0xffff, 0xffff, 0xffff,
    0xfbbb, 0xfbbb, 0xfbbb, 0xfbbb, 0xfbbb, 0xfbbb, 0xfbbb, 0xfbbb, 0xffff,
    0xffff, 0xffff, 0xffff, 0xffff, 0xffff, 0xffff, 0xffff, 0xfbbb, 0xfbbb,
    0xfbbb, 0xfbbb, 0xfbbb, 0xfbbb, 0xfbbb, 0xfbbb, 0xffff, 0xffff, 0xffff,
    0xffff, 0xffff, 0xffff, 0xffff, 0xffff, 0xfbbb, 0xfbbb, 0xfbbb, 0xfbbb,
...
CTextureExplorerDebugMenuOption::CTextureExplorerDebugMenuOption()
    : mSelectedIdx(0), mDisplayTexture(false), mScaleFactor(2),
      mTextureOffset(0, 0) {
  mCheckerTexture = CNativeTexture::Create(gCheckTextureWidth,
                                           gCheckTextureHeight, TexFmt_4444);

  if (mCheckerTexture != nullptr) {
    DAEDALUS_ASSERT(mCheckerTexture->GetBytesRequired() ==
                        sizeof(gCheckTexture),
                    "Incorrect size for checker texture");
...
bool CTextureExplorerDebugMenuOption::OverrideDisplay() const {
  ...
  if (mCheckerTexture != nullptr) {
    u32 num_verts(2);
    TextureVtx *p_verts =
        (TextureVtx *)sceGuGetMemory(num_verts * sizeof(TextureVtx));
    ...
    sceGuDrawArray(GU_SPRITES, TEXTURE_VERTEX_FLAGS | GU_TRANSFORM_2D,
                   num_verts, nullptr, p_verts);
  }
  ...
  if (texture != nullptr) {
    ...
    p_verts[1].pos =
        v3(left_offset + display_width, top_offset + display_height, 0.0f);
...
void CTextureExplorerDebugMenuOption::Display() const {
  ...
  for (s32 i = min_to_show; i <= max_to_show; ++i) {
    DAEDALUS_ASSERT(i >= 0 && i < s32(mSnapshot.size()),
    ...
    printf(" %s%03d %c%08x (%d,%d -> %dx%d, %d) %s/%dbpp, %04x, %04x\n",
           text_col, i, selected ? '*' : ' ', ti.GetLoadAddress(), left, top,
           ti.GetWidth(), ti.GetHeight(), ti.GetPitch(), ti.GetFormatName(),
           ti.GetSizeInBits(), ti.GetTmemAddress(), ti.GetPalette());
  }
  printf("%s\n", TERMINAL_WHITE);
}

void CTextureExplorerDebugMenuOption::Update(const SPspPadState &pad_state,
                                             float elapsed_time) {
  ...
  if (pad_state.OldButtons != pad_state.NewButtons) {
    ...
    if (pad_state.NewButtons & PSP_CTRL_DOWN) {
      mSelectedIdx =
          (mSelectedIdx < texture_count - 1) ? mSelectedIdx + 1 : mSelectedIdx;
      InvalidateDisplay();
    }
  ...
  if (mDisplayTexture) {
    ...
    if (fabsf(pad_state.Stick.x) < 0.001f &&
        fabsf(pad_state.Stick.y) < 0.001f) {
      mTextureOffset *= powf(0.997f, elapsed_time * 1000.0f);
    } else {
      mTextureOffset +=
          pad_state.Stick * STICK_ADJUST_PIXELS_PER_SECOND * elapsed_time;
    }
  }
}
...
CDisplayListLengthDebugMenuOption::CDisplayListLengthDebugMenuOption(u32 total,
                                                                     u32 *limit)
    : mTotalInstructionCount(total), mInstructionCountLimit(limit),
      mFractionalAdjustment(0.0f) {}

void CDisplayListLengthDebugMenuOption::Display() const {
  printf("Display list length %d / %d:\n", *mInstructionCountLimit,
         mTotalInstructionCount);
  printf("   Use [] to return\n");
  printf("   Use up/down to adjust\n");
}

void CDisplayListLengthDebugMenuOption::Update(const SPspPadState &pad_state,
                                               float elapsed_time) {
  ...
  new_adjustment += pad_state.Stick.y * STICK_ADJUST_PER_SECOND *
                    rate_adjustment * elapsed_time;
  ...
  if (adjustment != 0) {
    ...
    *mInstructionCountLimit =
        u32(Clamp<s32>(new_limit, 0, mTotalInstructionCount));
...
void CDecalOffsetDebugMenuOption::Update(const SPspPadState &pad_state,
                                         float elapsed_time) {
  ...
  if (pad_state.Stick.x != 0) {
    ...
    if (pad_state.Stick.x > 0)
      TEST_VARX += CHANGE_PER_SECOND * elapsed_time;
    else
      TEST_VARX -= CHANGE_PER_SECOND * elapsed_time;
    InvalidateDisplay();
  }
  if (pad_state.Stick.y != 0) {
    if (pad_state.Stick.y < 0)
      TEST_VARY += CHANGE_PER_SECOND * elapsed_time;
    else
      TEST_VARY -= CHANGE_PER_SECOND * elapsed_time;
    InvalidateDisplay();
  }
  ...
  while ((pad_state.NewButtons & PSP_CTRL_HOME) != 0 || !menu_button_pressed) {
    ...
    if (need_update_display) {
      ...
      printf("Dlist took %dms (%fHz) [%d/%d]\n", s32(elapsed_ms), framerate,
             instruction_limit, total_instruction_count);
      printf("\n");
      ...
      if (p_current_option != nullptr) {
        ...
        u32 idx = 0;
        for (DebugMenuOptionVector::const_iterator it = menu_options.begin();
             it != menu_options.end(); ++it, idx++) {
    ...
    if (pad_state.OldButtons != pad_state.NewButtons) {
      ...
      if (pad_state.NewButtons & PSP_CTRL_LTRIGGER) {
        gGlobalPreferences.ViewportType = EViewportType(
            (gGlobalPreferences.ViewportType + 1) % NUM_VIEWPORT_TYPES);
