...
static void UpdateFramerate() {
  ...
  fprintf(gFramerateFile, "%d,%f\n", gTotalFrames,
          f32(now - gFirstFrameTime) / f32(gTicksPerSecond));
#endif
  ...
  if (ticks_since_recalc > gTicksPerSecond) {
    // gCurrentVblrate = float( gVblCount * gTicksPerSecond ) / float(
    // ticks_since_recalc );
    gCurrentFramerate =
        float(gFlipCount * gTicksPerSecond) / float(ticks_since_recalc);
...
CGraphicsPluginImpl::~CGraphicsPluginImpl() = default;
...
void CGraphicsPluginImpl::UpdateScreen() {
  ...
  if (current_origin != last_origin) {
      ...
      if (gGlobalPreferences.DisplayFramerate) {
        ...
        case 2:
          pspDebugScreenPrintf(
              "FPS[%#.1f] VB[%d/%d] Sync[%#.1f%%]   ", gCurrentFramerate,
              u32(Fsync * f32(FramerateLimiter_GetTvFrequencyHz())),
              FramerateLimiter_GetTvFrequencyHz(), Fsync * 100.0f);
          break;
        case 3:
#ifdef DAEDALUS_DEBUG_DISPLAYLIST
          pspDebugScreenPrintf(
              "Dlist[%d] Cull[%d] | Tris[%d] Cull[%d] | Rect[%d] Clip[%d] ",
              gNumInstructionsExecuted, gNumDListsCulled,
              gRenderer->GetNumTrisRendered(), gRenderer->GetNumTrisClipped(),
              gRenderer->GetNumRect(), gNumRectsClipped);
#else
          pspDebugScreenPrintf("%#.1f  ", gCurrentFramerate);
#endif
          break;
    ...
    case FV_DISABLED:
      ...
      break;
    case FV_AUTO1:
      if (!Old_FrameskipActive && (Fsync < 0.965f))
        gFrameskipActive = true;
      else
        gFrameskipActive = false;
      break;
    case FV_AUTO2:
      if ((!Old_FrameskipActive | !Older_FrameskipActive) && (Fsync < 0.965f))
        gFrameskipActive = true;
      else
        gFrameskipActive = false;
      break;
...
CGraphicsPlugin *CreateGraphicsPlugin() {
  ...
  auto *plugin = new CGraphicsPluginImpl;
