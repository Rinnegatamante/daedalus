...
public:
  CButtonMappingNegate(CButtonMapping *p_mapping) : mpMapping(p_mapping) {}

  ~CButtonMappingNegate() override { delete mpMapping; }
...
public:
  ...
  void SetJoySwap(const char *swap) {
    mJoySwap = swap[0];
  } // Save only first char which should be A|B|C for the joystick swapping
...
void CControllerConfig::SetButtonMapping(EN64Button button,
                                         CButtonMapping *p_mapping) {
...
void IInputManager::SwapJoyStick(OSContPad *pPad, SceCtrlData *pad) {
  ...
  case 'B': // Swap Joystick with PSP Dpad
  {
    ...
    // Make a digital version of the Analogue stick that can be mapped to N64
    // buttons //Corn
    pad->Buttons &= ~(PSP_CTRL_UP | PSP_CTRL_DOWN | PSP_CTRL_RIGHT |
                      PSP_CTRL_LEFT); // Clear out original DPAD
    if (pPad->stick_x > 40)
      pad->Buttons |= PSP_CTRL_RIGHT;
    else if (pPad->stick_x < -40)
      pad->Buttons |= PSP_CTRL_LEFT;

    if (pPad->stick_y > 40)
      pad->Buttons |= PSP_CTRL_UP;
    else if (pPad->stick_y < -40)
      pad->Buttons |= PSP_CTRL_DOWN;

    // Set Stick
    if (tmp & PSP_CTRL_RIGHT)
      pPad->stick_x = s8(N64_ANALOGUE_STICK_RANGE);
    else if (tmp & PSP_CTRL_LEFT)
      pPad->stick_x = -s8(N64_ANALOGUE_STICK_RANGE);
    else
      pPad->stick_x = 0;

    if (tmp & PSP_CTRL_UP)
      pPad->stick_y = s8(N64_ANALOGUE_STICK_RANGE);
    else if (tmp & PSP_CTRL_DOWN)
      pPad->stick_y = -s8(N64_ANALOGUE_STICK_RANGE);
    else
      pPad->stick_y = 0;
  } break;
  ...
  case 'C': // Swap Joystick with PSP buttons
  {
    ...
    // Make a digital version of the Analogue stick that can be mapped to N64
    // buttons //Corn
    pad->Buttons &= ~(PSP_CTRL_TRIANGLE | PSP_CTRL_CROSS | PSP_CTRL_CIRCLE |
                      PSP_CTRL_SQUARE); // Clear out original buttons
    if (pPad->stick_x > 40)
    ...
    // Set Stick
    if (tmp & PSP_CTRL_CIRCLE)
      pPad->stick_x = s8(N64_ANALOGUE_STICK_RANGE);
    else if (tmp & PSP_CTRL_SQUARE)
      pPad->stick_x = -s8(N64_ANALOGUE_STICK_RANGE);
    else
      pPad->stick_x = 0;

    if (tmp & PSP_CTRL_TRIANGLE)
      pPad->stick_y = s8(N64_ANALOGUE_STICK_RANGE);
    else if (tmp & PSP_CTRL_CROSS)
      pPad->stick_y = -s8(N64_ANALOGUE_STICK_RANGE);
    else
      pPad->stick_y = 0;
  } break;
  ...
  {
    // PS vita Right Stick
    if (pad->Rsrv[0] - 128 > 40)
      pad->Buttons |= PSP_CTRL_RIGHT;
    if (pad->Rsrv[0] - 128 < -40)
      pad->Buttons |= PSP_CTRL_LEFT;

    if (pad->Rsrv[1] - 128 > -40)
      pad->Buttons |= PSP_CTRL_UP;
    if (pad->Rsrv[1] - 128 < 40)
      pad->Buttons |= PSP_CTRL_DOWN;
  } break;
...
void IInputManager::GetState(OSContPad pPad[4]) {
  ...
  //
  v2 stick(f32(normalised_x) / PSP_ANALOGUE_STICK_RANGE,
           f32(normalised_y) / PSP_ANALOGUE_STICK_RANGE);

  stick = ApplyDeadzone(stick, gGlobalPreferences.StickMinDeadzone,
  ...
  DAEDALUS_ASSERT(mpControllerConfig != NULL,
                  "We should always have a valid controller");
#endif
  ...
  // Synchronise the input - this will overwrite the real pad data when playing
  // back input
  for (u32 cont = 0; cont < 4; cont++) {
...
const SButtonNameMapping gButtonNameMappings[] = {
    {"PSP.Start", PSP_CTRL_START},       {"PSP.Cross", PSP_CTRL_CROSS},
    {"PSP.Square", PSP_CTRL_SQUARE},     {"PSP.Triangle", PSP_CTRL_TRIANGLE},
    {"PSP.Circle", PSP_CTRL_CIRCLE},     {"PSP.LTrigger", PSP_CTRL_LTRIGGER},
    {"PSP.RTrigger", PSP_CTRL_RTRIGGER}, {"PSP.Up", PSP_CTRL_UP},
    {"PSP.Down", PSP_CTRL_DOWN},         {"PSP.Left", PSP_CTRL_LEFT},
    {"PSP.Right", PSP_CTRL_RIGHT},       {"PSP.Select", PSP_CTRL_SELECT},
...
bool IsOperatorChar(char c) { return c == '&' || c == '|' || c == '!'; }

bool IsIdentifierChar(char c) { return isalpha(c) || isdigit(c) || c == '.'; }
...
void CButtonMappingExpressionEvaluator::HandleOperator(
    char op, std::stack<CButtonMapping *> &operands) {
  switch (op) {
  case '!':
    if (!operands.empty()) {
...
CButtonMapping *
CButtonMappingExpressionEvaluator::Parse(const char *expression) {
  mNumErrors = 0;
  mErrorStream.Clear();
  ...
  for (; p_cur < p_end; ++p_cur) {
    ...
    if (isspace(c)) {
      ...
      while (!found_opening) {
        ...
        // This must be an operand. Pop the arguments from the mapping stack and
        // construct outputs
        switch (op) {
        case '(':
          // We've matched the opening parenthesis - can discard it and continue
          // parsing
          found_opening = true;
          break;
        case '!':
      ...
      if (!found_opening) {
        ReportError() << "Couldn't match closing parenthesis at char "
                      << u32(p_cur - expression) << ", '..." << p_cur << "'\n";
      }
  ...
  //
  //	At this point we have evaluated the expression string, but might still
  //have 	unhandled operators lying on the expression stack. Spin through it
...
CControllerConfig *IInputManager::BuildDefaultConfig() {
  ...
  p_config->SetDescription(
      "The default Daedalus controller configuration. The Circle button "
      "toggles the PSP DPad between the N64 DPad and CButtons.");
  ...
  p_config->SetButtonMapping(N64Button_Down,
                             eval.Parse("PSP.Circle & PSP.Down"));
  p_config->SetButtonMapping(N64Button_Left,
                             eval.Parse("PSP.Circle & PSP.Left"));
  p_config->SetButtonMapping(N64Button_Right,
                             eval.Parse("PSP.Circle & PSP.Right"));

  p_config->SetButtonMapping(N64Button_CUp, eval.Parse("!PSP.Circle & PSP.Up"));
  p_config->SetButtonMapping(N64Button_CDown,
                             eval.Parse("!PSP.Circle & PSP.Down"));
  p_config->SetButtonMapping(N64Button_CLeft,
                             eval.Parse("!PSP.Circle & PSP.Left"));
  p_config->SetButtonMapping(N64Button_CRight,
                             eval.Parse("!PSP.Circle & PSP.Right"));
...
CControllerConfig *IInputManager::BuildControllerConfig(const char *filename) {
  ...
  //
  const CIniFileSection *p_button_section(
      p_ini_file->GetSectionByName("Buttons"));
...
//*****************************************************************************
const char *
IInputManager::GetConfigurationDescription(u32 configuration_idx) const {
  if (configuration_idx < mControllerConfigs.size()) {
...
#ifdef DAEDALUS_DEBUG_CONSOLE
    DBGConsole_Msg(0, "Setting the controller to [c%s]",
                   mpControllerConfig->GetName());
#endif
  }
#ifdef DAEDALUS_DEBUG_CONSOLE
  else {
...
v2 ApplyDeadzone(const v2 &in, f32 min_deadzone, f32 max_deadzone) {
  ...
  DAEDALUS_ASSERT(min_deadzone >= 0.0f && min_deadzone <= 1.0f,
                  "Invalid min deadzone");
  DAEDALUS_ASSERT(max_deadzone >= 0.0f && max_deadzone <= 1.0f,
                  "Invalid max deadzone");
#endif
