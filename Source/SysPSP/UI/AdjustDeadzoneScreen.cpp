...
private:
  ...
  void DrawStick(s32 x, s32 y, s32 r, const v2 &stick, f32 min_deadzone,
                 f32 max_deadzone);
...
IAdjustDeadzoneScreen::IAdjustDeadzoneScreen(CUIContext *p_context)
    : CUIScreen(p_context), mIsFinished(false), mPspStick(0.0f, 0.0f),
      mN64Stick(0.0f, 0.0f), mAdjustingMinDeadzone(true),
...
void IAdjustDeadzoneScreen::Update(float elapsed_time, const v2 &stick,
                                   u32 old_buttons, u32 new_buttons) {
  if (new_buttons & PSP_CTRL_DOWN) {
    if (mAdjustingMinDeadzone) {
      mStickMinDeadzone = Clamp(mStickMinDeadzone - DEADZONE_INCREMENT, 0.0f,
                                mStickMaxDeadzone);
    } else {
      mStickMaxDeadzone = Clamp(mStickMaxDeadzone - DEADZONE_INCREMENT,
                                mStickMinDeadzone, 1.0f);
    }
  }
...
void IAdjustDeadzoneScreen::DrawStick(s32 x, s32 y, s32 r, const v2 &stick,
                                      f32 min_deadzone, f32 max_deadzone) {
  ...
  if (max_deadzone >= 0.0f) {
    DrawCircle(x, y, s32(r * max_deadzone),
               mAdjustingMinDeadzone ? white : red);
  }

  if (min_deadzone >= 0.0f) {
    DrawCircle(x, y, s32(r * min_deadzone),
               mAdjustingMinDeadzone ? red : white);
  }
...
void IAdjustDeadzoneScreen::Render() {
  ...
  mpContext->DrawTextAlign(0, SCREEN_WIDTH, AT_CENTRE, TITLE_Y, TITLE_TEXT,
                           DrawTextUtilities::TextWhite);

  mpContext->DrawText(PSP_TITLE_X, PSP_TITLE_Y, "PSP",
                      DrawTextUtilities::TextWhite);
  mpContext->DrawText(N64_TITLE_X, N64_TITLE_Y, "N64",
                      DrawTextUtilities::TextWhite);

  DrawStick(PSP_CIRCLE_X, PSP_CIRCLE_Y, DISPLAY_RADIUS, mPspStick,
            mStickMinDeadzone, mStickMaxDeadzone);
  DrawStick(N64_CIRCLE_X, N64_CIRCLE_Y, DISPLAY_RADIUS, mN64Stick, -1.0f,
            -1.0f);

  char str[128];
  sprintf(str, "%s: %d, %s: %d", Translate_String("Min"),
          s32(100.f * mStickMinDeadzone), Translate_String("Max"),
          s32(100.0f * mStickMaxDeadzone));
  mpContext->DrawText(PSP_CIRCLE_X - DISPLAY_RADIUS,
                      PSP_CIRCLE_Y + DISPLAY_RADIUS + 10, str,
                      DrawTextUtilities::TextWhite);

  mpContext->DrawTextArea(
      LIST_TEXT_LEFT, BELOW_MENU_MIN, LIST_TEXT_WIDTH - LIST_TEXT_LEFT,
      (ROM_INFO_TEXT_X - 30) - BELOW_MENU_MIN, INSTRUCTIONS_TEXT,
      DrawTextUtilities::TextWhite, VA_BOTTOM);
}

void IAdjustDeadzoneScreen::Run() { CUIScreen::Run(); }
