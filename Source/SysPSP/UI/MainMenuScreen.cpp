...
const char *const gMenuOptionNames[NUM_MENU_OPTIONS] = {
    "Global Settings", "Roms", "Selected Rom", "Savestates", "About",
};
} // namespace
...
private:
  ...
  EMenuOption GetPreviousOption() const {
    return AsMenuOption(mCurrentOption - 1);
  }
...
IMainMenuScreen::IMainMenuScreen(CUIContext *p_context)
    : CUIScreen(p_context), mIsFinished(false), mCurrentOption(MO_ROMS),
      mCurrentDisplayOption(mCurrentOption) {
  for (u32 i{0}; i < NUM_MENU_OPTIONS; ++i) {
    mOptionComponents[i] = nullptr;
  }

  mSelectedRomComponent = CSelectedRomComponent::Create(
      mpContext, new CMemberFunctor<IMainMenuScreen>(
                     this, &IMainMenuScreen::OnStartEmulation));

  mOptionComponents[MO_GLOBAL_SETTINGS] =
      CGlobalSettingsComponent::Create(mpContext);
  mOptionComponents[MO_ROMS] = CRomSelectorComponent::Create(
      mpContext, new CMemberFunctor1<IMainMenuScreen, const char *>(
                     this, &IMainMenuScreen::OnRomSelected));
  mOptionComponents[MO_SELECTED_ROM] = mSelectedRomComponent;
  mOptionComponents[MO_SAVESTATES] = CSavestateSelectorComponent::Create(
      mpContext, CSavestateSelectorComponent::AT_LOADING,
      new CMemberFunctor1<IMainMenuScreen, const char *>(
          this, &IMainMenuScreen::OnSavestateSelected),
      0);
...
void IMainMenuScreen::Update(float elapsed_time, const v2 &stick,
  ...
  mCurrentDisplayOption =
      mCurrentDisplayOption + (mCurrentOption - mCurrentDisplayOption) * 0.1f;

  mOptionComponents[GetCurrentOption()]->Update(elapsed_time, stick,
...
void IMainMenuScreen::Render() {
  ...
  for (s32 i = -2; i <= 2; ++i) {
    ...
    s32 extreme(diff < 0 ? SCREEN_LEFT
                         : s32(SCREEN_RIGHT - (text_width * min_scale)));
...
void IMainMenuScreen::OnRomSelected(const char *rom_filename) {
  ...
  if (ROM_GetRomDetailsByFilename(rom_filename, &mRomID, &rom_size,
                                  &boot_type)) {
