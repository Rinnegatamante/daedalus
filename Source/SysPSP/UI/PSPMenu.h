...
const char *const pspModel[MAX_PSP_MODEL] = {
    "PSP PHAT (01g)",  "PSP 2000 (02g)", "PSP BRITE(03g)",  "PSP BRITE (04g)",
    "PSP GO (05G)",    "UNKNOWN PSP",    "PSP BRITE (07g)", "UNKNOWN PSP",
    "PSP BRITE (09g)", "UNKNOWN PSP",    "PSP STREET (11g)"};

// Adjust Dead Zone Screen

const char *const INSTRUCTIONS_TEXT =
    "Adjust the minimum and maximum deadzone regions. Up/Down: Increase or "
    "decrease the deadzone. Left/Right: Select minimum or maximum deadzone for "
    "adjusting. Triangle: Reset to defaults. Start/X: Confirm. Select/Circle: "
    "Cancel";
...
const u32 N64_CIRCLE_X =
    CENTRE_X + DISPLAY_RADIUS + ((HALF_WIDTH - DISPLAY_WIDTH) / 2);
...
const f32 DEFAULT_MIN_DEADZONE =
    0.28f; // Kind of gross - share somehow with IInputManager?
...
const char *const gRomsDirectories[] = {
    ...
    // For ease of developing with multiple source trees, common folder for roms
    // can be placed at host1: in usbhostfs
    "host1:/",
#endif
};
...
const f32 PREVIEW_SCROLL_WAIT =
    0.500f; // seconds to wait for scrolling to stop before loading preview
            // (prevent thrashing)
