...
private:
  static EMenuOption GetNextOption(EMenuOption option) {
    return EMenuOption((option + 1) % NUM_MENU_OPTIONS);
  }
  static EMenuOption GetPreviousOption(EMenuOption option) {
    return EMenuOption((option + NUM_MENU_OPTIONS - 1) % NUM_MENU_OPTIONS);
  }

  EMenuOption GetPreviousOption() const {
    return GetPreviousOption(mCurrentOption);
  }
...
IPauseScreen::IPauseScreen(CUIContext *p_context)
    : CUIScreen(p_context), mIsFinished(false),
      mCurrentOption(MO_PAUSE_OPTIONS) {
  for (auto &mOptionComponent : mOptionComponents) {
    mOptionComponent = NULL;
  }

  mOptionComponents[MO_GLOBAL_SETTINGS] =
      CGlobalSettingsComponent::Create(mpContext);
  mOptionComponents[MO_PAUSE_OPTIONS] = CPauseOptionsComponent::Create(
      mpContext,
      new CMemberFunctor<IPauseScreen>(this, &IPauseScreen::OnResume),
      new CMemberFunctor<IPauseScreen>(this, &IPauseScreen::OnReset));
...
bool IPauseScreen::IsOptionValid(EMenuOption option) const { return true; }

void IPauseScreen::Update(float elapsed_time, const v2 &stick, u32 old_buttons,
                          u32 new_buttons) {
  ...
  mOptionComponents[mCurrentOption]->Update(elapsed_time, stick, old_buttons,
                                            new_buttons);
}
...
void IPauseScreen::Render() {
  ...
  if (!scePowerIsBatteryCharging()) {
    sprintf(info, " [%s %d%% %s %2dh %2dm]", Translate_String("Battery / "),
            bat, Translate_String("Time"), batteryLifeTime / 60,
            batteryLifeTime - 60 * (batteryLifeTime / 60));
  }

  // Battery Info
  mpContext->SetFontStyle(CUIContext::FS_REGULAR);
  mpContext->DrawTextAlign(0, SCREEN_WIDTH - LIST_TEXT_LEFT, AT_RIGHT,
                           CATEGORY_TEXT_TOP, info,
                           DrawTextUtilities::TextWhiteDisabled,
                           DrawTextUtilities::TextBlueDisabled);

  p_option_text = gMenuOptionNames[previous];
  mpContext->DrawTextAlign(
      LIST_TEXT_LEFT, LIST_TEXT_WIDTH, AT_LEFT, y + mpContext->GetFontHeight(),
      p_option_text, IsOptionValid(previous) ? valid_colour : invalid_colour);

  mpContext->SetFontStyle(CUIContext::FS_HEADING);
  p_option_text = gMenuOptionNames[current];
  mpContext->DrawTextAlign(LIST_TEXT_LEFT, LIST_TEXT_WIDTH, AT_CENTRE,
                           y + mpContext->GetFontHeight(), p_option_text,
                           IsOptionValid(current) ? valid_colour
                                                  : invalid_colour);
  mpContext->SetFontStyle(CUIContext::FS_REGULAR);

  p_option_text = gMenuOptionNames[next];
  mpContext->DrawTextAlign(LIST_TEXT_LEFT, LIST_TEXT_WIDTH, AT_RIGHT,
                           y + mpContext->GetFontHeight(), p_option_text,
                           IsOptionValid(next) ? valid_colour : invalid_colour);
...
void IPauseScreen::OnResume() { mIsFinished = true; }
