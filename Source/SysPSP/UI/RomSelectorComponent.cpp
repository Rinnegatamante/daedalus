...
#ifdef DAEDALUS_ENABLE_ASSERTS
  DAEDALUS_ASSERT(category >= 0 && category < NUM_CATEGORIES,
                  "Invalid category");
#endif
...
struct SRomInfo {
  ...
  SRomInfo(const char *filename) : mFilename(filename) {
...
static bool SortByGameName(const SRomInfo *a, const SRomInfo *b) {
  ...
  return (strcmp(a->mSettings.GameName.c_str(), b->mSettings.GameName.c_str()) <
          0);
}
...
public:
  IRomSelectorComponent(CUIContext *p_context,
                        CFunctor1<const char *> *on_rom_selected);
...
private:
  ...
  void DrawInfoText(CUIContext *p_context, s32 y, const char *field_txt,
...
private:
  ...
  u32 mPreviewIdx;
  float mPreviewLoadedTime; // How long the preview has been loaded (so we can
                            // fade in)
...
CRomSelectorComponent::Create(CUIContext *p_context,
                              CFunctor1<const char *> *on_rom_selected) {
...
IRomSelectorComponent::IRomSelectorComponent(
    CUIContext *p_context, CFunctor1<const char *> *on_rom_selected)
    : CRomSelectorComponent(p_context), OnRomSelected(on_rom_selected)
      //,	mCurrentSelection( 0 )
      ,
      mCurrentScrollOffset(0), mSelectionAccumulator(0), mpPreviewTexture(NULL),
      mPreviewIdx(u32(-1)), mPreviewLoadedTime(0.0f), mTimeSinceScroll(0.0f),
      mRomDelete(false), mQuitTriggered(false), mQuitInit(false) {
...
//
//*************************************************************************************
void IRomSelectorComponent::AddRomDirectory(const char *p_roms_dir,
                                            RomInfoList &roms) {
...
//
//*************************************************************************************
void IRomSelectorComponent::DrawInfoText(CUIContext *p_context, s32 y,
                                         const char *field_txt,
                                         const char *value_txt) {
  c32 colour(p_context->GetDefaultTextColour());

  p_context->DrawTextAlign(PREVIEW_IMAGE_LEFT,
                           PREVIEW_IMAGE_LEFT + PREVIEW_IMAGE_WIDTH, AT_LEFT, y,
                           field_txt, colour);
  p_context->DrawTextAlign(PREVIEW_IMAGE_LEFT,
                           PREVIEW_IMAGE_LEFT + PREVIEW_IMAGE_WIDTH, AT_RIGHT,
                           y, value_txt, colour);
}
...
void IRomSelectorComponent::RenderPreview() {
  ...
  if (mpPreviewTexture != NULL) {
    ...
    if (mPreviewLoadedTime < PREVIEW_FADE_TIME) {
      colour = c32(255, 255, 255,
                   u8(mPreviewLoadedTime * 255.f / PREVIEW_FADE_TIME));
    }

    mpContext->DrawRect(PREVIEW_IMAGE_LEFT, BELOW_MENU_MIN, PREVIEW_IMAGE_WIDTH,
                        PREVIEW_IMAGE_HEIGHT, c32::Black);
    mpContext->RenderTexture(mpPreviewTexture, tl, wh, colour);
  } else {
    mpContext->DrawRect(PREVIEW_IMAGE_LEFT, BELOW_MENU_MIN, PREVIEW_IMAGE_WIDTH,
                        PREVIEW_IMAGE_HEIGHT, c32::White);
    mpContext->DrawRect(PREVIEW_IMAGE_LEFT + 2, BELOW_MENU_MIN + 2,
                        PREVIEW_IMAGE_WIDTH - 4, PREVIEW_IMAGE_HEIGHT - 4,
                        c32::Black);
    mpContext->DrawTextAlign(
        PREVIEW_IMAGE_LEFT, PREVIEW_IMAGE_LEFT + PREVIEW_IMAGE_WIDTH, AT_CENTRE,
        BELOW_MENU_MIN + PREVIEW_IMAGE_HEIGHT / 2, "No Preview Available",
        c32::White);
  }
  ...
  if (mCurrentSelection < mRomsList.size()) {
    ...
    DrawInfoText(mpContext, y, "Boot:", cic_name);
    y += line_height;
    DrawInfoText(mpContext, y, "Country:", country);
    y += line_height;
    DrawInfoText(mpContext, y, "Size:", buffer);
    y += line_height;

    DrawInfoText(mpContext, y,
                 "Save:", ROM_GetSaveTypeName(p_rominfo->mSettings.SaveType));
    y += line_height;
    DrawInfoText(
        mpContext, y, "EPak:",
        ROM_GetExpansionPakUsageName(p_rominfo->mSettings.ExpansionPakUsage));
    y += line_height;

  } else {
    DrawInfoText(mpContext, y, "Boot:", "");
    y += line_height;
...
void IRomSelectorComponent::RenderRomList() {
  ...
  sceGuScissor(LIST_TEXT_LEFT, BELOW_MENU_MIN, LIST_TEXT_LEFT + LIST_TEXT_WIDTH,
               BELOW_MENU_MIN + LIST_TEXT_HEIGHT);
  ...
  for (u32 i = 0; i < mRomsList.size(); ++i) {
    ...
    //
    if (s32(y + line_height) >= s32(BELOW_MENU_MIN) &&
        s32(y - line_height) < s32(BELOW_MENU_MIN + LIST_TEXT_HEIGHT)) {
      c32 colour{};
...
void IRomSelectorComponent::Render() {
  ...
  const char *const message[] = {"(X) -> Load", "([ ]) -> Settings",
                                 "(/\\) -> File Names", "(HOME) -> Quit",
                                 "(SELECT) -> Delete"};
  ...
  if (mRomsList.empty()) {
    ...
    for (auto i : gNoRomsText) {
      offset += mpContext->DrawTextArea(
          LIST_TEXT_LEFT, BELOW_MENU_MIN + offset, LIST_TEXT_WIDTH,
          LIST_TEXT_HEIGHT - offset, i, DrawTextUtilities::TextWhite, VA_TOP);
      offset += 4;
    }
  } else {
    RenderRomList();
  ...
  c32 color{};
  if (count & 0x80)
    color = c32(~count << 1, 0, 0, 255);
  else
    color = c32(count << 1, 0, 0, 255);

  if (mRomDelete) {
    mpContext->DrawTextAlign(0, 470, AT_RIGHT,
                             CATEGORY_TEXT_TOP + mpContext->GetFontHeight(),
                             "(X) -> Confirm", color);
  } else {
    mpContext->DrawTextAlign(0, 470, AT_RIGHT,
                             CATEGORY_TEXT_TOP + mpContext->GetFontHeight(),
                             message[(count >> 8) % ARRAYSIZE(message)], color);
  }
...
//
//*************************************************************************************
void IRomSelectorComponent::Update(float elapsed_time, const v2 &stick,
  ...
  v2 stick_dead(ApplyDeadzone(stick, gGlobalPreferences.StickMinDeadzone,
                              gGlobalPreferences.StickMaxDeadzone));
  ...
  if (old_buttons != new_buttons) {
    ...
    if (new_buttons != 0)
      mRomDelete =
          false; // DONT CHANGE ORDER clear it if any button has been pressed
  ...
  //	We add on 'current_vel * 2' to keep the selection highlight as close to
  //the 	center as possible (as if we're predicting 2 frames ahead)
  //
  ...
  if (mRomsList.size() * line_height > LIST_TEXT_HEIGHT) {
    s32 current_selection_y =
        s32((mCurrentSelection + current_vel * 2) * line_height) +
        (line_height / 2) + mCurrentScrollOffset;
    ...
    s32 new_scroll_offset =
        mCurrentScrollOffset + s32(float(adjust_amount) * d);
  ...
  //
  //	Increase a timer is the current selection is still the same (i.e. if
  //we've not scrolled)
  //
  ...
  //
  //	If the current selection is different from the preview, invalidate the
  //picture.
  //
  //
  if (mCurrentSelection < mRomsList.size() &&
    ...
    //
    //	If we've waited long enough since starting to scroll, try and load the
    //preview image 	Note that it may fail, so we sort out the other flags
    //regardless.
    //
    if (mTimeSinceScroll > PREVIEW_SCROLL_WAIT) {
      ...
      if (!mRomsList[mCurrentSelection]->mSettings.Preview.empty()) {
        ...
        IO::Path::Combine(
            preview_filename, gPreviewDirectory,
            mRomsList[mCurrentSelection]->mSettings.Preview.c_str());

        mpPreviewTexture =
            CNativeTexture::CreateFromPng(preview_filename, TexFmt_8888);
      }
    }
  }
