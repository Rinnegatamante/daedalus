...
public:
  ISavestateSelectorComponent(CUIContext *p_context, EAccessType accetype,
                              CFunctor1<const char *> *on_slot_selected,
...
private:
  ...
  s8 mPVExists[NUM_SAVESTATE_SLOTS]{}; // 0=skip, 1=file exists, -1=show no
                                       // preview
...
CSavestateSelectorComponent::Create(CUIContext *p_context, EAccessType accetype,
                                    CFunctor1<const char *> *on_slot_selected,
                                    const char *running_rom) {
  return new ISavestateSelectorComponent(p_context, accetype, on_slot_selected,
                                         running_rom);
}

namespace {
void MakeSaveSlotPath(char *path, char *png_path, u32 slot_idx,
                      char *slot_path) {
...
ISavestateSelectorComponent::ISavestateSelectorComponent(
    CUIContext *p_context, EAccessType accetype,
    CFunctor1<const char *> *on_slot_selected, const char *running_rom)
    : CSavestateSelectorComponent(p_context), mAccessType(accetype),
      mOnSlotSelected(on_slot_selected), mSelectedSlot(INVALID_SLOT),
      mIsFinished(false), deleteButtonTriggered(false) {
...
void ISavestateSelectorComponent::LoadFolders() {
  ...
  u32 i{0};
  const char *description_text(mAccessType == AT_SAVING
                                   ? "Select the slot in which to save"
                                   : "Select the slot from which to load");
  IO::Filename full_path;
  // We're using the same vector for directory names and slots, so we have to
  // clear it
  mElements.Clear();
  ...
  if (IO::FindFileOpen("ms0:/n64/SaveStates", &find_handle, find_data)) {
    do {
      ...
      if (IO::Directory::IsDirectory(full_path) && strlen(find_data.Name) > 2) {
        ...
        CUIElement *element;
        str << find_data.Name;
        CFunctor1<u32> *functor_1(
            new CMemberFunctor1<ISavestateSelectorComponent, u32>(
                this, &ISavestateSelectorComponent::OnFolderSelected));
    ...
    CUIElement *element;
    element = new CUICommandDummy("There are no Savestates to load",
                                  "There are no Savestates to load");
    mElements.Add(element);
  }
}
...
void ISavestateSelectorComponent::LoadSlots() {
  const char *description_text(
      mAccessType == AT_SAVING
          ? "Select the slot in which to save [X:save O:back]"
          : "Select the slot from which to load [X:load O:back []:delete]");
  SceIoStat file_stat;
  char date_string[30];
  // We're using the same vector for directory names and slots, so we have to
  // clear it
  ...
  for (u32 i = 0; i < NUM_SAVESTATE_SLOTS; ++i) {
    ...
    CUIElement *element;
    if (!rom_id.Empty() &&
        CRomSettingsDB::Get()->GetSettings(rom_id, &settings)) {
      IO::File::Stat(filename_ss, &file_stat);
      sprintf(date_string, "%02d/%02d/%d %02d:%02d:%02d",
              file_stat.st_ctime.month, file_stat.st_ctime.day,
              file_stat.st_ctime.year, file_stat.st_ctime.hour,
              file_stat.st_ctime.minute,
              file_stat.st_ctime.second); // settings.GameName.c_str();
      str << date_string;
      mSlotEmpty[i] = false;
    } else {
    ...
    if (mAccessType == AT_LOADING && mSlotEmpty[i]) {
      ...
      CFunctor1<u32> *functor_1(
          new CMemberFunctor1<ISavestateSelectorComponent, u32>(
              this, &ISavestateSelectorComponent::OnSlotSelected));
      CFunctor *curried(new CCurriedFunctor<u32>(functor_1, i));
...
void ISavestateSelectorComponent::Update(float elapsed_time, const v2 &stick,
                                         u32 old_buttons, u32 new_buttons) {
  //
  //	Trigger the save on the first update AFTER mSelectedSlot was set.
  //	This ensures we get at least one frame where we can display "Saving..."
  //etc.
  //
  if (mSelectedSlot != INVALID_SLOT && !mIsFinished) {
    ...
    IO::Filename filename_ss;
    IO::Filename filename_png;
    MakeSaveSlotPath(filename_ss, filename_png, mSelectedSlot,
                     current_slot_path);
...
void ISavestateSelectorComponent::Render() {
  ...
  if (mSelectedSlot == INVALID_SLOT) {
    mElements.Draw(mpContext, LIST_TEXT_LEFT, LIST_TEXT_WIDTH, AT_LEFT,
                   BELOW_MENU_MIN -
                       mElements.GetSelectedIndex() * (font_height + 2));
    ...
    if (element != NULL) {
      ...
      if (mPVExists[mElements.GetSelectedIndex()] == 1) {
        ...
        if (mPreviewTexture == NULL ||
            mElements.GetSelectedIndex() != mLastPreviewLoad) {
          mPreviewTexture = CNativeTexture::CreateFromPng(
              mPVFilename[mElements.GetSelectedIndex()], TexFmt_8888);
          mLastPreviewLoad = mElements.GetSelectedIndex();
        }

        mpContext->DrawRect(PREVIEW_IMAGE_LEFT, BELOW_MENU_MIN,
        ...
        mpContext->RenderTexture(mPreviewTexture, tl, wh, c32::White);
                 ...
                 mElements.GetSelectedIndex() < NUM_SAVESTATE_SLOTS) {
        mpContext->DrawRect(PREVIEW_IMAGE_LEFT, BELOW_MENU_MIN,
                            PREVIEW_IMAGE_WIDTH, PREVIEW_IMAGE_HEIGHT,
                            c32::White);
        ...
        mpContext->DrawTextAlign(
            PREVIEW_IMAGE_LEFT, PREVIEW_IMAGE_LEFT + PREVIEW_IMAGE_WIDTH,
            AT_CENTRE, BELOW_MENU_MIN + PREVIEW_IMAGE_HEIGHT / 2,
            "No Preview Available", c32::White);
      }
    ...
    const char *title_text(mAccessType == AT_SAVING ? SAVING_STATUS_TEXT
                                                    : LOADING_STATUS_TEXT);

    s32 y((mpContext->GetScreenHeight() - font_height) / 2 + font_height);
    mpContext->DrawTextAlign(0, mpContext->GetScreenWidth(), AT_CENTRE, y,
                             title_text, mpContext->GetDefaultTextColour());
  }

  if (deleteButtonTriggered)
    mpContext->DrawTextAlign(
        0, 480, AT_CENTRE, 135, "Press Triangle to delete this savestate",
