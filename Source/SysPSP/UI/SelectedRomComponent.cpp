...
CSelectedRomComponent::~CSelectedRomComponent() = default;

//

CSelectedRomComponent *
CSelectedRomComponent::Create(CUIContext *p_context,
...
ISelectedRomComponent::ISelectedRomComponent(CUIContext *p_context,
                                             CFunctor *on_start_emulation)
    : CSelectedRomComponent(p_context), OnStartEmulation(on_start_emulation) {
  mElements.Add(new CUICommandImpl(
      new CMemberFunctor<ISelectedRomComponent>(
          this, &ISelectedRomComponent::EditPreferences),
      "Edit Preferences", "Edit various preferences for this rom."));
  mElements.Add(new CUICommandImpl(
      new CMemberFunctor<ISelectedRomComponent>(
          this, &ISelectedRomComponent::AdvancedOptions),
      "Advanced Options", "Edit advanced options for this rom."));
  mElements.Add(
      new CUICommandImpl(new CMemberFunctor<ISelectedRomComponent>(
                             this, &ISelectedRomComponent::CheatOptions),
                         "Cheats", "Enable and select cheats for this rom."));

  mElements.Add(new CUISpacer(16));

  u32 i = mElements.Add(new CUICommandImpl(
      new CMemberFunctor<ISelectedRomComponent>(
          this, &ISelectedRomComponent::StartEmulation),
      "Start Emulation", "Start emulating the selected rom."));

  mElements.SetSelected(i);
}

ISelectedRomComponent::~ISelectedRomComponent() { delete OnStartEmulation; }

void ISelectedRomComponent::Update(float elapsed_time, const v2 &stick,
...
void ISelectedRomComponent::EditPreferences() {
  CRomPreferencesScreen *edit_preferences(
      CRomPreferencesScreen::Create(mpContext, mRomID));
...
void ISelectedRomComponent::AdvancedOptions() {
  CAdvancedOptionsScreen *advanced_options(
      CAdvancedOptionsScreen::Create(mpContext, mRomID));
...
void ISelectedRomComponent::CheatOptions() {
  CCheatOptionsScreen *cheat_options(
      CCheatOptionsScreen::Create(mpContext, mRomID));
  cheat_options->Run();
