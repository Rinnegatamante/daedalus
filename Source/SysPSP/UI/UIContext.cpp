...
public:
  ...
  c32 GetDefaultTextColour() const override {
    return DrawTextUtilities::TextWhite;
  }
  c32 GetSelectedTextColour() const override {
    return mColourPulser.GetCurrentColour();
  }

  void Update(float elapsed_time) override;

  void RenderTexture(const CNativeTexture *texture, const v2 &tl, const v2 &wh,
                     c32 colour) override;
  void RenderTexture(const CNativeTexture *texture, s32 x, s32 y,
                     c32 colour) override;
  ...
  u32 DrawText(s32 x, s32 y, const char *text, u32 length, c32 colour,
               c32 drop_colour) override;
  u32 DrawTextScale(s32 x, s32 y, float scale, const char *text, u32 length,
                    c32 colour) override;
  u32 DrawTextScale(s32 x, s32 y, float scale, const char *text, u32 length,
                    c32 colour, c32 drop_colour) override;
  u32 DrawTextAlign(s32 min_x, s32 max_x, EAlignType align_type, s32 y,
                    const char *text, u32 length, c32 colour) override;
  u32 DrawTextAlign(s32 min_x, s32 max_x, EAlignType align_type, s32 y,
                    const char *text, u32 length, c32 colour,
                    c32 drop_colour) override;
  s32 DrawTextArea(s32 left, s32 top, u32 width, u32 height, const char *text,
                   c32 colour, EVerticalAlign vertical_align) override;

  u32 GetFontHeight() const override;
  u32 GetTextWidth(const char *text) const override;

private:
  s32 AlignText(s32 min_x, s32 max_x, const char *p_str, u32 length,
                EAlignType align_type);
...
IUIContext::IUIContext()
    : mCurrentFont(CDrawText::F_REGULAR),
      mColourPulser(COLOUR_SELECTION_DIM, COLOUR_SELECTION_BRIGHT,
                    MS_PER_COLOUR_CYCLE),
...
void IUIContext::RenderTexture(const CNativeTexture *texture, s32 x, s32 y,
                               c32 colour) {
  if (texture == NULL)
    return;
...
void IUIContext::RenderTexture(const CNativeTexture *texture, const v2 &tl,
                               const v2 &wh, c32 colour) {
  if (texture == NULL)
    return;

  u32 num_verts(2);
  auto *p_verts = (BackgroundTextureVtx *)sceGuGetMemory(
      num_verts * sizeof(BackgroundTextureVtx));
  ...
  sceGuDrawArray(GU_SPRITES, BackgroundTextureVtx::Flags | GU_TRANSFORM_2D,
                 num_verts, NULL, p_verts);
}
...
void IUIContext::DrawRect(s32 x, s32 y, u32 w, u32 h, c32 colour) {
  u32 num_verts(2);
  auto *p_verts = (BackgroundColourVtx *)sceGuGetMemory(
      num_verts * sizeof(BackgroundColourVtx));
  ...
  sceGuDrawArray(GU_SPRITES, BackgroundColourVtx::Flags | GU_TRANSFORM_2D,
                 num_verts, NULL, p_verts);
}
...
void IUIContext::DrawLine(s32 x0, s32 y0, s32 x1, s32 y1, c32 colour) {
  ...
  sceGuDrawArray(GU_LINES, BackgroundColourVtx::Flags | GU_TRANSFORM_2D,
                 num_verts, NULL, p_verts);
}
...
void IUIContext::SetFontStyle(EFontStyle font_style) {
  ...
  case FS_REGULAR:
    ...
    return;
  case FS_HEADING:
    mCurrentFont = CDrawText::F_LARGE_BOLD;
    return;
  }
...
s32 IUIContext::AlignText(s32 min_x, s32 max_x, const char *p_str, u32 length,
                          EAlignType align_type) {
  s32 x;
  ...
  case AT_CENTRE:
    x = min_x + ((max_x - min_x) -
                 CDrawText::GetTextWidth(mCurrentFont, p_str, length)) /
                    2;
    break;
...
u32 IUIContext::DrawText(s32 x, s32 y, const char *text, u32 length,
                         c32 colour) {
...
u32 IUIContext::DrawTextScale(s32 x, s32 y, float scale, const char *text,
...
u32 IUIContext::DrawTextAlign(s32 min_x, s32 max_x, EAlignType align_type,
                              s32 y, const char *text, u32 length, c32 colour) {
...
namespace {
s32 VerticalAlign(EVerticalAlign vertical_align, s32 top, u32 height,
                  u32 text_height) {
  switch (vertical_align) {
  case VA_TOP:
    return top;
  case VA_BOTTOM:
    return top + height - text_height;
  }
#ifdef DAEDALUS_DEBUG_CONSOLE
...
s32 IUIContext::DrawTextArea(s32 left, s32 top, u32 width, u32 height,
                             const char *text, c32 colour,
                             EVerticalAlign vertical_align) {
  const u32 font_height(CDrawText::GetFontHeight(mCurrentFont));
  u32 length = strlen(text);
  std::vector<u32> lengths;
  bool match = false;
  DrawTextUtilities::WrapText(mCurrentFont, width,
                              CDrawText::Translate(text, length), length,
                              lengths, match);

  s32 x(left);
  s32 y(
      VerticalAlign(vertical_align, top, height, lengths.size() * font_height));
...
void IUIContext::BeginRender() { CGraphicsContext::Get()->BeginFrame(); }
