...
CUIElement::~CUIElement() = default;

CUIElementBag::CUIElementBag() : mSelectedIdx(0) {}
...
void CUIElementBag::Draw(CUIContext *context, s32 min_x, s32 max_x,
                         EAlignType halign, s32 y) const {
  ...
  for (u32 i = 0; i < mElements.size(); ++i) {
    ...
    if (y > MINTOP && y < MAXBOT)
      element->Draw(context, min_x, max_x, halign, y, i == mSelectedIdx);
    y += element->GetHeight(context);
  }
}

void CUIElementBag::DrawCentredVertically(CUIContext *context, s32 min_x,
                                          s32 min_y, s32 max_x,
                                          s32 max_y) const {
  s32 total_height(0);
