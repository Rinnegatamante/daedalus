...
CUIScreen::CUIScreen(CUIContext *p_context) : mpContext(p_context) {}
...
void CUIScreen::Run() {
...
#endif
  const u32 PSP_BUTTONS_MASK(
      0xffff); // Mask off e.g. PSP_CTRL_HOME, PSP_CTRL_HOLD etc
  ...
  //  We do this to avoid any undesirable button input after returning to the
  //  emulation from pause menu.
  //
