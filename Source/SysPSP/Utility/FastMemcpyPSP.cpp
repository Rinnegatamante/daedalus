...
//*****************************************************************************
// Uses VFPU if possible else normal memcpy() (orignal code by Alex? Modified by
// Corn)
//*****************************************************************************
//
...
void memcpy_vfpu_byteswap(void *dst, const void *src, size_t size) {
  ...
  if (size >= 4) {
    ...
    if (size >= 4) {
      ...
      if ((((u32)src8 & 0x3) == 0)) {
        // Not atleast 16 bytes at this point? -> not worth trying any VFPU
        // optimisations...
        if (size > 15) {
          src32 = (u32 *)src8;
          dst32 = (u32 *)dst8;

          // Align dst to 16 bytes to gain from vfpu aligned stores or just
        ...
        u32 srcTmp{*src32++};
        u32 dstTmp{0};
        u32 size32{static_cast<u32>(
            size >> 2)}; // Size in dwords (to avoid a sltiu in loop)
