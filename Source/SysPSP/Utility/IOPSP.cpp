...
struct SceIoDirentGuarded {
  SceIoDirent Dirent;
  char Guard[256]; // Not sure, but it looks like SceIoDirent is somehow
                   // mis-declared in the pspsdk, so it ends up scribbling over
                   // stack?
};
...
bool Delete(const char *p_file) { return sceIoRemove(p_file) >= 0; }
...
namespace Directory {
bool Create(const char *p_path) { return sceIoMkdir(p_path, 0777) == 0; }
  ...
  if (fh) {
    while (sceIoDread(fh, &gDirEntry.Dirent)) {
      ...
      if ((stat.st_mode & 0x1000) == 0x1000) {
        if (strcmp(gDirEntry.Dirent.d_name, ".") != 0 &&
            strcmp(gDirEntry.Dirent.d_name, "..")) {
          // printf("Found directory\n");
        }
      } else {
        if (strcmp(FindExtension(file), p_extension) == 0) {
...
bool FindFileOpen(const char *path, FindHandleT *handle, FindDataT &data) {
  *handle = sceIoDopen(path);
  if (*handle >= 0) {
    // To support findfirstfile() API we must return the first result
    // immediately
