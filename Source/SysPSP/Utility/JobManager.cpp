...
CJobManager::CJobManager(u32 job_buffer_size, ETaskMode task_mode)
    : mJobBuffer(malloc_64(job_buffer_size)), mJobBufferSize(job_buffer_size),
      mTaskMode(task_mode), mThread(kInvalidThreadHandle),
      mWorkReady(sceKernelCreateSema("JMWorkReady", 0, 0, 1,
                                     0)) // Initval is 0 - i.e. no work ready
      ,
      mWorkEmpty(sceKernelCreateSema("JMWorkEmpty", 0, 1, 1,
                                     0)) // Initval is 1 - i.e. work done
      ,
      mWantQuit(false) {
...
bool CJobManager::AddJob(SJob *job, u32 job_size) {
  ...
  // clear Cache -> this one is very important without it the CheckME(mei) will
  // not return with the ME status.
  sceKernelDcacheWritebackInvalidateAll();
