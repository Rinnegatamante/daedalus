...
void VolatileMemInit() {
  ...
  if (result == 0) {
    scePowerLock(0); // This used to avoid suspending while we are using the
                     // volatile memory
...
void *malloc_volatile_PSP(size_t size) {
  ...
  //info.arena, malloc_p5_memory_used);

  SceUID uid =
      sceKernelAllocPartitionMemory(5, "", PSP_SMEM_Low, size + 8, nullptr);
  if (uid >= 0) {
