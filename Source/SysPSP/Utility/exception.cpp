...
const char *const pspModel[6] = {"PSP PHAT",  "PSP SLIM", "PSP BRITE",
                                 "PSP BRITE", "PSP GO",   "UNKNOWN PSP"};
...
static void DumpInformation(PspDebugRegBlock *regs) {
          ...
          "\nCPU registers (PSP): ('*' -> pointing inside RDRAM else ':')\n");
  {
    for (int i{0}; i < 32; i += 4)
      fprintf(fp, "\t%s%s%08X %s%s%08X %s%s%08X %s%s%08X\n", regName[i + 0],
              (((u32)regs->r[i + 0] >= RDRAM_base) &
               ((u32)regs->r[i + 0] <= RDRAM_end))
                  ? "*"
                  : ":",
              (int)regs->r[i + 0], regName[i + 1],
              (((u32)regs->r[i + 1] >= RDRAM_base) &
               ((u32)regs->r[i + 1] <= RDRAM_end))
                  ? "*"
                  : ":",
              (int)regs->r[i + 1], regName[i + 2],
              (((u32)regs->r[i + 2] >= RDRAM_base) &
               ((u32)regs->r[i + 2] <= RDRAM_end))
                  ? "*"
                  : ":",
              (int)regs->r[i + 2], regName[i + 3],
              (((u32)regs->r[i + 3] >= RDRAM_base) &
               ((u32)regs->r[i + 3] <= RDRAM_end))
                  ? "*"
                  : ":",
              (int)regs->r[i + 3]);
  }
  ...
  fprintf(fp, "\nFPU registers (PSP):\n");
  {
    for (int i{0}; i < 32; i += 4)
      fprintf(fp, "\t%#+12.7f %#+12.7f %#+12.7f %#+12.7f\n",
              (f32)regs->fpr[i + 0], (f32)regs->fpr[i + 1],
              (f32)regs->fpr[i + 2], (f32)regs->fpr[i + 3]);
  }
  ...
  fprintf(fp, "\nCPU State (N64):\n");
  {
    for (int i{0}; i < 32; i += 4)
      fprintf(fp, "\t%s:%08X-%08X %s:%08X-%08X %s:%08X-%08X %s:%08X-%08X\n",
              regName[i + 0], gCPUState.CPU[i + 0]._u32_1,
              gCPUState.CPU[i + 0]._u32_0, regName[i + 1],
              gCPUState.CPU[i + 1]._u32_1, gCPUState.CPU[i + 1]._u32_0,
              regName[i + 2], gCPUState.CPU[i + 2]._u32_1,
              gCPUState.CPU[i + 2]._u32_0, regName[i + 3],
              gCPUState.CPU[i + 3]._u32_1, gCPUState.CPU[i + 3]._u32_0);
  ...
  const OpCode *op_start(
      reinterpret_cast<const OpCode *>(p_base - inst_before_epc * 4));
  const OpCode *op_end(
      reinterpret_cast<const OpCode *>(p_base + inst_after_epc * 4));
  ...
  while (op_start < op_end) {
    ...
    fprintf(fp, "\t%s%p: <0x%08x> %s\n",
            (u32)p_base == (u32)op_start ? "*" : " ", op_start, op._u32,
            opinfo);
...
void ExceptionHandler(PspDebugRegBlock *regs) {
  ...
  while (!exit) {
    ...
    pspDebugScreenPrintf("Exception[%s] RDRAM[%08X:%08X]\n\n",
                         codeTxt[(regs->cause >> 2) & 31], (int)RDRAM_base,
                         (int)RDRAM_end);
    // pspDebugScreenPrintf("Game Name - %s\n",
    // g_ROM.settings.GameName.c_str()); pspDebugScreenPrintf("Firmware  -
    // %08X\n", sceKernelDevkitVersion()); pspDebugScreenPrintf("Model	- %s\n",
    // pspModel[ kuKernelGetModel() ]);
    // pspDebugScreenPrintf("64MB	 - %s\n", PSP_IS_SLIM ? "Yes" : "No");
    // pspDebugScreenPrintf("Exception - %s\n",codeTxt[(regs->cause >> 2) &
    // 31]);
      ...
      pspDebugScreenPrintf(" %s%s%08X %s%s%08X %s%s%08X %s%s%08X\n",
                           ...
                           (((u32)regs->r[i + 0] >= RDRAM_base) &
                            ((u32)regs->r[i + 0] <= RDRAM_end))
                               ? "*"
                               : ":",
                           (u32)regs->r[i + 0], regName[i + 1],
                           (((u32)regs->r[i + 1] >= RDRAM_base) &
                            ((u32)regs->r[i + 1] <= RDRAM_end))
                               ? "*"
                               : ":",
                           (u32)regs->r[i + 1], regName[i + 2],
                           (((u32)regs->r[i + 2] >= RDRAM_base) &
                            ((u32)regs->r[i + 2] <= RDRAM_end))
                               ? "*"
                               : ":",
                           (u32)regs->r[i + 2], regName[i + 3],
                           (((u32)regs->r[i + 3] >= RDRAM_base) &
                            ((u32)regs->r[i + 3] <= RDRAM_end))
                               ? "*"
                               : ":",
