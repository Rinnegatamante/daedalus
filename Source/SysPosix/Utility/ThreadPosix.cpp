...
//	The real thread is passed in as an argument. We call it and return the
//result
void *StartThreadFunc(void *arg) {
  SDaedThreadDetails *thread_details(
      reinterpret_cast<SDaedThreadDetails *>(arg));
...
ThreadHandle CreateThread(const char *name, DaedThread function,
  ...
  SDaedThreadDetails *thread_details =
      new SDaedThreadDetails(function, argument);

  s32 result =
      ::pthread_create(&thread, &thread_attr, &StartThreadFunc, thread_details);
  if (result == 0) {
...
void ThreadSleepMs(u32 ms) { usleep(ms * 1000); }
