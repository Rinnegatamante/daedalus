...
public:
  CAssemblyWriterX86(CAssemblyBuffer *p_buffer) : mpAssemblyBuffer(p_buffer) {}

public:
  CAssemblyBuffer *GetAssemblyBuffer() const { return mpAssemblyBuffer; }
  void SetAssemblyBuffer(CAssemblyBuffer *p_buffer) {
...
private:
public:
  // I sometimes use this to force an exceptions so I can analyse the generated
  // code
  ...
  inline void NOP() { EmitBYTE(0x90); }

  inline void INT3() { EmitBYTE(0xcc); }
  ...
  void ADD_REG_MEM_IDXx4(EIntelReg destreg, void *mem,
                         EIntelReg idxreg); // add reg, dword ptr[ mem + idx*4 ]

  void AND(EIntelReg reg1, EIntelReg reg2);
  void AND_EAX(u32 mask); // and		eax, 0x00FF	- Mask off top
                          // bits!
  ...
  void CMP_MEM32_I32(const void *p_mem,
                     u32 data); // cmp		dword ptr p_mem, data
  void CMP_MEM32_I8(const void *p_mem,
                    u8 data); // cmp		dword ptr p_mem, data
  ...
  void CALL_MEM_PLUS_REGx4(void *mem,
                           EIntelReg reg); // call dword ptr [mem + reg*4]
  void RET();

  static inline bool IsValidMov8Reg(EIntelReg isrc) { return isrc <= EBX_CODE; }
  ...
  void MOV8_MEM_BASE_REG(EIntelReg ibase,
                         EIntelReg isrc); // mov byte ptr [base], src
  void MOV8_REG_MEM_BASE(EIntelReg idst,
                         EIntelReg ibase); // mov dst, byte ptr [base]

  void MOV16_MEM_BASE_REG(EIntelReg ibase,
                          EIntelReg isrc); // mov word ptr [base], src
  void MOV16_REG_MEM_BASE(
      EIntelReg idst,
      EIntelReg ibase); // mov dst, word ptr[base] e.g. mov ax, word ptr [eax]

  void MOV_MEM_BASE_OFFSET32_REG(
      EIntelReg ibase, s32 offset,
      EIntelReg isrc); // mov dword ptr [base + nnnnnnnn], src
  void
  MOV_MEM_BASE_OFFSET8_REG(EIntelReg ibase, s8 offset,
                           EIntelReg isrc); // mov dword ptr [base + nn], src
  void MOV_MEM_BASE_OFFSET_REG(EIntelReg ibase, s32 offset, EIntelReg isrc);
  void MOV_MEM_BASE_REG(EIntelReg ibase,
                        EIntelReg isrc); // mov dword ptr [base], src

  void
  MOV_REG_MEM_BASE_OFFSET32(EIntelReg idst, EIntelReg ibase,
                            s32 offset); // mov dst, dword ptr [base + nnnnnnnn]
  void MOV_REG_MEM_BASE_OFFSET8(EIntelReg idst, EIntelReg ibase,
                                s8 offset); // mov dst, dword ptr [base + nn]
  void MOV_REG_MEM_BASE_OFFSET(EIntelReg idst, EIntelReg ibase, s32 offset);
  void MOV_REG_MEM_BASE(EIntelReg idst,
                        EIntelReg ibase); // mov dst, dword ptr [base]
  ...
  void MOVSX8(
      EIntelReg idst,
      EIntelReg isrc); // movsx	dst, src		(e.g. movsx eax, al)
  void MOVSX16(
      EIntelReg idst,
      EIntelReg isrc); // movsx	dst, src		(e.g. movsx eax, ax)
...
private:
  ...
  inline void EmitBYTE(u8 byte) { mpAssemblyBuffer->EmitBYTE(byte); }

  inline void EmitWORD(u16 word) { mpAssemblyBuffer->EmitWORD(word); }

  inline void EmitDWORD(u32 dword) { mpAssemblyBuffer->EmitDWORD(dword); }
