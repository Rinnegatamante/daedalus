...
The second buffer is used to hold conditionally executed code pieces that will
...
The second buffer must come AFTER the first in the memory layout, otherwise
branch prediction will be screwed up.

This is the same algorithm used in the Linux schedule() function
(/usr/src/linux/kernel/sched.c)

The only problem of this system is that it uses 32-bit relative addresses (and
thus 6-byte long instructions) are used. However managing 8-bit +127/-128
relative displacements would be challenging since they would interfere with the
normal code Otherwise a 16-bit override prefix could be used (but is it
advantageous?)
*/
...
public:
  CCodeBufferManagerX86()
      : mpBuffer(NULL), mBufferPtr(0), mBufferSize(0), mpSecondBuffer(NULL),
        mSecondBufferPtr(0), mSecondBufferSize(0) {}
...
bool CCodeBufferManagerX86::Initialise() {
  ...
  mpBuffer = (u8 *)VirtualAlloc(NULL, 256 * 1024 * 1024, MEM_RESERVE,
...
CCodeGenerator *CCodeBufferManagerX86::StartNewBlock() {
  ...
  // This is a bit of a hack. We assume that no single entry will generate more
  // than 32k of storage. If there appear to be problems with this assumption,
  // this value can be enlarged
  if (mBufferPtr + 32768 > mBufferSize) {
    ...
    pNewAddress =
        VirtualAlloc(mpBuffer, mBufferSize, MEM_COMMIT, PAGE_EXECUTE_READWRITE);
    if (pNewAddress == 0) {
      DBGConsole_Msg(
          0, "SR Buffer allocation failed"); // maybe this should be an abort?
    } else {
  ...
  if (mSecondBufferPtr + 32768 > mSecondBufferSize) {
    ...
    pNewAddress = VirtualAlloc(mpSecondBuffer, mSecondBufferSize, MEM_COMMIT,
                               PAGE_EXECUTE_READWRITE);
    if (pNewAddress == 0) {
      DBGConsole_Msg(0,
                     "SR Second Buffer allocation failed"); // maybe this should
                                                            // be an abort?
    } else {
...
u32 CCodeBufferManagerX86::FinaliseCurrentBlock() {
  ...
  mSecondBufferPtr =
      ((mSecondBufferPtr - 1) & 0xfffffff0) + 0x10; // align to 16-byte boundary
