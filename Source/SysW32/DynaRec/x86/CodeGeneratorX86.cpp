...
// XX this optimisation works very well on the PSP, option to disable it was
// removed
...
class CRegisterStatus {
  struct CachedRegInfo {
    EIntelReg iCachedIReg; // INVALID_CODE if uncached, else ???_CODE of intel
                           // reg we're cached in
    bool BLoValid; // If cached, true if value in intel register is valid
    bool BLoDirty; // If cached, true if value has been modified

    u32 HiValue;   // If BHiUnknown is false, this is the value of the register
                   // (through setmipshi..)
    bool BHiDirty; // Do we need to write information about this register back
                   // to memory?
...
//
//*****************************************************************************
CCodeGeneratorX86::CCodeGeneratorX86(CAssemblyBuffer *p_primary,
                                     CAssemblyBuffer *p_secondary)
    : CCodeGenerator(), CAssemblyWriterX86(p_primary), mSpCachedInESI(false),
      mSetSpPostUpdate(0), mpPrimary(p_primary), mpSecondary(p_secondary) {}

//*****************************************************************************
//
//*****************************************************************************
void CCodeGeneratorX86::Finalise(
...
//
//*****************************************************************************
void CCodeGeneratorX86::Initialise(u32 entry_address, u32 exit_address,
...
//
//*****************************************************************************
CJumpLocation CCodeGeneratorX86::GenerateExitCode(u32 exit_address,
                                                  u32 jump_address,
  ...
  // found
  CJumpLocation jump_to_next_fragment(GenerateBranchIfNotSet(
      const_cast<u32 *>(&gCPUState.StuffToDo), next_fragment));
...
//*****************************************************************************
void CCodeGeneratorX86::GenerateEretExitCode(u32 num_instructions,
                                             CIndirectExitMap *p_map) {
  MOVI(ECX_CODE, num_instructions);
  CALL(CCodeLabel(CPU_UpdateCounter));

  // We always exit to the interpreter, regardless of the state of
  // gCPUState.StuffToDo
...
//*****************************************************************************
void CCodeGeneratorX86::GenerateIndirectExitCode(u32 num_instructions,
                                                 CIndirectExitMap *p_map) {
  MOVI(ECX_CODE, num_instructions);
  CALL(CCodeLabel(CPU_UpdateCounter));
  ...
  CJumpLocation jump_to_next_fragment(GenerateBranchIfNotSet(
      const_cast<u32 *>(&gCPUState.StuffToDo), no_target));
...
//
//*****************************************************************************
void CCodeGeneratorX86::GenerateExceptionHander(
    ExceptionHandlerFn p_exception_handler_fn,
    const std::vector<CJumpLocation> &exception_handler_jumps) {
  ...
  for (std::vector<CJumpLocation>::const_iterator it =
           exception_handler_jumps.begin();
       it != exception_handler_jumps.end(); ++it) {
...
//
//*****************************************************************************
void CCodeGeneratorX86::GenerateBranchHandler(CJumpLocation branch_handler_jump,
...
//
//*****************************************************************************
CJumpLocation CCodeGeneratorX86::GenerateBranchIfSet(const u32 *p_var,
                                                     CCodeLabel target) {
...
//
//*****************************************************************************
CJumpLocation CCodeGeneratorX86::GenerateBranchIfNotSet(const u32 *p_var,
                                                        CCodeLabel target) {
  MOV_REG_MEM(EAX_CODE, p_var);
  TEST(EAX_CODE, EAX_CODE);
...
//
//*****************************************************************************
CJumpLocation CCodeGeneratorX86::GenerateBranchIfEqual32(const u32 *p_var,
                                                         u32 value,
                                                         CCodeLabel target) {
...
//
//*****************************************************************************
CJumpLocation CCodeGeneratorX86::GenerateBranchIfEqual8(const u32 *p_var,
                                                        u8 value,
                                                        CCodeLabel target) {
...
//
//*****************************************************************************
CJumpLocation CCodeGeneratorX86::GenerateBranchIfNotEqual32(const u32 *p_var,
                                                            u32 value,
                                                            CCodeLabel target) {
  CMP_MEM32_I32(p_var, value);
...
//
//*****************************************************************************
CJumpLocation CCodeGeneratorX86::GenerateBranchIfNotEqual8(const u32 *p_var,
                                                           u8 value,
                                                           CCodeLabel target) {
  CMP_MEM32_I8(p_var, value);
...
//*****************************************************************************
CJumpLocation CCodeGeneratorX86::GenerateOpCode(const STraceEntry &ti,
  ...
  // For LW, SW, SWC1, LB etc, only generate an exception handler if access
  // wasn't done through the stack (handle = false) This will have to be
  // reworked once we handle accesses other than the stack!
  case OP_LW:
  ...
  case OP_ADDI:
    ...
    //		case SpecOp_SLL:	GenerateSLL( rd, rt, sa );	handled = true;
    //break; 		case SpecOp_SRA:	GenerateSRA( rd, rt, sa );	handled = true;
    //break; 		case SpecOp_SRL:	GenerateSRL( rd, rt, sa );	handled = true;
    //break;
    //		}
    //	}
    //	break;
  ...
  if (exception) {
    exception_handler =
        GenerateBranchIfSet(const_cast<u32 *>(&gCPUState.StuffToDo), no_target);
  }
  ...
  if (p_branch != NULL) {
    ...
    if (p_branch->Direct) {
      if (p_branch->ConditionalBranchTaken) {
        *p_branch_jump =
            GenerateBranchIfNotEqual8(&gCPUState.Delay, DO_DELAY, no_target);
      } else {
        *p_branch_jump =
            GenerateBranchIfEqual8(&gCPUState.Delay, DO_DELAY, no_target);
      }
    } else {
      ...
      if (p_branch->Eret) {
        ...
        *p_branch_jump = GenerateBranchIfNotEqual32(
...
//
//*****************************************************************************
void CCodeGeneratorX86::GenerateGenericR4300(OpCode op_code,
...
//
//*****************************************************************************
CJumpLocation CCodeGeneratorX86::ExecuteNativeFunction(CCodeLabel speed_hack,
                                                       bool check_return) {
  CALL(speed_hack);
  if (check_return) {
    TEST(EAX_CODE, EAX_CODE);
...
void CCodeGeneratorX86::GenerateLoad(u32 memBase, EN64Reg base, s16 offset,
                                     u8 twiddle, u8 bits) {
  MOV_REG_MEM(ECX_CODE, &gCPUState.CPU[base]._u32_0);
  if (twiddle == 0) {
...
void R4300_CALL_TYPE _EnterDynaRec(const void *p_function,
                                   const void *p_base_pointer,
                                   const void *p_rebased_mem, u32 mem_limit) {
