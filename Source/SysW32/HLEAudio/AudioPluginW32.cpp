...
// :( Would be nice to get it working correctly, since running audio in the main
// thread is abit jerky #define AUDIO_THREADED
...
CAudioPluginW32::CAudioPluginW32()
    : Dacrate(0), AIReady(false), lpds(NULL), lpdsbuf(NULL), lpdsNotify(NULL) {}
...
//
//*****************************************************************************
CAudioPluginW32 *CAudioPluginW32::Create() { return new CAudioPluginW32(); }
...
bool CAudioPluginW32::StartEmulation() {
  ...
  if (FAILED(hr = IDirectSound8_SetCooperativeLevel(lpds, GetConsoleWindow(),
                                                    DSSCL_PRIORITY))) {
    return false;
  }
  for (count = 0; count < NUMCAPTUREEVENTS; count++) {
    rghEvent[count] = CreateEvent(NULL, false, false, NULL);
    if (rghEvent[count] == NULL) {
      return false;
    }
  }
  Dacrate = 0;
...
void CAudioPluginW32::LenChanged() {
  ...
  Snd1Len = (len & 0x3FFF8);
  Snd1ReadPos =
      g_pu8RamBase + (Memory_AI_GetRegister(AI_DRAM_ADDR_REG) & 0x00FFFFF8);
  AIReady = true;
...
//
//*****************************************************************************
u32 CAudioPluginW32::ReadLength() { return Snd1Len; }
...
void CAudioPluginW32::Update(bool Wait) {
  ...
  if (Wait && Playing) {
    ...
    dwEvt = MsgWaitForMultipleObjects(NUMCAPTUREEVENTS, rghEvent, false, 0,
                                      QS_ALLINPUT);
  }
  dwEvt -= WAIT_OBJECT_0;
  ...
  // Speed Sync
  if (Playing && (SndBuffer[2] == Buffer_Full) ||
      (SndBuffer[1] == Buffer_Full) || (SndBuffer[0] == Buffer_Full)) {
    ThreadSleepMs(10);
  }
}
...
//
//*****************************************************************************
CAudioPlugin *CreateAudioPlugin() { return CAudioPluginW32::Create(); }
...
void CAudioPluginW32::SetupDSoundBuffers(void) {
  ...
  if (FAILED(hr = IDirectSound8_CreateSoundBuffer(lpds, &dsbdesc, &lpdsbuf,
                                                  NULL))) {
  ...
  if (FAILED(hr = IDirectSound8_QueryInterface(lpdsbuf, IID_IDirectSoundNotify,
                                               (VOID **)&lpdsNotify))) {
    return;
  }

  // Set capture buffer notifications.
  if (FAILED(hr = IDirectSoundNotify_SetNotificationPositions(
                 lpdsNotify, NUMCAPTUREEVENTS, rgdscbpn))) {
    return;
  }
...
bool CAudioPluginW32::FillBufferWithSilence(LPDIRECTSOUNDBUFFER lpDsb) {
  ...
  if (FAILED(IDirectSoundBuffer8_GetFormat(lpDsb, &wfx, sizeof(WAVEFORMATEX),
                                           (LPDWORD)&dwSizeWritten))) {
    return false;
  }

  if (SUCCEEDED(IDirectSoundBuffer8_Lock(lpDsb, 0, 0, (LPVOID *)&pb1,
                                         (LPDWORD)&cb1, NULL, NULL,
...
void CAudioPluginW32::FillSectionWithSilence(int buffer) {
  ...
  if (FAILED(IDirectSoundBuffer8_Lock(lpdsbuf, BufferSize * buffer, BufferSize,
                                      &lpvData, (LPDWORD)&dwBytesLocked, NULL,
                                      NULL, 0))) {
...
void CAudioPluginW32::FillBuffer(int buffer) {
  ...
  if (SndBuffer[buffer] == Buffer_Empty) {
    if (Snd1Len >= BufferSize) {
      if (FAILED(IDirectSoundBuffer8_Lock(
              lpdsbuf, BufferSize * buffer, BufferSize, &lpvData,
      ...
      if (FAILED(IDirectSoundBuffer8_Lock(lpdsbuf, BufferSize * buffer, Snd1Len,
                                          &lpvData, (LPDWORD)&dwBytesLocked,
                                          NULL, NULL, 0))) {
        IDirectSoundBuffer8_Unlock(lpdsbuf, lpvData, dwBytesLocked, NULL, 0);
        DAEDALUS_ERROR("FAILED lock");
        return;
    ...
    if (Snd1Len >= SpaceLeft) {
      if (FAILED(IDirectSoundBuffer8_Lock(
              lpdsbuf, (BufferSize * (buffer + 1)) - SpaceLeft, SpaceLeft,
              &lpvData, (LPDWORD)&dwBytesLocked, NULL, NULL, 0))) {
        IDirectSoundBuffer8_Unlock(lpdsbuf, lpvData, dwBytesLocked, NULL, 0);
        DAEDALUS_ERROR("FAILED lock");
        return;
      }
      ...
      if (FAILED(IDirectSoundBuffer8_Lock(
              lpdsbuf, (BufferSize * (buffer + 1)) - SpaceLeft, Snd1Len,
              &lpvData, (LPDWORD)&dwBytesLocked, NULL, NULL, 0))) {
        IDirectSoundBuffer8_Unlock(lpdsbuf, lpvData, dwBytesLocked, NULL, 0);
        DAEDALUS_ERROR("FAILED lock");
        return;
