...
FileNameHandler::FileNameHandler(LPCTSTR szSectionName, LPCTSTR szFilter,
                                 int nFilterIndex, LPCTSTR szDefExt,
...
void FileNameHandler::GetDefaultDirectory(LPTSTR szOutBuffer) {
  ...
  if (RegCreateKeyEx(HKEY_CURRENT_USER,
                     "Software\\Daedalus\\Default Directory\\", 0,
                     m_szSectionName, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS,
  ...
  if (nResult == REG_CREATED_NEW_KEY) {
    ...
    if (RegQueryValueEx(hRegKey, m_szSectionName, NULL, &datatype,
                        (LPBYTE)szOutBuffer, &datasize) != ERROR_SUCCESS) {
      bNeedToSetKey = TRUE;
    }
  } else {
  ...
  if (bNeedToSetKey) {
    ...
    RegSetValueEx(hRegKey, m_szSectionName, 0, REG_SZ, (LPBYTE)szOutBuffer,
                  lstrlen(szOutBuffer) + 1);
  }
...
void FileNameHandler::SetDefaultDirectory(LPCTSTR szDir) {
  ...
  RegSetValueEx(hRegKey, m_szSectionName, 0, REG_SZ, (LPBYTE)szDir,
                lstrlen(szDir) + 1);
