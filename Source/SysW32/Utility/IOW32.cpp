...
bool Delete(const char *p_file) { return ::DeleteFile(p_file) ? true : false; }
...
char *RemoveBackslash(char *p_path) { return ::PathRemoveBackslash(p_path); }

bool RemoveFileSpec(char *p_path) {
  return ::PathRemoveFileSpec(p_path) ? true : false;
}

void RemoveExtension(char *p_path) { ::PathRemoveExtension(p_path); }
