...
ThreadHandle CreateThread(const char *name, DaedThread function,
  ...
  HANDLE h(::CreateThread(NULL, 0, StartThreadFunc, thread_details,
                          CREATE_SUSPENDED, &id));
...
void ThreadSleepMs(u32 ms) { ::Sleep(ms); }
...
void ThreadYield() { ::Sleep(0); }

void TerminateThread(long handle) { ::TerminateThread((HANDLE)handle, 0); }
