...
static EAssertResult BatchAssertHook(const char *expression, const char *file,
...
static void SprintRunDirectory(IO::Filename &rundir, const char *batchdir,
                               u32 run_id) {
  char filename[64];
...
void BatchTestMain(int argc, char *argv[]) {
  ...
  bool random_order(false); // Whether to randomise the order of processing, to
                            // help avoid hangs
  ...
  while (!roms.empty()) {
    ...
    // Picking roms in a random order means we can work around roms which crash
    // the emulator a little more easily
    if (random_order)
      idx = rand() % roms.size();
    ...
    if (!update_results && result_exists) {
      ...
      if (!gRomLogFH) {
        ...
        // TODO: use ROM_GetRomDetailsByFilename and the alternative form of
        // ROM_LoadFile with overridden preferences (allows us to test if roms
        // break by changing prefs)
        System_Open(r.c_str());
        ...
        const char *reason(CBatchTestEventHandler::GetTerminationReasonString(
            gBatchTestEventHandler->GetTerminationReason()));
        ...
        if (!IO::File::Move(tmpfilepath, rom_logpath)) {
          fprintf(gBatchFH, "%#.3f: Coping %s -> %s failed\n",
                  timer.GetElapsedSecondsSinceReset(), tmpfilepath,
                  rom_logpath);
        }
      }
    }
...
CBatchTestEventHandler::CBatchTestEventHandler()
    : mNumDisplayListsCompleted(0), mNumVerticalBlanksSinceDisplayList(0),
...
EAssertResult CBatchTestEventHandler::OnAssert(const char *expression,
                                               const char *file,
                                               unsigned int line,
  ...
  std::vector<u32>::iterator it(
      std::lower_bound(mAsserts.begin(), mAsserts.end(), assert_hash));
  if (it == mAsserts.end() || *it != assert_hash) {
    if (gRomLogFH) {
      fprintf(gRomLogFH, "! Assert Failed: Location: %s(%d), [%s] %s\n", file,
              line, expression, formatted_msg);
    }
...
const char *
CBatchTestEventHandler::GetTerminationReasonString(ETerminationReason reason) {
  switch (reason) {
  case TR_UNKNOWN:
    return "Unknown";
