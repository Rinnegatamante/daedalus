...
static u32 gTicksBetweenVbls =
    0; // How many ticks we want to delay between vertical blanks
...
static u32 gVblsSinceFlip = 0; // The number of vertical blanks that have
                               // occurred since the last n64 flip
...
bool FramerateLimiter_Reset() {
  ...
  if (NTiming::GetPreciseFrequency(&frequency)) {
    ...
    DAEDALUS_ASSERT(g_ROM.TvType <= sizeof(gTvFrequencies) / sizeof(u32),
                    "Unknown TV type: %d", g_ROM.TvType);
#endif
...
void FramerateLimiter_Limit() {
  ...
  gCurrentAverageTicksPerVbl =
      FramerateLimiter_UpdateAverageTicksPerVbl(elapsed_ticks / gVblsSinceFlip);

  if (gSpeedSyncEnabled && !gAuxSyncFn) {
    u32 required_ticks = gTicksBetweenVbls * gVblsSinceFlip;

    if (gSpeedSyncEnabled == 2)
      required_ticks = required_ticks << 1; // Slow down to 1/2 speed //Corn

    // FIXME the constant here will need to be adjusted for different platforms.
    s32 delay_ticks = required_ticks - elapsed_ticks -
                      50; // Remove ~50 ticks for additional processing
...
u32 FramerateLimiter_GetTvFrequencyHz() { return gTvFrequencies[g_ROM.TvType]; }
