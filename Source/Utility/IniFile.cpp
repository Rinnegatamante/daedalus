...
//
//*****************************************************************************
CIniFileProperty::~CIniFileProperty() = default;
...
public:
  ...
  const char *GetName() const override { return mName.c_str(); }
  bool FindProperty(const char *p_name,
                    const CIniFileProperty **p_property) const override;
...
private:
  ...
  struct SCompareProperties {
    bool operator()(const IIniFileProperty *a,
                    const IIniFileProperty *b) const {
...
//
//*****************************************************************************
CIniFileSection::~CIniFileSection() = default;
...
//
//*****************************************************************************
bool IIniFileSection::FindProperty(const char *p_name,
                                   const CIniFileProperty **p_property) const {
  auto it(std::lower_bound(mProperties.begin(), mProperties.end(), p_name,
                           SCompareProperties()));
...
void IIniFileSection::AddProperty(const IIniFileProperty *p_property) {
  auto it(std::lower_bound(mProperties.begin(), mProperties.end(),
                           p_property->GetName(), SCompareProperties()));
#ifdef DAEDALUS_ENABLE_ASSERTS
  DAEDALUS_ASSERT(it == mProperties.end() ||
                      strcmp((*it)->mName.c_str(), p_property->GetName()) != 0,
...
public:
  ...
  GetSectionByName(const char *section_name) const override;
  ...
  while (fgets(readinfo, BUFFER_LEN, fh) != NULL) {
    ...
    if (*readinfo != 0) {
      ...
      if (readinfo[0] == '{' || readinfo[0] == '[') {
        ...
        Tidy(value);
#ifdef DAEDALUS_ENABLE_ASSERTS
        DAEDALUS_ASSERT(p_current_section != NULL,
...
//*****************************************************************************
const CIniFileSection *
IIniFile::GetSectionByName(const char *section_name) const {
