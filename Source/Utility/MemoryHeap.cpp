...
//  Taken from
//  http://svn.ps2dev.org/filedetails.php?repname=psp&path=%2Ftrunk%2Fpspgl%2Fpspgl_vidmem.c&rev=0&sc=0
//
//	We probably need something a bit more suitable for our requirements
//eventually.
//
...
//
//*****************************************************************************
CMemoryHeap *CMemoryHeap::Create(u32 size) { return new IMemoryHeap(size); }
...
//
//*****************************************************************************
CMemoryHeap::~CMemoryHeap() = default;

//*****************************************************************************
//
//*****************************************************************************
IMemoryHeap::IMemoryHeap(u32 size)
    : mBasePtr(new u8[size]), mTotalSize(size), mDeleteOnDestruction(true),
      mpMemMap(NULL), mMemMapLen(0)
#ifdef SHOW_MEM
      ,
      mMemAlloc(0)
#endif
...
IMemoryHeap::IMemoryHeap(void *base_ptr, u32 size)
    : mBasePtr(reinterpret_cast<u8 *>(base_ptr)), mTotalSize(size),
      mDeleteOnDestruction(false), mpMemMap(NULL), mMemMapLen(0)
#ifdef SHOW_MEM
      ,
      mMemAlloc(0)
#endif
...
void *IMemoryHeap::InsertNew(u32 idx, u8 *adr, u32 size) {
  auto *tmp = reinterpret_cast<Chunk *>(
      realloc(mpMemMap, (mMemMapLen + 1) * sizeof(mpMemMap[0])));
  if (tmp == NULL) {
    return NULL;
  }

  mpMemMap = tmp;
  memmove(&mpMemMap[idx + 1], &mpMemMap[idx],
          (mMemMapLen - idx) * sizeof(mpMemMap[0]));
  mMemMapLen++;
...
void *IMemoryHeap::Alloc(u32 size) {
  // FIXME(strmnnrmn): This code was removed at some point - does something else
  // help guarantee alignment? Might be that we just want to lower it to 4 or 8.
...
#ifdef DAEDALUS_ENABLE_ASSERTS
  DAEDALUS_ASSERT(ptr == NULL || IsFromHeap(ptr),
                  "Memory is not from this heap");
#endif
  if (ptr == NULL)
    return;
...
#endif
      mMemMapLen--;
      memmove(&mpMemMap[i], &mpMemMap[i + 1],
              (mMemMapLen - i) * sizeof(mpMemMap[0]));
      tmp = reinterpret_cast<Chunk *>(
          realloc(mpMemMap, mMemMapLen * sizeof(mpMemMap[0])));
      if (tmp != NULL) {
        mpMemMap = tmp;
      }
    }
