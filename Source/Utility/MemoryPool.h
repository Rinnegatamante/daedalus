...
public:
  ...
  inline void Deallocate(void *ptr) {
    DAEDALUS_ASSERT(ptr == NULL ||
                        ptr >= mMemory && ptr < &mMemory[MaxItems * ItemSize],
                    "Trying to free block outside of our chunk");
...
public:
  ...
  pointer allocate(size_type size,
                   CMemoryPoolAllocator<void>::const_pointer hint = 0) const {
    if (size == 1) {
      return mPool.Allocate();
    }

    DAEDALUS_ERROR("Can't handle multiple allocations - don't use this "
                   "allocator e.g. with std::vector");
    return NULL;
  }
...
inline bool operator==(const CMemoryPoolAllocator<T> &,
                       const CMemoryPoolAllocator<U> &) {
  return true;
}
