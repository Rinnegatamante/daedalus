...
static const ETextureHashFrequency kDefaultTextureHashFrequency =
    THF_EVERY_FRAME;
#endif
...
public:
  ...
  bool GetRomPreferences(const RomID &id,
                         SRomPreferences *preferences) const override;
  void SetRomPreferences(const RomID &id,
                         const SRomPreferences &preferences) override;

private:
  void OutputSectionDetails(const RomID &id, const SRomPreferences &preferences,
                            FILE *fh);
...
CPreferences::~CPreferences() = default;
...
//		INT_SETTING( gGlobalPreferences, Language, defaults );
    ...
    if (section->FindProperty("Language", &property)) {
      gGlobalPreferences.Language =
          Translate_IndexFromName(property->GetValue());
    }
#endif
    if (section->FindProperty("GuiColor", &property)) {
      u32 value(property->GetIntValue(defaults.GuiColor));
      if (value <
          NUM_COLOR_TYPES) // value >= 0 && not needed as it's always True
      {
    ...
    if (section->FindProperty("ViewportType", &property)) {
      ...
      if (value <
          NUM_VIEWPORT_TYPES) // value >= 0 && Not need as it's always True
      {
  ...
  for (u32 section_idx = 0; section_idx < p_ini_file->GetNumSections();
       ++section_idx) {
    ...
    if (section->FindProperty("CheckTextureHashFrequency", &property)) {
      preferences.CheckTextureHashFrequency =
          GetTextureHashFrequencyFromFrames(atoi(property->GetValue()));
    }
    if (section->FindProperty("Frameskip", &property)) {
      preferences.Frameskip =
          GetFrameskipValueFromInt(atoi(property->GetValue()));
    }
    ...
    if (section->FindProperty("Controller", &property)) {
      preferences.ControllerIndex =
          CInputManager::Get()->GetConfigurationFromName(property->GetValue());
    }
#endif
...
void IPreferences::OutputSectionDetails(const RomID &id,
                                        const SRomPreferences &preferences,
                                        FILE *fh) {
  ...
  fprintf(
      fh, "CheckTextureHashFrequency=%d\n",
...
#define OUTPUT_LANGUAGE(b, nm, def)                                            \
  fprintf(fh, "%s=%s\n", #nm, Translate_NameFromIndex((b).nm));
    ...
    for (PreferencesMap::const_iterator it = mPreferences.begin();
         it != mPreferences.end(); ++it) {
...
// not in the database
bool IPreferences::GetRomPreferences(const RomID &id,
                                     SRomPreferences *preferences) const {
...
// Update the preferences for the specified rom - creates a new entry if
// necessary
void IPreferences::SetRomPreferences(const RomID &id,
                                     const SRomPreferences &preferences) {
  auto it(mPreferences.find(id));
  if (it != mPreferences.end()) {
...
#ifdef DAEDALUS_DEBUG_DISPLAYLIST
      ,
      HighlightInexactBlendModes(false), CustomBlendModes(true)
#endif
      ,
      BatteryWarning(false), LargeROMBuffer(true), ForceLinearFilter(false),
      RumblePak(false), GuiColor(BLACK), StickMinDeadzone(0.28f),
      StickMaxDeadzone(1.0f), Language(0), ViewportType(VT_FULLSCREEN),
      TVEnable(false), TVLaced(false), TVType(TT_4_3) {
}

void SGlobalPreferences::Apply() const {}

SRomPreferences::SRomPreferences()
    : PatchesEnabled(true), DynarecEnabled(true), DynarecLoopOptimisation(true),
      DynarecDoublesOptimisation(true), DoubleDisplayEnabled(true),
      CleanSceneEnabled(false), ClearDepthFrameBuffer(false),
      AudioRateMatch(false), VideoRateMatch(false), FogEnabled(false),
      MemoryAccessOptimisation(true), CheatsEnabled(false)
      //	,	AudioAdaptFrequency( false )
      ,
      CheckTextureHashFrequency(kDefaultTextureHashFrequency),
      Frameskip(FV_DISABLED), AudioEnabled(kDefaultAudioPluginMode),
      ZoomX(1.0f), SpeedSyncEnabled(1), ControllerIndex(0) {}
...
void SRomPreferences::Apply() const {
  ...
  gDynarecLoopOptimisation =
      DynarecLoopOptimisation; // && g_ROM.settings.DynarecLoopOptimisation;
  gDynarecDoublesOptimisation =
      g_ROM.settings.DynarecDoublesOptimisation || DynarecDoublesOptimisation;
  gDoubleDisplayEnabled =
      g_ROM.settings.DoubleDisplayEnabled &&
      DoubleDisplayEnabled; // I don't know why DD won't disabled if we set ||
  gCleanSceneEnabled = g_ROM.settings.CleanSceneEnabled || CleanSceneEnabled;
  gClearDepthFrameBuffer =
      g_ROM.settings.ClearDepthFrameBuffer || ClearDepthFrameBuffer;
  ...
  gCheckTextureHashFrequency =
      GetTexureHashFrequencyAsFrames(CheckTextureHashFrequency);
  gMemoryAccessOptimisation =
      g_ROM.settings.MemoryAccessOptimisation || MemoryAccessOptimisation;
...
#ifdef DAEDALUS_PSP
  CInputManager::Get()->SetConfiguration(
      ControllerIndex); // Used after initialization
#endif
}
...
const char *
Preferences_GetTextureHashFrequencyDescription(ETextureHashFrequency thf) {
  if (thf >= 0 && thf < NUM_THF) {
