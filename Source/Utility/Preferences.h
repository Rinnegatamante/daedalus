...
struct SRomPreferences {
  ...
  u32 ControllerIndex;
  //	u32							PAD1;	//Some Bug in GCC that require
  //to pad the struct some times...(?)
...
public:
  ...
  virtual bool GetRomPreferences(const RomID &id,
                                 SRomPreferences *preferences) const = 0;
  virtual void SetRomPreferences(const RomID &id,
                                 const SRomPreferences &preferences) = 0;
};
