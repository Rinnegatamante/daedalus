...
public:
  ...
  CProfileItem(const char *p_str) : mName(p_str) {}
...
inline u32 ComputeCallstackHash(const std::vector<CProfileItem *> &items) {
  ...
  return murmur2_hash(reinterpret_cast<const u8 *>(&items[0]),
                      items.size() * sizeof(CProfileItem *), 0);
}

class CProfileCallstack {
public:
  CProfileCallstack(const CProfileCallstack *parent,
                    const std::vector<CProfileItem *> &items)
      : mParent(parent), mItems(items), mHash(ComputeCallstackHash(items)),
        mTotalTime(0), mStartTime(0), mHitCount(0) {}

  CProfileItem *GetBack() const {
    return mItems.empty() ? NULL : mItems.back();
  }
...
private:
  ...
  u32 mHitCount;  // The total number of times StartTiming() has been called
...
CProfilerImpl::~CProfilerImpl() {
  for (ProfileItemList::iterator it = mItems.begin(); it != mItems.end();
       ++it) {
...
void CProfilerImpl::Update() {
  DAEDALUS_ASSERT(mActiveCallstacks.size() == mActiveItems.size(),
  ...
  for (CallstackStatsMap::const_iterator it = mCallstackStatsMap.begin();
       it != mCallstackStatsMap.end(); ++it) {
  ...
  std::sort(active_callstacks.begin(), active_callstacks.end(),
            SortByCallstack());

  //       0         1         2         3         4         5         6 7 8
  //       012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789
  printf(" Function                                         Time us  Parent "
         "Overall  Hits\n");
  ...
  for (u32 i = 0; i < active_callstacks.size(); ++i) {
    ...
    u32 total_us =
        u32(callstack->GetTotalTime() * 1000.0f * 1000.0f * mFrequencyInv);
    ...
    if (parent_time != 0) {
      percent_parent_time =
          100.0f * f32(callstack->GetTotalTime()) / f32(parent_time);
    }
    if (total_root_time != 0) {
      percent_total_time =
          100.0f * f32(callstack->GetTotalTime()) / f32(total_root_time);
    }

    char line[1024];
    sprintf(line, "\033[2K%x%*s%s", depth, depth, "",
            callstack->GetBack()->GetName());
    Pad(line, 54);
    printf("%s %6.2f %6.1f%% %6.1f%% %5d%s\n", line, (f32)total_us / 1000.0f,
           percent_parent_time, percent_total_time, hit_count,
           TERMINAL_ERASE_TO_EOL);
    // DBGConsole_Msg( 0, "%*s %s %d,%03dms (%d calls)", depth, "",
    // p_item->GetName(), total_us / 1000, total_us % 1000, hit_count );
  }
  ...
  for (CallstackStatsMap::iterator it = mCallstackStatsMap.begin();
       it != mCallstackStatsMap.end(); ++it) {
...
CProfileCallstack *CProfilerImpl::GetActiveStats() {
  ...
  if (it == mCallstackStatsMap.end()) {
    CProfileCallstack *parent_callstack(
        mActiveCallstacks.empty() ? NULL : mActiveCallstacks.back());
...
CProfiler::CProfiler() : mpImpl(new CProfilerImpl()) {}

CProfiler::~CProfiler() { delete mpImpl; }
...
void CProfiler::Enter(SProfileItemHandle handle) { mpImpl->Enter(handle); }

void CProfiler::Exit(SProfileItemHandle handle) { mpImpl->Exit(handle); }

void CProfiler::Update() { mpImpl->Update(); }

void CProfiler::Display() { mpImpl->Display(); }
