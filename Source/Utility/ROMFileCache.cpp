...
struct SChunkInfo {
  ...
  bool InUse() const { return StartOffset != INVALID_ADDRESS; }
};
...
ROMFileCache::ROMFileCache()
    : mpROMFile(NULL), mChunkMapEntries(0), mpChunkMap(NULL), mMRUIdx(0) {
#ifdef DAEDALUS_PSP
  CHUNK_SIZE = 16 * 1024;
  if (PSP_IS_SLIM) {
    // 32MB cache(SLIM)
  ...
  DAEDALUS_ASSERT((1 << (sizeof(CacheIdx) * 8)) > CACHE_SIZE,
                  "Need to increase size of CacheIdx typedef to allow "
                  "sufficient entries to be indexed");
#endif
...
//
//*****************************************************************************
inline u32 AddressToChunkMapIndex(u32 address) { return address / CHUNK_SIZE; }
...
void ROMFileCache::PurgeChunk(CacheIdx cache_idx) {
  ...
  if (chunk_info.InUse()) {
    // DBGConsole_Msg( 0, "[CRomCache - purging %02x %08x-%08x", cache_idx,
    // chunk_info.StartOffset, chunk_info.StartOffset + CHUNK_SIZE );
    u32 chunk_map_idx(AddressToChunkMapIndex(current_chunk_address));
#ifdef DAEDALUS_ENABLE_ASSERTS
    DAEDALUS_ASSERT(chunk_map_idx < mChunkMapEntries,
                    "Chunk address is out of range?");
    DAEDALUS_ASSERT(mpChunkMap[chunk_map_idx] == cache_idx,
                    "Chunk map inconsistancy");
#endif
...
ROMFileCache::CacheIdx ROMFileCache::GetCacheIndex(u32 address) {
  ...
  if (idx == INVALID_IDX) {
    ...
    // DBGConsole_Msg( 0, "[CRomCache - loading %02x, %08x-%08x", selected_idx,
    // chunk_info.StartOffset, chunk_info.StartOffset + CHUNK_SIZE );
...
//
//*****************************************************************************
bool ROMFileCache::GetChunk(u32 rom_offset, u8 **p_p_chunk_base,
                            u32 *p_chunk_offset, u32 *p_chunk_size) {
...
#ifdef DAEDALUS_ENABLE_ASSERTS
    DAEDALUS_ASSERT(AddressToChunkMapIndex(chunk_info.StartOffset) ==
                        chunk_map_idx,
                    "Inconsistant map indices");
    DAEDALUS_ASSERT(chunk_info.ContainsAddress(rom_offset),
                    "Address is out of range for chunk");
#endif
