...
bool ROMFileCompressed::Open(COutputStream &messages) {
  ...
  if (err != UNZ_OK) {
    ...
    do {
      err = unzGetCurrentFileInfo(mZipFile, &file_info, rom_filename,
                                  ARRAYSIZE(rom_filename), NULL, 0, NULL, 0);
      if (err != UNZ_OK) {
      ...
      if (file_info.uncompressed_size > 1 * 1024 * 1024) {
        ...
        if (err == UNZ_OK) {
          ...
          if (bytes_read == u32size) {
            ...
            if (magic == 0x80371240 || magic == 0x40123780 ||
                magic == 0x12408037) {
...
//
//*****************************************************************************
bool ROMFileCompressed::LoadRawData(u32 bytes_to_read, u8 *p_bytes,
  ...
  //
  //	It is assumed that the file is already openened and ready for reading
  //here
  //
  ...
  if (u32(bytes_read) != bytes_to_read) {
    ...
    if (bytes_read < 0) {
      ...
      messages << "Unable to read sufficent bytes from zip.\nRead "
               << bytes_read << ", wanted " << bytes_to_read;
    }
#endif
    unzCloseCurrentFile(mZipFile);
    return false;
...
//	Uses the specified buffer as a scratch pad to temporarilly decompress
//data.
//*****************************************************************************
bool ROMFileCompressed::Seek(u32 offset, u8 *p_scratch_block, u32 block_size) {
...
#ifdef DAEDALUS_ENABLE_ASSERTS
  DAEDALUS_ASSERT((current_offset % block_size) == 0,
                  "Performance: Trying to seek and current offset isn't a "
                  "multiple of the block size");
  DAEDALUS_ASSERT(
      (offset % block_size) == 0,
      "Performance: Trying to seek to an address which isn't the block size");
#endif
  if (current_offset > offset) {
#ifdef DAEDALUS_DEBUG_CONSOLE
    DBGConsole_Msg(0, "[CRomCache - seeking from %08x to %08x in zip",
  ...
  while (current_offset < offset) {
    ...
    u32 bytes_read(
        unzReadCurrentFile(mZipFile, p_scratch_block, bytes_to_read));
...
#ifdef DAEDALUS_ENABLE_ASSERTS
  DAEDALUS_ASSERT(u32(unztell(mZipFile)) == offset,
                  "Failed to seek to the specified offset");
#endif
  return true;
}
