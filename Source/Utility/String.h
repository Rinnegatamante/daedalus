...
public:
  ...
  //
  // Access for functions which need to write to our buffer. Should try to avoid
  // these!
  //
...
private:
  void Copy(CConstString string) {
    ...
    DAEDALUS_ASSERT(string.Length() <= MAX_LENGTH,
                    "String '%s' is too long for copy, truncation will occur",
                    string.c_str());
#endif
  ...
  void Append(CConstString string) {
    ...
    DAEDALUS_ASSERT(Length() + string.Length() < MAX_LENGTH,
                    "String '%s' is too long append, truncation will occur",
                    string.c_str());
#endif
...
#ifdef DAEDALUS_ENABLE_ASSERTS
    DAEDALUS_ASSERT(length > Length(),
                    "Resize should always increase buffer length");
#endif
...
struct ConstStringRef {
  ...
  /*explicit */ ConstStringRef(const char *str)
  ...
  bool operator==(const char *rhs) const {
    return operator==(ConstStringRef(rhs));
  }
