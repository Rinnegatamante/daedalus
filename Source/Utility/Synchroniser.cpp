...
void CSynchroniser::HandleOutOfSynch(const char *msg) {
  char message[512];
  sprintf(message, "Synchronisation Failed at 0x%08x: %s", gCPUState.CurrentPC,
          msg);
  gCPUState.Dump();
...
//
//*****************************************************************************
bool ISynchProducer::IsOpen() const { return mStream.IsOpen(); }

//*****************************************************************************
//
//*****************************************************************************
CSynchroniser::ESynchResult ISynchProducer::SynchPoint(const void *data,
                                                       u32 length) {
  if (mStream.IsOpen()) {
...
//
//*****************************************************************************
bool ISynchConsumer::IsOpen() const { return mStream.IsOpen(); }

//*****************************************************************************
//
//*****************************************************************************
CSynchroniser::ESynchResult ISynchConsumer::SynchPoint(const void *data,
                                                       u32 length) {
  if (mStream.IsOpen()) {
    //
    ...
    while (bytes_remaining > 0) {
      ...
      if (!mStream.ReadData(buffer, bytes_to_process)) {
        // Actually, we may have gone out of synch just before running out of
        // data..
...
bool CSynchroniser::InitialiseSynchroniser() {
  ...
  if (!IO::File::Exists(filename)) {
    ...
    DBGConsole_Msg(0, "Start Synchroniser in [Gproducer] mode to create [C%s]",
                   filename);
  } else {
    p_synch = CSynchroniser::CreateConsumer(filename);
    DBGConsole_Msg(0, "Start Synchroniser in [Gconsumer] mode to read [C%s]",
                   filename);
  }
