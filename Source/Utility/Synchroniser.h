...
enum ESynchFlags {
  ...
  DAED_SYNC_FRAGMENT_PC =
      0x00000020, // e.g. program counter on entry/exit from fragments
};
...
static const u32 DAED_SYNC_MASK(DAED_SYNC_FRAGMENT_PC | DAED_SYNC_REG_GPR |
                                DAED_SYNC_REG_CCR0);
...
public:
  ...
  template <typename T> static void SynchData(T &data) {
    if (mpSynchroniser != NULL) {
      ...
      case SR_OK:
        return;
      case SR_OUT_OF_STORAGE:
        HandleOutOfStorage();
        return;
      case SR_OUT_OF_INPUT:
        HandleOutOfInput();
        return;
      case SR_OUT_OF_SYNCH:
        DAEDALUS_ERROR("Shouldn't ever be out of synch?");
        return;
      default:
        NODEFAULT;
      }
