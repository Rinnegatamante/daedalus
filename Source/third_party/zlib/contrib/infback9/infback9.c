 ...
 */
int ZEXPORT inflateBack9Init_(strm, window, version,
                              stream_size) z_stream FAR *strm;
...
void makefixed9(void) {
  ...
  for (;;) {
    if ((low % 6) == 0)
      printf("\n        ");
    printf("{%u,%u,%d}", lenfix[low].op, lenfix[low].bits, lenfix[low].val);
    if (++low == size)
      break;
    putchar(',');
  }
  ...
  for (;;) {
    if ((low % 5) == 0)
      printf("\n        ");
    printf("{%u,%u,%d}", distfix[low].op, distfix[low].bits, distfix[low].val);
    if (++low == size)
      break;
    putchar(',');
  }
 ...
 */
int ZEXPORT inflateBack9(strm, in, in_desc, out, out_desc) z_stream FAR *strm;
in_func in;
void FAR *in_desc;
out_func out;
void FAR *out_desc;
...
int ZEXPORT inflateBack9End(strm) z_stream FAR *strm;
{
