...
voidpf ZCALLBACK win32_open_file_func OF((voidpf opaque, const char *filename,
                                          int mode));
...
voidpf ZCALLBACK win32_open64_file_func(voidpf opaque, const void *filename,
  ...
  if ((filename != NULL) && (dwDesiredAccess != 0))
    hFile = CreateFile2((LPCTSTR)filename, dwDesiredAccess, dwShareMode,
                        dwCreationDisposition, NULL);
#else
  if ((filename != NULL) && (dwDesiredAccess != 0)) {
    WCHAR filenameW[FILENAME_MAX + 0x200 + 1];
    MultiByteToWideChar(CP_ACP, 0, (const char *)filename, -1, filenameW,
                        FILENAME_MAX + 0x200);
...
#ifdef IOWIN32_USING_WINRT_API
  if ((filename != NULL) && (dwDesiredAccess != 0))
    hFile = CreateFile2((LPCWSTR)filename, dwDesiredAccess, dwShareMode,
                        dwCreationDisposition, NULL);
#else
  if ((filename != NULL) && (dwDesiredAccess != 0))
...
voidpf ZCALLBACK win32_open_file_func(voidpf opaque, const char *filename,
                                      int mode) {
  const char *mode_fopen = NULL;
  DWORD dwDesiredAccess, dwCreationDisposition, dwShareMode,
      dwFlagsAndAttributes;
...
static BOOL MySetFilePointerEx(HANDLE hFile, LARGE_INTEGER pos,
