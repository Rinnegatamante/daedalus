...
int ReadFileMemory(const char *filename, long *plFileSize,
...
int main(int argc, char *argv[]) {
  ...
  if (argc <= 1) {
    printf("run TestZlib <File> [BlockSizeCompress] [BlockSizeUncompress] "
           "[compres. level]\n");
    return 0;
  }
  ...
  BeginCountRdtsc(&li_rdtsc);
  {
    ...
    do {
      ...
      ret = deflate(&zcpr,
                    (zcpr.avail_in == lOrigToDo) ? Z_FINISH : Z_SYNC_FLUSH);
