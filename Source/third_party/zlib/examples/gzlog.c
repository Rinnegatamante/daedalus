...
local int log_mark(struct log *log, int op) {
  ...
  fsync(log->fd);
  ret =
      lseek(log->fd, HEAD, SEEK_SET) < 0 || write(log->fd, ext, EXTRA) != EXTRA
          ? -1
          : 0;
  fsync(log->fd);
  return ret;
...
local int log_append(struct log *log, unsigned char *data, size_t len) {
  ...
  PUT4(buf, log->tcrc);
  PUT4(buf + 4, log->tlen);
  if (write(log->fd, buf, 8) != 8 || (end = lseek(log->fd, 0, SEEK_CUR)) < 0 ||
...
local int log_compress(struct log *log, unsigned char *data, size_t len) {
  ...
  if (len) {
    ...
    if (lseek(log->fd, log->first - (log->back > 8 ? 2 : 1), SEEK_SET) < 0 ||
...
local void log_log(struct log *log, int op, char *record) {
  ...
  fprintf(rec, "%.24s %s recovery: %s\n", ctime(&now),
          op == APPEND_OP ? "append"
                          : (op == COMPRESS_OP ? "compress" : "replace"),
          record);
  fclose(rec);
  return;
}
